...
* oracle.lpad(string, length [, fill])  - Oracle compatible rpad
...
because oracle.substr, oracle.lpad, oracle.rpad are installed side-by-side with pg_catalog.substr, pg_catalog.lpad, pg_catalog.rpad, respectively.

Note that in case of lpad and rpad, parameters string and fill can be of types CHAR, VARCHAR, TEXT, VARCHAR2 or NVARCHAR2 (note that the last two are orafce-provided types). The default fill character is a half-width space.
