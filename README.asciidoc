...
These dictionary can be named (second column in table `utl_file.utl_file_dir`).
The `location` parameter can be the name or the dictionary path. The `location`
is interpreted and checked as a name of dictionary. When this name is not found,
then location is interpreted and checked as dictionary.
...
* For these types is possible to use null safe || operator, when you enable
  ...
  Attention: - when result is empty string, then result is NULL. This behave is
  disabled by default.
  ...
  Is not possible to create functional indexes over stable or volatile expressions.
