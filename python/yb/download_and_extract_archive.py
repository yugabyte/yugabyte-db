...
class LockFile:
    """
    Locks a specific file. After the given timeout, gives up and deletes the file and takes the
    ownership of the lock anyway. Does not delete the lock file.
    """
    def __init__(self, path, timeout_sec):
        self.path = path
        self.timeout_sec = timeout_sec

    def __enter__(self):
        self.lock_fd = os.open(self.path, os.O_WRONLY | os.O_CREAT)
        start_time_sec = time.time()
        if g_verbose:
            logging.info("Trying to acquire lock %s", self.path)

        while True:
            try:
                fcntl.lockf(self.lock_fd, fcntl.LOCK_EX | fcntl.LOCK_NB)

                # If this succeeds, we're holding the lock.
                if g_verbose:
                    logging.info("Acquired lock %s", self.path)
                break
            except IOError as e:
                if e.errno != errno.EAGAIN:
                    logging.error(
                        "Got an unexpected errno while attempting to lock %s (expected %d): %d",
                        self.path, errno.EAGAIN, e.errno)
                    raise e

            elapsed_time_sec = time.time() - start_time_sec
            if elapsed_time_sec > self.timeout_sec:
                logging.error(
                    "Failed to acquire lock on %s in %1.f sec (elapsed time: %.1f sec), "
                    "deleting the file",
                    self.path, self.timeout_sec, elapsed_time_sec)
                os.remove(self.path)

    def __exit__(self, exc_type, exc, exc_tb):
        if g_verbose:
            logging.info("Releasing the lock %s", self.path)
        # TODO: is this necessary, or would closing the file be enough?
...
def download_and_extract(url, dest_dir_parent, local_cache_dir, nfs_cache_dir):
    ...
    lock_file_path = '/tmp/yb_download_%s' % install_dir_name
    with LockFile(lock_file_path, timeout_sec=LOCK_TIMEOUT_SEC):
        dest_dir = os.path.join(dest_dir_parent, install_dir_name)
        if os.path.isdir(dest_dir):
            if g_verbose:
                logging.info("Acquired lock %s, but directory %s already exists, someone must have "
                             "created it concurrently.", lock_file_path, dest_dir)
            return

        start_time_sec = time.time()
        logging.info("Instaling %s into directory %s", url, dest_dir)
        ...
        if install_dir_name.startswith('linuxbrew'):
            ...
            if os.path.exists(orig_brew_home):
                logging.info("'%s' already exists, cannot move '%s' to it" % (
...
def main():
    ...
    parser.add_argument(
        '--local-cache-dir',
        default='/opt/yb-build/download-cache',
...
if __name__ == '__main__':
    logging.basicConfig(
        level=logging.INFO,
        format="%(filename)s:%(lineno)d " + socket.gethostname() +
