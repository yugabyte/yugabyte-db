...
populate_mvn_config() {
  if [[ -f $MVN_SETTINGS_PATH ]]; then
    log "$MVN_SETTINGS_PATH already exists, not attempting to generate based on .s3cfg"
    return
  fi
  local s3cfg_path=""
  local s3cfg_path_candidate
  for s3cfg_path_candidate in "${S3CFG_PATHS[@]}"; do
    if [[ -f $s3cfg_path_candidate ]]; then
      s3cfg_path=$s3cfg_path_candidate
      break
    fi
  done

  if [[ -z $s3cfg_path ]]; then
    log "s3cmd configuration not found at any of these paths: ${S3CFG_PATHS[*]}," \
        "cannot create $MVN_SETTINGS_PATH based on it"
    return
  fi

  log "Creating $MVN_SETTINGS_PATH based on AWS access key and secret key from $s3cfg_path"
  ( set -x; mkdir -p "${MVN_SETTINGS_PATH%/*}" )

  get_aws_key_from_s3cfg "$s3cfg_path" access 20
  local aws_access_key=$get_aws_key_from_s3cfg_rv

  get_aws_key_from_s3cfg "$s3cfg_path" secret 40
  local aws_secret_key=$get_aws_key_from_s3cfg_rv

  cat >"$MVN_SETTINGS_PATH" <<-EOT
<!-- Automatically generated by common-build-env.sh (included by $0) on $( get_timestamp ). -->
<!-- Based on access and secret keys from $s3cfg_path. -->
<settings>
  <servers>
    <server>
      <id>s3.release</id>
