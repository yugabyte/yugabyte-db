        ...
        for file_dir, dirs, file_names in os.walk(prefix_dir):
            for file_name in file_names:
                if file_name.endswith('_patchelf_tmp'):
                    continue
                # TODO: rename this to something that does not contain ELF (Mac OS X binary format
                # is different).
                ...
                if ((os.access(elf_file_path, os.X_OK) or
                    ...
                    if is_mac:
                        ...
                        if install_name_tool_exit_code != 0:
                            logging.error(
                                ("Failed to update librocksdb path for '%s' using "
                                 "install_name_tool") % elf_flie_path)
                            return False
                        ...
                        if default_deps_lib_dir not in rpath_entries:
                            ...
                            if os.system(
                                ...
                                raise RuntimeError(
                                    "Failed to add rpath to '{}'".format(elf_file_path))
                    ...
                    if is_main_build:
                        all_libs_found, missing_libs = run_ldd(elf_file_path,
                    ...
                    for rpath_entry in rpath_entries:
                        rel_path = os.path.relpath(os.path.abspath(rpath_entry),
                    ...
                    if original_rpath_entries == new_rpath_entries:
                        ...
                        # This will actually set RUNPATH as newer replacement for RPATH, with the
                        # difference being that RUNPATH can be overwritten by LD_LIBRARY_PATH, but
                        # we're OK with it.
                        ...
                        logging.info(
                            "Setting rpath on '%s' to '%s'" % (elf_file_path, new_rpath_str))
                        if set_rpath_exit_code != 0:
                            logging.warn("Could not set rpath on '%s': exit code %d" % (
                                elf_file_path, set_rpath_exit_code))
                            is_success = False
