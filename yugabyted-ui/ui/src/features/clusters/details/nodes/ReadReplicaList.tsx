...
export const ReadReplicaList: FC = () => {
  ...
  const NODES_TABLE_COLUMNS = [
    ...
    },
    {
        ...
        options: {
          filter: true,
          display: columns.read_ops || columns.active_connections
        },
        subColumns: [
          {
              name: 'read_ops',
  ...
  return (
    <>
      <Typography variant="h5" className={classes.title}>{t('clusterDetail.nodes.readReplicaClusters')}</Typography>
      <Box className={classes.filterRow}>
        ...
        >
                <MenuItem value={''}>{t('clusterDetail.nodes.allClusters')}</MenuItem>
