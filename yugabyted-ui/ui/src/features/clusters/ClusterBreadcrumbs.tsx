...
export const ClusterBreadcrumbs: FC = () => {
  ...
  const releaseTrack = releaseTracks?.data?.find(
    ({ info: releaseInfo }) => releaseInfo?.id === clusterData?.data?.spec?.software_info?.track_id
  );
  //Release Tracks
  ...
  const releaseName = releaseTrack?.spec?.name ? `${releaseTrack?.spec?.name} ${t('clusters.track')}` : '';
