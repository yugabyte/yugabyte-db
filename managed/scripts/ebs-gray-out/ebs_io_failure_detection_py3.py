...
def identify_process_pids(pid_gid_dict):
    logging.info("Attempting to identify the pids of the processes: %s", list(pid_gid_dict.keys()))
    try:
...
def stop_process(pid_gid_dict):
    logging.info("Attempting to stop the processes: %s", list(pid_gid_dict.keys()))
            ...
            for line in file:
                ...
                if line.startswith(DOUBLE_HYPHEN):
                    ...
                    if key == PROCESS_NAMES_ARG:
                        ...
                        args[key] = int(value)
                    elif key in [FILE_PATHS_ARG, FS_DATA_DIRS_ARG]:
                        args[key] = value.split(',')
    return args
