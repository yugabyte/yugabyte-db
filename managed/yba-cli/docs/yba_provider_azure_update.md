...
```
      ...
      --edit-region stringArray          [Optional] Edit region details associated with the Azure provider. Provide the following comma separated fields as key-value pairs:"region-name=<region-name>,vnet=<virtual-network>,sg-id=<security-group-id>,yb-image=<custom-ami>". Region name is a required key-value pair. Virtual network and Security Group ID are optional. Each region needs to be modified using a separate --edit-region flag.
