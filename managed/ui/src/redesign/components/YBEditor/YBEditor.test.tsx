...
describe('YBEditor render test', () => {
  ...
  it('should load all plugins', () => {
    const { editorRef } = setup({
      editorProps: { 'data-testid': 'ybeditor' },
      loadPlugins: {
    ...
    const basicPlugin = editorRef.current?.['pluginsList'].find(
      (e: IYBSlatePlugin) => e.name === 'Basic'
    );
  ...
  it('should load only selected plugins', () => {
    const { editorRef } = setup({
      editorProps: { 'data-testid': 'ybeditor' },
      loadPlugins: { singleLine: true, basic: false }
    });
    ...
    const singLinePlugin = editorRef.current?.['pluginsList'].find(
      (e: IYBSlatePlugin) => e.name === 'Single Line Plugin'
    );
