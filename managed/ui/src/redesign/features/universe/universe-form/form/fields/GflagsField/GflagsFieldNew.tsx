...
import { Alert, Button, Tooltip, OverlayTrigger, Popover } from 'react-bootstrap';
...
import { ReactComponent as AddCircleIcon } from '../../../../../../assets/add-circle-blue.svg';
import { ReactComponent as CaretDownIcon } from '../../../../../../assets/caret-down-blue.svg';
import '../../../../../../../components/universes/UniverseForm/UniverseForm.scss';

import { YBDropdown, mui, YBButton } from '@yugabyte-ui-library/core';
...
export const GFlagsFieldNew = ({
  ...
  const OPTIONS = [
    {
      ...
      btnLabel: 'Add Configuration Flag'
    },
    {
      ...
      btnLabel: 'Add Configuration Flag as JSON'
    }
  ];
    ...
    if (editMode) {
      ...
      return (
        <Box
          ...
          mt={2}
        >
          {t('universeForm.gFlags.noFlagsCreate')}
        </Box>
      );
  };
  ...
  return (
    <Box display="flex" width="100%" height="100%" flexDirection="column">
      ...
      {isLoading && <span>{t('universeForm.gFlags.loadingGflagGroups')}</span>}
      <Box flexShrink={1} mt={0.5} flexDirection="row" display="flex" gridColumnGap={8}>
        {!isReadOnly &&
          OPTIONS.map((option) => {
            const { optionName, buttonProps, btnLabel } = option;
            return (
              <YBDropdown
                key={optionName}
                origin={
                  <YBButton {...(buttonProps as any)} disabled={isLoading}>
                    {' '}
                    {btnLabel}
                  </YBButton>
                }
