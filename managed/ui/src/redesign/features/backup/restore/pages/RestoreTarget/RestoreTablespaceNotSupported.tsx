...
export const RestoreTablespaceNotSupported: FC<RestoreTablespaceNotSupportedProps> = ({
  visible,
  onHide
}) => {
  const [{ preflightResponse }, , { hideModal }] = GetRestoreContext();
  ...
  return (
    <YBModal
      ...
      onClose={() => {
        setValue('target.useTablespaces', false, { shouldValidate: true });
        onHide();
      }}
      onSubmit={() => {
        setValue('target.useTablespaces', false, { shouldValidate: true });
            ...
            components={{
              ...
              a: (
                <a
                  ...
                  href="https://docs.yugabyte.com/preview/explore/ysql-language-features/going-beyond-sql/tablespaces"
                  rel="noreferrer"
                  target="_blank"
                ></a>
              )
      ...
      {conflictingTablespaces.length !== 0 && (
        <div className={classes.errMsgPane}>
          ...
          {conflictingTablespaces.map((t) => (
            <div className={classes.tablespaceItem} key={t}>
              {t}
            </div>
          ))}
        </div>
