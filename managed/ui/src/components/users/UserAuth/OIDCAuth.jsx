...
export const OIDCAuth = (props) => {
  const { fetchRunTimeConfigs, setRunTimeConfig, deleteRunTimeConfig, runtimeConfigs } = props;
  ...
  const saveOIDCConfigs = async (values) => {
    ...
    const promiseArray = Object.keys(formValues).reduce((promiseArr, key) => {
      if (formValues[key] !== initValues[key]) {
        ...
        const value =
          key !== 'oidc_default_role' && isString(formValues[key])
            ? `"${formValues[key]}"`
            : formValues[key];
        promiseArr.push(
          formValues[key] !== ''
  ...
  const isOIDCMappingEnabled =
    configEntries.find((config) => config.key === 'yb.security.oidc_enable_auto_create_users')
      ?.value === 'true';
                      ...
                      {isFetching ? (
                        ...
                        <Row className="ldap-sec-container ">
                          <Col
                            ...
                            md={10}
                            lg={6}
                            className="ua-field-row-c adv-settings"
                          >
                            <Row className="ua-field-row">
                                ...
                                <div className="ua-box-c default-ldap-role-b">
                                          ...
                                          ({ label, value }) => (
                                            ...
                                            >
                                              <Field
                                                ...
                                                value={value}
                                                checked={
                                                  `${value}` === `${values['oidc_default_role']}`
                                                }
                                                disabled={isDisabled}
                                              />
                                              &nbsp;&nbsp;{label}&nbsp;
                                  ...
                                  <Row key="oidc_group_claim_token">
                                    <Col
                                      ...
                                      md={10}
                                      lg={6}
                                      className="ua-field-row-c group-claim"
                                    >
                                      <Row className="ua-field-row">
                                        <Col className="ua-label-c">
                                          <div>
                        ...
                        <Col className="ua-label-c">
                          <div>
                            ...
                            <YBInfoTip
                              ...
                              content={
                                <div>
                                  If you have an airgapped installation, provide an OpenID Provider
                                  Configuration Document for your provider. <br />
                                  This contains the provider OIDC endpoints, supported claims, and
                                  other metadata.
                                  <br />
                                  YugabyteDB Anywhere uses the metadata to discover the URLs to use
                                  for authentication and the authentication service&apos;s public
                                  signing keys
                                </div>
                              }
                            >
