...
export default class StepProgressBar extends Component {
  ...
  normalizeTasks = (taskDetails) => {
    const taskDetailsNormalized = [...taskDetails,{
  ...
  render() {
    ...
    const progressbarClass = this.isFailedIndex(taskDetailsNormalized )>-1 ? "failed" : (
      this.isRunningIndex(taskDetailsNormalized ) > -1 ? "running" : "finished"
    );
    const barWidth = taskIndex === -1 
      ? "100%" 
      : 100*(taskIndex+(this.isFailedIndex(taskDetailsNormalized ) > -1 ? 0 : 0.5))/(tasksTotal - 1)+"%";

    const listLabels = taskDetailsNormalized.map(function(item, idx) {
      taskClassName = getTaskClass(item.state);
      return (
        <li key={idx} className={taskClassName+" "+item.class}><span>{item.title}</span></li>
      );
    }, this);
    return (
      <ul className="progressbar">
        <div className={"progressbar-bar "+progressbarClass+" "+(taskIndex > -1 ? getTaskClass(taskDetailsNormalized[taskIndex].state) : "")} style={{ width:barWidth }}></div>
        {listLabels}
      </ul>
    );
  }
