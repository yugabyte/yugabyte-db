...
export default class RollingUpgradeForm extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      getPromiseState(rollingUpgrade).isSuccess() &&
      getPromiseState(prevProps.universe.rollingUpgrade).isLoading()
    ) {
  ...
  setRollingUpgradeProperties = (values) => {
    ...
    if (isNonEmptyArray(values.masterGFlags)) {
      masterGFlagList = values.masterGFlags
        .map(function (masterFlag, masterIdx) {
    ...
    if (isNonEmptyArray(values.tserverGFlags)) {
      tserverGFlagList = values.tserverGFlags
        .map(function (tserverFlag, tserverIdx) {
  ...
  render() {
    ...
    const shouldEnableRollingUpgradeDelay = upgradeOption === 'Rolling';
    ...
    const softwareVersionOptions = softwareVersions.map(function (item, idx) {
      return (
        <option key={idx} disabled={item === currentVersion ? true : false} value={item}>
          {item}
        </option>
      );
    });
          ...
          <Tabs defaultActiveKey={1} className="gflag-display-container" id="gflag-container">
            <Tab eventKey={1} title="Master" className="gflag-class-1" bsClass="gflag-class-2">
              <FieldArray
                name="masterGFlags"
                component={FlagItems}
                resetRollingUpgrade={resetRollingUpgrade}
              />
            </Tab>
    ...
    return visibleModal === 'softwareUpgradesModal' ? (
      <YBModal
        ...
        error={error}
        footerAccessory={
          this.state.pickedVersion !== currentVersion ? (
            <YBCheckBox
