...
export default class YBTextArea extends Component {
  ...
  render() {
    ...
    const { input, type, className, placeHolder, onValueChanged, isReadOnly, label, meta, insetError, infoContent,
      infoTitle, infoPlacement, subLabel } = this.props;
    ...
    const ybLabelContent = isNonEmptyString(subLabel) ? <DescriptionItem title={subLabel}>
    ...
    </DescriptionItem> : <FormControl {...input} componentClass="textarea" placeholder={placeHolder} type={type} className={className}
                                      onChange={onChange} readOnly={isReadOnly} />;
