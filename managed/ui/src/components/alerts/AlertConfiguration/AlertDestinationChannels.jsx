...
const Composer = React.lazy(() =>
  import('../../../redesign/features/alerts/TemplateComposer/Composer')
);
...
export const AlertDestinationChannels = (props) => {
  ...
  const { customer, featureFlags } = props;
  const isReadOnly = isNonAvailable(customer.data.features, 'alert.channels.actions');

  const enableNotificationTemplates =
    featureFlags.test.enableNotificationTemplates ||
    featureFlags.released.enableNotificationTemplates;

  const enableCustomEmailTemplates =
    ...
    return (
      <>
        <DropdownButton
          ...
          pullRight
        >
          <RbacValidator accessRequiredOn={ApiPermissionMap.MODIFY_ALERT_DESTINATION} isControl>
            <MenuItem
              onClick={() => {
        ...
        <RbacValidator accessRequiredOn={ApiPermissionMap.GET_ALERT_CONFIGURATIONS}>
          <>
            <div className="alert-destinations">
              <div>
                A notification channel defines the means by which an alert is sent (ex: Email) as
                well as who should receive the notification.
              </div>
              {!isReadOnly && (
                <div>
