...
const ScheduledBackupCard: FC<ScheduledBackupCardProps> = ({
  ...
  if (schedule.backupInfo.backupType === TableType.YQL_TABLE_TYPE && !schedule.backupInfo.fullBackup) {
    schedule.backupInfo.keyspaceList.forEach((keyspace) => {
      isTableMissingToDoBackup = !keyspace.tableUUIDList?.every(tableUUID => find(tablesInUniverse, { tableUUID, keySpace: keyspace.keyspace }));
    });
  }
    ...
    <div className="schedule-item">
      <Row className="name-and-actions">
        <Col lg={6} md={6} className={`name-field ${schedule.status}`}>
          ...
          {
            isTableMissingToDoBackup && (<OverlayTrigger trigger={["hover", "focus"]} placement="top" overlay={<Popover id="more-tables-popover">One or more of selected tables to backup does not exists in the keyspace.</Popover>}><span> <WarningIcon /></span></OverlayTrigger>)
          }
        </Col>
