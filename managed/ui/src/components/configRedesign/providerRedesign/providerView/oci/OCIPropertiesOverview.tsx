...
import styles from './OCIPropertiesOverview.module.scss';

interface OCIPropertiesOverviewProps {
  providerConfig: YBProvider;
  isProviderInUse: boolean;
}

export const OCIPropertiesOverview = ({
  providerConfig,
  isProviderInUse
}: OCIPropertiesOverviewProps) => {
  const [isOverviewExpanded, setIsOverviewExpanded] = useState<boolean>(false);

  if (providerConfig.code !== ProviderCode.OCI) {
    return null;
  }
