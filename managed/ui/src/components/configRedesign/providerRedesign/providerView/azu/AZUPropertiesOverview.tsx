...
import styles from './AZUPropertiesOverview.module.scss';

interface AZUPropertiesOverviewProps {
  providerConfig: YBProvider;
  isProviderInUse: boolean;
}

export const AZUPropertiesOverview = ({
  providerConfig,
  isProviderInUse
}: AZUPropertiesOverviewProps) => {
  const [isOverviewExpanded, setIsOverviewExpanded] = useState<boolean>(false);

  if (providerConfig.code !== ProviderCode.AZU) {
    return null;
  }
