...
const YBPagination = (props) => {
  ...
  const curActivePage = props.activePage ? Math.min(Math.max(props.activePage, firstPage), numPages) : activePage
  ...
  const isSibling = (num) => curActivePage - siblingCount <= num && num <= curActivePage + siblingCount
  ...
  for (let num = firstPage; num <= numPages; num++) {
    if (isBoundary(num) ||
      ...
      items.push(
        <Pagination.Item key={num} active={num === curActivePage} onClick={() => handlePageChange(num)}>
          {num}
        </Pagination.Item>
      );
    } else if (num == firstPage + 1 && showEllipsis(side.LEFT) || num == numPages - 1 && showEllipsis(side.RIGHT)) {
  ...
  return (
    <React.Fragment>
      <Pagination>
        <Pagination.Prev onClick={() => handlePageChange(nav.PREV)} disabled={curActivePage == firstPage} />
        {items}
        <Pagination.Next onClick={() => handlePageChange(nav.NEXT)} disabled={curActivePage == numPages} />
      </Pagination>
    </React.Fragment>
  )
}
