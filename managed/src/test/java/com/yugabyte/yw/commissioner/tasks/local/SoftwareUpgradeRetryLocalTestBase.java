...
/**
 * Shared machinery for the local-provider fault-injection tests covering {@link
 ...
 * until the upgrade fully succeeds against real loopback yb-master/yb-tserver processes. A single
 * such sweep takes minutes, so the tests are split across {@link SoftwareUpgradeRetryLocalTest} and
 * {@link SoftwareUpgradeRetryPg15LocalTest} to let them run in two forks instead of one - the class
 * was previously the slowest in the suite and therefore set its wall-clock floor on its own.
 */
@Slf4j
public abstract class SoftwareUpgradeRetryLocalTestBase extends LocalProviderUniverseTestBase {
  ...
  protected static final String PG_11_DB_VERSION = "2024.2.3.0-b116";
  ...
  protected static final String PG_15_DB_VERSION = "2025.1.1.0-b73";
  private static final String PG_15_DB_VERSION_URL =
      "https://s3.us-west-2.amazonaws.com/uploads.dev.yugabyte.com/"
          + "local-provider-test/2025.1.1.0-b73/yugabyte-2025.1.1.0-b73-%s-%s.tar.gz";

  protected static final long WAIT_SLEEP_MS = 500;
  protected static final int WAIT_MAX_RETRIES = 3600;
  private static final int MAX_PRECHECK_ADVANCES = 20;
  ...
  protected void runUpgradeThenFaultInjectedRollback(
  ...
  protected void runFaultInjectedUpgrade(Supplier<UUID> submitTask, Runnable verifyFinalState)
   ...
   */
  protected void runCanaryFaultInjectedUpgradeWithPauses(
   ...
   */
  protected void finalizeCanaryUpgradeWithFaultInjection(UUID universeUUID)
      throws InterruptedException {
    assertEquals(
  ...
  private SubtaskCapture captureSubtasks(UUID taskUuid) throws InterruptedException {
    ...
    Optional<Integer> freezePosition = freezePosition(taskInfo);
    // Upgrade tasks run comprehensive prechecks ahead of FreezeUniverse, and the pause at position
    // 0 sometimes lands while only that first precheck group exists. Capturing there would yield a
    // two-subtask "graph" and silently reduce the sweep below to a no-op, so step the pause forward
    // until the frozen universe - and with it the real graph - is visible.
    for (int i = 0; !freezePosition.isPresent() && i < MAX_PRECHECK_ADVANCES; i++) {
      int lastPosition =
          taskInfo.getSubTasks().stream().mapToInt(TaskInfo::getPosition).max().orElse(0);
      setPausePosition(lastPosition + 1);
      commissioner.resumeTask(taskUuid);
      waitForTaskPaused(taskUuid);
      taskInfo = TaskInfo.getOrBadRequest(taskUuid);
      freezePosition = freezePosition(taskInfo);
    }
  ...
  private Optional<Integer> freezePosition(TaskInfo taskInfo) {
  ...
  protected Universe createRollbackCapableUniverse() throws InterruptedException {
  ...
  protected Universe createPg11Universe() throws InterruptedException {
  ...
  protected SoftwareUpgradeParams buildCanaryUpgradeParams(
      Universe universe, String targetVersion) {
