...
public class NodeInstanceControllerTest extends FakeDBApplication {
  ...
  public void testListByZoneWrongZone() {
    ...
    String error = "Invalid com.yugabyte.yw.models.AvailabilityZoneUUID: " + wrongUuid.toString();
    checkNotOk(r, error);
    assertAuditEntry(0, customer.uuid);
  }
  ...
  public void testInvalidNodeAction() {
    for (NodeActionType nodeActionType : NodeActionType.values()) {
      ...
      customer.save();
      verify(mockCommissioner, times(0)).submit(any(), any());
      Result r = performNodeAction(customer.uuid, u.universeUUID, "fake-n1", nodeActionType, true);
