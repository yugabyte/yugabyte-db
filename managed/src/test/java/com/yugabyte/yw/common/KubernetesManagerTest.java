...
public class KubernetesManagerTest extends FakeDBApplication {
    ...
    switch (commandType) {
      case HELM_INSTALL:
        kubernetesManager.helmInstall(
            configProvider, defaultProvider.uuid, "demo-universe", "/tmp/override.yml");
        break;
      case HELM_UPGRADE:
  ...
  public void helmDelete() {
    ...
    assertEquals(
        ImmutableList.of("helm", "delete", "demo-universe", "-n", "demo-universe"),
        command.getValue());
    assertEquals(config.getValue(), configProvider);
  }
  ...
  public void deleteStorage() {
    ...
    assertEquals(
        ImmutableList.of(
            ImmutableList.of(
                "kubectl", "delete", "pvc", "--namespace", "demo-universe", "-l", "app=yb-master"),
            ImmutableList.of(
                "kubectl",
                "delete",
                "pvc",
