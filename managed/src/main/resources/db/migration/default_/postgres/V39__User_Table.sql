...
INSERT into users (uuid, customer_uuid, email, password_hash, creation_date, auth_token, auth_token_issue_date, api_token, role, is_primary)
  SELECT  uuid_in(md5(random()::text || clock_timestamp()::text)::cstring), uuid, email, password_hash, creation_date, auth_token, auth_token_issue_date, api_token, 'Admin', 'true'
  FROM customer;
