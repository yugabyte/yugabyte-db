...
public class ImageBundleManagementHandler {
  ...
  public ImageBundle getImageBundle(UUID cUUID, UUID providerUUID, UUID iBUUID) {
    if (Provider.get(cUUID, providerUUID) == null) {
      throw new PlatformServiceException(
          NOT_FOUND, String.format("Could not find provider %s", providerUUID));
    }
    com.yugabyte.yw.models.ImageBundle bundle =
        com.yugabyte.yw.models.ImageBundle.get(providerUUID, iBUUID);
    if (bundle == null) {
      throw new PlatformServiceException(
          NOT_FOUND, String.format("Could not find image bundle %s", iBUUID));
    }
    return ImageBundleMapper.INSTANCE.toApi(bundle);
  }
  ...
  public ImageBundlePagedResp pageListImageBundles(
    ...
    return HandlerPagingSupport.pagedResponse(
        ...
        com.yugabyte.yw.models.ImageBundle.getPagedList(providerUUID, arch, normalized),
