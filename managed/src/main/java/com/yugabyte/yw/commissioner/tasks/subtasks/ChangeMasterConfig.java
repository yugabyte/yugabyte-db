...
public class ChangeMasterConfig extends UniverseTaskBase {
  ...
  private static final String CHANGE_MASTER_CONFIG_CHECK_ENABLED_PARAM =
      "yb.checks.change_master_config.enabled";
  private static final String CHANGE_MASTER_CONFIG_CHECK_TIMEOUT_PARAM =
  ...
  public void run() {
    ...
    if (isChangeMasterConfigDone(universe, node, isAddMasterOp, ipToUse)) {
      ...
      if (isAddMasterOp) {
        ...
        waitForChangeToComplete(universe, config, node, ipToUse);
      }
      return;
    }
  ...
  private void waitForChangeToComplete(
      Universe universe, YbClientConfig clientConfig, NodeDetails node, String masterIp) {

    boolean waitForMasterConfigChangeCheckEnabled =
        runtimeConfigFactory
            .forUniverse(universe)
            .getBoolean(CHANGE_MASTER_CONFIG_CHECK_ENABLED_PARAM);
    ...
    Duration maxWaitTime =
        runtimeConfigFactory
            .forUniverse(universe)
            .getDuration(CHANGE_MASTER_CONFIG_CHECK_TIMEOUT_PARAM);
