...
public class Universe extends Model {
  ...
  /**
   * Modifies lists of allowed for nodes actions depending on the universe state. Actions are
   * added/removed directly into/from the json representation. Initial values of allowed actions are
   * set by NodeDetails.getAllowedActions().
   *
   * @param universeDetailsJson
   * @param nodes
   */
  void updateNodesDynamicActions(ObjectNode universeDetailsJson, Collection<NodeDetails> nodes) {
  ...
  public static Universe create(UniverseDefinitionTaskParams taskParams, Long customerId) {
    ...
    LOG.debug(
        "Created universe {} with details [{}] with name {}.",
  ...
  private static synchronized Universe readModifyWrite(UUID universeUUID, UniverseUpdater updater)
