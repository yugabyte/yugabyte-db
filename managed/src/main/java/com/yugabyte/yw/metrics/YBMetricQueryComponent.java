  ...
  */
  ...
  private List<ResultSet> queryRunner(
    ...
    if (params.has("exported_instance")) {
      ...
      ResultSet rs =
          cassandraTserverSelectQuery(
              metricName, tserverMap.get(tserverName), session, universe, start, end);
      ...
      for (Entry<String, String> entry : tserverMap.entrySet()) {
        ResultSet rs =
            cassandraTserverSelectQuery(
                metricName, entry.getValue(), session, universe, start, end);
        if (rs != null && rs.iterator().hasNext()) {
          results.add(rs);
        }
      }
   ...
   *
   * @param queryParams, Query params like start, end timestamps, even filters Ex: {"metricKey":
