...
public class CloudProviderController extends AuthenticatedController {
  ...
  private static JsonNode KUBERNETES_CLOUD_INSTANCE_TYPE =
      Json.parse("{\"instanceTypeCode\": \"cloud\", \"numCores\": 0.5, \"memSizeGB\": 1.5}");
  private static JsonNode KUBERNETES_DEV_INSTANCE_TYPE =
      Json.parse("{\"instanceTypeCode\": \"dev\", \"numCores\": 0.5, \"memSizeGB\": 0.5}");
  private static JsonNode KUBERNETES_INSTANCE_TYPES =
      Json.parse(
          "["
  ...
  public Result create(UUID customerUUID) throws IOException {
    Form<CloudProviderFormData> formData =
        formFactory.form(CloudProviderFormData.class).bindFromRequest();
  ...
  private void updateGCPConfig(Provider provider, Map<String, String> config) {
    ...
    try {
      gcpCredentialsFile = accessManager.createCredentialsFile(provider.uuid, Json.toJson(config));
    } catch (IOException e) {
      LOG.error(e.getMessage());
  ...
  public Result setupDocker(UUID customerUUID) {
    ...
    try {
      ...
      regionMetadata.forEach(
          (regionCode, metadata) -> {
            Region region =
                Region.createWithMetadata(newProvider, regionCode, Json.toJson(metadata));
            Arrays.asList("a", "b", "c")
                .forEach(
                    (zoneSuffix) -> {
  ...
  public Result cleanup(UUID customerUUID, UUID providerUUID) {
    ...
    Form<CloudBootstrapFormData> formData =
        formFactory.form(CloudBootstrapFormData.class).bindFromRequest();
    if (formData.hasErrors()) {
      return ApiResponse.error(BAD_REQUEST, formData.errorsAsJson());
    }
  ...
  private Result validateAwsHostedZoneUpdate(Provider provider, String hostedZoneId) {
    ...
    ShellProcessHandler.ShellResponse response =
        dnsManager.listDnsRecord(provider.uuid, hostedZoneId);
    if (response.code != 0) {
      return ApiResponse.error(
