...
public class YBUniverseReconciler extends AbstractReconciler<YBUniverse> {
  ...
  private void rememberResolvedUniverse(YBUniverse ybUniverse, UUID universeUUID) {
    ...
    if (annotations.containsKey(ResourceAnnotationKeys.YBA_RESOURCE_ID)) {
      return;
    }
    annotations.put(ResourceAnnotationKeys.YBA_RESOURCE_ID, universeUUID.toString());
    try {
      resourceClient
      ...
      log.warn(
          "Failed to persist YBA resource ID '{}' annotation to universe resource {}",
          universeUUID,
          metadata.getName(),
          e);
    }
