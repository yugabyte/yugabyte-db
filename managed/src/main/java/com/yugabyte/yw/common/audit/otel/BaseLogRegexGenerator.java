...
@Singleton
public abstract class BaseLogRegexGenerator {

  protected Map<String, LogPrefixTokens> tokensMap =
      Arrays.stream(LogPrefixTokens.values())
          .collect(Collectors.toMap(LogPrefixTokens::getPlaceholder, Function.identity()));

  /**
   * Common method to parse log prefix and generate regex pattern. This extracts the shared logic
   * between audit and query log regex generation.
   */
  protected LogRegexResult parseLogPrefix(String logPrefix, boolean onlyPrefix) {
    String result = "";
    ...
    // Add the specific suffix based on log type
    result += getLogSuffix();
    return new LogRegexResult(result, namedTokens);
  }

  /** Abstract method to be implemented by subclasses to provide the specific log suffix. */
  protected abstract String getLogSuffix();

  /** Common method to output tokens and generate regex patterns. */
