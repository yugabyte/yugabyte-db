...
public class PlacementInfoUtil {
  ...
  public static void updateUniverseDefinition(
    ...
    if (cluster.placementInfo == null && clusterOpType.equals(ClusterOperationType.CREATE)) {
      taskParams.nodeDetailsSet.removeIf(n -> n.isInPlacement(placementUuid));
      cluster.placementInfo =
          getPlacementInfo(
              cluster.clusterType, cluster.userIntent, cluster.userIntent.replicationFactor);
    ...
    boolean primaryClusterEdit =
        cluster.clusterType.equals(PRIMARY) && clusterOpType.equals(ClusterOperationType.EDIT);
    boolean readOnlyClusterEdit =
        cluster.clusterType.equals(ASYNC) && clusterOpType.equals(ClusterOperationType.EDIT);
        ...
        if (numNodesToBeAdded > NodeInstance.listByZone(azUUID, instanceType).size()) {
          LOG.error(
              "Not enough nodes configured for given AZ/Instance type combo, "
                  + "required {} found {} in AZ {} for Instance type {}",
              numNodesToBeAdded,
  ...
  public static void configureNodeEditUsingPlacementInfo(UniverseDefinitionTaskParams taskParams) {
    ...
    Cluster currentCluster =
        taskParams.currentClusterType.equals(PRIMARY)
  ...
  public static String computeMasterAddresses(
    ...
    for (Entry<UUID, Integer> entry : azToNumMasters.entrySet()) {
      ...
      for (int idx = 0; idx < entry.getValue(); idx++) {
        String master =
            String.format(
                "yb-master-%d.yb-masters.%s-%s.%s:%d",
                idx, nodePrefix, az.code, domain, masterRpcPort);
        masters.add(master);
      }
    }
