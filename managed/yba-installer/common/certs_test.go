...
func TestCertificateGeneration(t *testing.T) {
	...
	_, err := ExecuteBashCommand("openssl", []string{"x509", "-in", "/tmp/ca_cert.pem", "-text", "-noout"})
	if err != nil {
		t.Fatalf("Failed to open certificate with openssl (is openssl installed?)")
	}

	_, err = ExecuteBashCommand("openssl", []string{"rsa", "-in", "/tmp/ca_key.pem", "-text", "-noout"})
	if err != nil {
	...
	_, err = ExecuteBashCommand("openssl", []string{"x509", "-in", "/tmp/server_cert.pem", "-text", "-noout"})
	if err != nil {
		t.Fatalf("Failed to open certificate with openssl (is openssl installed?)")
	}

	_, err = ExecuteBashCommand("openssl", []string{"rsa", "-in", "/tmp/server_key.pem", "-text", "-noout"})
	if err != nil {
		t.Fatalf("Failed to open key file with openssl (is openssl installed?)")
	}

	_, err = ExecuteBashCommand("openssl", []string{"verify", "-CAfile", "/tmp/ca_cert.pem", "/tmp/server_cert.pem"})
	if err != nil {
		t.Fatalf("Failed to open key file with openssl (is openssl installed?)")
	}
