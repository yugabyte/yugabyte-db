...
func getFreeBytes(path string) (uint64, error) {
	if _, existsErr := os.Stat(path); existsErr == nil {
		...
		if err != nil {
			return 0, fmt.Errorf("%s is not writeable: %w", path, err)
		}
	...
	if err != nil {
		return 0, fmt.Errorf("No valid parent dir for install dir %s: %w", path, err)
	}

	var stat unix.Statfs_t
	err = unix.Statfs(validParentDir, &stat)
	if err != nil {
		return 0, fmt.Errorf("Cannot read disk availability of %s: %w", validParentDir, err)
	}
