...
create_backup() {
  ...
  cleanup "${tar_name}"
  delete_postgres_backup "${db_backup_path}"
...
restore_backup() {
  ...
  if [[ "${migration}" = true ]]; then
    ...
    $tar_cmd "${input_path}" --directory "${untar_dir}" "${skip_old_files}"
  fi
...
case $command in
  ...
  restore)
    ...
    "$disable_version_check" "$pgrestore_path" "$ybai_data_dir" "$skip_old_files" "$skip_dump_check"
    exit 0
    ;;
  *)
