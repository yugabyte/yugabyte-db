...
def apply_proc(child_table, partman_schema):
    ...
    cur = conn.cursor()
    sql = "SELECT " + partman_schema + ".apply_constraints(%s, %s, %s)"
    debug = False;
    if not args.quiet:
        debug = True
