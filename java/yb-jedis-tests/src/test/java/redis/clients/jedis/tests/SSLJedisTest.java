...
public class SSLJedisTest extends JedisCommandTestBase {
  ...
  private static void setJvmTrustStore(String trustStoreFilePath,
                                       String trustStoreType) {
    assertTrue(String.format("Could not find trust store at '%s'.",
                             trustStoreFilePath),
   ...
   * NOTE: This test relies on a feature that is only available as of Java 7 and
   * later.
   * It is commented out but not removed in case support for Java 6 is dropped
   * or
   * we find a way to have the CI run a specific set of tests on Java 7 and
   * above.
   */
  /*
  @Test
  ...
  createTrustStoreSslSocketFactory();
    ...
    JedisShardInfo shardInfo = new JedisShardInfo(uri, sslSocketFactory,
  sslParameters, null);
  ...
  */
  ...
  public void connectWithShardInfoAndCustomHostnameVerifierByIpAddress() {
    ...
    try {
      ...
      assertEquals(
          "The JedisConnectionException does not contain the expected message.",
          "The connection to '127.0.0.1' failed ssl/tls hostname verification.",
          e.getMessage());
    }
  ...
  public void connectWithShardInfoAndEmptyTrustStore() throws Exception {
    ...
    try {
      ...
      assertEquals("Unexpected third inner exception.",
