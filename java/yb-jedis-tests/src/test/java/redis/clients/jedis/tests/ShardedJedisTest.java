...
public class ShardedJedisTest extends BaseYBClassForJedis {
  private static HostAndPort redis1;
  private static HostAndPort redis2;

  @Before
  public void initializeHosts() {
    redis1 = HostAndPortUtil.getRedisServers().get(0);
    redis2 = HostAndPortUtil.getRedisServers().get(1);
  }
   ...
   */
  @Test
  @Ignore public void testAvoidLeaksUponDisconnect() throws InterruptedException {
  ...
  public void checkKeyTags() {
    ...
    shards.add(new JedisShardInfo(redis2.getHost(), redis2.getPort()));
    ShardedJedis jedis =
        new ShardedJedis(shards, ShardedJedis.DEFAULT_KEY_TAG_PATTERN);
  ...
  public void testMasterSlaveShardingConsistency() {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      JedisShardInfo jedisShardInfo2 =
          sharded2.getShardInfo(Integer.toString(i));
  ...
  public void testMasterSlaveShardingConsistencyWithShardNaming() {
    List<JedisShardInfo> shards = new ArrayList<JedisShardInfo>(3);
    shards.add(
        new JedisShardInfo("localhost", Protocol.DEFAULT_PORT, "HOST1:1234"));
    shards.add(new JedisShardInfo("localhost", Protocol.DEFAULT_PORT + 1,
                                  "HOST2:1234"));
    shards.add(new JedisShardInfo("localhost", Protocol.DEFAULT_PORT + 2,
                                  "HOST3:1234"));
    ...
    List<JedisShardInfo> otherShards = new ArrayList<JedisShardInfo>(3);
    otherShards.add(
        new JedisShardInfo("otherhost", Protocol.DEFAULT_PORT, "HOST2:1234"));
    otherShards.add(new JedisShardInfo("otherhost", Protocol.DEFAULT_PORT + 1,
                                       "HOST3:1234"));
    otherShards.add(new JedisShardInfo("otherhost", Protocol.DEFAULT_PORT + 2,
                                       "HOST1:1234"));
