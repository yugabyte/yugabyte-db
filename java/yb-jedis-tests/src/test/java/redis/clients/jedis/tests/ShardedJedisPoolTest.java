...
import static org.yb.AssertionWrappers.assertNotSame;
...
public class ShardedJedisPoolTest extends BaseYBClassForJedis {
  ...
  @Test
  @Ignore public void checkConnections() {
  ...
  @Test
  @Ignore public void checkConnectionWithDefaultPort() {
    ShardedJedisPool pool = new ShardedJedisPool(new GenericObjectPoolConfig(), shards);
    ShardedJedis jedis = pool.getResource();
    jedis.set("foo", "bar");
    assertEquals("bar", jedis.get("foo"));
  ...
  @Test
  @Ignore public void checkJedisIsReusedWhenReturned() {
    ShardedJedisPool pool = new ShardedJedisPool(new GenericObjectPoolConfig(), shards);
    ShardedJedis jedis = pool.getResource();
    jedis.set("foo", "0");
    pool.returnResource(jedis);
  ...
  @Test
  @Ignore public void checkPoolRepairedWhenJedisIsBroken() {
    ShardedJedisPool pool = new ShardedJedisPool(new GenericObjectPoolConfig(), shards);
    ShardedJedis jedis = pool.getResource();
    jedis.disconnect();
  ...
  @Ignore public void checkPoolOverflow() {
  ...
  @Test
  @Ignore public void shouldNotShareInstances() {
    GenericObjectPoolConfig config = new GenericObjectPoolConfig();
  ...
  @Test
  @Ignore public void checkFailedJedisServer() {
    ShardedJedisPool pool = new ShardedJedisPool(new GenericObjectPoolConfig(), shards);
    ShardedJedis jedis = pool.getResource();
    jedis.incr("foo");
    pool.returnResource(jedis);
  ...
  @Test
  @Ignore public void shouldReturnActiveShardsWhenOneGoesOffline() {
  ...
  @Ignore public void startWithUrlString() {
    ...
    shards.add(new JedisShardInfo("redis://:foobared@" + redis2.getHost()
                + ":" + redis2.getPort() + ""));
    shards.add(new JedisShardInfo("redis://:foobared@" + redis1.getHost()
                + ":" + redis1.getPort() + ""));
  ...
  @Ignore public void startWithUrl() throws URISyntaxException {
    ...
    List<JedisShardInfo> shards = new ArrayList<JedisShardInfo>();
    shards.add(new JedisShardInfo(new URI("redis://:foobared@" + redis2.getHost()
                    + ":" + redis2.getPort() + "")));
    shards.add(new JedisShardInfo(new URI("redis://:foobared@" + redis1.getHost()
                    + ":" + redis1.getPort() + "")));
  ...
  @Test
  @Ignore public void returnResourceShouldResetState() throws URISyntaxException {
    GenericObjectPoolConfig config = new GenericObjectPoolConfig();
    config.setMaxTotal(1);
    config.setBlockWhenExhausted(false);
  ...
  @Test
  @Ignore public void checkResourceIsCloseable() throws URISyntaxException {
    GenericObjectPoolConfig config = new GenericObjectPoolConfig();
    config.setMaxTotal(1);
    config.setBlockWhenExhausted(false);
