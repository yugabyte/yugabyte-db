...
public class CDCSubscriber {
  ...
  private void createStreamUtil(String cpType) throws Exception {
    ...
    if (dbStreamId == null || dbStreamId.isEmpty()) {
      ...
      dbStreamId = syncClient.createCDCStream(table,
        "yugabyte", FORMAT, checkpointingType).getStreamId();
  ...
  /**
   * This function is used to parse the GetChangesResponse in case of Json records
   * and add it to the provided List in order for further processing
   *
   * @param response The GetChangesResponse we get on calling getChangesCDCSDK
   * @param records a List to add the records to after parsing it from response
   * @see GetChangesResponse
   * @see CdcService.CDCSDKRecordPB
   * @see List
   */
  private void addJsonRecords(GetChangesResponse response,
                              List<CdcService.CDCSDKRecordPB> records) {
    for (CdcService.CDCSDKRecordPB record : response.getResp().getCdcSdkRecordsList()) {
      try {
        records.add(record);
      } catch (Exception e) {
        LOG.error("Exception caught while adding records, cannot add records", e);
        e.printStackTrace();
        fail();
      }
    }
   ...
   * <strong>default record format as "json"</strong> and if a record format is specified then
  ...
  public void getSnapshotFromCDC(List records) throws Exception {
    ...
    for (LocatedTablet tablet : locatedTablets) {
      ...
      LOG.debug(String.format("Polling for tablet: %s", tabletId));
    ...
    for (String tabletId : tabletIds) {
      ...
      if (FORMAT.equalsIgnoreCase("PROTO")) {
        ...
        // add records json
        addJsonRecords(changesResponse, records);
      }
    }
  }

  public void getSnapshotFromCheckpoint(List records, long term, long index) throws Exception {
    List<LocatedTablet> locatedTablets = table.getTabletsLocations(30000);
    List<String> tabletIds = new ArrayList<>();
