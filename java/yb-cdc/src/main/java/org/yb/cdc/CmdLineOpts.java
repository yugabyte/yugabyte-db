...
public class CmdLineOpts {
    ...
    String configFileHelpMessage = "\nCreate a config.properties file with the" +
            ...
            "pass its path in config"
                    ...
                    .concat("\tstream.id=<stream-id-if-any>" + lineSeparator +
                            ...
                            "leave this empty)")
                    .concat(lineSeparator)
                    .concat("\tformat=<json/proto>")
                    .concat(lineSeparator)
                    .concat("\t default is proto")
                    .concat(lineSeparator)
                    .concat(lineSeparator)
    ...
    String helpMessage = lineSeparator
            ...
            .concat("    Table to capture change from, in format <namespace>.<table>")
            .concat(lineSeparator)
            ...
            .concat(lineSeparator)
            .concat("  --format").concat(lineSeparator)
            .concat("    Flag to specify record format (proto/json), default is proto")
            .concat(lineSeparator)
    ...
    public static CmdLineOpts createFromArgs(String[] args) throws Exception {
        ...
        // option to pass the config path from CommandLine
        options.addOption("config_file", true,
                "Path to config.properties file");

        // option to specify whether to create a new cdc db stream id
        ...
        options.addOption("format", true,
                "Option to specify record format");
        ...
        // options to provide client certificate and key files
        ...
        // provide the maximum number of tablets the client can poll for
    ...
    public void initialize(CommandLine commandLine) throws Exception {
        if (commandLine.hasOption("help")) {
            // if help option is there, the program would simply display the help and exit
            LOG.info(helpMessage);
            System.exit(0);
        }

        if (commandLine.hasOption("show_config_help")) {
            // if this option is there the program would display the config example and exit
        ...
        if (commandLine.hasOption("table_name")) {
            ...
            if (fullTableName.length != 2) {
                throw new Exception(String.format("Expected a namespace and table" +
                        " name for --table_name in" +
        ...
        if (commandLine.hasOption("config_file")) {
            ...
            if (commandLine.hasOption("create_new_db_stream_id")) {
                ...
                // would be created
                // even when the config.properties file contains a stream id
                // the default behaviour in case this option is not passed would be to take whatever
                // there is in the config file, if the stream id is empty
                // then a new one would be created
                // and if there is a pre-existing stream id in the file, it would be used
            ...
            if (commandLine.hasOption("format")) {
                propConfig.setProperty("format", commandLine.getOptionValue("format"));
            } else {
                if (propConfig.getProperty("format").toString().isEmpty()) {
