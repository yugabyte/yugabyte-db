...
public class LogClient implements OutputClient {
  ...
  public void applyChange(YBTable table, CdcService.CDCSDKRecordPB changeRecord) throws Exception {
    LOG.info(changeRecord.toString());
    ...
    LOG.info(String.format("Inserts: %d, Updates: %d, Deletes: %d, Writes: %d, " +
        "Snapshot Records: %d", inserts, updates, deletes, writes, snapshotRecords));
  }
}
