...
public class TestMisc extends BaseYsqlConnMgr {
  ...
  @Test
  public void testStickyCreateType() throws Exception {
    ResultSet rs;
    ...
    try (Connection conn = getConnectionBuilder()
      ...
      // The shell type query will make the session sticky.
      stmt.execute("CREATE TYPE shell");
      for (int i = 0; i < 10; i++) {
        rs = stmt.executeQuery("SELECT pg_backend_pid()");
        assertTrue(rs.next());
        pids.add(rs.getInt(1));
      ...
      stmt.execute("CREATE TYPE int42");
      // Make dummy I/O routines using the existing internal support for int4, text
      stmt.execute("CREATE FUNCTION int42_in(cstring)\n" + //
                "   RETURNS int42\n" + //
                "   AS 'int4in'\n" + //
                "   LANGUAGE internal STRICT IMMUTABLE");

      stmt.execute("CREATE FUNCTION int42_out(int42)\n" + //
                "   RETURNS cstring\n" + //
                "   AS 'int4out'\n" + //
                "   LANGUAGE internal STRICT IMMUTABLE");
    }
    ...
    try (Connection conn = getConnectionBuilder()
                    ...
                    .connect();
                    Statement stmt = conn.createStatement()) {
          // The base type of create type query will make the session sticky.
          stmt.execute("CREATE TYPE int42 (\n" + //
                        "   internallength = 4,\n" + //
                        "   input = int42_in,\n" + //
                        "   output = int42_out,\n" + //
    ...
    try (Connection conn = getConnectionBuilder()
        ...
        // Range type
        stmt.execute("CREATE TYPE intrange as range (subtype = int)");
        for (int i = 0; i < 10; i++) {
          rs = stmt.executeQuery("SELECT pg_backend_pid()");
          assertTrue(rs.next());
          pids.add(rs.getInt(1));
        ...
        // Composite type
        stmt.execute("CREATE TYPE addr_type as (street text, city text)");
        for (int i = 0; i < 10; i++) {
          rs = stmt.executeQuery("SELECT pg_backend_pid()");
          assertTrue(rs.next());
          pids.add(rs.getInt(1));
