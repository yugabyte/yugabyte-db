...
import org.yb.client.ColumnRangePredicate;
import org.yb.client.YBTable;
import org.yb.client.Operation;
...
/**
 * Utility class to setup MR jobs that use YB as an input and/or output.
 */
...
public class YBTableMapReduceUtil {
  ...
  private static abstract class AbstractMapReduceConfigurator<S> {
    ...
    /**
     * Sets whether this job should add YB's dependencies to the distributed cache. Turned on
     * by default.
  ...
  /**
   * Builder-like class that sets up the required configurations and classes to write to YB.
   * <p>
  ...
  /**
   * Builder-like class that sets up the required configurations and classes to read from YB.
  ...
  /**
   * Add the YB dependency jars as well as jars for any of the configured
   * job classes to the job configuration, so that JobClient will ship them
   * to the cluster and add them to the DistributedCache.
   */
  public static void addDependencyJars(Job job) throws IOException {
    addYBDependencyJars(job.getConfiguration());
    try {
  ...
  /**
   * Add YB and its dependencies (only) to the job configuration.
   * <p>
   * This is intended as a low-level API, facilitating code reuse between this
   * class and its mapred counterpart. It also of use to external tools that
   * need to build a MapReduce job that interacts with YB but want
   ...
   */
  public static void addYBDependencyJars(Configuration conf) throws IOException {
    addDependencyJars(conf,
        // explicitly pull a class from each module
        Operation.class,                      // yb-client
