...
public class TestDocumentDBGateway extends BasePgSQLTest {
  ...
  private static final int BASE_GATEWAY_PORT = 10260;
  ...
  public void setUp() throws Exception {
    if (!extensionCreated) {
      ...
      // until the extension schemas exist. Kill the gateway processes so that the
      // postmaster restarts them (bgw_restart_time=1s) with the schemas now available.
  ...
  /**
   * Kill the DocumentDB Gateway background worker processes so the postmaster restarts them.
   * This is needed after CREATE EXTENSION because the gateway starts before the extension
   * schemas exist and caches the failure. The bgworker is registered with restart_time=1s,
   * so the postmaster will automatically restart it after a SIGKILL crash.
   * (SIGTERM via pg_terminate_backend causes a clean exit which does not trigger restart.)
   */
  private void restartGatewayWorkers() throws Exception {
    ...
    // Wait for the postmaster to detect the crash and restart the bgworkers.
    Thread.sleep(5000);
    // Reconnect since the postmaster restarts all backends after a bgworker crash.
