...
public class TestPgBatch extends BasePgSQLTest {
  ...
  protected void testSyncInLongBatchHelper(
    ...
    try (Connection c = getConnectionBuilder().connect();
      ...
      // In v2.18, RC doesn't advance the read time on DocDB, so the "conflicts with committed
      // transaction" error is returned, and we're unable to retry in batch mode.
      // In RR, there is a single read time for the entire transaction and thus the transaction
      // cannot be re-applied.
      assertNotEquals(isolationLevel, SR);
      return;
    }
    // In v2.18, we skip the BatchUpdateException with single-row update statements in the
    // following situations:
    //  * Serializable isolation always
    //  * RC, if we received a SYNC message, which is after statement 255
    if (conflictingStatement == 256) {
      assertTrue(isolationLevel == SR || isolationLevel == RC);
    } else {
