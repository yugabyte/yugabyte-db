...
public class TestSchemaVersionMismatch extends BasePgSQLTest {
  ...
  private static final String updateSqlTemplate =
      "WITH updated AS (" + "  UPDATE todo " + "  SET status = false " + "  WHERE id = %s "
          + "  RETURNING * " + ") " + "SELECT pg_sleep(1), * FROM updated;";

  private static final String updateSqlWithPlaceholder = String.format(updateSqlTemplate, "?");
  private static final String updateSqlWithUuid =
      String.format(updateSqlTemplate, "'" + uuid + "'");
  ...
  private void setupTestTable(Statement stmt) throws SQLException {
    ...
    stmt.execute(
        "CREATE TABLE todo (" + "    id         uuid PRIMARY KEY," + "    task       VARCHAR(255),"
            + "    status     boolean," + "    created_at timestamptz default now()" + ");");
    ...
    for (int i = 0; i < 100; i++) {
      ...
      insertBuilder.append(
          String.format("('%s', 'task %d', %b)", UUID.randomUUID().toString(), i + 2, i % 2 == 0));
    }
    insertBuilder.append(";");
  ...
  public void testBasic() throws Exception {
    ...
    try (Connection simpleModeConn = getConnectionBuilder().withPreferQueryMode("simple").connect();
      ...
      assertGreaterThan("Expected at least one schema version mismatch error",
          numExpectedErrors.get(), 0);
    }
  }
   ...
   */
  private Thread createExtendedDmlThread(Connection conn, String sqlWithPlaceholder,
      long duration_ms, boolean use_txn_block) {
    return new Thread(() -> {
   ...
   */
  private Thread createExtendedDmlThreadWithRetry(Connection conn, String sqlWithPlaceholder,
      long duration_ms) {
    return new Thread(() -> {
      long endTime = System.currentTimeMillis() + duration_ms;
      while (System.currentTimeMillis() < endTime) {
  ...
  private void processError(String threadType, SQLException e) {
    ...
    // statement changes.
    if ("40001".equals(e.getSQLState()) || "25P02".equals(e.getSQLState())
        || "0A000".equals(e.getSQLState()) || e.getMessage().contains("Invalid column number")
