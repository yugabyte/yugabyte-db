...
void CDCServiceImpl::UpdateMetrics() {
  std::unordered_set<TabletStreamInfo, TabletStreamInfo::Hash> latest_tablet_stream_entries;
  TabletInfoToLastReplicationTimeMap cdc_state_tablets_to_last_replication_time;
  ...
  if (last_seen_tablet_stream_entries_) {
    ...
    last_seen_tablet_stream_entries_ =
        std::make_shared<std::unordered_set<TabletStreamInfo, TabletStreamInfo::Hash>>();
  }
...
Result<bool> CDCServiceImpl::CheckBeforeImageActive(
  ...
  if (FLAGS_ysql_yb_enable_replica_identity && IsReplicationSlotStream(stream_metadata)) {
    ...
    // are present in the stream metadata's replica identity map. If before image is active for any
    // such tables then we should return true.
