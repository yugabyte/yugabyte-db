...
class CDCServiceImpl::Impl {
 public:
  ...
  void AddTabletCheckpoint(
      OpId op_id, const CDCStreamId& stream_id, const TabletId& tablet_id) {
...
bool CheckKeyExists(const QLMapValuePB& map_value, const std::string& key) {
  ...
  if (!req->has_tablet_id() || req->tablet_id().empty()) {
    ...
    CDCSDKCheckpointPB parent_checkpoint_pb;
    {
      ...
      auto result = GetLastCDCSDKCheckpoint(
          req->table_info().stream_id(), req->tablet_id(), session, CDCRequestSource::CDCSDK);
      RPC_RESULT_RETURN_ERROR(result, resp->mutable_error(), CDCErrorPB::INTERNAL_ERROR, context);
    ...
    for (const auto& child_tablet_id : child_tablet_ids) {
      ...
      auto result = GetLastCDCSDKCheckpoint(
          req->table_info().stream_id(), child_tablet_id, session, CDCRequestSource::CDCSDK);
      RPC_RESULT_RETURN_ERROR(result, resp->mutable_error(), CDCErrorPB::INTERNAL_ERROR, context);
...
Result<TabletIdCDCCheckpointMap> CDCServiceImpl::PopulateTabletCheckPointInfo(
  ...
  for (const auto& row : client::TableRange(**cdc_state_table_result, options)) {
    ...
    if (!row.column(4).IsNull()) {
      ...
      if (safe_time_result.ok() && (record.GetRecordType() == CDCRecordType::ALL ||
                                    CheckKeyExists(map_value, kCDCSDKSnapshotKey))) {
    ...
    if (op_id == OpId::Min() && last_replicated_time_str.empty() &&
...
Status CDCServiceImpl::GetTabletIdsToPoll(
  ...
  for (const auto& row : rows) {
    ...
    checkpoint_pb.set_term(checkpoint_result->term);
    checkpoint_pb.set_index(checkpoint_result->index);
    ...
    if (!row.column(4).IsNull()) {
      ...
      if (snapshot_time_result.ok()) {
        checkpoint_pb.set_snapshot_time(snapshot_time_result.get());
      } else {
        checkpoint_pb.set_snapshot_time(0);
      }
    }
...
void CDCServiceImpl::BootstrapProducer(
  ...
  XClusterProducerBootstrap xcluster_producer_bootstrap(
      this, *req, resp, &creation_state, context_.get());
