...
TEST_F_EX(
    YbAdminSnapshotScheduleTest, YB_DISABLE_TEST_IN_SANITIZERS(SysCatalogRetentionWithClone),
    YbAdminSnapshotScheduleTestWithYsqlRetention) {
  auto schedule_id = ASSERT_RESULT(PreparePg(YsqlColocationConfig::kNotColocated, 30s, 600s));
  auto conn = ASSERT_RESULT(PgConnect(client::kTableName.namespace_name()));
  // Create 10 tables.
...
TEST_F(YbAdminSnapshotScheduleTestWithYsql, ListScheduleAfterDatabaseRename) {
  ...
  auto& options = ASSERT_RESULT(GetMember(schedule, "options")).get();
  ...
  auto& new_options = ASSERT_RESULT(GetMember(schedule, "options")).get();
