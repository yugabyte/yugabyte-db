...
// The hazard the two-tier record exists to avoid: a merged chunk derives its own scale, so
// decoding int8 would feed already-quantized values into a fresh quantization and lose more on
// every merge. The rerank copy round-trips exactly.
TEST_F(YbHnswStorageWrapperTest, IterationReadsTheRerankCopy) {
  ...
  for (const auto& entry : *index) {
    ...
    // Exact float16 equality, not a tolerance: an int8 decode would be off by up to half a
    // quantization step, orders of magnitude more.
    std::vector<std::byte> narrowed(
        vector_index::CoordinateBytes(VectorStorageKind::kFloat16, dimensions_));
    vector_index::NarrowCoordinates(
...
// Feeding iteration's output back through a fresh chunk models a merge; repeating it would
// expose compounding error.
TEST_F(YbHnswStorageWrapperTest, Int8SurvivesRepeatedCompaction) {
  ...
  for (size_t generation = 0; generation != kGenerations; ++generation) {
    ...
    if (generation != 0) {
      // Bit-identical across generations is the fixed point. Per id so a failure names the
      // vector that drifted rather than dumping the chunk.
    ...
    // Built from stored records rather than the originals, as a real compaction does.
    for (size_t i = 0; i != kNumVectors; ++i) {
...
// Distance() feeds VectorLSM's comparison of in-flight vectors against chunk results, so it must
// be in the metric's units; the int8 metric returns quantized units, meaningful in one chunk.
TEST_F(YbHnswStorageWrapperTest, Int8DistanceUsesTheRerankMetric) {
  ...
  for (size_t i = 0; i != 50; ++i) {
    ...
    // So within float16's error of full precision, not int8's.
    ASSERT_LE(std::fabs(i8->Distance(lhs, rhs) - exact) / exact, 0.01) << "pair " << i;
    ASSERT_EQ(i8->Distance(lhs, lhs), 0.0f) << "pair " << i;
  }
}

// Two chunks of one index hold different scales, so a query quantized with a cached or shared
// scale would decode one of them wrong.
TEST_F(YbHnswStorageWrapperTest, ScalesDifferAcrossChunks) {
  ...
  // A query from a chunk's own range finds its exact match at zero only if quantized with that
  // chunk's scale.
  ...
  // Reranking puts both into the metric's units regardless of scale, so VectorLSM can merge.
...
// Why the encoding is shippable: int8 traversal alone loses recall; reranking recovers it.
TEST_F(YbHnswStorageWrapperTest, Int8SearchAgreesWithFullPrecision) {
  ...
  // float16 is the ceiling. That the rerank ran is asserted below by the distance's units, not
  // by a recall gap against `bare`, which depends on the data.
  ASSERT_GE(i8_overlap, f16_overlap - 0.02);

  // Proves the rerank ran: each distance must be exactly what the float16 index gives, which is
  // what MakeResult computes. Without the tier the quantized distance is reported.
