...
// The compaction hazard the two-tier record exists to avoid.
//
// Compaction reads source chunks through this iterator and re-inserts what it yields into a new
// chunk, which derives its own quantization scale. Decoding the int8 coordinates would therefore
// feed already-quantized values into a fresh quantization, losing a little more on every merge
// until the index degrades to noise. The rerank copy round-trips exactly, so reading it instead
// makes a vector's value a fixed point across arbitrarily many compactions.
TEST_F(YbHnswStorageWrapperTest, IterationReadsTheRerankCopy) {
  ...
  for (const auto& [vector_id, vector] : *index) {
    ...
    // What iteration yields must be the float16 round trip of the original, not the int8 one.
    // Asserting equality against float16 rather than a loose tolerance is what distinguishes the
    // two: an int8 decode would be off by up to half a quantization step, orders of magnitude
    // more than float16's error.
    std::vector<std::byte> narrowed(
        vector_index::CoordinateBytes(VectorStorageKind::kFloat16, dimensions_));
    vector_index::NarrowCoordinates(
