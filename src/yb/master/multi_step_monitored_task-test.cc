...
class TraceObservingCatalogEntityTask : public MultiStepCatalogEntityTask {
 public:
  ...
  dist_trace::trace::SpanContext first_step_context_ = dist_trace::trace::SpanContext::GetInvalid();
  dist_trace::trace::SpanContext second_step_context_ =
...
class CatalogEntityTaskTraceTest : public YBTest {
 protected:
  void SetUp() override {
    YBTest::SetUp();
    ANNOTATE_UNPROTECTED_WRITE(FLAGS_otel_collector_traces_endpoint) =
...
TEST_F(CatalogEntityTaskTraceTest, NoTraceContextCarriedWhenNoneActive) {
  ...
  ASSERT_FALSE(task->first_step_context_.IsValid());
  ASSERT_FALSE(task->second_step_context_.IsValid());
}
