...
DEFINE_int32(load_balancer_max_concurrent_tablet_remote_bootstraps, -1,
...
DEFINE_int32(load_balancer_max_inbound_remote_bootstraps_per_tserver, 4,
...
void ClusterLoadBalancer::RunLoadBalancerWithOptions(Options* options) {
  ...
  for (const auto& table : GetTableMap()) {
    ...
    if (it == per_table_states_.end()) {
      ...
      VLOG(2) << "Unable to find the state for table " << table.first << ". Skipping.";
      continue;
    }
    ...
    if (!handle_analyze_tablets.ok()) {
      LOG(WARNING) << "Skipping load balancing for table " << table.first << ": "
  ...
  for (const auto& table : GetTableMap()) {
    ...
    if (it == per_table_states_.end()) {
      ...
      VLOG(2) << "Unable to find table state for table " << table.first
    ...
    VLOG(2) << "Per table state for table: " << table.first << ", " << state_->ToString();
    VLOG(2) << "Global state: " << table.first << ", " << state_->ToString();
    VLOG(2) << "Sorted load: " << table.first << ", " << GetSortedLoad();
    VLOG(2) << "Global load: " << table.first << ", " << GetSortedLeaderLoad();
      ...
      for (; remaining_removals > 0; --remaining_removals) {
        if (state_->allow_only_leader_balancing_) {
          YB_LOG_EVERY_N_SECS_OR_VLOG(INFO, 30, 2)
              << "Skipping removing replicas. Only leader balancing table " << table.first;
          break;
        }
        auto handle_remove = HandleRemoveReplicas(&out_tablet_id, &out_from_ts);
        if (!handle_remove.ok()) {
          LOG(WARNING) << "Skipping removing replicas for " << table.first << ": "
    ...
    for ( ; remaining_adds > 0; --remaining_adds) {
      if (state_->allow_only_leader_balancing_) {
        YB_LOG_EVERY_N_SECS_OR_VLOG(INFO, 30, 2)
            << "Skipping adding replicas. Only leader balancing table " << table.first;
        break;
      }
    ...
    VLOG(2) << "Remaining leader moves for table " << table.first << " is "
            << table_remaining_leader_moves;
