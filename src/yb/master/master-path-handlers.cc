...
DEFINE_uint64(master_maximum_heartbeats_without_lease, 10,
    "Deprecated - use leaderless_tablet_alert_delay_secs instead."
    "After this number of heartbeats without a valid lease for a tablet, treat it as leaderless.");

DEFINE_int32(leaderless_tablet_alert_delay_secs, 2 * 60,
    "From master's view, if the tablet doesn't have a valid leader for this amount of seconds, "
    "alert it as a leaderless tablet.");
TAG_FLAG(leaderless_tablet_alert_delay_secs, runtime);
      ...
      *output << "<table class='table table-borderless table-hover'>\n";
      for (const auto& table : tserver_tree[tserver]) {
        ...
        *output << Substitute("<td><h4><a href='http://$0/table?id=$1'>"
                              ...
                              EscapeForHtmlToString(HostPortPBToString(reg.http_addresses(0))),
...
string MasterPathHandlers::TSDescriptorToHtml(const TSDescriptor& desc,
  ...
  if (reg.common().http_addresses().size() > 0) {
    return Substitute(
        ...
        EscapeForHtmlToString(HostPortPBToString(reg.common().http_addresses(0))),
