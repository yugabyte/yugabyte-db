...
Status CatalogManager::AbortTableCreation(TableInfo* table,
  ...
  if (indexed_table_write_lock) {
    indexed_table_write_lock->Unlock();
  }
...

  {
    ...
    if (PREDICT_FALSE(!s.ok())) {
      return AbortTableCreation(
          ...
          resp, nullptr /* indexed_table_write_lock */);
    }
...
Status CatalogManager::CreateTable(const CreateTableRequestPB* orig_req,
  ...
  if (PREDICT_FALSE(!s.ok())) {
    return AbortTableCreation(table.get(), tablets, s, resp, &indexed_table_write_lock);
  }
