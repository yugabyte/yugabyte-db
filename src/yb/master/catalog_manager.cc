...
DEFINE_bool(enable_tablet_split_of_cdcsdk_streamed_tables, false,
...
DEFINE_bool(master_join_existing_universe, false,
...
void CatalogManager::LoadSysCatalogDataTask() {
  ...
  SysCatalogLoaded(term, std::move(state));
  ...
  for (auto& [table_id, indexes] : indexes_map) {
    ...
    GetBackfillStatus(
        ...
        [this, tables_to_persist = DCHECK_NOTNULL(tables_to_persist)](
          ...
          auto index_table = CHECK_RESULT(FindTableById(index_id));
