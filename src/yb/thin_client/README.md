...
`yb-tserver`'s `yb.tserver.ThinClientService.Perform` RPC via
`ThinClientServiceProxy`. It is meant to be built here, shipped as a shared
...
closure it links (the deploy set) is the `ThinClientServiceProxy` closure
(`thin_client_proto`, `pg_client_proto`, `yb_common`, `yb_dockv`, `yrpc`, `yb_util` + transitive proto
