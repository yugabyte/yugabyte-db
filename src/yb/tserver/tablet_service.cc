...
DEFINE_test_flag(int32, TEST_leader_stepdown_delay_ms, 0,
...
void TabletServiceImpl::Read(const ReadRequestPB* req,
  ...
  if (FLAGS_simulate_time_out_failures_msecs > 0 && RandomUniformInt(0, 10) < 2) {
    LOG(INFO) << "Marking request as timed out for test: " << req->ShortDebugString();
    SleepFor(MonoDelta::FromMilliseconds(FLAGS_simulate_time_out_failures_msecs));
