...
void MaybePauseReadWithPagingStateForTesting(const PgPerformRequestPB& req) {
...
DeferReadPoint GetDeferReadPoint(const Req& req) {
  return DeferReadPoint(req.options().defer_read_point());
}
 ...
 public:
  ...
  Status DoAcquireObjectLock(const ObjectLockQueryDataPtr& data, CoarseTimePoint deadline) {
    ...
    if (data->req.is_session_lock()) {
      ...
      RETURN_NOT_OK(SetupSession(options, deadline, GetInTxnLimit(options, clock().get())));
    ...
    if (IsTableLockTypeGlobal(lock_type)) {
      ...
      auto lock_req = AcquireRequestFor<master::AcquireObjectLocksGlobalRequestPB>(
          ...
          txn_meta_res->status_tablet, opt_bg_txn_meta);
  ...
  Result<SetupSessionResult> SetupSession(
    ...
    if (options.use_catalog_session()) {
      ...
      RETURN_NOT_OK(GetDdlTransactionMetadata(
          ...
          IsTxnUsingTableLocks(options.is_using_table_locks()),
          DeferReadPoint(options.defer_read_point()), options.priority(),
