...
DEFINE_uint64(tablet_validator_retain_delete_markers_validation_period_sec, 60,
...
TAG_FLAG(tablet_validator_retain_delete_markers_validation_period_sec, runtime);

DEFINE_test_flag(int32, tablet_validator_cache_retention_interval_sec, 3600,
    "Used in tests to indicate the time period in seconds "
    "to retain tablet validator cache's entries.");

DEFINE_test_flag(uint64, tablet_validator_max_tables_number_per_rpc, 50,
...
std::chrono::milliseconds SanitizeRetainDeleteMarkersValidationPeriod(const uint64_t value) {
...
void TabletMetadataValidator::Impl::TriggerMetadataUpdate(
  ...
  for (const auto& [index_tablet_id, index_tables] : indexes_for_update) {
    auto result = tablet_manager_.LookupTablet(index_tablet_id);
    if (!result.ok()) {
