...
class TabletServerIf : public LocalTabletServer {
 public:
  ...
  // is enabled, read from the cache instead of actual table. Stale-tolerant callers (Read RPCs)
  // can opt in for the fast path; callers that need authoritative versions (e.g.
  // WaitForYsqlBackends catalog version) must leave it false. Tserver implementations ignore
  // this parameter and always read their local shared-memory view.
