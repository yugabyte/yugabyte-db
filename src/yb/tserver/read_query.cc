...
Status ReadQuery::DoPerform() {
  ...
  if (serializable_isolation || has_row_mark) {
    std::shared_ptr<tserver::WriteResponsePB> response;
    const bool use_async_write = req_->use_async_write();
    if (use_async_write) {
      response = std::make_shared<tserver::WriteResponsePB>();
    }
...
Status ReadQuery::Complete() {
  for (;;) {
    ...
    if (!allow_retry_) {
      ...
      resp_->set_restart_read_key(
          dockv::SubDocKey::DebugSliceToString(result.key.AsSlice()));
      break;
    }
