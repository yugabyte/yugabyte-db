...
client::YBqlWriteOpPtr NewSetMutationsOp(
    const client::TableHandle& table, client::YBSession& session, const TableId& table_id,
    int64_t mutations) {
...
client::YBqlWriteOpPtr NewSubtractMutationsOp(
    ...
    const PgAutoAnalyzeMutationSnapshot& snapshot) {
  auto update_op = table.NewWriteOp(session.arena(), QLWriteRequestPB::QL_STMT_UPDATE);
  auto* const update_req = update_op->mutable_request();
  const auto mutations_col_id = MutationsColumnId(table);
...
Status ResetPgAutoAnalyzeMutationCounts(
  ...
  for (const auto& table_id : table_ids) {
    auto update_op = NewSetMutationsOp(table, session, table_id, 0);
...
Status SaturatingSubtractPgAutoAnalyzeMutationCounts(
  ...
  for (const auto& snapshot : snapshots) {
    ...
    auto reset_op = NewSetMutationsOp(table, session, snapshot.table_id, 0);
    ...
    auto subtract_op = NewSubtractMutationsOp(table, session, snapshot);
    AddExistsAndMutationsCondition(
