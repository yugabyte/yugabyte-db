...
#define YB_PG_CLIENT_AFTER_MOCKABLE_METHODS \
    (Perform) \
    (AcquireObjectLock) \
    /**/
...
#define YB_PG_CLIENT_MOCK_METHOD_SETTER_DECLARE(r, data, method) \
  [[nodiscard]] Handle BOOST_PP_CAT(Mock, method)( \
      const std::function<Status( \
          ...
          BOOST_PP_CAT(BOOST_PP_CAT(Pg, method), ResponsePB)*, PgClientMockCallContext*)>& mock);
...
#define YB_PG_CLIENT_MOCK_BEFORE_SETTER_DECLARE(r, data, method) \
  [[nodiscard]] Handle BOOST_PP_CAT(BOOST_PP_CAT(Mock, method), Before)( \
      const std::function<Status( \
          const BOOST_PP_CAT(BOOST_PP_CAT(Pg, method), RequestPB)*, \
          BOOST_PP_CAT(BOOST_PP_CAT(Pg, method), ResponsePB)*, PgClientMockCallContext*)>& before);

  BOOST_PP_SEQ_FOR_EACH(
      YB_PG_CLIENT_MOCK_BEFORE_SETTER_DECLARE, ~, YB_PG_CLIENT_MOCKABLE_METHODS);
...
#define YB_PG_CLIENT_MOCK_AFTER_SETTER_DECLARE(r, data, method) \
  [[nodiscard]] Handle BOOST_PP_CAT(BOOST_PP_CAT(Mock, method), After)( \
      const std::function<Status( \
          const BOOST_PP_CAT(BOOST_PP_CAT(Pg, method), RequestPB)*, \
          BOOST_PP_CAT(BOOST_PP_CAT(Pg, method), ResponsePB)*, PgClientMockCallContext*)>& after);

  BOOST_PP_SEQ_FOR_EACH(
      YB_PG_CLIENT_MOCK_AFTER_SETTER_DECLARE, ~, YB_PG_CLIENT_AFTER_MOCKABLE_METHODS);
