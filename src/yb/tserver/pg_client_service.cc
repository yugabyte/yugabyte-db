...
class LockablePgClientSession {
 ...
 public:
  ...
  Status StartExchange(const std::string& instance_id, YBThreadPool& thread_pool) {
    ...
    session_.SetupOldestReadPointSerialNo(shared_mem_manager_.OldestReadPointSerialNoPtr());
  ...
  PgClientSessionDbHistoryRetentionPin GetDbHistoryRetentionPin() {
    if (!session_.HasPublishedOldestReadPointSerial()) {
      session_.ClearReadTimePin();
