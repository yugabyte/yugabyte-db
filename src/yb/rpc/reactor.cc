...
DEFINE_bool(reactor_check_current_thread, true,
...
DEFINE_int32(stuck_outbound_call_default_timeout_sec, 120,
...
DEFINE_int32(stuck_outbound_call_check_interval_sec, 30,
...
DEFINE_int32(reactor_based_outbound_call_expiration_delay_ms, 1000,
...
Status Reactor::Init() {
  ...
  auto old_state = ReactorState::kNotStarted;
  {
    ...
    SCHECK(
        ...
        IllegalState,
        "State was expected to be kNotStarted but was $1 during reactor initialization",
        old_state);
  }
...
void DelayedTask::Run(Reactor* reactor) {
  reactor->CheckCurrentThread();
  DCHECK(reactor_atomic_.load(std::memory_order_acquire) == nullptr)
