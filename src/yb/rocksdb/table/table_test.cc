...
TEST_F(TableTest, YB_LINUX_ONLY_TEST(BlockCacheWithHardlink)) {
  ...
  // GetMiddleKey doesn't use block cache, so need to Get it to populate block cache
  InternalKey internal_key(mkey, 0, kTypeValue);
  std::string encoded_key = internal_key.Encode().ToString();
  std::string value;
  GetContext get_context(options.comparator, nullptr, nullptr, nullptr,
                         GetContext::kNotFound, mkey, &value, nullptr,
                         nullptr, nullptr);

  const auto tw_1 = ASSERT_RESULT(db->TEST_GetLargestSstTableReader());
  auto* table_reader_1 = dynamic_cast<BlockBasedTable*>(tw_1);
  ASSERT_OK(table_reader_1->Get(ReadOptions(), encoded_key, &get_context));
