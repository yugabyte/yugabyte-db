...
#include "yb/vtables/yql_peers_vtable.h"
...
Result<std::shared_ptr<QLRowBlock>> PeersVTable::RetrieveData(
  ...
  vector<shared_ptr<yb::master::TSDescriptor>> descs;
  ...
  struct Entry {
    size_t index;
    yb::master::TSInformationPB ts_info;
  ...
  for (const auto& desc : descs) {
    ...
    yb::master::TSInformationPB ts_info = *desc->GetTSInformationPB();
  ...
  for (const auto& entry : entries) {
    ...
    RETURN_NOT_OK(
        SetColumnValue(kSystemTablesReleaseVersionColumn, kSystemTablesReleaseVersion, &row));
