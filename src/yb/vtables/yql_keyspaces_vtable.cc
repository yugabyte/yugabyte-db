...
#include "yb/vtables/yql_keyspaces_vtable.h"
...
Result<std::shared_ptr<QLRowBlock>> YQLKeyspacesVTable::RetrieveData(
  ...
  std::vector<scoped_refptr<yb::master::NamespaceInfo>> namespaces;
  catalog_manager().GetAllNamespaces(&namespaces, true);
  for (scoped_refptr<yb::master::NamespaceInfo> ns : namespaces) {
