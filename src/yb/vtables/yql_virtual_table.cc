...
void YQLVirtualTable::GetSortedLiveDescriptors(
    std::vector<std::shared_ptr<yb::master::TSDescriptor>>* descs) const {
  master_->ts_manager()->GetAllLiveDescriptors(descs);
  std::sort(
      descs->begin(), descs->end(),
      [](const std::shared_ptr<yb::master::TSDescriptor>& a,
         const std::shared_ptr<yb::master::TSDescriptor>& b) -> bool {
        return a->permanent_uuid() < b->permanent_uuid();
      });
}

yb::master::CatalogManagerIf& YQLVirtualTable::catalog_manager() const {
