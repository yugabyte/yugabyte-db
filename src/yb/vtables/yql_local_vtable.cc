...
#include "yb/vtables/yql_local_vtable.h"
...
Result<std::shared_ptr<QLRowBlock>> LocalVTable::RetrieveData(
  ...
  vector<std::shared_ptr<yb::master::TSDescriptor>> descs;
  ...
  size_t index = 0;
  for (const std::shared_ptr<yb::master::TSDescriptor>& desc : descs) {
    ++index;
  ...
  for (const auto& entry : entries) {
    ...
    RETURN_NOT_OK(SetColumnValue(kSystemTablesReleaseVersionColumn,
                                 kSystemTablesReleaseVersion, &row));
...
Schema LocalVTable::CreateSchema() const {
  ...
  CHECK_OK(builder.AddColumn(kSystemTablesReleaseVersionColumn,
                             QLType::Create(DataType::STRING)));
