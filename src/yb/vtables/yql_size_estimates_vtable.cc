...
#include "yb/vtables/yql_size_estimates_vtable.h"
...
Result<std::shared_ptr<QLRowBlock>> YQLSizeEstimatesVTable::RetrieveData(
  ...
  for (const auto& table : tables) {
    ...
    for (const scoped_refptr<yb::master::TabletInfo>& tablet : tablets) {
      yb::master::TabletLocationsPB tablet_locations_pb;
