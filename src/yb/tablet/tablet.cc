...
docdb::ConsensusFrontiers* InitFrontiers(const Data& data, docdb::ConsensusFrontiers* frontiers) {
  return InitFrontiers(yb::OpId::FromPB(data.op_id), data.log_ht, frontiers);
}
...
Status Tablet::ApplyOperationState(
  ...
  auto frontiers_ptr = InitFrontiers(
      yb::OpId::FromPB(operation_state.op_id()),
...
Result<docdb::ApplyTransactionState> Tablet::ApplyIntents(const TransactionApplyData& data) {
  ...
  docdb::ConsensusFrontiers frontiers;
  auto frontiers_ptr = data.op_id.IsInitialized() ? InitFrontiers(data, &frontiers) : nullptr;
