...
class Tablet::RegularRocksDbListener : public Tablet::RocksDbListener {
 ...
 private:
  ...
  void FillMinSchemaVersion(rocksdb::DB* db, MinSchemaVersionMap* table_id_to_min_schema_version) {
    ...
    {
      auto smallest = db->GetInMemoryFrontier(rocksdb::UpdateUserValueType::kSmallest);
      if (smallest) {
...
struct Tablet::IntentsDbFlushFilterState {
  ...
  void AddLargestFlushedIndex(
      const rocksdb::UserFrontierPtr& flushed_frontier,
      const rocksdb::UserFrontierPtr& smallest_mem_frontier) {
...
Result<bool> Tablet::IntentsDbFlushFilter(
  ...
  if (frontiers) {
    ...
    if (state->largest_flushed_index.empty()) {
      ...
      // flushed.
      const rocksdb::FrontierKinds frontier_kinds{
          rocksdb::FrontierKind::kFlushed, rocksdb::FrontierKind::kInMemorySmallest};
