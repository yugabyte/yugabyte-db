...
Status TabletTestHarness::Create(bool first_time) {
  ...
  auto table_info = TableInfo::TEST_Create(
      "YBTableTest", "test", "YBTableTest", options_.table_type, schema_, partition.first);
...
TabletInitData TabletTestHarness::MakeTabletInitData(const RaftGroupMetadataPtr& metadata) {
  return TabletInitData {
      ...
      .parent_block_based_table_mem_tracker = std::shared_ptr<MemTracker>(),
      .parent_block_based_table_builder_mem_tracker = std::shared_ptr<MemTracker>(),
