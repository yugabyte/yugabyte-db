...
Status TabletRetentionPolicy::RegisterReaderTimestamp(HybridTime timestamp) {
  ...
  if (timestamp < earliest_read_time_allowed) {
    ...
    // progress.  Compaction then never drops delete markers, though it still garbage-collects
    // regular values overwritten at or below the history cutoff.  That suffices for the backfill
    // writes that read at the backfill safe time: a value visible at that time and not overwritten
    // since is the latest version at or below the cutoff and always survives, a value overwritten
    // since is regenerated by the backfill scan of its base row (so conflicts reappear as
    // backfill-vs-backfill checks), and the retained delete markers keep maintenance deletes
    // shadowing late-arriving backfill writes.  Skip the SnapshotTooOld check for such readers.
    // The justification above assumes the forward/backward uniqueness checks regenerate
    // conflicts from re-read base rows. A SKIP_ALL build (active index-backfill ordering
    // generation) removes those checks, so the bypass is unsound there -- and unnecessary:
    // the generation's retention barrier holds the cutoff below the verification window, so
    // legitimate readers in the window never reach this branch.
    const bool bypass_allowed = ShouldRetainDeleteMarkersInMajorCompaction() &&
                                !metadata_.index_backfill_ordering_generation().active;
    if (!bypass_allowed) {
      return STATUS(
          SnapshotTooOld,
          Format(
