...
class Tablet : public AbstractTablet,
 ...
 public:
  ...
  // skips the regular-DB Put when bit 0 is clear (this branch's external-apply path does not feed
  // vector indexes). Tablet-bootstrap replay narrows it -- clearing the regular bit for a fused
  // external WRITE_OP already durably flushed to the regular DB, so it is not written again
  // (GH#31899); every other caller passes All(). The argument has no default, so each caller
