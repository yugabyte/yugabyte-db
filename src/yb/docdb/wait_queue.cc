...
struct WaitingTxn : public std::enable_shared_from_this<WaitingTxn> {
 public:
  ...
  WaiterDataPtr PurgeWaiter(int64_t request_id) EXCLUDES(mutex_) {
    auto purged_waiters = PurgeWaiters([request_id](const auto& waiter_data) {
      return waiter_data->request_id == request_id;
    });
    if (purged_waiters.empty()) {
      return nullptr;
    }
    if (purged_waiters.size() == 1) {
      return purged_waiters.at(0);
    }
    LOG_WITH_PREFIX(DFATAL) << "Found multiple waiters with the same request_id " << request_id;
    WaiterDataPtr waiter;
    for (const auto& duplicate_waiter : purged_waiters) {
      if (!waiter || duplicate_waiter->wq_entry_time > waiter->wq_entry_time) {
        waiter = duplicate_waiter;
      }
    }
    return waiter;
  }
...
class WaitQueue::Impl {
  ...
  void UpdateWaitersOnBlockerPromotion(const TransactionId& id,
    ...
    if (promoted_blocker) {
      ...
      for (const auto& waiter : waiters_to_signal) {
        ...
        if (auto waiter_to_signal = txn_it->second->PurgeWaiter(waiter->request_id)) {
          waiter_runner_.Submit(waiter_to_signal, Status::OK(), res.status_time);
        } else {
          LOG(DFATAL) << "Did not find waiter to signal during promotion " << waiter->id
                      << ", request_id: " << waiter->request_id;
        }
      }
    }
 ...
 private:
  ...
  void InvokeWaiterCallback(
    ...
    WaiterDataPtr found_waiter;
    {
      UniqueLock l(mutex_);
      auto it = waiter_status_.find(waiter_data->id);
      if (it != waiter_status_.end()) {
        found_waiter = it->second->PurgeWaiter(waiter_data->request_id);
      }
    }
