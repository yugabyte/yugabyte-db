...
void GenerateKeyAndValidateSchemaKeyOffsets(
    size_t hash_key_cols, bool use_colocation_id = false, bool use_cotable_id = false) {
  auto build_key = [](const Schema& schema) -> KeyBytes {
    KeyBytes result;

    const std::vector<KeyEntryValue> single_key_component{KeyEntryValue::Int32(5)};
    const std::vector<KeyEntryValue> two_key_component{
        KeyEntryValue::Int32(5), KeyEntryValue::Int32(6)};

    const size_t hash_key_cols = schema.num_hash_key_columns();
    const size_t range_key_cols = schema.num_range_key_columns();
    if (hash_key_cols && range_key_cols) {
      DocKey(schema, /* hash = */ 100, single_key_component, single_key_component)
          .AppendTo(&result);
    } else if (hash_key_cols) {
      DocKey(schema, /* hash = */ 100, two_key_component).AppendTo(&result);
    } else {
      DocKey(schema, two_key_component).AppendTo(&result);
    }
  ...
  LOG(INFO) << "Testing with use_colocation_id=" << use_colocation_id
            << ", use_cotable_id=" << use_cotable_id << ", hash key cols=" << hash_key_cols
            << " out of key_cols=" << key_cols;
  ...
  if (use_colocation_id) {
    ASSERT_FALSE(use_cotable_id);
    schema.set_colocation_id(1);
  } else if (use_cotable_id) {
    ASSERT_FALSE(use_colocation_id);
    schema.set_cotable_id(Uuid::Generate());
  }

  const auto& doc_key_offsets = schema.doc_key_offsets();
  ASSERT_TRUE(doc_key_offsets.has_value());

  auto key_bytes = build_key(schema);
  LOG(INFO) << "Generated key: " << SubDocKey::DebugSliceToString(key_bytes.AsSlice()) << " ("
            << key_bytes.AsSlice().ToDebugHexString() << ")";
  LOG(INFO) << "Expected: HashPartSize = " << doc_key_offsets->hash_part_size
            << ", DocKeySize = " << doc_key_offsets->doc_key_size
            << ", KeyOffsets = " << AsString(doc_key_offsets->key_offsets);

  // Use decoder to decode an element at a time and validate the offsets.
  auto start = key_bytes.AsSlice().data();
  auto decoder = DocKeyDecoder(key_bytes);
  ASSERT_EQ(schema.has_colocation_id(), ASSERT_RESULT(decoder.DecodeColocationId()));
  ASSERT_EQ(schema.has_cotable_id(), ASSERT_RESULT(decoder.DecodeCotableId()));
  bool has_hash_components = ASSERT_RESULT(decoder.DecodeHashCode());
  ASSERT_EQ(schema.num_hash_key_columns() > 0, has_hash_components);

  KeyEntryValue key_entry_value;
  if (has_hash_components) {
    for (size_t i = 0; i < schema.num_hash_key_columns(); i++) {
      ASSERT_EQ(decoder.ConsumedSizeFrom(start), doc_key_offsets->key_offsets[i]);
      ASSERT_OK(decoder.DecodeKeyEntryValue(&key_entry_value));
    }
    ASSERT_OK(decoder.ConsumeGroupEnd());
  }

  ASSERT_EQ(decoder.ConsumedSizeFrom(start), doc_key_offsets->hash_part_size);
  ...
  ASSERT_OK(decoder.ConsumeGroupEnd());
  ASSERT_EQ(decoder.ConsumedSizeFrom(start), doc_key_offsets->doc_key_size);
  ASSERT_TRUE(decoder.left_input().empty());
}

TEST_F(DocKeyTest, ComputeKeyOffsets) {
  // Hash key cols.
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 2, /* use_colocation_id = */ false, /* use_cotable_id = */ false);
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 2, /* use_colocation_id = */ true, /* use_cotable_id = */ false);
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 2, /* use_colocation_id = */ false, /* use_cotable_id = */true);

  // Range key cols.
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 0, /* use_colocation_id = */ false, /* use_cotable_id = */ false);
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 0, /* use_colocation_id = */ true, /* use_cotable_id = */ false);
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 0, /* use_colocation_id = */ false, /* use_cotable_id = */true);

  // Hash and range key cols.
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 1, /* use_colocation_id = */ false, /* use_cotable_id = */ false);
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 1, /* use_colocation_id = */ true, /* use_cotable_id = */ false);
  GenerateKeyAndValidateSchemaKeyOffsets(
      /* hash_key_cols = */ 1, /* use_colocation_id = */ false, /* use_cotable_id = */true);
}
