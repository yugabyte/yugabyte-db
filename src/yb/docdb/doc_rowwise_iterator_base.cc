...
Status DocRowwiseIteratorBase::Init(const qlexpr::YQLScanSpec& doc_spec, SkipSeek skip_seek) {
  ...
  VLOG(2) << "DocKey Bounds " << DocKey::DebugSliceToString(lower_doc_key.AsSlice()) << ", "
  ...
  if (is_forward_scan_) {
    ...
    if (has_bound_key_) {
      ...
      if (lower_doc_key.data()[0] != dockv::KeyEntryTypeAsChar::kLowest) {
        ...
        // table rows cannot have a key before kNullLow, we could use it as a bound instead of -Inf.
        bound_key_.Clear();
        bound_key_.AppendKeyEntryType(dockv::KeyEntryType::kNullLow);
