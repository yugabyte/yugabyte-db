...
class DiscreteScanChoices : public ScanChoices {
 public:
  ...
  bool IsInitialPositionKnown() const override { return true; }
...
DiscreteScanChoices::DiscreteScanChoices(
...
DiscreteScanChoices::DiscreteScanChoices(
    const DocPgsqlScanSpec& doc_spec, const KeyBytes& lower_doc_key, const KeyBytes& upper_doc_key)
    : ScanChoices(doc_spec.is_forward_scan()) {
...
Status DiscreteScanChoices::SeekToCurrentTarget(IntentAwareIteratorIf* db_iter) {
...
Status RangeBasedScanChoices::SeekToCurrentTarget(IntentAwareIteratorIf* db_iter) {
  VLOG(2) << __PRETTY_FUNCTION__ << " Advancing iterator towards target";
...
ScanChoicesPtr ScanChoices::Create(
  ...
  if (doc_spec.range_options()) {
    return std::make_unique<DiscreteScanChoices>(doc_spec, lower_doc_key, upper_doc_key);
  }

  if (doc_spec.range_bounds()) {
    return std::make_unique<RangeBasedScanChoices>(schema, doc_spec);
  }
