...
class DocKeyColumnPathBuilderHelper {
 public:
  template<class... Args>
  explicit DocKeyColumnPathBuilderHelper(
      Args&&... args) : builder_(&buffer_, std::forward<Args>(args)...) {}

  [[nodiscard]] DocKeyColumnPathBuilder& builder() {
    return builder_;
  }
...
class IntentMode {
 public:
  IntentMode(IsolationLevel level, RowMarkType row_mark, KeyOnlyRequested key_only_requested) {
    auto read_intents = dockv::GetIntentTypesForRead(level);
    auto lock_intents = dockv::GetIntentTypes(row_mark);
    DCHECK(!(read_intents.None() && lock_intents.None()));
...
[[nodiscard]] bool IsOnlyKeyColumnsRequested(
    const Schema& schema, const PgsqlReadRequestPB& req) {
  ...
  return !Find(req.column_refs().ids(),
  ...
  switch (mode) {
    case GetDocPathsMode::kLock: {
      if (request_.stmt_type() == PgsqlWriteRequestPB::PGSQL_UPDATE) {
        ...
        if (!has_expression) {
          DocKeyColumnPathBuilderHelper helper(encoded_doc_key_.as_slice());
          paths->emplace_back(helper.builder().Build(dockv::SystemColumnIds::kLivenessColumn));
          return Status::OK();
        }
      }
      break;
...
Status PgsqlReadOperation::GetIntents(
  ...
  if (!(has_batch_arguments || request_.has_ybctid_column_value())) {
    IntentMode mode(level, row_mark, KeyOnlyRequested::kFalse);
    inserter.Add(VERIFY_RESULT(accessor.GetEncoded(request_)), mode);
    return Status::OK();
  }

  IntentMode mode(level, row_mark, KeyOnlyRequested(IsOnlyKeyColumnsRequested(schema, request_)));
