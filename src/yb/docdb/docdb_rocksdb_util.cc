...
DEFINE_NON_RUNTIME_int32(rocksdb_max_background_flushes, -1,
    "Number threads to do background flushes.");
DEFINE_NON_RUNTIME_bool(rocksdb_disable_compactions, false, "Disable rocksdb compactions.");
DEFINE_NON_RUNTIME_bool(rocksdb_compaction_measure_io_stats, false,
    "Measure stats for rocksdb compactions.");
DEFINE_NON_RUNTIME_int32(rocksdb_base_background_compactions, -1,
             "Number threads to do background compactions.");
DEFINE_NON_RUNTIME_int32(rocksdb_max_background_compactions, -1,
...
DEFINE_NON_RUNTIME_int32(rocksdb_level0_file_num_compaction_trigger, 5,
             "Number of files to trigger level-0 compaction. -1 if compaction should not be "
             "triggered by number of files at all.");

DEFINE_NON_RUNTIME_int32(rocksdb_level0_slowdown_writes_trigger, -1,
             "The number of files above which writes are slowed down.");
DEFINE_NON_RUNTIME_int32(rocksdb_level0_stop_writes_trigger, -1,
             "The number of files above which compactions are stopped.");
DEFINE_NON_RUNTIME_int32(rocksdb_universal_compaction_size_ratio, 20,
             "The percentage upto which files that are larger are include in a compaction.");
DEFINE_NON_RUNTIME_uint64(rocksdb_universal_compaction_always_include_size_threshold, 64_MB,
             "Always include files of smaller or equal size in a compaction.");
DEFINE_NON_RUNTIME_int32(rocksdb_universal_compaction_min_merge_width, 4,
             "The minimum number of files in a single compaction run.");
DEFINE_NON_RUNTIME_int64(rocksdb_compact_flush_rate_limit_bytes_per_sec, 1_GB,
...
DEFINE_NON_RUNTIME_uint64(rocksdb_compaction_size_threshold_bytes, 2ULL * 1024 * 1024 * 1024,
             "Threshold beyond which compaction is considered large.");
DEFINE_NON_RUNTIME_uint64(rocksdb_max_file_size_for_compaction, 0,
...
DEFINE_NON_RUNTIME_int64(db_block_size_bytes, 32_KB,
             "Size of RocksDB data block (in bytes).");

DEFINE_NON_RUNTIME_int64(db_filter_block_size_bytes, 64_KB,
             "Size of RocksDB filter block (in bytes).");

DEFINE_NON_RUNTIME_int64(db_index_block_size_bytes, 32_KB,
             "Size of RocksDB index block (in bytes).");

DEFINE_NON_RUNTIME_int64(db_min_keys_per_index_block, 100,
             "Minimum number of keys per index block.");

DEFINE_NON_RUNTIME_int64(db_write_buffer_size, -1,
             "Size of RocksDB write buffer (in bytes). -1 to use default.");

DEFINE_NON_RUNTIME_int32(memstore_size_mb, 128,
             "Max size (in mb) of the memstore, before needing to flush.");

DEFINE_NON_RUNTIME_bool(use_docdb_aware_bloom_filter, true,
...
DEFINE_NON_RUNTIME_int32(max_nexts_to_avoid_seek, 2,
             "The number of next calls to try before doing resorting to do a rocksdb seek.");

DEFINE_NON_RUNTIME_bool(use_multi_level_index, true, "Whether to use multi-level data index.");
...
DEFINE_NON_RUNTIME_uint64(initial_seqno, 1ULL << 50, "Initial seqno for new RocksDB instances.");

DEFINE_NON_RUNTIME_int32(num_reserved_small_compaction_threads, -1,
    "Number of reserved small compaction "
    "threads. It allows splitting small vs. large compactions.");

DEFINE_NON_RUNTIME_bool(enable_ondisk_compression, true,
            "Determines whether SSTable compression is enabled or not.");

DEFINE_NON_RUNTIME_int32(priority_thread_pool_size, -1,
...
DEFINE_NON_RUNTIME_int32(block_restart_interval, kDefaultDataBlockRestartInterval,
             "Controls the number of keys to look at for computing the diff encoding.");

DEFINE_NON_RUNTIME_int32(index_block_restart_interval, kDefaultIndexBlockRestartInterval,
