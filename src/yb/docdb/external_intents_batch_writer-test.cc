...
class ExternalIntentsBatchWriterTest : public DocDBTestBase {
 public:
  ...
  Status SendWriteBatch(const docdb::LWKeyValueWriteBatchPB& put_batch, HybridTime write_ht) {
    rocksdb::WriteBatch intents_write_batch;
    docdb::ExternalTxnIntentsState external_txn_intents_state;
    ExternalIntentsBatchWriter batcher(
        put_batch, write_ht, intents_db(), &intents_write_batch, &external_txn_intents_state,
        nullptr);
...
TEST_F(ExternalIntentsBatchWriterTest, SimpleTransaction) {
  ...
  const DocKey hk1(0, KeyEntryValues("h1"));
  const DocKey hk2(1, KeyEntryValues("h2"));
  ...
  // Write to intentsdb.
  ASSERT_OK(SendWriteBatch(put_batch, kBatchHT));
...
TXN EXT 30303030-3030-3030-3030-303030303031 HT{ physical: 5000 w: 1 } -> \
  ...
  ASSERT_OK(SendWriteBatch(put_batch, kWriteHT));
...
TEST_F(ExternalIntentsBatchWriterTest, ApplyFilterOnHashExternalIntents) {
  ...
  for (int i = 0; i < kNumKeys; ++i) {
    const DocKey hk(i * 100, KeyEntryValues(Format("h$0", i)));
    std::vector<ExternalIntent> intents = {
...
TXN EXT 30303030-3030-3030-3030-303030303031 HT{ physical: 5000 w: 9 } -> \
  ...
  put_batch.Clear();
  AddApplyExternalTxn(
      &put_batch, txn1, kWriteHT, "", PartitionSchema::EncodeMultiColumnHashValue(400));
  ASSERT_OK(SendWriteBatch(put_batch, kWriteHT));
...
TXN EXT 30303030-3030-3030-3030-303030303031 HT{ physical: 5000 w: 9 } -> \
  ...
  put_batch.Clear();
  AddApplyExternalTxn(
      &put_batch, txn1, kWriteHT, PartitionSchema::EncodeMultiColumnHashValue(700), "");
  ASSERT_OK(SendWriteBatch(put_batch, kWriteHT));
  ASSERT_DOC_DB_DEBUG_DUMP_STR_EQ(R"#(
SubDocKey(DocKey(0x0000, ["h0"], []), [HT{ physical: 6000 w: 1 }]) -> "value1"
SubDocKey(DocKey(0x0000, ["h0"], []), [HT{ physical: 6000 }]) -> "value0"
...
TXN EXT 30303030-3030-3030-3030-303030303031 HT{ physical: 5000 w: 6 } -> \
  ...
  AddApplyExternalTxn(
      &put_batch, txn1, kWriteHT, PartitionSchema::EncodeMultiColumnHashValue(400),
      PartitionSchema::EncodeMultiColumnHashValue(700));
  ASSERT_OK(SendWriteBatch(put_batch, kWriteHT));
  ASSERT_DOC_DB_DEBUG_DUMP_STR_EQ(R"#(
SubDocKey(DocKey(0x0000, ["h0"], []), [HT{ physical: 6000 w: 1 }]) -> "value1"
SubDocKey(DocKey(0x0000, ["h0"], []), [HT{ physical: 6000 }]) -> "value0"
...
TEST_F(ExternalIntentsBatchWriterTest, ApplyFilterOnRangedExternalIntents) {
  ...
  // Mimic xCluster and group each key in separate batches.
  for (int i = 0; i < kNumKeys; ++i) {
    const DocKey rk(KeyEntryValues(Format("r$0", i)));
    std::vector<ExternalIntent> intents = {
