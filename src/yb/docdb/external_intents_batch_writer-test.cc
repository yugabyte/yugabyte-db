...
class ExternalIntentsBatchWriterTest : public DocDBTestBase {
 public:
  ...
  std::string GetEncodedHashPartitionKey(uint16_t hash) {
    ...
    dockv::DocKeyEncoderAfterTableIdStep(&encoded_key).Hash(hash, dockv::KeyEntryValues());
...
TXN EXT 30303030-3030-3030-3030-303030303031 HT{ physical: 5000 w: 9 } -> \
  ...
  put_batch.Clear();
  AddApplyExternalTxn(&put_batch, txn1, kWriteHT, "", GetEncodedHashPartitionKey(400));
  ASSERT_OK(SendWriteBatch(put_batch, kWriteHT, kBatchHT));
...
TXN EXT 30303030-3030-3030-3030-303030303031 HT{ physical: 5000 w: 9 } -> \
  ...
  put_batch.Clear();
  AddApplyExternalTxn(&put_batch, txn1, kWriteHT, GetEncodedHashPartitionKey(700), "");
  ASSERT_OK(SendWriteBatch(put_batch, kWriteHT, kBatchHT));
  ASSERT_DOC_DB_DEBUG_DUMP_STR_EQ(R"#(
SubDocKey(DocKey(0x0000, ["h0"], []), [HT{ physical: 6000 w: 1 }]) -> "value1"
SubDocKey(DocKey(0x0000, ["h0"], []), [HT{ physical: 6000 }]) -> "value0"
...
TXN EXT 30303030-3030-3030-3030-303030303031 HT{ physical: 5000 w: 6 } -> \
  ...
  put_batch.Clear();
  AddApplyExternalTxn(
      &put_batch, txn1, kWriteHT, GetEncodedHashPartitionKey(400), GetEncodedHashPartitionKey(700));
  ASSERT_OK(SendWriteBatch(put_batch, kWriteHT, kBatchHT));
  ASSERT_DOC_DB_DEBUG_DUMP_STR_EQ(R"#(
SubDocKey(DocKey(0x0000, ["h0"], []), [HT{ physical: 6000 w: 1 }]) -> "value1"
SubDocKey(DocKey(0x0000, ["h0"], []), [HT{ physical: 6000 }]) -> "value0"
