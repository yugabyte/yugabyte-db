...
TEST_F(DocDBTestWrapper, HistoryRetentionWithCotables) {
  ...
  auto hash_components =
      KeyEntryValues("cotablekey", 10000);
  ...
  auto hash_components2 =
      KeyEntryValues("cotablekey2", 10000);
  std::string table_id2 = GetPgsqlTableId(db_oid2, table_oid2);
  Uuid cotable_id2 = ASSERT_RESULT(Uuid::FromHexString(table_id2));
  auto encoded_doc_key2 = DocKey(cotable_id2, key_hash2, hash_components2).Encode();
  ASSERT_OK(SetPrimitive(
      DocPath(encoded_doc_key2, KeyEntryValue("subkey1")),
      QLValue::Primitive("value1"), 1000_usec_ht));
  // Another key without cotable.
  DocKeyHash non_cotable_hash = 2;
  auto non_cotable_hash_components =
      KeyEntryValues("noncotablekey", 10000);
  auto encoded_non_cotable_key =
      DocKey(non_cotable_hash, non_cotable_hash_components).Encode();
  ASSERT_OK(SetPrimitive(
...
TEST_F(DocDBTestWrapper, HistoryRetentionWithColocatedTables) {
  ...
  DocKeyHash key_hash = 1;
  auto hash_components =
      KeyEntryValues("colocationkey", 10000);
  uint32_t colocation_id = 16384;
  auto encoded_doc_key = DocKey(colocation_id, key_hash, hash_components).Encode();
  ASSERT_OK(SetPrimitive(
      DocPath(encoded_doc_key, KeyEntryValue("subkey1")),
      QLValue::Primitive("value1"), 1000_usec_ht));
  ASSERT_OK(SetPrimitive(
  ...
  DocKeyHash hash2 = 2;
  auto hash_components2 =
      KeyEntryValues("colocationkey2", 10000);
  uint32_t colocation_id2 = 16389;
  auto encoded_key2 =
      DocKey(colocation_id2, hash2, hash_components2).Encode();
  ASSERT_OK(SetPrimitive(
  ...
  DocKeyHash hash3 = 3;
  auto hash_components3 =
      KeyEntryValues("colocationkey3", 10000);
  uint32_t colocation_id3 = 16394;
  auto encoded_key3 =
      DocKey(colocation_id3, hash3, hash_components3).Encode();
  ASSERT_OK(SetPrimitive(
