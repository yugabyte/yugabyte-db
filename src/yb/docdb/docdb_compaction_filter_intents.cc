...
DEFINE_NON_RUNTIME_uint64(aborted_intent_cleanup_ms, 60000,
    // 1 minute by default, 1 sec for testing
    "Duration in ms after which to check if a transaction is aborted.");

DEFINE_NON_RUNTIME_uint64(aborted_intent_cleanup_max_batch_size, 256,
    // Cleanup 256 transactions at a time
    "Number of transactions to collect for possible cleanup.");

DEFINE_NON_RUNTIME_int32(external_intent_cleanup_secs, 60 * 60 * 24, // 24 hours by default
             "Duration in secs after which to cleanup external intents.");

DEFINE_NON_RUNTIME_uint64(intents_compaction_filter_max_errors_to_log, 100,
