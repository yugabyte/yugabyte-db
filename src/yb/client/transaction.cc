...
class YBTransaction::Impl final : public internal::TxnBatcherIf {
 ...
 private:
  ...
  // Stops waiting on the participants whose UpdateTransactionStatusLocation rpc is never going to
  // be sent. DoSendUpdateTransactionStatusLocationRpcs() skips a participant until one of its
  // batches completes, so a participant whose only batches failed keeps its entry in
  ...
  void DropPendingStatusMovesWithoutMetadata() REQUIRES(mutex_) {
    for (auto it = transaction_status_move_tablets_.begin();
      ...
      VLOG_WITH_PREFIX(1) << "Tablet " << *it << " has no metadata, dropping its"
                          << " UpdateTransactionStatusLocation rpc";
