...
class QLDmlTest : public QLDmlTestBase {
 public:
  ...
  shared_ptr<YBqlWriteOp> InsertRow(
    ...
    table_.SetInt32ColumnValue(req->add_hashed_column_values(), "h1", h1);
    table_.SetStringColumnValue(req->add_hashed_column_values(), "h2", h2);
    table_.SetInt32ColumnValue(req->add_range_column_values(), "r1", r1);
    table_.SetStringColumnValue(req->add_range_column_values(), "r2", r2);
...
TEST_F(QLDmlTest, TestInsertUpdateAndSelect) {
...

  {
    ...
    auto* const req = op->mutable_request();
    table_.SetInt32ColumnValue(req->add_hashed_column_values(), "h1", 1);
    table_.SetStringColumnValue(req->add_hashed_column_values(), "h2", "a");
    table_.SetInt32ColumnValue(req->add_range_column_values(), "r1", 2);
    table_.SetStringColumnValue(req->add_range_column_values(), "r2", "b");
...
TEST_F(QLDmlTest, FlushedOpId) {
  ...
  const shared_ptr<YBqlReadOp> op = table_.NewReadOp();
  auto* const req = op->mutable_request();
  table_.SetInt32ColumnValue(req->add_hashed_column_values(), "h1", kHashInt);
  table_.SetStringColumnValue(req->add_hashed_column_values(), "h2", kHashStr);
...
TEST_F(QLDmlTest, TestConditionalDelete) {
...

  {
    ...
    table_.SetStringColumnValue(req->add_hashed_column_values(), "h2", "a");
    table_.SetInt32ColumnValue(req->add_range_column_values(), "r1", 2);
    table_.SetStringColumnValue(req->add_range_column_values(), "r2", "c");
    auto* const condition = req->mutable_if_expr()->mutable_condition();
    condition->set_op(QL_OP_EXISTS);
    const shared_ptr<YBSession> session(client_->NewSession(false /* read_only */));
    CHECK_OK(session->Apply(op));
