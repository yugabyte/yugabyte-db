 ...
 protected:
  ...
  Result<PromotedTxn> StartAndPromoteTransaction() {
    auto txn = std::make_shared<YBTransaction>(
        &transaction_manager_.value(), TransactionLocality::LOCAL);
    ...
    RETURN_NOT_OK(txn->PromoteToGlobal());
    RETURN_NOT_OK(WaitFor([&txn]() -> Result<bool> { return txn->OldTransactionAborted(); },
  ...
  void TestRestartsMidTransaction(std::initializer_list<const RestartInfo> restarts) {
    ...
    for (const auto& restart : restarts) {
      ...
      LOG(INFO) << "DocDB dump:\n" << tablet->TEST_DocDBDumpStr(tablet::IncludeIntents::kTrue);
    }
