...
// Disabled RefreshPartitions test in TSAN for 2.6 until we also backport
// https://github.com/yugabyte/yugabyte-db/commit/8025ea6dd8cfb79a7556501b410650c635cadbf8
// that fixes concurrent Table::table_type_ update (only rarely happened with tablet splitting on).
...
TEST_F_EX(ClientTest, YB_DISABLE_TEST_IN_TSAN(EmptiedBatcherFlush), ClientTestWithHashAndRangePk) {
  ...
  for (int i = 0; i != kWriters; ++i) {
    thread_holder.AddThreadFunctor([this, &stop = thread_holder.stop_flag(), &next_batch_hash_key,
      ...
      while (!stop.load(std::memory_order_acquire)) {
        ...
        if (!flush_status.ok()) {
          LogSessionErrorsAndDie(session, flush_status);
        }
      }
    });
  }
