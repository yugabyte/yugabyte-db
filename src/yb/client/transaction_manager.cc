...
class TransactionTableState {
 public:
  ...
  // Update transaction hash and return true if changed.
  bool UpdateTransactionStatusHash(uint64_t hash) EXCLUDES(mutex_) {
    std::lock_guard<yb::RWMutex> lock(mutex_);
    if (transaction_status_hash_ == hash) {
      return false;
    }
    transaction_status_hash_ = hash;
    return true;
  }
 ...
 private:
  ...
  uint64_t transaction_status_hash_ GUARDED_BY(mutex_) = 0;
...
class TransactionManager::Impl {
 public:
  ...
  void UpdateTransactionStatusHash(uint64_t hash) {
    if (!table_state_.UpdateTransactionStatusHash(hash)) {
      return;
    }
 ...
 private:
  ...
  // Version of set of transaction status tablets. Each time the transaction
  // status hash changes, this is incremented. This version is internal and specific to the
...
void TransactionManager::UpdateTransactionStatusHash(uint64_t hash) {
  impl_->UpdateTransactionStatusHash(hash);
}
