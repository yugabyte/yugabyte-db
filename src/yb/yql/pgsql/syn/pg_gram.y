...
#include "yb/yql/pgsql/syn/pg_inactive_nodes.h"
...
typedef PgTExpr::SharedPtr              PExpr;
typedef PgTRef::SharedPtr               PRef;
typedef PgTExprListNode::SharedPtr      PExprListNode;
typedef PgTConstInt::SharedPtr          PConstInt;
typedef PgTCollection::SharedPtr        PCollection;
typedef PgTValues::SharedPtr            PValues;
typedef PgTSelectStmt::SharedPtr        PSelectStmt;
typedef PgTTableRef::SharedPtr          PTableRef;
typedef PgTTableRefListNode::SharedPtr  PTableRefListNode;
typedef PgTOrderBy::SharedPtr           POrderBy;
typedef PgTOrderByListNode::SharedPtr   POrderByListNode;
typedef PgTAssign::SharedPtr            PAssign;
typedef PgTAssignListNode::SharedPtr    PAssignListNode;

typedef PgTName::SharedPtr              PName;
typedef PgTQualifiedName::SharedPtr     PQualifiedName;
typedef PgTQualifiedNameListNode::SharedPtr PQualifiedNameListNode;

typedef PgTBaseType::SharedPtr          PType;
typedef PgTCharBaseType::SharedPtr      PCharBaseType;
...
#define PARSER_NOTIMP(loc) parser_->Error(loc, ErrorCode::FEATURE_NOT_YET_IMPLEMENTED)
}
...
%type <PExpr>             // Expression clause. These expressions are used in a specific context.
                          where_clause opt_where_clause opt_using_clause
                          where_or_current_clause opt_where_or_current_clause
                          opg_tselect_limit select_limit limit_clause select_limit_value
...
%type <PListNode>         // Clauses as list of tree nodes.
                          ...
                          ColQualList columnList

                          // Create index clauses.
                          index_params

%type <PExpr>             // Expressions.
                          a_expr b_expr ctext_expr c_expr AexprConst
                          target_el in_expr
...
// Name nodes.
%type <PName>             database_name indirection_el columnElem
...
%type <PString>           createdb_opg_tname RoleId opg_ttype foreign_server_version
                          opt_foreign_server_version opt_in_database OptSchemaName copy_file_name
                          access_method_clause access_method attr_name name
...
%type <dbehavior>         opg_tdrop_behavior
...
%type <UndefType>         inactive_stmt inactive_schema_stmt
                          ...
                          AlterOwnerStmt AlterSeqStmt AlterSystemStmt
                          ...
                          DeclareCursorStmt DefineStmt DiscardStmt DoStmt
                          DropOpClassStmt DropOpFamilyStmt DropPLangStmt
                          DropAssertStmt DropTrigStmt DropRuleStmt DropCastStmt DropRoleStmt
                          DropPolicyStmt DropUserStmt DropTableSpaceStmt DropFdwStmt
                          DropTransformStmt
                          DropForeignServerStmt DropUserMappingStmt ExplainStmt FetchStmt
                          GrantStmt GrantRoleStmt ImportForeignSchemaStmt
                          ListenStmt LoadStmt LockStmt NotifyStmt ExplainableStmt PreparableStmt
                          CreateFunctionStmt AlterFunctionStmt IndexStmt ReindexStmt RemoveAggrStmt
                          ...
                          select_offset_value2 opg_tselect_fetch_first_value
                          SeqOptElem
...
%type <UndefListType>     alter_table_cmds alter_type_cmds createdb_opt_list createdb_opt_items
                          ...
                          RuleActionMulti opt_column_list opg_tname_list
                          name_list role_list
                          ...
                          TableFuncElementList opg_ttype_modifiers prep_type_clause
...
// Op - multi-character operator. See the calls to  GramProcessor::make_Op() by lexer.
%left     Op OPERATOR
%left     '+' '-'
%left     '*' '/' '%'
%left     '^'
...
CreatedbStmt:
  CREATE DATABASE database_name createdb_opt_list {
    $$ = MAKE_NODE(@1, PgTCreateDatabase, $3);
  }
;

database_name:
  ColId {
    $$ = MAKE_NODE(@1, PgTName, $1);
  }
;
...
createdb_opt_item:
  createdb_opg_tname opt_equal SignedIconst {
  }
  | createdb_opg_tname opt_equal opt_boolean_or_string {
  }
  | createdb_opg_tname opt_equal DEFAULT {
  }
;
...
DropdbStmt:
  DROP DATABASE database_name {
    $$ = MAKE_NODE(@1, PgTDropDatabase, $3, false);
  }
  | DROP DATABASE IF_P EXISTS database_name {
    $$ = MAKE_NODE(@1, PgTDropDatabase, $5, true);
  }
;
...
//--------------------------------------------------------------------------------------------------
// TODO(neil) SCHEMA should be created as sub-database, and its name should be <dbname>.<schema>.
// "YQLDatabase" for a SCHEMA should be YQL_DATABASE_PGSQL_SUBDB.
//
// Currently, an error is raised when SCHEMA is created.
CreateSchemaStmt:
  CREATE SCHEMA ColId OptSchemaEltList {
    $$ = MAKE_NODE(@1, PgTCreateSchema, $3, false);
  }
  | CREATE SCHEMA IF_P NOT EXISTS ColId OptSchemaEltList {
    $$ = MAKE_NODE(@1, PgTCreateSchema, $6, true);
  }
...
CreateStmt:
  ...
  OptInherit OptWith OnCommitOption OptTableSpace {
    $$ = MAKE_NODE(@1, PgTCreateTable, $4, $6, false);
  }
  | CREATE OptTemp TABLE IF_P NOT EXISTS qualified_name '(' OptTableElementList ')'
  OptInherit OptWith OnCommitOption OptTableSpace {
    $$ = MAKE_NODE(@1, PgTCreateTable, $7, $9, true);
  }
...
columnDef:
  ColId Typename create_generic_options ColQualList {
    $$ = MAKE_NODE(@1, PgTColumnDefinition, $1, $2, $4);
  }
;
...
DropStmt:
  DROP drop_type IF_P EXISTS any_name_list opg_tdrop_behavior {
    $$ = MAKE_NODE(@1, PgTDropStmt, $2, $5, true);
  }
  | DROP drop_type any_name_list opg_tdrop_behavior {
    $$ = MAKE_NODE(@1, PgTDropStmt, $2, $3, false);
  }
  | DROP TYPE_P type_name_list opg_tdrop_behavior {
    PARSER_UNSUPPORTED(@2);
  }
  | DROP TYPE_P IF_P EXISTS type_name_list opg_tdrop_behavior {
    PARSER_UNSUPPORTED(@2);
  }
  | DROP DOMAIN_P type_name_list opg_tdrop_behavior {
    PARSER_UNSUPPORTED(@2);
  }
  | DROP DOMAIN_P IF_P EXISTS type_name_list opg_tdrop_behavior {
    PARSER_UNSUPPORTED(@2);
  }
  | DROP INDEX CONCURRENTLY any_name_list opg_tdrop_behavior {
    PARSER_UNSUPPORTED(@2);
  }
  | DROP INDEX CONCURRENTLY IF_P EXISTS any_name_list opg_tdrop_behavior {
    PARSER_UNSUPPORTED(@2);
  }
;
...
any_name_list:
  any_name {
    $$ = MAKE_NODE(@1, PgTQualifiedNameListNode, $1);
  }
...
any_name:
  ColId {
    $$ = MAKE_NODE(@1, PgTQualifiedName, $1);
  }
  | ColId attrs {
    PgTName::SharedPtr name_node = MAKE_NODE(@1, PgTName, $1);
    $2->Prepend(name_node);
    $$ = $2;
  }
;
...
attrs:
  '.' attr_name {
    $$ = MAKE_NODE(@2, PgTQualifiedName, $2);
  }
  | attrs '.' attr_name {
    PgTName::SharedPtr name_node = MAKE_NODE(@3, PgTName, $3);
    $1->Append(name_node);
    $$ = $1;
  }
;
...
alter_table_cmd:
  ...
  | DROP opt_column IF_P EXISTS ColId opg_tdrop_behavior {
  }
  /* ALTER TABLE <name> DROP [COLUMN] <colname> [RESTRICT|CASCADE] */
  | DROP opt_column ColId opg_tdrop_behavior {
  }
  /*
  ...
  | DROP CONSTRAINT IF_P EXISTS name opg_tdrop_behavior {
  }
  /* ALTER TABLE <name> DROP CONSTRAINT <name> [RESTRICT|CASCADE] */
  | DROP CONSTRAINT name opg_tdrop_behavior {
  }
...
alter_type_cmd:
  ...
  ADD_P ATTRIBUTE TableFuncElement opg_tdrop_behavior {
  }
  /* ALTER TYPE <name> DROP ATTRIBUTE IF EXISTS <attname> [RESTRICT|CASCADE] */
  | DROP ATTRIBUTE IF_P EXISTS ColId opg_tdrop_behavior {
  }
  /* ALTER TYPE <name> DROP ATTRIBUTE <attname> [RESTRICT|CASCADE] */
  | DROP ATTRIBUTE ColId opg_tdrop_behavior {
  }
  /* ALTER TYPE <name> ALTER ATTRIBUTE <attname> [SET DATA] TYPE <typename> [RESTRICT|CASCADE] */
  | ALTER ATTRIBUTE ColId opt_set_data TYPE_P Typename opt_collate_clause opg_tdrop_behavior {
  }
;
...
select_no_parens:
  ...
  | simple_select opt_sort_clause for_locking_clause opg_tselect_limit {
    PARSER_UNSUPPORTED(@3);
  }
  ...
  | with_clause simple_select opt_sort_clause for_locking_clause opg_tselect_limit {
    PARSER_UNSUPPORTED(@1);
  }
...
simple_select:
  ...
  group_clause having_clause opt_window_clause {
    $$ = MAKE_NODE(@1, PgTSelectStmt, false, $3, $5, $6, $7, $8, nullptr, nullptr);
  }
  | SELECT distinct_clause target_list into_clause from_clause opt_where_clause
  group_clause having_clause opt_window_clause {
    $$ = MAKE_NODE(@1, PgTSelectStmt, true, $3, $5, $6, $7, $8, nullptr, nullptr);
  }
  | TABLE relation_expr {
    PARSER_UNSUPPORTED(@1);
  }
...
common_table_expr:
  name opg_tname_list AS '(' PreparableStmt ')' {
  }
;
...
sortby_list:
  sortby {
    $$ = MAKE_NODE(@1, PgTOrderByListNode, $1);
  }
  | sortby_list ',' sortby {
    $1->Append($3);
    $$ = $1;
...
sortby:
  a_expr opt_asc_desc opt_nulls_order {
    $$ = MAKE_NODE(@1, PgTOrderBy, $1, PgTOrderBy::Direction($2), PgTOrderBy::NullPlacement($3));
  }
...
target_el:
  a_expr AS ColLabel {
    $$ = MAKE_NODE(@1, PgTExprAlias, $1, $3);
  }
  | a_expr {
    $$ = $1;
  }
  | '*' {
    $$ = MAKE_NODE(@1, PgTAllColumns);
  }
...
limit_clause:
  ...
  // SQL:2008 syntax
  | FETCH first_or_next opg_tselect_fetch_first_value row_or_rows ONLY {
    $$ = nullptr;
  }
;
...
opt_indirection:
  ...
  | opt_indirection indirection_el {
    if ($1 == nullptr) {
      $$ = MAKE_NODE(@1, PgTQualifiedName, $2);
    } else {
      $1->Append($2);
      $$ = $1;
    }
...
DeleteStmt:
  ...
  opt_using_clause opt_where_or_current_clause returning_clause {
    $$ = MAKE_NODE(@2, PgTDeleteStmt, $4, $6);
  }
;
...
UpdateStmt:
  ...
  from_clause opt_where_or_current_clause returning_clause {
    $$ = MAKE_NODE(@2, PgTUpdateStmt, $3, $5, $7);
  }
;

set_clause_list:
  set_clause {
    $$ = MAKE_NODE(@1, PgTAssignListNode, $1);
  }
...
from_list:
  table_ref {
    $$ = MAKE_NODE(@1, PgTTableRefListNode, $1);
  }
  | from_list ',' table_ref {
    $1->Append($3);
    $$ = $1;
...
relation_expr_opt_alias:
  relation_expr %prec UMINUS  {
    $$ = MAKE_NODE(@1, PgTTableRef, $1, nullptr);
  }
  | relation_expr AS ColId {
    $$ = MAKE_NODE(@1, PgTTableRef, $1, $3);
  }
  | relation_expr ColId {
    PARSER_UNSUPPORTED(@2);
  }
...
a_expr:
  ...
  | '-' a_expr          %prec UMINUS {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $2);
    auto name = parser_->MakeString("-");
    $$ = MAKE_NODE(@1, PgTBcall, name, args);
  }
  | a_expr '+' a_expr {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    args->Append($3);
    auto name = parser_->MakeString("+");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | a_expr '-' a_expr {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    args->Append($3);
  ...
  | a_expr AND a_expr {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    args->Append($3);
    auto name = parser_->MakeString("and");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | a_expr OR a_expr {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    args->Append($3);
    auto name = parser_->MakeString("or");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | NOT a_expr {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@2, PgTExprListNode, $2);
    auto name = parser_->MakeString("not");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  ...
  | a_expr LIKE a_expr {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    args->Append($3);
    auto name = parser_->MakeString("like");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  ...
  | a_expr NOT_LA LIKE a_expr             %prec NOT_LA {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    args->Append($4);
    auto name = parser_->MakeString("not like");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  ...
  | a_expr IS NULL_P              %prec IS {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    auto name = parser_->MakeString("is null");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | a_expr ISNULL {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    auto name = parser_->MakeString("isnull");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | a_expr IS NOT NULL_P            %prec IS {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    auto name = parser_->MakeString("is not null");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | a_expr NOTNULL {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    auto name = parser_->MakeString("notnull");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | row OVERLAPS row {
    PARSER_UNSUPPORTED(@1);
  ...
  | a_expr IS TRUE_P              %prec IS {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    auto name = parser_->MakeString("is true");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | a_expr IS NOT TRUE_P            %prec IS {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    auto name = parser_->MakeString("is not true");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | a_expr IS FALSE_P             %prec IS {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    auto name = parser_->MakeString("is false");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
  | a_expr IS NOT FALSE_P           %prec IS {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode, $1);
    auto name = parser_->MakeString("is not false");
    $$ = MAKE_NODE(@2, PgTBcall, name, args);
  }
...
func_application:
  func_name '(' ')' {
    PgTExprListNode::SharedPtr args = MAKE_NODE(@1, PgTExprListNode);
    $$ = MAKE_NODE(@1, PgTBcall, $1, args);
  }
  | func_name '(' func_arg_list opt_sort_clause ')' {
    if ($4 != nullptr) {
      PARSER_UNSUPPORTED(@1);
    }
    $$ = MAKE_NODE(@1, PgTBcall, $1, $3);
  }
  ...
  | func_name '(' '*' ')' {
    if (*$1 == "count") {
      PgTExpr::SharedPtr arg = MAKE_NODE(@3, PgTStar);
      PgTExprListNode::SharedPtr args = MAKE_NODE(@2, PgTExprListNode, arg);
      $$ = MAKE_NODE(@1, PgTBcall, $1, args);
    } else {
      PARSER_INVALID(@1);
    }
...
columnref:
  ColId {
    PgTQualifiedName::SharedPtr name_node = MAKE_NODE(@1, PgTQualifiedName, $1);
    $$ = MAKE_NODE(@1, PgTRef, name_node);
  }
  | ColId indirection {
    PgTName::SharedPtr name_node = MAKE_NODE(@1, PgTName, $1);
    $2->Prepend(name_node);
    $$ = MAKE_NODE(@1, PgTRef, $2);
  }
;

indirection_el:
  '.' attr_name {
    $$ = MAKE_NODE(@1, PgTName, $2);
  }
  | '.' '*' {
    $$ = MAKE_NODE(@1, PgTNameAll);
  }
  | '[' a_expr ']' {
    PARSER_UNSUPPORTED(@1);
  }
...
AexprConst:
  ICONST {
    $$ = MAKE_NODE(@1, PgTConstVarInt, $1);
  }
  | FCONST {
    $$ = MAKE_NODE(@1, PgTConstDecimal, $1);
  }
  | Sconst {
    $$ = MAKE_NODE(@1, PgTConstText, $1);
  }
  | BCONST {                                                           // Binary string (BLOB type)
    $$ = MAKE_NODE(@1, PgTConstBinary, $1);
  }
  | XCONST {                                                                        // Hexadecimal.
    PARSER_NOTIMP(@1);
  }
  ...
  | TRUE_P {
    $$ = MAKE_NODE(@1, PgTConstBool, true);
  }
  | FALSE_P {
    $$ = MAKE_NODE(@1, PgTConstBool, false);
  }
  | NULL_P {
    $$ = MAKE_NODE(@1, PgTNull, nullptr);
  }
;
...
Iconst: ICONST {
  ...
  if (!val.ok()) {
    PARSER_ERROR_MSG(@1, SYNTAX_ERROR, "Invalid integer");
  } else {
    $$ = *val;
  }
};
...
// constants for them.
GenericType:
  type_function_name opg_ttype_modifiers {
  }
  | type_function_name attrs opg_ttype_modifiers {
  }
;
...
Numeric:
  ...
  | TINYINT {
    $$ = MAKE_NODE(@1, PgTTinyInt);
  }
  | SMALLINT {
    $$ = MAKE_NODE(@1, PgTSmallInt);
  }
  | BIGINT {
    $$ = MAKE_NODE(@1, PgTBigInt);
  }
  | REAL {
    $$ = MAKE_NODE(@1, PgTFloat, 24);
  }
  | FLOAT_P opt_float {
    $$ = MAKE_NODE(@1, PgTFloat, $2);
  }
  | DOUBLE_P PRECISION {
    $$ = MAKE_NODE(@1, PgTDouble);
  }
  | DECIMAL_P opg_ttype_modifiers {
    $$ = MAKE_NODE(@1, PgTDecimal);
  }
  | DEC opg_ttype_modifiers {
    PARSER_UNSUPPORTED(@1);
  }
  | NUMERIC opg_ttype_modifiers {
    PARSER_UNSUPPORTED(@1);
  }
;
...
ConstDatetime:
  TIMESTAMP opt_timezone {
    $$ = MAKE_NODE(@1, PgTTimestamp); // TODO ignoring timezone option for now
  }
...
DropPLangStmt:
  DROP opt_procedural LANGUAGE NonReservedWord_or_Sconst opg_tdrop_behavior {
  }
  | DROP opt_procedural LANGUAGE IF_P EXISTS NonReservedWord_or_Sconst opg_tdrop_behavior {
  }
;
...
DropFdwStmt:
  DROP FOREIGN DATA_P WRAPPER name opg_tdrop_behavior {
  }
  | DROP FOREIGN DATA_P WRAPPER IF_P EXISTS name opg_tdrop_behavior {
  }
;
...
CreateForeignServerStmt:
  CREATE SERVER name opg_ttype opt_foreign_server_version
...
DropForeignServerStmt:
  DROP SERVER name opg_tdrop_behavior {
  }
  |  DROP SERVER IF_P EXISTS name opg_tdrop_behavior {
  }
;
...
DropPolicyStmt:
  DROP POLICY name ON any_name opg_tdrop_behavior {
  }
  | DROP POLICY IF_P EXISTS name ON any_name opg_tdrop_behavior {
  }
;
...
DropTrigStmt:
  DROP TRIGGER name ON any_name opg_tdrop_behavior {
  }
  | DROP TRIGGER IF_P EXISTS name ON any_name opg_tdrop_behavior {
  }
;
...
DropAssertStmt:
  DROP ASSERTION name opg_tdrop_behavior {
  }
;
...
DropOpClassStmt:
  DROP OPERATOR CLASS any_name USING access_method opg_tdrop_behavior {
  }
  | DROP OPERATOR CLASS IF_P EXISTS any_name USING access_method opg_tdrop_behavior {
  }
;

DropOpFamilyStmt:
  DROP OPERATOR FAMILY any_name USING access_method opg_tdrop_behavior {
  }
  | DROP OPERATOR FAMILY IF_P EXISTS any_name USING access_method opg_tdrop_behavior {
  }
;
 ...
 *****************************************************************************/
DropOwnedStmt:
  DROP OWNED BY role_list opg_tdrop_behavior {
  }
;
...
TruncateStmt:
  TRUNCATE opt_table relation_expr_list opt_restart_seqs opg_tdrop_behavior {
  }
;
...
RevokeStmt:
  REVOKE privileges ON privilege_target FROM grantee_list opg_tdrop_behavior {
  }
  | REVOKE GRANT OPTION FOR privileges ON privilege_target FROM grantee_list opg_tdrop_behavior {
  }
;
...
RevokeRoleStmt:
  REVOKE privilege_list FROM role_list opt_granted_by opg_tdrop_behavior {
  }
  | REVOKE ADMIN OPTION FOR privilege_list FROM role_list opt_granted_by opg_tdrop_behavior {
  }
;
...
DefACLAction:
  ...
  | REVOKE privileges ON defacl_privilege_target FROM grantee_list opg_tdrop_behavior {
  }
  | REVOKE GRANT OPTION FOR privileges ON defacl_privilege_target
  FROM grantee_list opg_tdrop_behavior {
  }
;
...
RemoveFuncStmt:
  DROP FUNCTION func_name func_args opg_tdrop_behavior {
  }
  | DROP FUNCTION IF_P EXISTS func_name func_args opg_tdrop_behavior {
  }
;

RemoveAggrStmt:
  DROP AGGREGATE func_name aggr_args opg_tdrop_behavior {
  }
  | DROP AGGREGATE IF_P EXISTS func_name aggr_args opg_tdrop_behavior {
  }
;

RemoveOperStmt:
  DROP OPERATOR any_operator oper_argtypes opg_tdrop_behavior {
  }
  | DROP OPERATOR IF_P EXISTS any_operator oper_argtypes opg_tdrop_behavior {
  }
;
...
DropCastStmt:
  DROP CAST opt_if_exists '(' Typename AS Typename ')' opg_tdrop_behavior {
  }
;
...
DropTransformStmt:
  DROP TRANSFORM opt_if_exists FOR Typename LANGUAGE name opg_tdrop_behavior {
  }
;
...
RenameStmt:
  ...
  | ALTER TYPE_P any_name RENAME ATTRIBUTE name TO name opg_tdrop_behavior {
  }
;
...
DropRuleStmt:
  DROP RULE name ON any_name opg_tdrop_behavior {
  }
  | DROP RULE IF_P EXISTS name ON any_name opg_tdrop_behavior {
  }
;
...
AlterDomainStmt:
  ...
  | ALTER DOMAIN_P any_name DROP CONSTRAINT name opg_tdrop_behavior {
  }
  /* ALTER DOMAIN <domain> DROP CONSTRAINT IF EXISTS <name> [RESTRICT|CASCADE] */
  | ALTER DOMAIN_P any_name DROP CONSTRAINT IF_P EXISTS name opg_tdrop_behavior {
  }
...
VacuumStmt:
  ...
  | VACUUM '(' vacuum_option_list ')' qualified_name opg_tname_list {
  }
;
...
AnalyzeStmt:
  ...
  | analyze_keyword opt_verbose qualified_name opg_tname_list {
  }
;
