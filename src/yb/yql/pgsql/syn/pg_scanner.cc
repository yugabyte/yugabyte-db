...
void LexProcessor::ScanInit(PgCompileContext *compile_context) {
  yyrestart(compile_context->input_file());
  ...
  if (compile_context_ != nullptr) {
    yy_flex_debug = compile_context->trace_scanning();
  }
}
...
int LexProcessor::LexerInput(char* buf, int max_size) {
  return compile_context_->Read(buf, max_size);
}
...
GramProcessor::symbol_type LexProcessor::ScanError(const char *token) {
  ...
  Status s = compile_context_->Error(Location(token_loc_),
...
GramProcessor::symbol_type LexProcessor::ScanError(const char *message, ErrorCode errcode) {
  // Flex scanner will raise exception by itself, so we don't return Status::Error here.
  Status s = compile_context_->Error(Location(token_loc_), message, errcode);
  VLOG(3) << s.ToString();
  return GramProcessor::make_SCAN_ERROR(cursor_);
}
...
void LexProcessor::TruncateIdentifier(const MCSharedPtr<MCString>& ident, bool warn) {
  ...
  if (len >= NAMEDATALEN) {
    ...
    if (warn) {
      ...
      compile_context_->Warn(Location(token_loc_), warn_msg, ErrorCode::NAME_TOO_LONG);
    }
    ident->resize(len);
  }
}
...
void LexProcessor::check_string_escape_warning(unsigned char ychar) {
  if (ychar == '\'') {
    ...
    if (warn_on_first_escape_ && escape_string_warning_)
      compile_context_->Warn(Location(token_loc_),
...
#include "yb/yql/pgsql/syn/pg_kwlist.h"
};
