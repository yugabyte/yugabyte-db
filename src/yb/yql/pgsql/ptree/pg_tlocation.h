...
#ifndef YB_YQL_PGSQL_PTREE_PG_TLOCATION_H_
#define YB_YQL_PGSQL_PTREE_PG_TLOCATION_H_
...
class PgTLocation : public MCBase {
 public:
  //------------------------------------------------------------------------------------------------
  // Public types.
  typedef MCSharedPtr<PgTLocation> SharedPtr;
  typedef MCSharedPtr<const PgTLocation> SharedPtrConst;

  typedef MCUniPtr<PgTLocation> UniPtr;
  typedef MCUniPtr<const PgTLocation> UniPtrConst;
...
operator<< (std::basic_ostream<YYChar>& ostr, const PgTLocation& loc) {
  // In general, we only need to record the starting location of a token where error occurs.
...
#endif  // YB_YQL_PGSQL_PTREE_PG_TLOCATION_H_
