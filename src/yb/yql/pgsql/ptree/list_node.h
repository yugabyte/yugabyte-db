...
#ifndef YB_YQL_PGSQL_PTREE_LIST_NODE_H_
#define YB_YQL_PGSQL_PTREE_LIST_NODE_H_
...
class TreeListNode : public TreeNode {
 public:
  ...
  CHECKED_STATUS Analyze(PgCompileContext *compile_context) override {
    for (auto tnode : node_list_) {
      RETURN_NOT_OK(tnode->Analyze(compile_context));
    }
    return Status::OK();
  }

  virtual CHECKED_STATUS Analyze(PgCompileContext *compile_context,
                                 TreeNodePtrOperator<PgCompileContext, NodeType> node_op) {
    for (auto tnode : node_list_) {
      RETURN_NOT_OK(node_op(tnode.get(), compile_context));
    }
    return Status::OK();
  }
...
#endif  // YB_YQL_PGSQL_PTREE_LIST_NODE_H_
