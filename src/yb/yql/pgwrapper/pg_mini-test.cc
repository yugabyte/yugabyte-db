...
// neither consumed nor populated), so every path that installs a new TableInfo has to arm it
// again. A path that does not leaves the table's cache off until the next ALTER or restart, and
// every scan of the table then does a full table-tombstone lookup (an IntentAwareIterator plus a
// seek) that the cache exists to avoid. These tests cover old-schema-version GC, which runs after
// compactions, and the index backfill-done metadata update.
...
// rebuilt DocReadContext must still be able to use the tombstone-time cache.
TEST_F_EX(PgMiniTest, TombstoneCacheArmedAfterSchemaGC, PgMiniTombstoneCacheRebuildTest) {
  ...
  ASSERT_NE(before_gc->doc_read_context->tombstone_cache_watermark(), HybridTime::kMax);
  ...
  // A base-table scan that would refill an armed cache.
  ASSERT_OK(conn_->FetchRow<int64_t>("SELECT sum(w) FROM t"));
  ...
  const auto diag = Format(
      "TOMBSTONE CACHE LEFT OFF: old-schema-version GC rebuilt the table's DocReadContext "
      "(packings now $0) and nothing re-armed it, so every scan does a full table-tombstone "
      "lookup. Regular-DB seeks for $1 single-row PK UPDATEs: $2 before the GC, $3 after.",
      ctx.schema_packing_storage.VersionsToString(), kNumUpdates, seeks_before_gc,
      seeks_after_gc);
...
TEST_F_EX(
  ...
  auto after = ASSERT_RESULT(ColocatedTableInfo(kTableColocationId));
  ...
  ASSERT_NE(after->doc_read_context->tombstone_cache_watermark(), HybridTime::kMax);
  ASSERT_TRUE(after->doc_read_context->table_tombstone_time().has_value());
}
...
// follows also arms every colocated table in the tablet, so this does not isolate the arm in
// MarkBackfillDone.
