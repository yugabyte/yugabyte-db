...
Result<bool> IsPerformOpOnCatalogTable(const yb::tserver::PgPerformRequestPB* req) {
  if (req->ops().empty()) {
    return STATUS(IllegalState, "No operations in Perform request");
  }

  const auto& first_op = req->ops(0);
  string table_id = first_op.has_read() ? first_op.read().table_id()
                                        : first_op.write().table_id();
...
Status MockPerformBeforeFunc(
    ...
    yb::tserver::PgPerformResponsePB* resp, tserver::PgClientMockCallContext* context) {
