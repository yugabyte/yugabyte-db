...
DEFINE_NON_RUNTIME_PREVIEW_bool(ysql_enable_documentdb_gateway, false,
    "Enable the DocumentDB Gateway for MongoDB wire protocol support. "
    "Requires ysql_enable_documentdb to be true.");
...
static bool ValidateDocumentDBGateway(const char* flag_name, bool value) {
  if (value && !FLAGS_ysql_enable_documentdb) {
    LOG_FLAG_VALIDATION_ERROR(flag_name, value)
        << "ysql_enable_documentdb must be true to enable the DocumentDB Gateway";
    return false;
  }
  return ValidateDocumentDB(flag_name, value);
}

DEFINE_validator(ysql_enable_documentdb_gateway, &ValidateDocumentDBGateway);
...
Result<string> WritePostgresConfig(const PgProcessConf& conf) {
  ...
  if (FLAGS_ysql_enable_documentdb) {
    ...
    if (FLAGS_ysql_enable_documentdb_gateway) {
  ...
  if (FLAGS_ysql_enable_documentdb_gateway) {
    // Write the gateway setup configuration file as JSON.
    string gateway_config_path = JoinPathSegments(conf.data_dir, "documentdb_gateway_config.json");
    {
      std::ofstream gw_conf_file;
      gw_conf_file.open(gateway_config_path, std::ios_base::out | std::ios_base::trunc);
      if (!gw_conf_file) {
        return STATUS_FORMAT(
            IOError,
            "Failed to write DocumentDB gateway config file '$0': errno=$1: $2",
            gateway_config_path, errno, ErrnoToString(errno));
      }
      gw_conf_file
          << "{\n"
          << Format("  \"NodeHostName\": \"$0\",\n", conf.listen_addresses)
          << "  \"BlockedRolePrefixes\": "
          << "[ \"documentdb\", \"citus\", \"pg\", \"internal_role\" ],\n"
          << Format("  \"PostgresPort\": $0,\n", conf.pg_port)
          << Format("  \"GatewayListenPort\": $0,\n", FLAGS_ysql_documentdb_gateway_port)
          << "  \"CertificateOptions\": {\n"
