...
Status WriteDocumentDBGatewayConfig(
    const string& data_dir, const string& listen_addresses, uint16_t pg_port) {
  string gateway_config_path = JoinPathSegments(data_dir, "documentdb_gateway_config.json");
  ...
  gw_conf_file
      ...
      << "  \"BlockedRolePrefixes\": "
      << "[ \"documentdb\", \"citus\", \"pg\", \"internal_role\", \"yugabyte\" ],\n"
...
Result<string> WritePostgresConfig(const PgProcessConf& conf) {
  ...
  if (FLAGS_ysql_enable_documentdb) {
    string gateway_config_path = JoinPathSegments(
        conf.data_dir, "documentdb_gateway_config.json");
    RETURN_NOT_OK(WriteDocumentDBGatewayConfig(
