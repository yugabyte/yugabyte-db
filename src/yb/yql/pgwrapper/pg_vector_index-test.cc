...
TEST_P(PgVectorIndexTest, CompactionDuringShutdown) {
  // The scenario relies on dropping the table to tear the tablet down and free the VectorLSM out
  // from under the parked compaction task. For a colocated table DROP TABLE does not delete the
  // shared tablet, so the VectorLSM is not freed the same way; on this branch the colocated drop
  // also hits an unrelated DDL transaction abort. The fix is exercised by the non-colocated case.
  if (IsColocated()) {
    GTEST_SKIP() << "Scenario requires tablet teardown, which DROP TABLE does not do for a "
                    "colocated table";
  }
...
TEST_P(PgVectorIndexCompactionPoolTest, ShutdownNotBlockedByCompaction) {
  ...
  // Wait for an in-flight callback to finish: it reads the stack locals captured by reference
  // above, and the follower's compaction outlives this test body.
...
TEST_P(PgVectorIndexSingleServerTest, ManualSplitWithReverseMappingOnlySstFile) {
  ...
  ASSERT_RESULT(tablet->GetEncodedMiddleSplitKey());
