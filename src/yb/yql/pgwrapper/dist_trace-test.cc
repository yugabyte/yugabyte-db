 ...
 public:
  ...
  Result<Span> WaitForRemoteChildSpan(
    ...
    return WaitForParentedSpan(
        trace_id,
        [&](const Span& server) {
          if (server.service_name != server_service || !server.op_name.starts_with(op_prefix)) {
            return false;
          }
          auto sys_it = server.str_attrs.find("rpc.system");
          return sys_it != server.str_attrs.end() && sys_it->second == expected_rpc_system;
        },
        [&](const Span& client) {
          return client.service_name == client_service && client.op_name.starts_with(op_prefix);
        },
