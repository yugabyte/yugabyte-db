...
struct Span {
  ...
  bool operator==(const Span& other) const {
    bool name_match = service_name == other.service_name && op_name == other.op_name;
    bool text_match = query_text == other.query_text;
    bool id_match = trace_id == other.trace_id;
    bool db_id_match = db_id == other.db_id;
    bool user_id_match = user_id == other.user_id;
    // parent_span_id and span_id are not used for comparison as they are generated by the server
    return name_match && text_match && id_match && db_id_match && user_id_match;
  }

  bool operator<(const Span& other) const {
    // Sort primarily by op_name then by other fields to ensure a deterministic order.
  ...
  friend std::ostream& operator<<(std::ostream& os, const Span& s) {
    return os << "{service=" << s.service_name << " op=" << s.op_name
              ...
              << " parent_span_id=" << s.parent_span_id << " span_id=" << s.span_id << "}";
  }
};
...
struct Trace {
  ...
  bool operator==(const Trace& other) const {
    bool id_match = trace_id == other.trace_id;
    bool query_text_match = query_text == other.query_text;
    return id_match && query_text_match;
  }

  friend std::ostream& operator<<(std::ostream& os, const Trace& t) {
    os << "{trace_id=" << t.trace_id << " query=" << t.query_text << " spans=[";
    for (size_t i = 0; i < t.spans.size(); ++i) {
      os << t.spans[i];
      if (i < t.spans.size() - 1) os << ", ";
    }
    return os << "]}";
  }
};
...
class OtlpHttpCollector {
 public:
  ...
  Status VerifySpansMatch(Trace& actual, const Trace& expected) {
    SCHECK_EQ(actual.spans.size(), expected.spans.size(), IllegalState,
        Format("Span count mismatch for query '$0': expected $1, got $2",
               expected.query_text, expected.spans.size(), actual.spans.size()));

    std::sort(actual.spans.begin(), actual.spans.end());

    SCHECK_EQ(
        actual.spans, expected.spans, IllegalState,
        Format("Spans mismatch for query '$0'", expected.query_text));
    return Status::OK();
  }
  ...
  Status VerifyAgainstCollectorTraces(const std::vector<Trace>& expected_traces)
    ...
    RETURN_NOT_OK(WaitFor(
        [this, &expected_traces]() -> Result<bool> {
          ...
          for (const auto& expected : expected_traces) {
            ...
            if (it == traces_.end() || it->second.spans.size() < expected.spans.size()) {
              return false;
            }
          }
          return true;
    ...
    for (const auto& expected : expected_traces) {
      ...
      SCHECK_EQ(it->second, expected, IllegalState,
          Format("Traces mismatch for query '$0'", expected.query_text));
