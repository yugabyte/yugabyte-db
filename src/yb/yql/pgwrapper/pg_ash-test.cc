...
// Enables QPM (yb_pg_stat_plans_track=top) so plan_ids are recorded for TestPlanIdExistsInStatPlans.
class PgAshSingleNodeWithQPM : public PgAshSingleNode {
 public:
  void UpdateMiniClusterOptions(ExternalMiniClusterOptions* options) override {
    options->extra_tserver_flags.push_back(
        "--ysql_pg_conf_csv=yb_pg_stat_plans_track=top");
...
TEST_F(PgAshSingleNode, TestDmlPlanIdNonZero) {
  ...
  auto zero_plan_id_count = ASSERT_RESULT(conn_->FetchRow<int64_t>(Format(
      "SELECT COUNT(*) FROM yb_active_session_history "
      "WHERE query_id = $0 AND plan_id = 0", query_id)));
  ASSERT_EQ(zero_plan_id_count, 0);
}

TEST_F(PgAshSingleNode, TestUtilityPlanIdZero) {
  static constexpr auto kTableName = "plan_id_util_test";

  ASSERT_OK(conn_->Execute(Format("CREATE TABLE $0 (k INT PRIMARY KEY, v INT)", kTableName)));
  ASSERT_OK(conn_->Execute(Format("INSERT INTO $0 SELECT i, i FROM generate_series($1, $2) AS i",
      kTableName, 1, (kIsDebug ? 100000 : 10000000))));

  // CREATE INDEX is a utility statement (ProcessUtility path, plan_id = 0).
  // Backfilling makes it long-running enough for ASH sampling.
  ASSERT_OK(conn_->Execute(Format("CREATE INDEX ON $0 (v)", kTableName)));

  auto query_id = ASSERT_RESULT(conn_->FetchRow<int64_t>(
      "SELECT queryid FROM pg_stat_statements WHERE query LIKE 'CREATE INDEX%'"));
...
TEST_F(PgAshSingleNodeWithQPM, TestPlanIdExistsInStatPlans) {
  ...
  const auto num_rows = kIsDebug ? 100000 : 1000000;
  ASSERT_OK(conn_->Execute(Format(
  ...
  // A JOIN query generates hints that QPM can record.
  ...
  // Verify ASH has plan_ids for this query.
  auto ash_plan_id_count = ASSERT_RESULT(conn_->FetchRow<int64_t>(Format(
      "SELECT COUNT(DISTINCT plan_id) FROM yb_active_session_history "
      "WHERE query_id = $0 AND plan_id != 0", query_id)));
  ASSERT_GT(ash_plan_id_count, 0);

  // Verify QPM recorded the plan.
  auto qpm_plan_count = ASSERT_RESULT(conn_->FetchRow<int64_t>(Format(
      "SELECT COUNT(*) FROM yb_pg_stat_plans WHERE queryid = $0", query_id)));
  ASSERT_GT(qpm_plan_count, 0);
}

TEST_F(PgAshSingleNode, TestPlanIdRestoredAfterNestedQuery) {
  ASSERT_OK(conn_->Execute("CREATE TABLE outer_t (k INT PRIMARY KEY, v INT)"));
  ASSERT_OK(conn_->Execute("CREATE TABLE inner_t (k SERIAL PRIMARY KEY, src INT)"));

  ASSERT_OK(conn_->Execute(
      "CREATE OR REPLACE FUNCTION trg_fn() RETURNS TRIGGER AS $$ "
      "BEGIN "
      "  INSERT INTO inner_t (src) VALUES (NEW.k); "
      "  RETURN NEW; "
      "END; $$ LANGUAGE plpgsql"));

  ASSERT_OK(conn_->Execute(
      "CREATE TRIGGER trg AFTER INSERT ON outer_t "
      "FOR EACH ROW EXECUTE FUNCTION trg_fn()"));

  // With 'all', nested queries exercise the (query_id, plan_id) stack
  ASSERT_OK(conn_->Execute("SET pg_stat_statements.track = 'all'"));

  ASSERT_OK(conn_->Execute(Format(
      "INSERT INTO outer_t SELECT i, i FROM generate_series(1, $0) AS i",
      (kIsDebug ? 10000 : 50000))));

  auto query_id = ASSERT_RESULT(conn_->FetchRow<int64_t>(
      "SELECT queryid FROM pg_stat_statements WHERE query LIKE "
      "'INSERT INTO outer_t%'"));
  ...
  // After each trigger's inner query, the outer query's plan_id must be
  // restored correctly from the stack - expect exactly one distinct value
  auto distinct_plan_ids = ASSERT_RESULT(conn_->FetchRow<int64_t>(Format(
      "SELECT COUNT(DISTINCT plan_id) FROM yb_active_session_history "
      "WHERE query_id = $0 AND plan_id != 0", query_id)));
  ASSERT_EQ(distinct_plan_ids, 1);
}

TEST_F(PgAshTest, TestTServerPlanIdPropagation) {
  static constexpr auto kTableName = "plan_id_prop_test";
  ...
  auto plan_ids = ASSERT_RESULT(conn_->FetchRows<int64_t>(Format(
      "SELECT DISTINCT plan_id FROM yb_active_session_history "
      "WHERE query_id = $0 AND plan_id != 0", query_id)));
  ASSERT_EQ(plan_ids.size(), 1);
  auto plan_id = plan_ids[0];

  for (auto* ts : cluster_->tserver_daemons()) {
    auto conn = ASSERT_RESULT(ConnectToTs(*ts));
    LOG_WITH_FUNC(INFO) << "query " << ts->uuid();
    const auto count = ASSERT_RESULT((conn.FetchRow<int64_t>(Format(
        "SELECT COUNT(*) FROM yb_active_session_history "
        "WHERE query_id = $0 AND plan_id = $1", query_id, plan_id))));
    ASSERT_GT(count, 0);
  }
  LOG_WITH_FUNC(INFO) << "done";
}
