...
Status PgDdlAtomicityTestBase::VerifyAllFailingDdlsNotRolledBack(client::YBClient* client,
  ...
  if (VERIFY_RESULT(client->ListTables(kAddPk, false /* exclude_ysql */)).size() != 2)
    return STATUS_FORMAT(IllegalState, "Expected 2 tables for $0", kAddPk);
  if (VERIFY_RESULT(client->ListTables(kDropPk, false /* exclude_ysql */)).size() != 2)
...
Status PgDdlAtomicityTestBase::VerifyAllFailingDdlsRolledBack(PGConn *conn,
  ...
  RETURN_NOT_OK(LoggedWaitFor([&]() -> Result<bool> {
      auto addPkTables = VERIFY_RESULT(client->ListTables(kAddPk, false /* exclude_ysql */));
      auto dropPkTables = VERIFY_RESULT(client->ListTables(kDropPk, false /* exclude_ysql */));
      return std::count_if(addPkTables.begin(), addPkTables.end(), [&](const auto& table) {
        return table.namespace_name() == database && table.table_name() == kAddPk;
      }) == 1 && std::count_if(dropPkTables.begin(), dropPkTables.end(), [&](const auto& table) {
        return table.namespace_name() == database && table.table_name() == kDropPk;
      }) == 1;
...
Status PgDdlAtomicityTestBase::WaitForDdlVerificationAfterSuccessfulDdl(
  ...
  RETURN_NOT_OK(LoggedWaitFor([&]() -> Result<bool> {
      if (VERIFY_RESULT(client->ListTables(kAddPk, false /* exclude_ysql */)).size() == 1
          && VERIFY_RESULT(client->ListTables(kDropPk, false /* exclude_ysql */)).size() == 1)
        return true;
      return false;
