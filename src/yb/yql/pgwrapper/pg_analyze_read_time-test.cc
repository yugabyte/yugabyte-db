...
class PgAnalyzeBlockSamplingRetentionTest : public PgAnalyzeReadTimeTest {
 ...
 protected:
  ...
  void TestCompactionDuringAnalyze(
      std::function<bool(const PgsqlResponsePB&)> is_trigger_response) {
    if (!kIsDebug) {
    ...
    sync_point.SetCallBack(
        ...
        [this, &compact_conn, &injected, &is_trigger_response](void* arg) {
          ...
          if (read_resp->pgsql_batch().empty() ||
              !read_resp->pgsql_batch(0).has_sampling_state() ||
              !read_resp->pgsql_batch(0).sampling_state().is_blocks_sampling_stage() ||
              !is_trigger_response(read_resp->pgsql_batch(0)) ||
...
TEST_F_EX(
  ...
  TestCompactionDuringAnalyze([](const PgsqlResponsePB&) { return true; });
}
...
TEST_F_EX(
  ...
  TestCompactionDuringAnalyze(
      [tablets_completed = size_t{0}](const PgsqlResponsePB& resp) mutable {
