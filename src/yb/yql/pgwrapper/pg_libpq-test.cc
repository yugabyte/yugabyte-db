...
TEST_F(PgLibPqPgssQtextLeakTest, TestNoMemoryLeakOnCorruptedPgssQtext) {
  ...
  const int64_t kMaxRssGrowthKb = RegularBuildVsSanitizers(50, 300) * 1024;
...
class PgLibPqAmopNegCacheTest : public PgLibPqTest {
 protected:
  ...
  void TestAmopNegCacheMiss(bool preloaded) {
    ...
    Status s = conn2.Execute("SET yb_neg_catcache_ids='52252'");
    ASSERT_NOK(s);
    ASSERT_EQ(PgsqlError(s),  YBPgErrorCode::YB_PG_INVALID_PARAMETER_VALUE);

    s = conn2.Execute("SET yb_neg_catcache_ids='3.14'");
    ASSERT_NOK(s);
    ASSERT_EQ(PgsqlError(s),  YBPgErrorCode::YB_PG_INVALID_PARAMETER_VALUE);
