...
Result<string> PgLibPqTest::GetPostmasterPidViaShell(PGConn* conn) {
  auto backend_pid = VERIFY_RESULT(conn->FetchValue<int32_t>("SELECT pg_backend_pid()"));
...
TEST_F(PgOidCollisionReservedNormalOid, PgOidCollisionSystemPostgresTest) {
  ...
  auto query = "SELECT oid FROM pg_database WHERE oid >= 16384 ORDER BY oid"s;
  const auto values = ASSERT_RESULT(conn.FetchAll<int32_t>(query));
  decltype(values) expected = {{65532}, {65533}, {65534}, {65536}};
...
TEST_F(PgLibPqTest, InconsistentIndexRead) {
  ...
  auto initial_sum = ASSERT_RESULT(
      conn.FetchValue<int64_t>("SELECT SUM(balance) FROM bank WHERE id >= 0"));
  ...
  thread_holder.AddThreadFunctor([this, &stop = thread_holder.stop_flag(), initial_sum]() {
    ...
    while (!stop.load(std::memory_order_acquire)) {
      auto sum_balance = ASSERT_RESULT(reader_conn.FetchValue<int64_t>(
