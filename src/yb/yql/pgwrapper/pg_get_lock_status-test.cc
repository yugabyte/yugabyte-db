...
TEST_F(PgGetLockStatusTest, TestWaiterLockContainingColumnId) {
  ...
  auto column_id = ASSERT_RESULT(session.conn->FetchValue<std::string>(
  ...
  ASSERT_EQ(column_id, ASSERT_RESULT(session.conn->FetchValue<std::string>(
...
TEST_F(PgGetLockStatusTest, TestColocatedWaiterWriteLock) {
  ...
  auto value = ASSERT_RESULT(
      setup_conn.FetchValue<int64_t>("SELECT COUNT(*) FROM pg_locks WHERE granted = true"));
  ASSERT_EQ(value, 4);
  ...
  // 1 {WEAK_READ,WEAK_WRITE} on the table
  value = ASSERT_RESULT(
      setup_conn.FetchValue<int64_t>("SELECT COUNT(*) FROM pg_locks WHERE granted = false"));
...
TEST_F(PgGetLockStatusTest, TestWaitStartTimeIsConsistentAcrossWaiterReEntries) {
  ...
  auto init_micros = ASSERT_RESULT(conn.FetchValue<int64_t>(
  ...
  auto now_micros = ASSERT_RESULT(conn.FetchValue<int64_t>("SELECT NOW()"));

  auto wait_start_time_1_query =
      "SELECT DISTINCT(waitstart) FROM pg_locks WHERE NOT granted ORDER BY waitstart LIMIT 1";
  auto wait_start_time_1 = ASSERT_RESULT(conn.FetchValue<int64_t>(wait_start_time_1_query));
  ...
  auto wait_start_time_2 = ASSERT_RESULT(conn.FetchValue<int64_t>(wait_start_time_2_query));
  ...
  ASSERT_EQ(wait_start_time_1, ASSERT_RESULT(conn.FetchValue<int64_t>(wait_start_time_1_query)));
    ASSERT_EQ(wait_start_time_2, ASSERT_RESULT(conn.FetchValue<int64_t>(wait_start_time_2_query)));
...
TEST_F(PgGetLockStatusTest, TestLockStatusRespHasHostNodeSet) {
  ...
  auto conn = ASSERT_RESULT(Connect());
  ASSERT_OK(conn.ExecuteFormat("SET yb_locks_min_txn_age='$0ms'", kMinTxnAgeMs));
  ASSERT_EQ(ASSERT_RESULT(conn.FetchValue<int64>(kPgLocksQuery)), 0);
...
TEST_F(PgGetLockStatusTestRF3, TestLocksOfSingleShardWaiters) {
  ...
  ASSERT_OK(setup_conn.Execute("SET yb_locks_max_transactions=1"));
  auto num_txns = ASSERT_RESULT(setup_conn.FetchValue<int64>(
  ...
  num_txns = ASSERT_RESULT(setup_conn.FetchValue<int64>(
  ...
  auto waiter1_start_time = ASSERT_RESULT(setup_conn.FetchValue<int64>(
  ...
  num_txns = ASSERT_RESULT(setup_conn.FetchValue<int64>(
      "SELECT COUNT(DISTINCT(waitstart)) FROM pg_locks WHERE fastpath"));
  ASSERT_EQ(num_txns, 1);
  ASSERT_EQ(waiter1_start_time, ASSERT_RESULT(setup_conn.FetchValue<int64>(
...
TEST_F(PgGetLockStatusTest, TestPgLocksOutputAfterNodeOperations) {
  ...
  ASSERT_EQ(ASSERT_RESULT(setup_conn.FetchValue<int64>(kPgLocksQuery)), 2);
