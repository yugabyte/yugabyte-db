...
TEST_F(PgHeapSnapshotTest, TestYsqlLogHeapSnapshot) {
  ...
  auto pid = ASSERT_RESULT(conn1.FetchValue<PGUint32>("SELECT pg_backend_pid()"));
  ASSERT_GT(pid, 0) << "Expected a valid PID";

  // Call yb_log_backend_heap_snapshot from conn2 targeting conn1's PID
  auto return_value =
      ASSERT_RESULT(conn2.FetchValue<bool>(Format("SELECT yb_log_backend_heap_snapshot($0)", pid)));
  ...
  return_value = ASSERT_RESULT(
      conn2.FetchValue<bool>(Format("SELECT yb_log_backend_heap_snapshot(pg_backend_pid())")));
  ...
  return_value = ASSERT_RESULT(
      conn2.FetchValue<bool>(Format("SELECT yb_log_backend_heap_snapshot_peak($0)", pid)));
  ASSERT_EQ(return_value, true) << "Expected yb_log_backend_heap_snapshot_peak to return true";

  // Test with invalid PID
  return_value =
      ASSERT_RESULT(conn2.FetchValue<bool>(Format("SELECT yb_log_backend_heap_snapshot(0)")));
...
TEST_F(PgHeapSnapshotTest, TestYsqlLogHeapSnapshotPermissions) {
  ...
  auto superuser_pid = ASSERT_RESULT(
      conn_superuser1.FetchValue<PGUint32>("SELECT pg_backend_pid()"));
  ...
  auto test_user1_pid = ASSERT_RESULT(conn_test1.FetchValue<PGUint32>("SELECT pg_backend_pid()"));
  ASSERT_GT(test_user1_pid, 0) << "Expected a valid PID";

  // Connect as test_user2
  auto conn_test2 = ASSERT_RESULT(PgConnect("test_user2"));
  auto test_user2_pid = ASSERT_RESULT(conn_test2.FetchValue<PGUint32>("SELECT pg_backend_pid()"));
  ASSERT_GT(test_user2_pid, 0) << "Expected a valid PID";

  // Test logging the superuser's heap snapshot with test_user1
  // Should fail due to insufficient permissions
  auto res =
      conn_test1.FetchValue<bool>(Format("SELECT yb_log_backend_heap_snapshot($0)", superuser_pid));
  ...
  auto return_value = ASSERT_RESULT(conn_superuser2.FetchValue<bool>(
  ...
  // Should fail due to insufficient permissions
  res = conn_test1.FetchValue<bool>(
