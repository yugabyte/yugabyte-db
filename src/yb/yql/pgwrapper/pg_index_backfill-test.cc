...
TEST_F_EX(PgIndexBackfillTest,
          SnapshotTooOldOnIndexWritePath,
          PgIndexBackfillSnapshotTooOld) {
  auto client = ASSERT_RESULT(cluster_->CreateClient());
  constexpr int kTimeoutSec = 3;
...
TEST_F(PgIndexBackfillTest, SimulateEmptyIndexesForStackOverflow) {
  ASSERT_OK(conn_->ExecuteFormat("CREATE TABLE $0 (i int)", kTableName));
...
TEST_F(PgIndexBackfillTest, MultipleIndexesFirstOneInvalid) {
...
TEST_F_EX(
    PgIndexBackfillTest, DeleteAndInsertSameValueInTxn, PgIndexBackfillBlockDoBackfill) {
  TestDeleteAndInsertSameValue(/* use_single_txn= */ true);
}

// DELETE and INSERT as separate statements (different HTs).
TEST_F_EX(
    PgIndexBackfillTest, DeleteAndInsertSameValueSeparateStmts, PgIndexBackfillBlockDoBackfill) {
...
TEST_F_EX(PgIndexBackfillTest, DuplicatesExistBeforeBackfill, PgIndexBackfillBlockDoBackfill) {
  ASSERT_OK(conn_->ExecuteFormat("CREATE TABLE $0 (a int PRIMARY KEY, b int)", kTableName));
...
TEST_F_EX(PgIndexBackfillTest,
  ...
  BumpCatalogVersion(2, conn_.get());
...
TEST_F(PgIndexBackfillBlockDoBackfill, ConcurrentInplaceUpdateCoveringIndex) {
  constexpr int kNumRows = 10;
