...
TEST_F_EX(PgIndexBackfillTest,
  ...
  ASSERT_EQ(ASSERT_RESULT(GetValue<int64_t>(res.get(), 0, 1)), 0);
...
TEST_F(PgIndexBackfillTest,
  ...
  const auto res = ASSERT_RESULT(conn_->FetchFormat(
      "SELECT reltuples FROM pg_class WHERE relname='$0'", kTableName));
  const auto val = ASSERT_RESULT(GetInt32(res.get(), 0, 0));
  const auto num_rows_indexed_table = *pointer_cast<const float*>(&val);
...
TEST_F_EX(PgIndexBackfillTest,
  ...
  const auto res = ASSERT_RESULT(new_db_conn.FetchFormat(
      "SELECT reltuples FROM pg_class WHERE relname='$0'", kTableName));
  const auto val = ASSERT_RESULT(GetInt32(res.get(), 0, 0));
  const auto num_rows_indexed_table = *pointer_cast<const float*>(&val);
