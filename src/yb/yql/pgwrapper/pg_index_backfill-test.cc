...
class PgIndexBackfillReadCommittedBlockIndislive : public PgIndexBackfillTestEnableWait {
 public:
  void UpdateMiniClusterOptions(ExternalMiniClusterOptions* options) override {
    PgIndexBackfillTestEnableWait::UpdateMiniClusterOptions(options);
...
TEST_F_EX(PgIndexBackfillTest,
          YB_DISABLE_TEST_IN_SANITIZERS(CatVerBumps),
  ...
  ASSERT_OK(WaitForIndexScan(query));
  ASSERT_EQ(ASSERT_RESULT(conn_->FetchValue<PGUint64>(query)), 0);
}
