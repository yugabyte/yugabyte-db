...
TEST_F(PgCatalogVersionTest, InvalMessageExceedPgMaxNumMessagesTest) {
  RestartClusterWithInvalMessageEnabled(
      {"--ysql_yb_ddl_transaction_block_enabled=true",
       "--ysql_pg_conf_csv=yb_test_make_all_ddl_statements_incrementing=true,"
...
TEST_F(PgCatalogVersionTest, AnalyzeAllTables) {
  ...
  string expected = IsTransactionalDdlEnabled()
      ? "$0, 2, 120; $0, 3, 768; $0, 4, 624; $0, 5, 720; "
        ...
        "$0, 36, 216; $0, 37, 96; $0, 38, 192; $0, 39, 240; $0, 40, 168; "
...
TEST_F(PgCatalogVersionTest, InvalMessageDeltaTableLoad) {
  for (int i = 0; i < 2; i++) {
    ...
    if (i == 0) {
      ...
      ASSERT_EQ(get_schema_count_after - get_schema_count_before, 655);
    } else {
      ASSERT_EQ(open_table_count_after - open_table_count_before, 638);
      ASSERT_EQ(get_schema_count_after - get_schema_count_before, 755);
    }
  }
}
