...
// A statement that cannot be retried for a reason of its own settles the retry question before the
// write fastpath is ever consulted, so the error does not blame the fastpath. This mirrors the
// second permutation of the yb.orig.inplace_catalog_updates isolation test: the GRANT updates
// pg_class first, so the CREATE INDEX in the other transaction conflicts over that catalog row
// after its backfill has already skipped intents.
TEST_F(SkipIntentsRetryReasonTest, TestRetryReasonPrefersStatementOverSkippedIntents) {
  ...
  // The command-tag rejection returns without annotating the error on this branch - the
  // user-facing "retry of CREATE INDEX has not been validated" text arrives with the retry-reason
  // work that is not backported here - so the ordering shows up as the absence of the fastpath
  // reason rather than the presence of the statement's own.
  ASSERT_STR_NOT_CONTAINS(result.ToString(), "we have skipped intents write");
}
