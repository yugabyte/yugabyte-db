...
Result<std::string> FetchRangeSplitClause(PGConn* conn, Oid table_oid) {
  return conn->FetchValue<std::string>(Format(
      "SELECT range_split_clause from yb_get_range_split_clause($0)", table_oid));
}

Result<std::string> FetchRangeSplitClause(PGConn* conn, const std::string& table_name) {
  const auto table_oid = VERIFY_RESULT(FetchTableRelOid(conn, table_name));
...
TEST_P(PgPartitioningWaitQueuesOffTest, YB_DISABLE_TEST_IN_TSAN(RowLockWithSplit)) {
  ...
  for (const auto& key : keys) {
    ...
    const auto error = update_status.ToString();
    ASSERT_TRUE(
        error.find("could not serialize access due to concurrent update") != std::string::npos ||
        error.find("Conflicts with higher priority transaction") != std::string::npos)
