...
class ColumnOpCounter {
 public:
  ...
  bool is_valid() {
    // A. At most one condition can be set for a column (except for CONTAINS).
    ...
    // Note: We allow adding multiple CONTAINS condition on a column.
    if (in_count_ + eq_count_ + gt_count_ > 1 || in_count_ + eq_count_ + lt_count_ > 1 ||
        ...
        partial_col_eq_count_ + partial_col_gt_count_ + partial_col_in_count_ +
            partial_col_lt_count_ + contains_count_ > 0)) {
      return false;
    }
