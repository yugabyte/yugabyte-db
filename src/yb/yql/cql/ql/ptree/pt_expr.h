...
class PTExpr : public TreeNode {
 public:
  ...
  // TODO(Oleg) INDEX SUPPORT - Implement this function to generate unique column ref.
  // When INDEX is created, its column names is generated by YugaByte. For now, we use QLName()
  // to generate the name for the index columns.
  // Example:
  //   CREATE INDEX idx ON tab(xxx);
  //   - Internally INDEX idx() is implemented as a table.
  //   - Currently, the column of "idx" on tab(xxx) is also named "xxx".
  //   - However, that might not be sufficient for our index search.
  virtual string IndexColumnName() const {
    return QLName();
  }
