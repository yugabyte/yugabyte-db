...
std::string RedactPasswordLiterals(const std::string& operation) {
  ...
  while (i < n) {
    ...
    // the placeholder. Redact *every* occurrence: a rejected statement carrying two password
    // clauses is still audited via LogStatementError, and stopping after the first would log the
    // rest in cleartext.
