...
struct TableHolder {
  explicit TableHolder(const PgTableDescPtr& descr) : table_holder_(descr) {}
  PgTable table_holder_;
};

class PgDocReadOpWithPgTable : private TableHolder, public PgDocReadOp {
 public:
  PgDocReadOpWithPgTable(
      const PgTableDescPtr& descr,
      const PgSession::ScopedRefPtr& pg_session,
      std::unique_ptr<client::YBPgsqlReadOp> read_op)
    : TableHolder(descr), PgDocReadOp(pg_session, &table_holder_, std::move(read_op)) {}
};
...
CHECKED_STATUS FetchExistingYbctids(PgSession::ScopedRefPtr session,
  ...
  std::vector<std::pair<PgOid, std::shared_ptr<PgDocReadOpWithPgTable>>> doc_ops;
  ...
  for (auto it = ybctids->begin(); it != ybctids->end(); ) {
    ...
    auto doc_op = std::make_shared<PgDocReadOpWithPgTable>(desc, session, std::move(read_op));
    ...
    RETURN_NOT_OK(static_cast<PgDocOp*>(doc_op.get())->PopulateDmlByYbctidOps(&table_ybctids));
