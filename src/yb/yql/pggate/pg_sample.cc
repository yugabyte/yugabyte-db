    ...
    for (size_t partition = 0; partition < partition_keys.size(); ++partition) {
      ...
      if (VERIFY_RESULT(SetLowerUpperBound(&read_req, partition))) {
        ...
        if (!sample_blocks_feed.has_value() ||
          ...
          auto& read_op = *pgsql_ops_[partition];
...
class SamplePickerBase : public PgSelect {
 ...
 protected:
  SamplePickerBase(const PgSession::ScopedRefPtr& pg_session, scoped_refptr<ClockBase> clock)
 ...
 public:
  ...
  static Result<std::unique_ptr<SampleBlocksPicker>> Make(
    ...
    RETURN_NOT_OK(result->Prepare(
        table_id, is_region_local, targrows, rand_state,
        ysql_sampling_algorithm));
    return result;
  }

 private:
  SampleBlocksPicker(const PgSession::ScopedRefPtr& pg_session, scoped_refptr<ClockBase> clock)
  ...
  Status Prepare(
    ...
    RETURN_NOT_OK(
        SamplePickerBase::Prepare(table_id, is_region_local));
...
class SampleRowsPicker : public SamplePickerBase, public SampleRowsPickerIf {
 ...
 protected:
  SampleRowsPicker(const PgSession::ScopedRefPtr& pg_session, scoped_refptr<ClockBase> clock)
      : SamplePickerBase(pg_session, std::move(clock)) {
  }
  ...
  Status Prepare(
      ...
      const SampleRandomState& rand_state,
      YsqlSamplingAlgorithm ysql_sampling_algorithm) {
    RETURN_NOT_OK(SamplePickerBase::Prepare(table_id, is_region_local));
...
class TwoStageSampleRowsPicker : public SampleRowsPickerIf {
 public:
  ...
  static Result<std::unique_ptr<SampleRowsPickerIf>> Make(
    ...
    RETURN_NOT_OK(result->Prepare(
        table_id, is_region_local, targrows, rand_state, std::move(clock),
        ysql_sampling_algorithm));
    return result;
  }
 ...
 private:
  ...
  Status Prepare(
    ...
    sample_blocks_picker_ = VERIFY_RESULT(SampleBlocksPicker::Make(
        pg_session_, table_id, is_region_local, targrows, rand_state, clock,
        ysql_sampling_algorithm));
    sample_rows_picker_ = VERIFY_RESULT(SampleRowsPicker::Make(
        pg_session_, table_id, is_region_local, targrows, rand_state,
...
Result<std::unique_ptr<PgSample>> PgSample::Make(
  ...
  RETURN_NOT_OK(result->Prepare(table_id, is_region_local, targrows,
