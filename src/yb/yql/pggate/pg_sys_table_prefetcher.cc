...
DEFINE_bool(ysql_enable_read_request_caching, false, "Enable read request caching");
...
[[nodiscard]] std::string BuildCacheKey(
    Arena* arena, const ReadHybridTime& catalog_read_time,
...
class Loader {
 public:
  Loader(PgSession* session, const std::shared_ptr<Arena>& arena,
...
class PgSysTablePrefetcher::Impl {
 public:
  explicit Impl(const Settings& settings)
      : arena_(std::make_shared<Arena>()), settings_(settings) {}
