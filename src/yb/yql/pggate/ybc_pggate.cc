...
DEFINE_NON_RUNTIME_int32(ysql_client_read_write_timeout_ms, -1,
    "Timeout for YSQL's yb-client read/write "
    "operations. Falls back on max(client_read_write_timeout_ms, 600s) if set to -1." );
DEFINE_NON_RUNTIME_int32(pggate_num_connections_to_server, 1,
...
DEFINE_NON_RUNTIME_bool(ysql_enable_reindex, false,
            "Enable REINDEX INDEX statement.");
TAG_FLAG(ysql_enable_reindex, advanced);
TAG_FLAG(ysql_enable_reindex, hidden);

DEFINE_NON_RUNTIME_bool(ysql_disable_server_file_access, false,
