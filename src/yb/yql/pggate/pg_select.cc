...
Status PgSelect::Prepare(
  ...
  if (!index_info) {
    ...
    SetSecondaryIndex(VERIFY_RESULT(PgSelectIndex::Make(
        pg_session_, index_info->id, locality_info, skip_intents_info, std::move(index_req))));
  }
...
Result<std::unique_ptr<PgSelect>> PgSelect::Make(
  ...
  RETURN_NOT_OK(result->Prepare(table_id, locality_info, skip_intents_info, index_info));
  return result;
}
