...
// export.
//
// The choke points this relies on: this branch still has two writable-file implementations, so
// both are instrumented - yb::PosixWritableFile in env_posix.cc, behind the Raft WAL, and
// rocksdb::PosixWritableFile in file_system_posix.cc, behind everything RocksDB writes. Between
// them they cover every write without any caller knowing that drives exist.
...
// which is what makes the writable-file instrumentation cover the WAL, both RocksDB databases,
// tablet metadata, remote bootstrap and snapshot writes without any of them knowing about drives.
