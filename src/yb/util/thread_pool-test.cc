...
class ContextObservingTask : public ThreadPoolTask {
 public:
  ...
  const dist_trace::trace::SpanContext& observed() const { return observed_; }
 ...
 private:
  ...
  CountDownLatch* latch_;
  dist_trace::trace::SpanContext observed_ = dist_trace::trace::SpanContext::GetInvalid();
};
...
TEST_F(ThreadPoolTraceTest, TraceContextCarriedToWorker) {
  ...
  ASSERT_TRUE(task.observed().IsValid());
...
TEST_F(ThreadPoolTraceTest, NoTraceContextCarriedWhenNoneActive) {
  ...
  ASSERT_FALSE(task.observed().IsValid());
}
