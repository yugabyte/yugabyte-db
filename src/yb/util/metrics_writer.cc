...
Status PrometheusWriter::FlushAggregatedValues(
  ...
  for (const auto& metric_values : aggregated_values_) {
    if (!priority_regex.empty() && !std::regex_match(metric_values.first, p_regex)) {
      continue;
    }
    for (const auto& id_value : metric_values.second) {
      RETURN_NOT_OK(FlushSingleEntry(aggregated_attributes_[id_value.first],
                                     metric_values.first, id_value.second));
    }
