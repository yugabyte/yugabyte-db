...
TEST_F(TabletSplitITest, TestLogCopySetsCloseTimestampInFooter) {
  ...
  ASSERT_OK(WaitUntilTabletHasLeader(
      cluster_.get(), tablet_id, MonoTime::Now() + 5s, RequireLeaderIsReady::kTrue));
...
TEST_F(AutomaticTabletSplitITest, SizeRatio) {
  ...
  // splitting_table will be the largest tablet, candidate1 will be the tablet on the same tserver
