...
TEST_F(CDCSDKConsumptionConsistentChangesTest, TestIntentGC) {
  ...
  ASSERT_NOK(InitVirtualWAL(stream_id, {table.table_id()}));
}
...
TEST_F(CDCSDKConsumptionConsistentChangesTest, TestCDCWithSavePoint) {
  ...
  // Track transaction IDs to verify new transactions
  ...
  // Test scenario: starts a new transaction, perform DMLs and commit.
  ASSERT_OK(conn.Execute("BEGIN"));
  ASSERT_OK(conn.Execute("SAVEPOINT active_record_2"));
  ...
  // Test scenario: DMLs between ROLLBACK TO SAVEPOINT
  ASSERT_OK(conn.Execute("BEGIN"));
  ASSERT_OK(conn.Execute("SAVEPOINT sp1"));
  ...
  // DMLs after ROLLBACK TO SAVEPOINT
...
TEST_F(CDCSDKConsumptionConsistentChangesTest, TestReleaseOfRetentionBarriersOnDynamicIndexTablet) {
  ...
  CheckTabletsInCDCStateTable(
      ...
      stream_id, "Timed out waiting for index tablet to be added to cdc_state table");
...
void CDCSDKConsumptionConsistentChangesTest::TestFailureOnAddingUnqualifiedTableToVWAL(
  ...
  ASSERT_NOK(UpdatePublicationTableList(
      stream_id, {table_1.table_id(), table_2.table_id()}, kVWALSessionId1, 0 /* timeout */));
