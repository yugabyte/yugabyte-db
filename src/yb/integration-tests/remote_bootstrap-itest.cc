...
TEST_F(RemoteBootstrapITest, TestRBSWithCheckpointLockContention) {
  ...
  ASSERT_OK(itest::AddServer(
      leader_ts, tablet_id, follower_ts, PeerMemberType::PRE_VOTER, boost::none, kTimeout));
...
TEST_F(RemoteBootstrapITest, TestFetchDataIsRetriedOnFailures) {
  ...
  LOG(INFO) << "Shutting down TS " << ts_to_restart_uuid;
  cluster_->tablet_server(ts_to_restart_idx)->Shutdown();
  cluster_->master()->Shutdown();
  ASSERT_OK(cluster_->master()->Restart());
  ASSERT_OK(cluster_->WaitForTabletServerCount(num_tablet_servers - 1, timeout));
  ...
  ASSERT_OK(itest::AddServer(
      leader_ts, tablet_id, ts_to_restart_details, PeerMemberType::PRE_VOTER, boost::none,
      timeout));
...
void RemoteBootstrapITest::AddNewPeerWithDiskspaceCheck(
  ...
  // Populate small amunt of data to generate tablet
  TestYcqlWorkload workload(cluster_.get());
  workload.set_sequential_write(true);
  workload.Setup(YBTableType::YQL_TABLE_TYPE);
  workload.Start();
  ...
  cluster_->tablet_server(ts_to_add_peer)->Shutdown();
  TServerDetails* leader_ts;
  ASSERT_OK(FindTabletLeader(ts_map_, tablet_id, timeout, &leader_ts));

  // stop the ts.
  ASSERT_OK(RemoveServer(
      leader_ts, tablet_id, ts_to_add_peer_details, boost::none, MonoDelta::FromSeconds(5)));
  ...
  ASSERT_OK(cluster_->tablet_server(ts_to_add_peer)->Restart(
      ExternalMiniClusterOptions::kDefaultStartCqlProxy, disk_full_flags));
