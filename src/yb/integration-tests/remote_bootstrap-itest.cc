...
Result<std::string> RemoteBootstrapITest::SetUp3TabletServerClusterAndTable(
  ...
  auto yb_table_name = YBTableName(YQLDatabase::YQL_DATABASE_PGSQL, db_name, table_name);
  yb_table_name.set_namespace_name(db_name);
  RETURN_NOT_OK(client_->CreateNamespaceIfNotExists(
      yb_table_name.namespace_name(), yb_table_name.namespace_type()));
  ...
  table_creator
      ->table_name(YBTableName(
          YQLDatabase::YQL_DATABASE_PGSQL, yb_table_name.namespace_name(),
          yb_table_name.table_name()))
      .table_type(YBTableType::PGSQL_TABLE_TYPE)
      .num_tablets(1)
      .wait(true);
  RETURN_NOT_OK(table_creator->Create());
  SCHECK(!table_creator->get_table_id().empty(), IllegalState, "Table id should not be empty.");
