...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestNestedCompositeType)) {
  ...
  std::string table_id =
      ASSERT_RESULT(GetTableId(&test_cluster_, test_namespace_name, "emp_nested"));
  xrepl::StreamId stream_id = ASSERT_RESULT(CreateDBStream(IMPLICIT));
...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestArrayCompositeType)) {
  ...
  std::string table_id =
      ASSERT_RESULT(GetTableId(&test_cluster_, test_namespace_name, "emp_array"));
  xrepl::StreamId stream_id = ASSERT_RESULT(CreateDBStream(IMPLICIT));
...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestEnumWithMultipleTablets)) {
  ...
  for (int idx = 0; idx < total_stream_count; idx++) {
    ...
    table_id[idx] =
        ASSERT_RESULT(GetTableId(&test_cluster_, test_namespace_name, listTablesName[idx]));
...
TEST_F(CDCSDKYsqlTest, TestCDCStateEntryForReplicationSlot) {
  ...
  const auto test_namespace_name_2 = "test_namespace_2";
  ASSERT_OK(CreateDatabase(&test_cluster_, test_namespace_name_2));
  auto stream_id_2 = ASSERT_RESULT(
      CreateDBStream(CDCCheckpointType::EXPLICIT, CDCRecordType::CHANGE, test_namespace_name_2));
...
TEST_F(CDCSDKYsqlTest, TestSlotNameInCDCMetricsAttributes) {
  ...
  std::string test_namespace_name_2 = "test_namespace_for_old_model";
  ASSERT_OK(CreateDatabase(&test_cluster_, test_namespace_name_2));

  auto table_1 = ASSERT_RESULT(CreateTable(&test_cluster_, test_namespace_name, "test1"));
  auto table_2 = ASSERT_RESULT(CreateTable(&test_cluster_, test_namespace_name_2, "test2"));
...
TEST_F(CDCSDKYsqlTest, TestDropTableWithXcluster) {
  ...
  ASSERT_OK(test_client()->GetCDCStream(ReplicationSlotName(slot_name), &replica_identities));
  ASSERT_EQ(replica_identities.size(), 1);
}
...
TEST_F(CDCSDKYsqlTest, TestStreamIndependenceWhilePollingCatalogTablet) {
  ...
  // Create second DB.
  const auto test_namespace_name_2 = "test_namespace_2";
  ASSERT_OK(CreateDatabase(&test_cluster_, test_namespace_name_2, false /* colocated*/));

  // Create one table on each DB.
  ASSERT_RESULT(CreateTable(&test_cluster_, test_namespace_name, "test_table_1"));
  ASSERT_RESULT(CreateTable(&test_cluster_, test_namespace_name_2, "test_table_2"));

  auto conn_1 = ASSERT_RESULT(test_cluster_.ConnectToDB(test_namespace_name));
  auto conn_2 = ASSERT_RESULT(test_cluster_.ConnectToDB(test_namespace_name_2));
  ...
  auto stream_id_2 = ASSERT_RESULT(CreateConsistentSnapshotStreamWithReplicationSlot(
      "test_slot_2", CDCSDKSnapshotOption::NOEXPORT_SNAPSHOT, false, test_namespace_name_2));
