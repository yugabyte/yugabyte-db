...
TEST_F(DeleteTableTest, DeleteTransactionTable) {
  ...
  auto local_txn_table = std::ranges::find_if(result.tables(), [](const auto& table) {
    return table.name().find("transactions_") != std::string::npos;
  });
...
TEST_F(RemoveTablesFromMapsTest, RemoveDeletedTableFromMap) {
  ...
  auto cm = ASSERT_RESULT(cluster_->GetLeaderMiniMaster())->master()->catalog_manager_impl();
  {
    ...
    for (const auto& tablet :
             ASSERT_RESULT(table_info->GetTablets(master::IncludeInactive::kTrue))) {
