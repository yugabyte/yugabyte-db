...
class SnapshotTest : public SnapshotTestBase<MiniCluster> {
 public:
  ...
  void CheckAllSnapshots(const std::map<TxnSnapshotId, SysSnapshotEntryPB::State>& snapshot_info) {
    ListSnapshotsRequestPB list_req;
    ListSnapshotsResponsePB list_resp;

    LOG(INFO) << "Requested available snapshots.";
    const Status s = proxy_backup_->ListSnapshots(list_req, &list_resp, ResetAndGetController());
  ...
  Status WaitForSnapshotOpDone(const string& op_name, const TxnSnapshotId& snapshot_id) {
    return WaitTillComplete(op_name, [this, &snapshot_id, &op_name]() -> Result<bool> {
      ListSnapshotsRequestPB list_req;
      ListSnapshotsResponsePB list_resp;
      list_req.set_snapshot_id(snapshot_id.AsSlice().ToBuffer());

      Status s = proxy_backup_->ListSnapshots(list_req, &list_resp, ResetAndGetController());
      ...
      RETURN_NOT_OK(s);
      SCHECK_FORMAT(
          list_resp.snapshots_size() == 1, IllegalState,
  ...
  Status WaitForSnapshotRestorationDone(const TxnSnapshotRestorationId& restoration_id) {
    return WaitTillComplete("IsRestorationDone", [this, &restoration_id]() -> Result<bool> {
      ...
      RETURN_NOT_OK(
          proxy_backup_->ListSnapshotRestorations(list_req, &list_resp, ResetAndGetController()));
  ...
  Status WaitForCreateTableDone(const YBTableName& table_name) {
    return WaitTillComplete("IsCreateTableDone", [this, &table_name]() -> Result<bool> {
  ...
  void VerifySnapshotFiles(const TxnSnapshotId& snapshot_id) {
    ...
    for (size_t i = 0; i < cluster_->num_tablet_servers(); ++i) {
      ...
      for (const auto& tablet_peer : ts_tablet_peers) {
        ...
        ASSERT_OK(WaitFor(
            [tablet_peer, last_op_id]() {
              EXPECT_OK(tablet_peer->WaitUntilConsensusRunning(15s));
              return CHECK_RESULT(tablet_peer->GetConsensus())->GetLastCommittedOpId() >=
                     last_op_id;
            },
            15s, "Wait for op id commit"));
