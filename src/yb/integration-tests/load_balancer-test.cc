...
TEST_F_EX(LoadBalancerTest, TableWithNullPartitionInfo, LoadBalancerManyTabletsTest) {
...

  {
    ...
    rpc::RpcController rpc;
    rpc.set_timeout(kDefaultTimeout);
    auto proxy_ddl = ASSERT_RESULT(GetMasterLeaderProxy());
...

  {
    ...
    auto proxy_client = ASSERT_RESULT(GetMasterLeaderProxy());
    ...
    for (const auto& tablet_id : replicas[1]) {
      ASSERT_EQ(replicas[3].find(tablet_id), replicas[3].end());
    }
  }
}
