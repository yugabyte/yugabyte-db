...
class LoadBalancerColocatedTablesTest : public YBTableTestBase {
 protected:
  ...
  virtual std::unique_ptr<client::YBTableCreator> NewTableCreator() override {
...
TEST_F(LoadBalancerTablegroupsTest, YB_DISABLE_TEST_IN_TSAN(GlobalLoadBalancingWithTablegroups)) {
  ...
  // Add two tservers to z0 and wait for everything to be balanced.
  std::vector<std::string> extra_opts;
  extra_opts.push_back("--placement_cloud=c");
  extra_opts.push_back("--placement_region=r");
  extra_opts.push_back("--placement_zone=z0");
  ...
  ASSERT_OK(external_mini_cluster()->AddTabletServer(true, extra_opts));
  ASSERT_OK(
      external_mini_cluster()->WaitForTabletServerCount(num_tablet_servers() + 2, kDefaultTimeout));

  // Wait for load balancing to complete.
  WaitForLoadBalanceCompletion();

  ASSERT_OK(WaitFor(
      [&]() { return client_->IsLoadBalanced(rf); }, MonoDelta::FromSeconds(30), "IsLoadBalanced"));
