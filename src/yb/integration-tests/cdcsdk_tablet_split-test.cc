...
void CDCSDKTabletSplitTest::TestTransactionCommitAfterTabletSplit(
  ...
  auto table = ASSERT_RESULT(CreateTable(
      ...
      num_tablets, true, false, 0, false, "", "public",
      num_columns));
...
void CDCSDKTabletSplitTest::TestStreamMetaDataCleanupDropTableAfterTabletSplit(
  ...
  ASSERT_OK(WriteEnumsRows(
      100 /* start */, 200 /* end */, &test_cluster_, table_list_suffix[0], kTableName));
...
TEST_F(CDCSDKTabletSplitTest, TestNoEmptyBeginCommitAfterTabletSplit) {
  ...
  auto txn_conn = ASSERT_RESULT(test_cluster_.ConnectToDB(test_namespace_name));
