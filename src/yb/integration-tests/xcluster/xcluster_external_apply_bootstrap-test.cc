...
std::pair<string, string> EncodeExternalIntent(
  ...
  class Provider : public docdb::ExternalIntentsProvider {
   public:
    ...
    // boost::optional (not std::optional) to match this branch's ExternalIntentsProvider::Next().
    boost::optional<std::pair<Slice, Slice>> Next() override {  // NOLINT(build/boost_optional)
      if (consumed_) {
        return boost::none;
      }
      consumed_ = true;
...
//   * This branch has no mechanism to advance the intents flushed OpId to match the regular one on
//     flush, so the lag persists naturally across the restart.
//
...
TEST_F(XClusterExternalApplyBootstrapTest, ExternalApplyReplayedAfterRestartShadowsPackedRow) {
  ...
  ASSERT_EQ(ASSERT_RESULT(tablet->TEST_CountRegularDBRecords()), 1)
