...
TEST_F(MasterPathHandlersItest, TestTablesJsonEndpoint) {
  ...
  EXPECT_EQ(table_obj["message"].GetString(), std::string());
  EXPECT_EQ(table->id(), table_obj["uuid"].GetString());
  EXPECT_EQ(table_obj["ysql_oid"].GetString(), std::string());
...
TEST_F_EX(
  ...
  JsonReader reader(result_str);
  ASSERT_OK(reader.Init());
  const rapidjson::Value* json_obj = nullptr;
  ASSERT_OK(reader.ExtractObject(reader.root(), nullptr, &json_obj));
  ASSERT_EQ(rapidjson::kObjectType, CHECK_NOTNULL(json_obj)->GetType());
  ASSERT_TRUE(json_obj->HasMember("tablets"));
  ASSERT_EQ(rapidjson::kArrayType, (*json_obj)["tablets"].GetType());
  const rapidjson::Value::ConstArray tablets_json = (*json_obj)["tablets"].GetArray();
  ASSERT_EQ(tablets_json.Size(), kNumTablets);
  for (const auto& tablet_json : tablets_json) {
    ASSERT_TRUE(tablet_json.HasMember("partition"));
    auto partition = tablet_json["partition"].GetString();
