...
const unordered_map<int64_t, const char*> kQLErrorMessage {
  //------------------------------------------------------------------------------------------------
...
ErrorCode GetErrorCode(const Status& s) {
  return s.IsQLError() ? static_cast<ErrorCode>(s.ql_error_code()) : ErrorCode::FAILURE;
}

const char *ErrorText(const ErrorCode error_code) {
  return kQLErrorMessage.at(static_cast<int64_t>(error_code));
}

Status ErrorStatus(const ErrorCode code, const std::string& mesg) {
  return STATUS(QLError, ErrorText(code), mesg, static_cast<int64_t>(code));
}
