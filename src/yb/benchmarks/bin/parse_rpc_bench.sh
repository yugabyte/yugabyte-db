...
if [[ -z $FILE || $FILE == "-h" || $FILE == "--help" ]]; then
  ...
  echo '$ YB_ALLOW_SLOW_TESTS=1 ./build/latest/bin/rpc-bench --gtest_repeat=10 2>&1 | tee rpc-bench-output.log'
  echo '$ ./src/yb/benchmarks/bin/parse_rpc_bench.sh rpc-bench-output.log'
  echo
  echo 'Example output:'
  echo
