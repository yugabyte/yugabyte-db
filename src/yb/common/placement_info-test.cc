...
TEST(PlacementInfoTest, TestPreferredZoneJsonProcessing) {
  ...
  // Valid option with no preferred zones.
  {
    ...
    QLValuePB ql_val;
    ql_val.set_string_value(no_preferred_zone);
    PlacementInfoConverter::Placement result =
        ASSERT_RESULT(PlacementInfoConverter::FromQLValue(vector<QLValuePB>{ql_val}));
  ...
  // Negative priority.
  {
    ...
    QLValuePB ql_val;
    ql_val.set_string_value(negative_priority);
    ASSERT_NOK(PlacementInfoConverter::FromQLValue(vector<QLValuePB>{ql_val}));
  }
  ...
  // Zero priority.
  {
    ...
    QLValuePB ql_val;
    ql_val.set_string_value(zero_priority);
    ASSERT_NOK(PlacementInfoConverter::FromQLValue(vector<QLValuePB>{ql_val}));
  }
  ...
  // No priority 1.
  {
    ...
    QLValuePB ql_val;
    ql_val.set_string_value(no_priority_1);
    ASSERT_NOK(PlacementInfoConverter::FromQLValue(vector<QLValuePB>{ql_val}));
  }
  ...
  // Non contiguous priority.
  {
    ...
    QLValuePB ql_val;
    ql_val.set_string_value(non_cont_priority_1);
    ASSERT_NOK(PlacementInfoConverter::FromQLValue(vector<QLValuePB>{ql_val}));
  }
  ...
  // Non contiguous priority3.
  {
    auto non_cont_priority_3 = strings::Substitute(
        ...
        zone3 + R"#(,"leader_preference":3)#");
    QLValuePB ql_val;
    ql_val.set_string_value(non_cont_priority_3);
  ...
  // Only 1 zone has priority
  {
    auto one_zone_priority =
        strings::Substitute(format, zone1 + R"#(,"leader_preference":1)#", zone2, zone3);
    QLValuePB ql_val;
    ql_val.set_string_value(one_zone_priority);
    auto result = ASSERT_RESULT(PlacementInfoConverter::FromQLValue(vector<QLValuePB>{ql_val}));
    for (auto& info : result.placement_infos) {
  ...
  // Two zones have priority 1
  {
    auto two_zone_priority = strings::Substitute(
        ...
        zone3);
    QLValuePB ql_val;
    ql_val.set_string_value(two_zone_priority);
    auto result = ASSERT_RESULT(PlacementInfoConverter::FromQLValue(vector<QLValuePB>{ql_val}));
    for (auto& info : result.placement_infos) {
  ...
  // All unique priority
  {
    auto teo_zone_priority = strings::Substitute(
        ...
        zone3 + R"#(,"leader_preference":3)#");
    QLValuePB ql_val;
    ql_val.set_string_value(teo_zone_priority);
    auto result = ASSERT_RESULT(PlacementInfoConverter::FromQLValue(vector<QLValuePB>{ql_val}));
    for (auto& info : result.placement_infos) {
