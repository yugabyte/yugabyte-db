...
// The point of the whole design: int8 traversal coordinates cost recall, and reranking a modest
// over-fetch at float16 gets it back.
//
// The assertion that the un-reranked configuration is measurably worse is not redundant. Without
// it the test passes just as happily when the rerank tier does nothing at all, which is exactly
// the failure mode a capped candidate budget produces.
TEST_F(YbHnswStorageTest, Int8WithRerankMatchesFloat16Recall) {
  ...
  // Reranking cannot beat the encoding it reranks with, so float16's overlap is the ceiling.
  // The margin absorbs the graph's own non-determinism between the two files. Reranking having
  // happened at all is asserted deterministically below rather than by comparing recall, whose
  // gap depends on the data.
  ASSERT_GE(i8_overlap, f16_overlap - 0.02);

  // What proves the rerank ran, deterministically: for each returned row, the reported distance
  // must be exactly what the float16 metric gives for that row -- because that is the
  // computation MakeResult performs. The un-reranked index reports the traversal's quantized
  // distance instead, which is larger by 1/scale squared.
  for (size_t i = 0; i != 20; ++i) {
    auto query = RandomVector();
...
// Over-fetching has to work when ef <= max_num_results, and that is the case a naive
// min(max(ef, k), overfetch * k) budget silently breaks: it yields exactly k retained candidates,
// so reranking reorders the answer instead of correcting it. Every size assertion still passes in
// that state, so this test compares behaviour across over-fetch factors instead.
TEST_F(YbHnswStorageTest, OverFetchActuallyOverFetches) {
  ...
  for (auto [max_results, ef] : std::initializer_list<std::pair<size_t, size_t>>{
    ...
    // Reranking a superset cannot select a worse result, so summed over every query a larger
    // retained set must not be farther. Recall is deliberately not the assertion: it is not
    // monotone in the candidate budget, because a larger budget also moves the traversal's
    // termination bound and so changes which nodes get visited at all.
...
// quantize every coordinate in the chunk -- including all the well-behaved ones -- to zero, so a
// single bad vector would silently destroy the whole chunk's traversal while leaving the reranked
// distances looking plausible.
