...
- **Always push via `.agents/scripts/git-push.sh`** — never via raw `git push` (deny-listed in `.claude/settings.json`). The helper picks its push mode from the PR's review state; you don't have to.
- **Don't rebase or force-push a branch whose PR is out of draft.** Renewing the SHAs marks every reviewer line comment "outdated" and destroys the diff-since-last-look, which is the main thing a returning reviewer wants. Concretely, `git-push.sh` behaves in two modes:
  - **No PR yet, or the PR is still a draft** — nobody is reading, so history is free. The helper integrates commits already on the remote branch, rebases onto the latest upstream `<base>`, runs lint, and force-pushes (`--force-with-lease`).
  - **PR is ready for review** — append-only. The helper refuses to rebase, verifies the push is a fast-forward, and pushes plainly. A non-fast-forward exits `4`; integrate the remote branch with `git merge` (**not** a rebase) and re-run. Pass `-f` to rewrite anyway, and only when the author and reviewer have agreed the churn is worth it.
- **Refresh a live PR branch with a merge, not a rebase.** `git merge upstream/master` when you need the base's newer commits. The merge commit is harmless: the PR is squash-merged, so it never reaches `master`. `git-push.sh` reports how far behind the branch is but never merges for you — picking up the base mid-review is the author's call.
- **Stacked PRs live in the main repo, under `feature-stack/<feature-name>/<change-name>`.** GitHub can only chain PRs whose head branches it owns, so a stack member cannot sit in your fork. That branch prefix is the *only* thing allowed to push to `yugabyte/yugabyte-db`; the rulesets (`Block Creations`, `Require PR`, `yb_required`) exclude `refs/heads/feature-stack/**/*` and nothing else. `git-push.sh` detects the prefix, validates the two-segment shape, and targets upstream automatically. Point each PR at its parent with `create-pr.sh -b feature-stack/<feature>/<parent-change>`. Every PR in a stack runs its own full CI, so keep all but the bottom one in draft until the one below it lands.
