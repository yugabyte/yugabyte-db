...
- **Don't collapse commits locally yourself** (no `git rebase -i ... squash`, no hand-merging two commits into one). Each follow-up should stay as its own commit so reviewers can read it independently; the squash happens automatically at merge time on GitHub. The exception: `git-push.sh` auto-squashes a branch into a single commit once it has more than 20 commits since the merge-base with upstream — at that point per-commit history stops being a useful review aid and the rolling rebase makes conflict resolution painful, so the script collapses everything into one commit (preserving the first commit's subject and listing every squashed subject in the body).
