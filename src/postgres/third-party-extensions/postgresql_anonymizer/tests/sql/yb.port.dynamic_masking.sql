...
\! PGPASSWORD=CHANGEME ${YB_BUILD_ROOT}/postgres/bin/ysqlsh -U jimmy_mcnulty -c 'SELECT * FROM phone' # YB: use ysqlsh and the default database

-- Jimmy joins table to get the call history
\! PGPASSWORD=CHANGEME ${YB_BUILD_ROOT}/postgres/bin/ysqlsh -U jimmy_mcnulty -c 'SELECT p1.phone_owner as "from", p2.phone_owner as "to", c.call_start_time FROM phonecall c JOIN phone p1 ON c.call_sender = p1.phone_number JOIN phone p2 ON c.call_receiver = p2.phone_number' # YB: use ysqlsh and the default database
