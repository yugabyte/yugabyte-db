                        ...
                        agtype *edge_properties)
{
    ...
    /*
     * YB: simple bulk inserts (e.g. graph_generation builders) do not yet
     * supply meko_* tenant values, which are NOT NULL columns on YB-hosted
     * label tables. Fail loudly until the path is made YB-aware. See
     * TODO(#31338).
     */
    if (IsYBRelation(label_relation))
    {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                 errmsg("simple edge insertion is not supported on "
                          ...
                          agtype *vertex_properties)
{
    ...
    if (IsYBRelation(label_relation))
    {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                 errmsg("simple vertex insertion is not supported on "
                        "YugabyteDB yet (TODO(#31338))")));
    }
