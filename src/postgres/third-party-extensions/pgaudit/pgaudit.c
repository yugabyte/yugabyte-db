 ...
 */
static void stack_free(void *stackFree) {
 ...
 * store it.
 */
static AuditEventStackItem *stack_push() {
  MemoryContext contextAudit;
...
static void stack_pop(int64 stackId) {
  ...
  if (auditEventStack != NULL && auditEventStack->stackId == stackId)
    ...
    elog(ERROR, "pgaudit stack item " INT64_FORMAT " not found on top - cannot pop", stackId);
}
 ...
 */
static void stack_valid(int64 stackId) {
  AuditEventStackItem *nextItem = auditEventStack;
  ...
  if (nextItem == NULL)
    elog(
        ERROR, "pgaudit stack item " INT64_FORMAT " not found - top of stack is " INT64_FORMAT "",
        stackId, auditEventStack == NULL ? (int64)-1 : auditEventStack->stackId);
}
...
static void append_valid_csv(StringInfoData *buffer, const char *appendStr) {
   ...
   * quoted in CSV.
   */
  if (appendStr == NULL) return;

  /* Only format for CSV if appendStr contains: ", comma, \n, \r */
  if (strstr(appendStr, ",") || strstr(appendStr, "\"") || strstr(appendStr, "\n") ||
      strstr(appendStr, "\r")) {
...
static void log_audit_event(AuditEventStackItem *stackItem) {
  ...
  if (stackItem->auditEvent.logged) return;
  ...
  switch (stackItem->auditEvent.logStmtLevel) {
    ...
    case LOGSTMT_DDL:
      ...
      switch (stackItem->auditEvent.commandTag) {
        ...
        case T_RenameStmt:
        case T_DropStmt:
          if (pg_strcasecmp(stackItem->auditEvent.command, COMMAND_ALTER_ROLE) == 0 ||
              pg_strcasecmp(stackItem->auditEvent.command, COMMAND_DROP_ROLE) == 0) {
            className = CLASS_ROLE;
            class = LOG_ROLE;
          }
          break;
   ...
   */
  if (!stackItem->auditEvent.granted && !(auditLogBitmap & class)) return;
  ...
  if (!stackItem->auditEvent.statementLogged || !auditLogStatementOnce) {
    ...
    if (auditLogParameter) {
      ...
      for (paramIdx = 0; paramList != NULL && paramIdx < numParams; paramIdx++) {
        ...
        if (paramIdx != 0) appendStringInfoCharMacro(&paramStrResult, ',');

        /* Skip if null or if oid is invalid */
        if (prm->isnull || !OidIsValid(prm->ptype)) continue;
    ...
    appendStringInfoString(&auditStr, "<previously logged>,<previously logged>");
   ...
   */
  ereport(
      auditLogClient ? auditLogLevel : LOG_SERVER_ONLY,
      (errmsg(
 ...
 * considered.
 */
static bool audit_on_acl(Datum aclDatum, Oid auditOid, AclMode mask) {
  bool result = false;
  Acl *acl;
  AclItem *aclItemData;
  int aclIndex;
  ...
  if (!result) {
    for (aclIndex = 0; aclIndex < aclTotal; aclIndex++) {
      ...
      if (aclItem->ai_grantee == ACL_ID_PUBLIC || aclItem->ai_grantee == auditOid) continue;

      /*
       * Check that the role has the required privileges and that it is
       * inherited by auditOid.
       */
      if (aclItem->ai_privs & mask && has_privs_of_role(auditOid, aclItem->ai_grantee)) {
        result = true;
        break;
      }
    }
 ...
 */
static bool audit_on_relation(Oid relOid, Oid auditOid, AclMode mask) {
  bool result = false;
  HeapTuple tuple;
  Datum aclDatum;
  bool isNull;
 ...
 */
static bool audit_on_attribute(Oid relOid, AttrNumber attNum, Oid auditOid, AclMode mask) {
  bool result = false;
  HeapTuple attTuple;
  Datum aclDatum;
  bool isNull;
  ...
  attTuple = SearchSysCache2(ATTNUM, ObjectIdGetDatum(relOid), Int16GetDatum(attNum));
  if (!HeapTupleIsValid(attTuple)) return false;
 ...
 */
static bool audit_on_any_attribute(
    Oid relOid, Oid auditOid, Bitmapset *attributeSet, AclMode mode) {
  bool result = false;
  AttrNumber col;
  Bitmapset *tmpSet;
  ...
  if (bms_is_empty(attributeSet)) {
    ...
    if (!HeapTupleIsValid(classTuple)) return false;
    ...
    for (curr_att = 1; curr_att <= nattrs; curr_att++)
      if (audit_on_attribute(relOid, curr_att, auditOid, mode)) return true;
  }
  ...
  while ((col = bms_first_member(tmpSet)) >= 0) {
    ...
    if (col != InvalidAttrNumber && audit_on_attribute(relOid, col, auditOid, mode)) {
      result = true;
      break;
    }
  }
 ...
 */
static void log_select_dml(Oid auditOid, List *rangeTabls) {
  ListCell *lr;
  bool first = true;
  bool found = false;

  /* Do not log if this is an internal statement */
  if (internalStatement) return;
  ...
  foreach (lr, rangeTabls) {
    ...
    auditEventStack->auditEvent.objectName =
        quote_qualified_identifier(get_namespace_name(relNamespaceOid), get_rel_name(relOid));
       ...
       */
        ...
        if (auditPerms & ACL_SELECT)
          auditEventStack->auditEvent.granted =
              audit_on_any_attribute(relOid, auditOid, rte->selectedCols, ACL_SELECT);

        /*
         * Check the insert columns
         */
        if (!auditEventStack->auditEvent.granted && auditPerms & ACL_INSERT)
          auditEventStack->auditEvent.granted =
              audit_on_any_attribute(relOid, auditOid, rte->insertedCols, auditPerms);

        /*
         * Check the update columns
         */
        if (!auditEventStack->auditEvent.granted && auditPerms & ACL_UPDATE)
          auditEventStack->auditEvent.granted =
              audit_on_any_attribute(relOid, auditOid, rte->updatedCols, auditPerms);
      }
    }
 ...
 */
static void log_function_execute(Oid objectId) {
  HeapTuple proctup;
  Form_pg_proc proc;
  AuditEventStackItem *stackItem;
  ...
  if (!proctup) elog(ERROR, "cache lookup failed for function %u", objectId);
  ...
  stackItem->auditEvent.objectName =
      quote_qualified_identifier(get_namespace_name(proc->pronamespace), NameStr(proc->proname));
  ReleaseSysCache(proctup);
...
static void pgaudit_ExecutorStart_hook(QueryDesc *queryDesc, int eflags) {
   ...
   */
  if (stackItem) MemoryContextSetParent(stackItem->contextAudit, queryDesc->estate->es_query_cxt);
}
...
static bool pgaudit_ExecutorCheckPerms_hook(List *rangeTabls, bool abort) {
  ...
  if (next_ExecutorCheckPerms_hook && !(*next_ExecutorCheckPerms_hook)(rangeTabls, abort))
    return false;
 ...
 */
static void pgaudit_ProcessUtility_hook(
    PlannedStmt *pstmt,
    const char *queryString,
    ProcessUtilityContext context,
    ParamListInfo params,
  ...
  */
  ...
  if (stackItem && !IsAbortedTransactionBlockState()) {
     ...
     * aborted.
     */
    if (auditLogBitmap != 0 && !stackItem->auditEvent.logged) log_audit_event(stackItem);
  }
}
 ...
 * calls.
 */
static void pgaudit_object_access_hook(
    ObjectAccessType access, Oid classId, Oid objectId, int subId, void *arg) {
  if (auditLogBitmap & LOG_FUNCTION && access == OAT_FUNCTION_EXECUTE && auditEventStack &&
      !IsAbortedTransactionBlockState())
 ...
 */
Datum pgaudit_ddl_command_end(PG_FUNCTION_ARGS) {
  ...
  if (~auditLogBitmap & LOG_DDL && ~auditLogBitmap & LOG_ROLE) PG_RETURN_NULL();

  /* Be sure the module was loaded */
  if (!auditEventStack)
    elog(
        ERROR,
        "pgaudit not loaded before call to "
  ...
  query =
      "SELECT UPPER(object_type), object_identity, UPPER(command_tag)\n"
      "  FROM pg_catalog.pg_event_trigger_ddl_commands()";

  /* Attempt to connect */
  result = SPI_connect();
  if (result < 0) elog(ERROR, "pgaudit_ddl_command_end: SPI_connect returned %d", result);

  /* Execute the query */
  result = SPI_execute(query, true, 0);
  if (result != SPI_OK_SELECT)
    elog(ERROR, "pgaudit_ddl_command_end: SPI_execute returned %d", result);
  ...
  for (row = 0; row < SPI_processed; row++) {
    ...
    auditEventStack->auditEvent.command = SPI_getvalue(spiTuple, spiTupDesc, 3);
 ...
 */
Datum pgaudit_sql_drop(PG_FUNCTION_ARGS) {
  int result, row;
  TupleDesc spiTupDesc;
  const char *query;
  MemoryContext contextQuery;
  ...
  if (~auditLogBitmap & LOG_DDL) PG_RETURN_NULL();
  ...
  /* Attempt to connect */
  result = SPI_connect();
  if (result < 0) elog(ERROR, "pgaudit_ddl_drop: SPI_connect returned %d", result);

  /* Execute the query */
  result = SPI_execute(query, true, 0);
  if (result != SPI_OK_SELECT) elog(ERROR, "pgaudit_ddl_drop: SPI_execute returned %d", result);
 ...
 */
static bool check_pgaudit_log(char **newVal, void **extra, GucSource source) {
  List *flagRawList;
  char *rawVal;
  ListCell *lt;
  int *flags;
   ...
   */
  if (!(flags = (int *)malloc(sizeof(int)))) return false;
 ...
 */
static void assign_pgaudit_log(const char *newVal, void *extra) {
  if (extra) auditLogBitmap = *(int *)extra;
}
 ...
 * function.
 */
static bool check_pgaudit_log_level(char **newVal, void **extra, GucSource source) {
  int *logLevel;

  /* Allocate memory to store the log level */
  if (!(logLevel = (int *)malloc(sizeof(int)))) return false;
 ...
 * assignment is to be suppressed.
 */
static void assign_pgaudit_log_level(const char *newVal, void *extra) {
...
void _PG_init(void) {
  ...
  DefineCustomBoolVariable(
      ...
      NULL, &auditLogCatalog, true, PGC_SUSET, GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);
  ...
  DefineCustomBoolVariable(
      ...
      NULL, &auditLogClient, false, PGC_SUSET, GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);
  ...
  DefineCustomBoolVariable(
      ...
      NULL, &auditLogParameter, false, PGC_SUSET, GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);
  ...
  DefineCustomBoolVariable(
      ...
      NULL, &auditLogRelation, false, PGC_SUSET, GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);
  ...
  DefineCustomBoolVariable(
      ...
      NULL, &auditLogStatementOnce, false, PGC_SUSET, GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);
  ...
  DefineCustomStringVariable(
      ...
      NULL, &auditRole, "", PGC_SUSET, GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);
