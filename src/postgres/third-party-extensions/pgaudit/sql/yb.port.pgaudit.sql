...
CREATE PROPERTY GRAPH graph_test VERTEX TABLES (graph_vertex); -- YB: property graphs not supported
...
SELECT name
 ...
 ORDER BY name; -- YB: fails due to prior CREATE failure

SELECT 'yb'; -- YB: bump audit number to stay same with upstream

DROP PROPERTY GRAPH graph_test; -- YB: fails due to prior CREATE failure
...
CREATE SCHEMA schema_grant
	GRANT SELECT
	   ...
	   TO regress_schema_grant; -- YB: CREATE SCHEMA with elements not supported
...
CREATE SCHEMA AUTHORIZATION regress_schema_auth
	...
	GRANT SELECT
	   ON schema_auth_tbl
	   TO PUBLIC; -- YB: CREATE SCHEMA with elements not supported

SET pgaudit.log = 'none';
DROP SCHEMA schema_grant; -- YB: fails due to prior CREATE failure
DROP SCHEMA regress_schema_auth CASCADE; -- YB: fails due to prior CREATE failure
