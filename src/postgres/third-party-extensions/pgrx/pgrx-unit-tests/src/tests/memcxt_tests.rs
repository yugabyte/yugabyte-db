...
mod tests {
    ...
    fn switch_to_should_switch_back_on_panic() {
        ...
        PgTryBuilder::new(move || unsafe {
            ctx.switch_to(|_| {
                assert!(std::ptr::eq(pg_sys::YbCurrentMemoryContext, ctx_sys));
                panic!();
            });
        })
        .catch_others(|_| {})
        .execute();
        assert_ne!(unsafe { pg_sys::YbCurrentMemoryContext }, ctx_sys);
    }
    ...
    fn test_current_owned_memory_context_drop() {
        ...
        assert_eq!(unsafe { pg_sys::YbCurrentMemoryContext }, ctx.value());
        drop(ctx);
        assert_eq!(unsafe { pg_sys::YbCurrentMemoryContext }, another_ctx.value());
    }
    ...
    fn test_current_owned_memory_context_drop_when_set_current_twice() {
        ...
        drop(ctx);
        assert_eq!(unsafe { pg_sys::YbCurrentMemoryContext }, ctx_parent);
    }
