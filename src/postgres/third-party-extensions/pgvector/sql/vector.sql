...
CREATE FUNCTION yblocalannhandler(internal) RETURNS index_am_handler
	AS 'MODULE_PATHNAME' LANGUAGE C;

CREATE ACCESS METHOD yblocalann TYPE INDEX HANDLER yblocalannhandler;

COMMENT ON ACCESS METHOD yblocalann IS 'yblocalann index access method';

CREATE OPERATOR CLASS vector_l2_ops
	DEFAULT FOR TYPE vector USING yblocalann AS
	OPERATOR 1 <-> (vector, vector) FOR ORDER BY float_ops,
	FUNCTION 1 vector_l2_squared_distance(vector, vector);
