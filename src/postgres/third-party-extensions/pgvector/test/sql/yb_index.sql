...
-- Shouldn't work for now. yblocalann is only supported on colocated tables.
CREATE INDEX ON items USING yblocalann (embedding vector_l2_ops);
DROP TABLE items;
...
-- Create a table with a vector column of size 10
CREATE TABLE items (id serial PRIMARY KEY, embedding vector(10));
INSERT INTO items (embedding) VALUES ('[0.12, 0.78, 0.56, 0.34, 0.91, 0.29, 0.47, 0.68, 0.22, 0.85]');
