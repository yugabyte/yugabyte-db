...
-- Fixed bug introduced in 2.1.0 that wasn't setting the retention config columns in part_config properly for multi-level subpartitions
    ...
    -- If you encounter the error during the upgrade, run the following query using the parent table returned in the error for <<<schema.table>>> and setting <<<partman_schema>>> to the schema you installed pg_partman to. Only a single row should be returned. If not then fix the mismatched data.
/*
    WITH parent_info AS (
        SELECT c1.oid
