...
-- YB note: upstream loads 'passwordcheck_extra', but YB builds a single
-- 'passwordcheck' module that already includes the extra checks (its GUCs are
-- registered under the passwordcheck.* prefix), so we LOAD 'passwordcheck' here.
LOAD 'passwordcheck';
