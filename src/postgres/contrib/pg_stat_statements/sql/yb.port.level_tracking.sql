...
-- YB note: (1) YB EXPLAIN labels constant Result nodes "Result *RESULT*"; (2) CREATE TABLE AS
-- EXECUTE is not supported yet (errors gracefully, kept); (3) MERGE and CREATE/REFRESH
-- MATERIALIZED VIEW crash the backend and are commented out below (TODO(#19064) / TODO(#32650)).
...
-- TODO(#19064): MERGE crashes the YB backend; disabled until supported.
...
-- TODO(#32650): CREATE/REFRESH MATERIALIZED VIEW crashes the YB backend; the upstream matview
-- test blocks (all-level + top-level, create + refresh) are omitted here until it is fixed.
...
SELECT toplevel, calls, rows, plans, query FROM pg_stat_statements
  ORDER BY query COLLATE "C";
-- YB: RESET (upstream) would restore YB's TRUE default, so set FALSE explicitly.
SET pg_stat_statements.track_planning = FALSE;
