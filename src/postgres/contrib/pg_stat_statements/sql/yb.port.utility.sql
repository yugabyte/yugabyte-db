...
-- YB note: YB's online CREATE INDEX backfill emits internal statements (BACKFILL INDEX, a
-- pg_stat_activity catalog-version poll, SET default_transaction_isolation) that pgss records;
-- the display filters them out to stay deterministic and focused on the user DDL.
SELECT calls, rows, query FROM pg_stat_statements
  WHERE query NOT LIKE 'BACKFILL INDEX%'
    AND query NOT LIKE 'SELECT count(*) FROM pg_stat_activity%'
    AND query NOT LIKE 'SET default_transaction_isolation%'
...
-- TODO(#1125): YB does not support two-phase commit (PREPARE TRANSACTION / COMMIT PREPARED /
-- ROLLBACK PREPARED).
-- BEGIN;
...
-- COPY
-- YB note: PRIMARY KEY added so the COPY (UPDATE ... RETURNING) output below is deterministic
-- — YB stores no-PK tables in random ybrowid order, making multi-row RETURNING non-deterministic.
CREATE TABLE copy_stats (a int PRIMARY KEY, b int);
SELECT pg_stat_statements_reset() IS NOT NULL AS t;
...
-- TODO(#32650): CREATE/REFRESH MATERIALIZED VIEW crashes the YB backend; omitted until fixed.
SELECT calls, rows, query FROM pg_stat_statements ORDER BY query COLLATE "C";
SELECT pg_stat_statements_reset() IS NOT NULL AS t;
...
-- YB note: ALTER DOMAIN ... ADD CONSTRAINT is not supported in YB (graceful "ERROR: ALTER DOMAIN
-- ADD CONSTRAINT not supported yet"); kept verbatim, so the error is expected and no row for it
...
13	copy
\.
-- TODO(#32650): CREATE/REFRESH MATERIALIZED VIEW crashes the YB backend; the matview and its
-- cursor reads are omitted until fixed.
