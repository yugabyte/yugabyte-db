...
/*NESTED*/
/* Sourced from contrib/pg_stat_statements/pg_stat_statements--1.10-yb-1.0--1.10-yb-2.0.sql */
...
CREATE OR REPLACE FUNCTION pg_stat_statements(IN showtext boolean,
    ...
    OUT jit_emission_time float8,
    OUT yb_latency_histogram jsonb,
    OUT docdb_read_rpcs int8,
    OUT docdb_write_rpcs int8,
    OUT catalog_wait_time float8,
    OUT docdb_read_operations int8,
    OUT docdb_write_operations int8,
    OUT docdb_rows_scanned int8,
    OUT docdb_rows_returned int8,
    OUT docdb_wait_time float8,
    OUT conflict_retries int8,
    OUT read_restart_retries int8,
    OUT total_retries int8
)
RETURNS SETOF record
AS 'MODULE_PATHNAME', 'yb_2_0_pg_stat_statements_1_10'
LANGUAGE C STRICT VOLATILE PARALLEL SAFE;
