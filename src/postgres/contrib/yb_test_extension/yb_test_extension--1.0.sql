...
-- Test Language
-- Create a procedural language same as plpgsql
CREATE FUNCTION test_call_handler()
RETURNS LANGUAGE_HANDLER AS '$libdir/plpgsql',
'plpgsql_call_handler' LANGUAGE C;

CREATE FUNCTION test_inline_handler(internal)
RETURNS VOID AS '$libdir/plpgsql',
'plpgsql_inline_handler' LANGUAGE C;

CREATE FUNCTION test_validator(oid)
RETURNS VOID AS '$libdir/plpgsql',
'plpgsql_validator' LANGUAGE C;

CREATE LANGUAGE test_language HANDLER test_call_handler INLINE test_inline_handler
