...
typedef enum statementType
{
  ...
  CatCacheMisses_78,
  CatCacheMisses_79,
  CatCacheMisses_End = CatCacheMisses_79,
  kMaxStatementType
} statementType;
...
set_metric_names(void)
{
	...
	for (int i = CatCacheMisses_Start; i <= CatCacheMisses_End; ++i)
	{
		snprintf(ybpgm_table[i].count_help, YB_PG_METRIC_NAME_LEN,
				 ...
				 YbGetCatalogCacheIndexName(i - CatCacheMisses_Start));
...
_PG_init(void)
{
  ...
  static_assert(SysCacheSize == CatCacheMisses_End - CatCacheMisses_Start + 1,
