...
/* YB_TODO(neil@yugabyte) Add "min_attr" to context now.
 *
		...
		case T_SubscriptingRef:
			{
				...
				/* recurse directly for upper/lower container index lists */
				if (expression_tree_walker_min_attr((Node *) sbsref->refupperindexpr,
													walker, context, min_attr))
					return true;
				if (expression_tree_walker_min_attr((Node *) sbsref->reflowerindexpr,
													walker, context, min_attr))
					return true;
				/* walker must see the refexpr and refassgnexpr, however */
				if (walker(sbsref->refexpr, context, min_attr))
					return true;

				if (walker(sbsref->refassgnexpr, context, min_attr))
					return true;
			}
			break;
		case T_FuncExpr:
