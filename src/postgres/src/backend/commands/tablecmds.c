					  ...
					  AlterTableCmd *cmd, LOCKMODE lockmode)
{
	...
	if (IsYBRelation(rel) && !tab->rewrite)
	{
...
ATPostAlterTypeCleanup(List **wqueue, AlteredTableInfo *tab, LOCKMODE lockmode)
{
			...
			def_item, tab->changedConstraintDefs)
	{
		 ...
		 */
		if (!(IsYBRelationById(tab->relid) && OidIsValid(get_constraint_index(oldId))))
			add_exact_object_address(&obj, objects);
		 ...
		 */
		if (!(IsYBRelationById(tab->relid) && yb_reuse_index))
			add_exact_object_address(&obj, objects);
	}
					 ...
					 bool *yb_reuse_index, bool yb_conislocal)
{
	...
	foreach(list_item, querytree_list)
	{
		...
		if (IsA(stm, IndexStmt))
		{
			...
			if (IsYBRelationById(tab->relid) && yb_reuse_index != NULL && *yb_reuse_index)
			{
				/*
		...
		else if (IsA(stm, AlterTableStmt))
		{
			...
			foreach(lcmd, stmt->cmds)
			{
				...
				if (cmd->subtype == AT_AddIndex)
				{
					 ...
					 */
					else if (IsYBRelationById(tab->relid) && !yb_conislocal)
						continue;
					else
					{
								...
								AlterTableCmd *cmd, LOCKMODE lockmode)
{
	...
	pg_attribute = relation_open(AttributeRelationId, RowExclusiveLock);
	ScanKeyInit(&key[0],
				Anum_pg_attribute_attrelid,
				BTEqualStrategyNumber, F_OIDEQ,
	...
	systable_endscan(scan);
	relation_close(pg_attribute, RowExclusiveLock);
