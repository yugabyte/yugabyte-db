...
{
	/*
	 * Construct HeapTuple (db_oid, null) for computing ybctid using
	...
	HeapTuple tuple = heap_form_tuple(RelationGetDescr(logical_client_version_rel),
									  values, nulls);
	return YBCGetYBTupleIdFromTuple(logical_client_version_rel, tuple,
									RelationGetDescr(logical_client_version_rel));
}
