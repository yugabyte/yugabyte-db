...
ShouldPushdownScanKey(Relation relation, YbScanPlan scan_plan, AttrNumber attnum,
	...
	else
	{
		...
		{
			/*
			 * Only allow strategy here (i.e. IN .. or = ANY(..) conditions,
 ...
 */
static bool YbCheckScanTypes(YbScanDesc ybScan, YbScanPlan scan_plan, int i) {
	Oid	atttypid = ybc_get_atttypid(scan_plan->bind_desc, scan_plan->bind_key_attnums[i]);
