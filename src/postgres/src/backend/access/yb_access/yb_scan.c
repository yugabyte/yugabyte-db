...
ybcSetupScanKeys(YbScanDesc ybScan, YbScanPlan scan_plan)
{
	...
	if (!bms_is_subset(scan_plan->hash_key, scan_plan->sk_cols))
	{
		...
		if (ybScan->hash_code_keys != NIL ||
			sk_cols_has_ybctid)
		{
			scan_plan->sk_cols = bms_del_members(scan_plan->sk_cols,
												 scan_plan->hash_key);
		}
		else
		{
						...
						bool is_for_precheck)
{
	...
	for (int j = 0; j < subkey_count; j++)
	{
		 ...
		 *
		 * Stop at the first incompatible subkey.  Trailing unspecified
		 * columns are bound as +/-Inf, which is a correct conservative
		 * prefix.  If even the first subkey is incompatible, skip pushdown.
		 */
		{
			AttrNumber	attnum =
				scan_plan->bind_key_attnums[skey_index + 1 + j];
			Oid			col_typid =
