...
set_cheapest(RelOptInfo *parent_rel)
{
	...
	foreach(p, parent_rel->pathlist)
	{
		...
		/*
		 * YB: Do not consider provisional paths for this.
		 * Provisional paths always occur after the usual paths.
		 */
		if (path->yb_path_info.yb_uniqpath_provisional)
			break;
...
/*
 * YB: Provisional paths are unique paths generated by index scans that are not
 * ready to be used as part of normal operation yet.
 * For this reason, these paths are maintained at the end of the list.
 * Moreover, there must at least be one regular path accompanying any
  * provisional path since the latter paths may be rejected later on.
 */
static void
yb_check_pathlist_invariants(const List *pathlist)
{
	bool	  has_provisional_paths;
	bool	  has_regular_paths;
	ListCell *lc;

	if (pathlist == NIL)
		return;

	has_provisional_paths = false;
	has_regular_paths = false;
	foreach(lc, pathlist)
	{
		Path *path = (Path *) lfirst(lc);
		bool is_provisional = path->yb_path_info.yb_uniqpath_provisional;

		has_provisional_paths |= is_provisional;
		has_regular_paths |= !is_provisional;

		/* !is_provisional => !has_provisional_paths */
		Assert(is_provisional || !has_provisional_paths);
	}

	/* has_provisional_paths => has_regular_paths */
	Assert(!has_provisional_paths || has_regular_paths);
}
#else
#define yb_check_pathlist_invariants(l)
#endif
...
add_path(RelOptInfo *parent_rel, Path *new_path)
{
	...
	/* YB: These paths must go at the end of the list. */
	yb_newpath_provisional =
		new_path->yb_path_info.yb_uniqpath_provisional;
	...
	for (p1 = list_head(parent_rel->pathlist); p1 != NULL; p1 = p1_next)
	{
		...
		bool				yb_oldpath_provisional;

		yb_oldpath_provisional =
			old_path->yb_path_info.yb_uniqpath_provisional;

		/*
		 * YB: Skip processing old hidden paths for the usual new paths.
		 */
		if (!yb_newpath_provisional && yb_oldpath_provisional)
			break;
		...
		/*
		 * YB: Do not remove usual paths in favor of provisional paths.
		 * All provisional paths come after usual paths.
		 */
		if (yb_newpath_provisional && !yb_oldpath_provisional)
		{
			remove_old = false;
	...
	yb_check_pathlist_invariants(parent_rel->pathlist);
}
				  ...
				  List *pathkeys, Relids required_outer)
{
	...
	foreach(p1, parent_rel->pathlist)
	{
		...
		/*
		 * YB: Skip checking provisional paths.
		 * Provisional paths always occur after the regular paths.
		 */
		if (old_path->yb_path_info.yb_uniqpath_provisional)
			break;
...
yb_propagate_fields(YbPathInfo *parent_fields, YbPathInfo *child_fields)
{
	...
	parent_fields->yb_uniqpath_provisional =
		child_fields->yb_uniqpath_provisional;
}
					 ...
					 YbPathInfo *child2_fields)
{
	...
	parent_fields->yb_uniqpath_provisional =
		...
		child2_fields->yb_uniqpath_provisional;
}
...
yb_propagate_fields_list(YbPathInfo *parent_fields, List *child_paths)
{
	...
	parent_fields->yb_uniqkeys = NIL;
	parent_fields->yb_uniqpath_provisional = yb_uniqpath_provisional;
}
...
/*
 * YB: yb_create_skipscan_path
 *
 * Skip Scan (or distinct index scan) scans the underlying table for
 * distinct values of the prefix.
 * To be more precise, the scan may return duplicate values when not all keys
 * corresponding to the prefix are required by the query.
 *
 * Path costing is not entirely accurate but works reasonably well.
 *
 * Returns a polymorphic path.
 * - sometimes sticks an UpperUniquePath on top, see the comment below.
 */
Path *
yb_create_skipscan_path(PlannerInfo *root,
						IndexOptInfo *index,
						IndexPath *basepath,
						...
						int yb_distinct_nkeys)
{
	IndexPath  *pathnode = makeNode(IndexPath);
	int			numDistinctRows;
	int			prefixlen = list_length(yb_distinct_prefix);
	List	   *prefixExprs;
	ListCell   *lc;
	int			i;
	...
	/*
	 * A distinct index scan path created using the computed prefix may or
	 * may not be used by the planner.
	 *
	 * For example: SELECT DISTINCT VOLATILE_FN(t1.r * t2.r) FROM t1, t2;
	 * The baserel t1 is unaware of the volatile function in the final scanjoin
	 * target. Hence, the generated index scan path may be dropped because of
	 * which, these paths are usually accompanied by other paths such as
	 * sequential scans or index scans.
	 * yb_uniqpath_provisional is an indication that this path must be
	 * accompanied as such.
	 */
	pathnode->path.yb_path_info.yb_uniqpath_provisional = true;
	...
	Assert(prefixlen >= index->nhashcolumns);
	/*
	 * DocDB may return duplicate rows from different tablets.
	 * So, stick a upper unique node.
	 *
	 ...
	 * 	  this pathnode's pathkeys since r2 is not prefix (only a subset).
	 *
	 ...
	 * 	  r1 and r2 are the same, requiring the Unique node only DISTINCT'ify
	 ...
	 *
	 * 5. SELECT DISTINCT r1 WHERE r1 = 1
	 * 	  yb_distinct_nkeys == 0. In this case all tuples are considered equal.
	 * 	  Hence, 0 or 1 tuples are returned.
	 *
	 ...
	 * - yb_distinct_nkeys < 0 => Keys missing either from prefix or hash cols.
	 * 	 At least one key missing from prefix => No unique node necessary
	 * 	 because yb_has_uniqkeys_for reports excess keys.
	 * 	 i.e. Unique Node => yb_distinct_nkeys < 0.
	 * Hence, a unique node is unnecessary when there are no hash columns or
	 * some keys are missing from the prefix.
	 */
	if (index->nhashcolumns == 0 && yb_distinct_nkeys >= 0)
		return (Path *)
