				...
				List *yb_bitmap_idx_pushdowns, List **bitindexpaths)
{
	...
	foreach(lc, indexpaths)
	{
		 ...
		 * may be represented by a UniquePath on top of a distinct index
		 ...
		 *
		 * YB_TODO_PG19MERGE: PG removed UpperUniquePath (renamed to UniquePath
		 * to compile); distinct-pushdown task should replace this indirect
		 * marker.
		 */
		if ((index->amhasgetbitmap || index->yb_amhasgetbitmap) &&
			!(IsA(ipath, UniquePath) ||
...
{
	List	   *interpretations = get_op_index_interpretation(opno);
	ListCell   *lc;
	bool		result = false;

	foreach(lc, interpretations)
	{
		OpIndexInterpretation *interp = (OpIndexInterpretation *) lfirst(lc);

		if (interp->cmptype == COMPARE_EQ)
		{
			result = true;
			break;
		}
							...
							IndexOptInfo *index)
{
...

	{
		...
		else
		{
			...
			rc->cmptype = (CompareType) hc_strategy;
			rc->opnos = new_ops;
