...
range_out(PG_FUNCTION_ARGS)
{
  ...
  flags = range_get_flags(range);
	...
	if (RANGE_HAS_LBOUND(flags)) lbound_str = OutputFunctionCall(&cache->proc, lower.val);
	if (RANGE_HAS_UBOUND(flags)) ubound_str = OutputFunctionCall(&cache->proc, upper.val);
	}
