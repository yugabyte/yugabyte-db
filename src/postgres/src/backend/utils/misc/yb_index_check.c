...
indexrel_scan_plan(Relation indexrel)
{
	...
	attr = SystemAttributeDefinition(YBIdxBaseTupleIdAttributeNumber, false);
	...
	{
		attr = SystemAttributeDefinition(YBUniqueIdxKeySuffixAttributeNumber, false);
...
baserel_scan_plan(Relation baserel, Relation indexrel)
{
	...
	int resno = 1;
	const FormData_pg_attribute *attr =
		SystemAttributeDefinition(YBTupleIdAttributeNumber, false);
...
spurious_check_plan(Relation baserel, Relation indexrel)
{
	...
	TupleDesc indexrel_scan_desc = ExecTypeFromTL(indexrel_scan->targetlist, false);
	...
	/* Join claue */
	attr = SystemAttributeDefinition(YBTupleIdAttributeNumber, false);
...
check_spurious_index_rows(Relation baserel, Relation indexrel)
{
	...
	estate->es_range_table = list_make1(rte1);
...
/*
 * Check a single leaf index. lock_index_check_relations() already holds
 * AccessShareLock on indexrel and baserel; release those locks when done.
 */
static void
 ...
 * Locks on leaf indexes and their base tables are released by
 * yb_index_check_internal() when that leaf's check ends.
 */
static List *
...
yb_index_check(PG_FUNCTION_ARGS)
{
	 ...
	 */
	leaf_indexes = lock_index_check_relations(indexoid);

	validate_index_check_rls(leaf_indexes);

	foreach(lc, leaf_indexes)
		yb_index_check_internal(lfirst_oid(lc));
