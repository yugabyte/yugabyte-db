...
FetchUniqueConstraintName(Oid relation_id)
{
	...
	Oid			pkindex = OidIsValid(rel->rd_pkindex) ? rel->rd_pkindex :
		RelationGetPrimaryKeyIndex(rel, false /* deferrable_ok */ );
...
int			yb_parallel_range_size = 16 * 1024 * 1024;
bool		yb_disable_parallel_query_in_ddl = true;
bool		yb_enable_advanced_index_cond_fold = true;
...
YbRelationIsHashPartitioned(Relation rel)
{
	switch (rel->rd_rel->relkind)
	{
		...
		{
			Oid			pkidx = RelationGetPrimaryKeyIndex(rel, false /* deferrable_ok */ );
			bool		hash;
...
YbCanSkipIntents(Relation rel, bool is_write)
{
	 ...
	 * 2. rd_newRelfilelocatorSubid: The physical storage was swapped/created
