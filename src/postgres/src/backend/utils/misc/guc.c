 ...
 * PG commit a13833c35f9e changed the struct layout: config_generic now
 * contains a union of the type-specific structs (._bool, ._int, ...) rather
 * than each type-specific struct embedding `struct config_generic gen`
 * as its first field.
 *
 * The 5 ConfigureNames* arrays below + ConfigureNamesOid still use
...
build_guc_variables(void)
{
	...
	/*
	 * YB_TODO_PG19MERGE: YB GUCs need to be handled (either explicitly,
	 * or moved into guc_parameters.dat).
	 */
