...
main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "acd:E:f:gh:l:m:oOp:rsS:tU:vwWx", long_options, &optindex)) != -1)
	{
		switch (c)
		{
			case 'a':
	...
	{
		fprintf(stderr, _("%s: options --binary-upgrade and --include-yb-metadata cannot be used together\n"),
				progname);
		fprintf(stderr, _("Try \"%s --help\" for more information.\n"),
				progname);
		exit_nicely(1);
	...
	{
		fprintf(stderr, _("%s: option --dump-role-checks must be used only together with --include-yb-metadata\n"),
				progname);
		fprintf(stderr, _("Try \"%s --help\" for more information.\n"),
				progname);
		exit_nicely(1);
...
dumpGroups(PGconn *conn)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		for (j = 0; j < PQntuples(res2); j++)
		{
			...
			/*
			 * In YB PG11-based code server_version >= 110000.
			 * So this dumpGroups() function is never called in YB.
			 */
