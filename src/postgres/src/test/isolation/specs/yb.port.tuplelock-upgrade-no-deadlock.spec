...
# YB: Ported from tuplelock-upgrade-no-deadlock. Three permutations that test
# FIFO lock acquisition ordering are removed because YugabyteDB wait queues do
# not guarantee FIFO ordering completely among waiters.
...
# YB: The following three permutations from the original test are removed because
# they test FIFO lock ordering ("sessions that don't upgrade locks acquire them in
# order"), which YugabyteDB wait queues do not guarantee completely:
