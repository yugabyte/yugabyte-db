...
test_enc_setup(PG_FUNCTION_ARGS)
{
	...
	for (int i = 0; i < _PG_LAST_ENCODING_; i++)
	{
		...
		bigbuf[0] = buf[0];
		bigbuf[1] = buf[1];
		valid = pg_encoding_verifymb(i, bigbuf, sizeof(bigbuf));
		if (valid != 0)
			elog(WARNING,
...
yb_run_spi(PG_FUNCTION_ARGS)
{
	...
	if (SPI_processed <= 0)
		...
		return_count = SPI_tuptable->alloced - SPI_tuptable->free;
