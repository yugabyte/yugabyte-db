...
declare
    ln text;
begin
    /* YB: parallelism is hard to force. Just ignore the plan entirely.
    for ln in
...
set enable_indexscan = false; -- YB: try to force bitmap scans for the following plans
set yb_enable_bitmapscan = true; -- YB: try to force bitmap scans for the following plans
...
-- reset enable_indexscan; -- YB: keep forcing bitmap scans for the next plan
...
reset yb_enable_bitmapscan; -- YB
reset enable_indexscan; -- YB
...
create index on ma_test (b ASC); -- YB: change to ASC to allow for range queries
...
select tableoid::regclass, * from pph_arrpart order by 1,2; -- YB: add more ordering
...
create index on rangep (a ASC); -- YB: change to ASC to allow for range queries
