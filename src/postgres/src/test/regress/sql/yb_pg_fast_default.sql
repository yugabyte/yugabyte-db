...
WITH deleted AS (DELETE FROM T WHERE pk BETWEEN 10 AND 20 RETURNING *)
    SELECT * FROM deleted ORDER BY pk;
EXPLAIN (VERBOSE TRUE, COSTS FALSE)
...
SELECT * FROM vtype2 ORDER BY a;

-- YB Note: This test is disabled in YB because the INSERT will be aborted
-- due to the ALTER TABLE being executed in the same transaction.
-- Additionally, the referenced issue doesn't apply to YB relations because
-- missing column values for YB relations are filled in by DocDB,
-- so heap_getattr() will never have to look for missing attributes.
