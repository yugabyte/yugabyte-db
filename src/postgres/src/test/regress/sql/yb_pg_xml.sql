...
SELECT * FROM xmltest ORDER BY id;
...
SELECT xmlagg(data) FROM xmltest ORDER BY id;
SELECT xmlagg(data) FROM xmltest WHERE id > 10 ORDER BY id;
...
SELECT xpath(NULL, NULL) IS NULL FROM xmltest ORDER BY id;
SELECT xpath('', '<!-- error -->');
...
SELECT xmltable.* FROM xmltest2, LATERAL xmltable('/d/r' PASSING x COLUMNS a int PATH '' || lower(_path) || 'c') ORDER BY _path;
SELECT xmltable.* FROM xmltest2, LATERAL xmltable(('/d/r/' || lower(_path) || 'c') PASSING x COLUMNS a int PATH '.') ORDER BY _path;
SELECT xmltable.* FROM xmltest2, LATERAL xmltable(('/d/r/' || lower(_path) || 'c') PASSING x COLUMNS a int PATH 'x' DEFAULT ascii(_path) - 54) ORDER BY _path;
