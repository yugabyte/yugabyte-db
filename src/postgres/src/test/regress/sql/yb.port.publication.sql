...
-- YB Note: TABLES IN SCHEMA is blocked at runtime in YB.
...
-- YB Note: REPLICA IDENTITY USING INDEX is unsupported (#1124).
...
-- YB Note: regress tests use the 'yugabyte' database
...
-- will be successful.
-- YB Note: schema publications are blocked at runtime in YB.
...
-- YB Note: Clean up leaked objects for the next regress test. Multi-object DROP
-- PUBLICATION is unsupported (#880), and tables without a primary key can now be
-- part of publications (#29423), so some objects are not dropped earlier in the
-- test. Drop publications before their tables (a table that is part of a
-- publication cannot be dropped, #26659).
DROP PUBLICATION IF EXISTS testpub_default;
DROP PUBLICATION IF EXISTS testpib_ins_trunct;
DROP PUBLICATION IF EXISTS testpub_fortbl;
DROP PUBLICATION IF EXISTS testpub_fortable;
DROP PUBLICATION IF EXISTS testpub_forparted;
DROP PUBLICATION IF EXISTS testpub_forparted1;
...
DROP TABLE IF EXISTS testpub_tbl7;
DROP TABLE IF EXISTS testpub_tbl8;
DROP TABLE IF EXISTS testpub_tbl8_0;
DROP TABLE IF EXISTS testpub_tbl8_1;
DROP TABLE IF EXISTS testpub_tbl_both_filters;
DROP TABLE IF EXISTS testpub_rf_tbl1;
DROP TABLE IF EXISTS testpub_unloggedtbl;
DROP SCHEMA IF EXISTS pub_test CASCADE;
