...
-- YB_TODO: begin: remove this after tracking yb.port.partition_prune
...
-- YB_TODO: begin: remove after tracking yb.orig.join_batching
SET enable_hashjoin = off;
SET enable_mergejoin = off;
SET enable_seqscan = off;
SET enable_material = off;
...
-- YB_TODO: begin: remove after tracking yb.port.foreign_key
...
select * from outer_text where (f1, f2) not in (select * from inner_text) order by f2;
-- YB_TODO: begin: remove after tracking yb.orig.tablespaces
