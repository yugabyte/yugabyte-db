...
-- store the index's relfilenode before ALTER TYPE
CREATE TEMP TABLE index_info AS
    SELECT relfilenode FROM pg_class WHERE relname = 'idx_collation';
...
-- check if index was rebuilt (relfilenode changed) -- we expect it to be rebuilt
SELECT
    CASE
        WHEN old.relfilenode = new.relfilenode
        THEN 'Index reused'
...
-- store the table's relfilenode before ALTER TYPE
CREATE TEMP TABLE table_info AS
    SELECT relfilenode FROM pg_class WHERE relname = 'compatible_test';
-- store the index's relfilenode before ALTER TYPE
CREATE TEMP TABLE index_info AS
    SELECT relfilenode FROM pg_class WHERE relname = 'idx_compatible';
-- alter type without changing storage or semantics (just increase length)
...
SELECT
    ...
    END as result
FROM table_info old, pg_class new
WHERE new.relname = 'compatible_test';
-- check if index was rebuilt (relfilenode changed) -- we expect it to not be rebuilt
SELECT
    CASE
        WHEN old.relfilenode = new.relfilenode
        THEN 'Index reused'
    ...
    END as result
FROM index_info old, pg_class new
WHERE new.relname = 'idx_compatible';
