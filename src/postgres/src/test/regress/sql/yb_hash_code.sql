...
EXPLAIN (COSTS OFF, TIMING OFF, SUMMARY OFF, ANALYZE) /*+IndexOnlyScan(text_table textidx)*/ SELECT tj FROM text_table WHERE 63 >= yb_hash_code(tj);
/*+IndexOnlyScan(text_table textidx)*/ SELECT tj FROM text_table WHERE 63 >= yb_hash_code(tj);
