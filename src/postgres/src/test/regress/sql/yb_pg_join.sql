...
-- TODO: Add yb_pg_inherits once it is supported.
-- explain (costs off)
...
select * from
  j1_tbl full join
  (select * from j2_tbl order by j2_tbl.i desc, j2_tbl.k asc) j2_tbl
  on j1_tbl.i = j2_tbl.i and j1_tbl.i = j2_tbl.k order by 1, 2, 3, 4, 5;
...
-- Inherits not supported in YB. See https://github.com/YugaByte/yugabyte-db/issues/1129
...
select tt1.*, tt2.* from tt1 left join tt2 on tt1.joincol = tt2.joincol order by 1, 2, 3, 4;

select tt1.*, tt2.* from tt2 right join tt1 on tt1.joincol = tt2.joincol order by 1, 2, 3, 4;
...
select * from tt5,tt6 where tt5.f1 = tt6.f1 and tt5.f1 = tt5.f2 - tt6.f2 order by 1, 2, 3, 4;
...
from yy
     ...
     left join xx xxb on coalesce (xxa.pkxx, 1) = xxb.pkxx order by 1, 2, 3, 4, 5;
...
-- TODO: remove these when closing issue #5313
drop table a;
drop table b;
...
SELECT qq, unique1
  ...
  ( SELECT COALESCE(q2, -1) AS qq FROM int8_tbl b ) AS ss2
  USING (qq)
  INNER JOIN tenk1 c ON qq = unique2 order by 1, 2;
...
(values (1, array[10,20]), (2, array[20,30])) as v1(v1x,v1ys)
left join (values (1, 10), (2, 20)) as v2(v2x,v2y) on v2x = v1x
left join unnest(v1ys) as u1(u1y) on u1y = v2y order by 1, 2;
...
select t1.unique1 from
  tenk1 t1 join tenk2 t2 on t1.two = t2.two; -- error, prefer "t1" suggestion
select t2.unique1 from
...
select *, (select r from (select q1 as q2) x, (select q2 as r) y) from int8_tb order by 1, 2, 3;
select *, (select r from (select q1 as q2) x, lateral (select q2 as r) y) from int8_tbl order by 1, 2;
...
select * from generate_series(100,200) g,
  lateral (select * from int8_tbl a where g = q1 union all
           select * from int8_tbl b where g = q2) ss order by 1, 2, 3;
...
select * from int8_tbl a,
  int8_tbl x left join lateral (select a.q1 from int4_tbl y) ss(z)
    on x.q2 = ss.z
  order by a.q1, a.q2, x.q1, x.q2, ss.z order by 1, 2, 3, 4;
...
select * from
  ...
  lateral (values(x.q1,y.q1,y.q2)) v(xq1,yq1,yq2) order by 1, 2;
select * from
  int8_tbl x left join (select q1,coalesce(q2,0) q2 from int8_tbl) y on x.q2 = y.q1,
         ...
         lateral (select * from
                    (select * from int8_tbl t2
                     where q1 = any (select q2 from int8_tbl t3
                                     where q2 = (select greatest(t1.q1,t2.q2))
                                       and (select v.id=0)) offset 0) ss2) ss
         where t1.q1 = ss.q2) ss0 order by 1, 2, 3, 4, 5;
...
-- NOTE: This test has been removed for now because YB does not support Partitioning of Tables.
-- See: https://github.com/YugaByte/yugabyte-db/issues/1126
...
-- Since the rest of join.sql relies on savepoint, yb_pg_join ends here. 