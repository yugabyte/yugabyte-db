...
-- Test #32692: DISTINCT over UNION ALL or partitioned table was failing with "could not find pathkey item to sort"
CREATE TABLE u1(a INT);
CREATE TABLE u2(a INT);
CREATE INDEX u1_a_idx ON u1 (a ASC);
CREATE INDEX u2_a_idx ON u2 (a ASC);
INSERT INTO u1 (SELECT i%2 FROM GENERATE_SERIES(1, 100) AS i);
INSERT INTO u2 (SELECT i%3 FROM GENERATE_SERIES(1, 100) AS i);

EXPLAIN (COSTS OFF)
SELECT DISTINCT x FROM (SELECT a FROM u1 UNION ALL SELECT a FROM u2) s(x) ORDER BY 1;
...
CREATE TABLE pt(k INT, v INT) PARTITION BY RANGE (k);
CREATE TABLE pt1 PARTITION OF pt FOR VALUES FROM (0) TO (5);
CREATE TABLE pt2 PARTITION OF pt FOR VALUES FROM (5) TO (10);
CREATE INDEX pt_v_idx ON pt (v ASC);
INSERT INTO pt (SELECT i%10, i%3 FROM GENERATE_SERIES(1, 100) AS i);

EXPLAIN (COSTS OFF) SELECT DISTINCT v FROM pt ORDER BY 1;

SELECT DISTINCT v FROM pt ORDER BY 1;
