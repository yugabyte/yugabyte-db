...
-- 2024.2 has the single-argument yb_index_check(oid) form only.
\c yugabyte
...
RESET ROLE;
REVOKE EXECUTE ON FUNCTION yb_index_check(oid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION yb_index_check(oid)
  TO rls_admin, rls_owner, rls_exec;

-- Control: RLS disabled. Every caller with EXECUTE passes; the caller without
-- EXECUTE is rejected by the function ACL.
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- superuser
SET ROLE rls_admin;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- BYPASSRLS
RESET ROLE;
SET ROLE rls_owner;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- owner
RESET ROLE;
SET ROLE rls_exec;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- non-owner + EXECUTE
RESET ROLE;
SET ROLE rls_noexec;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- non-owner, no EXECUTE
RESET ROLE;

-- RLS enabled, FORCE disabled: the owner is not subject to RLS, so only the
-- non-owner with EXECUTE is rejected by the guardrail.
...
RESET ROLE;
SET ROLE rls_owner;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- owner (exempt)
RESET ROLE;
SET ROLE rls_exec;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- rejected
RESET ROLE;
SET ROLE rls_noexec;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- ACL error
RESET ROLE;
...
RESET ROLE;
SET ROLE rls_owner;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- owner rejected (FORCE)
RESET ROLE;
SET ROLE rls_exec;
SELECT yb_index_check('rls_tbl_body_idx'::regclass::oid); -- rejected
RESET ROLE;
...
DROP TABLE rls_tbl;
GRANT EXECUTE ON FUNCTION yb_index_check(oid) TO PUBLIC;
REVOKE CREATE ON SCHEMA public FROM rls_owner;
REVOKE EXECUTE ON FUNCTION yb_index_check(oid)
...
SELECT yb_index_check('rls_part_body_idx'::regclass::oid);
SELECT yb_index_check('rls_part_1_body_idx'::regclass::oid);
RESET ROLE;
...
SELECT yb_index_check('rls_part_2_body_idx'::regclass::oid);
SELECT yb_index_check('rls_part_1_body_idx'::regclass::oid);
RESET ROLE;
