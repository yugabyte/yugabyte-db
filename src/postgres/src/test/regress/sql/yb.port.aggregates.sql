...
--    sign function.
select f1, (select min(unique1) from tenk1 where unique1 > f1) AS gt
  from int4_tbl order by abs(f1) - sign(f1); -- YB ordering
...
/*+ Set(enable_hashagg off) */ explain (costs off) -- YB: disable hashagg to get the same plan as PG
  select distinct min(f1), max(f1) from minmaxtest;
/*+ Set(enable_hashagg off) */  -- YB: disable hashagg to get the same plan as PG
select distinct min(f1), max(f1) from minmaxtest;
...
explain (costs off)
  ...
  from int4_tbl t0 order by abs(f1) - sign(f1); -- YB: ordering
select f1, (select distinct min(t1.f1) from int4_tbl t1 where t1.f1 = t0.f1)
from int4_tbl t0 order by abs(f1) - sign(f1); -- YB: ordering
rollback;
...
/*+ HashJoin(t1 t2) Set(enable_sort false) */ explain (costs off) select * -- YB: force hash join + hash agg
from t1 inner join t2 on t1.a = t2.x and t1.b = t2.y
group by t1.a,t1.b,t1.c,t1.d,t2.x,t2.y,t2.z;

-- Test case where t1 can be optimized but not t2
/*+ HashJoin(t1 t2) Set(enable_sort false) */ explain (costs off) select t1.*,t2.x,t2.z -- YB: force hash join + hash agg
from t1 inner join t2 on t1.a = t2.x and t1.b = t2.y
...
create or replace aggregate my_sum(int4) -- YB: add REPLACE because DDL is not transactional
(
   stype = int4,
   sfunc = sum_transfn
);
...
/* YB: parallelism cannot be forced, so we cannot test the parallel aggregate combiner function.
