...
ROLLBACK; BEGIN ISOLATION LEVEL SERIALIZABLE, READ ONLY, DEFERRABLE; -- YB workaround of above failure
...
-- YB: above fails even without a savepoint in YB.  If that is fixed, port remaining queries
COMMIT;
...
-- YB: imitate PG ctid reallocation for the following updates: row a=0 should be first:
create temp table ybtmp as select * from xacttest where a > 0;
delete from xacttest where a > 0; -- YB
insert into xacttest select * from ybtmp;
drop table ybtmp;
...
select * from xacttest; -- YB: TODO(#22947): fix output
rollback;
