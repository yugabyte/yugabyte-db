...
ALTER TABLE part6 ATTACH PARTITION part6b FOR VALUES IN (66, 67, 68, 69, 70);
...
-- YB note: Introduce an explicit ORDER BY via CTE in 2.20 for consistent results
WITH cte AS (UPDATE part6a SET c = NOT c, a = a + 2 WHERE a BETWEEN 60 AND 65 RETURNING c, b, a) SELECT c, b, a FROM cte ORDER BY a;
