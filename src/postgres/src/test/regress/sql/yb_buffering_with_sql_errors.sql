...
-- #27552: Make the following table a temp table when speculative execution is supported for temp tables.
CREATE TABLE t_nottemp (k INT, v INT);
...
BEGIN
	INSERT INTO t_audit VALUES (NEW.k);
	INSERT INTO t_nottemp VALUES (NEW.k, NEW.v);
...
BEGIN
	...
	SELECT current_setting('client_encoding', TRUE) INTO vtext;
	INSERT INTO t_audit VALUES (NEW.k + 100), (NEW.k + 101);
  -- #27552: Uncomment when speculative execution is supported for temp tables.
	-- SELECT SUM(k) + SUM(v) FROM t_nottemp WHERE k > 10 INTO vint;
	INSERT INTO t_other VALUES (NEW.k, NEW.v);
	RETURN NEW;
END; $$;
...
BEGIN
	...
	IF TRUE THEN
		INSERT INTO t_nottemp VALUES (NEW.k, NEW.v);
	ELSE
		INSERT INTO t_nottemp VALUES (NEW.k + 100, NEW.v + 100);
	END IF;
	INSERT INTO t_other VALUES (NEW.k, NEW.v);
	RETURN NEW;
END; $$;
...
SELECT * FROM t_nottemp ORDER BY k;
