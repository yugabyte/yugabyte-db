...
CREATE INDEX partitioned_table_name_idx ON partitioned_table (name);
...
-- automatic indexes must share the partition's tablespace and implicit tablegroup
SELECT t.relname, t.reltablespace = c.reltablespace AS same_tablespace,
       (yb_table_properties(t.oid)).tablegroup_oid =
       (yb_table_properties(c.oid)).tablegroup_oid AS same_tablegroup
FROM pg_inherits h JOIN pg_index i ON i.indexrelid = h.inhrelid
    JOIN pg_class t ON t.oid = i.indrelid
    JOIN pg_class c ON c.oid = i.indexrelid
WHERE h.inhparent = 'partitioned_table_name_idx'::regclass
ORDER BY t.relname;
