-- YB note: derived from xml.sql with ordering added for deterministic output.
...
-- YB note: add ordering for deterministic output.
...
SELECT xmlelement(name employee, xmlforest(name, age, salary as pay)) FROM emp
  ORDER BY name, age, salary, manager;
...
SELECT xmlagg(data ORDER BY id) FROM xmltest WHERE id > 10;
   ...
   FROM (SELECT data FROM xmldata) x,
        LATERAL XMLTABLE('/ROWS/ROW'
                         ...
                         COLUMNS id int PATH '@id',
                                  ...
                                  premier_name text PATH 'PREMIER_NAME' DEFAULT 'not specified') -- yb
  ORDER BY id, _id; -- yb deterministic order
...
-- YB note: add ordering for deterministic output.
SELECT * FROM xmltableview1 ORDER BY id, _id;
...
-- YB note: add ordering for deterministic output.
SELECT * FROM xmltableview2 ORDER BY a;
...
SELECT xmltable.* FROM xmldata, LATERAL xmltable('/ROWS/ROW[COUNTRY_NAME="Japan" or COUNTRY_NAME="India"]' PASSING data COLUMNS "COUNTRY_NAME" text, "REGION_ID" int)
  ORDER BY "COUNTRY_NAME", "REGION_ID";
   ...
   FROM (SELECT data FROM xmldata) x,
        LATERAL XMLTABLE('/ROWS/ROW'
                         ...
                         COLUMNS id int PATH '@id',
                                  ...
                                  unit text PATH 'SIZE/@unit',
                                  premier_name text PATH 'PREMIER_NAME' DEFAULT 'not specified')
  WHERE region_id = 2 ORDER BY id, _id; -- yb deterministic order
