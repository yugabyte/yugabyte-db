...
-- Wait for the master->tserver heartbeat (and the tserver's shared-memory update) so this
-- backend picks up the new version on its next query. The in-backend cache-refresh fallback
-- still handles this race, but the sleep avoids the noisy "catalog snapshot used for this
-- transaction has been invalidated" retry round-trip that would otherwise occur.
SELECT pg_sleep(2);
