-- ----------------------------------------------------------------------------------------
-- Tests that perform operations on a table that was created/ swapped in
-- the same transaction block. Such operations might use the "Skip Intents DB"
-- optimization or not but the outcome should be the same irrespective of that.
-- ----------------------------------------------------------------------------------------
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- TEST 2: Statement shouldn't see its own writes
-- -----------------------------------------------------------------------------
-- Three shapes of the same requirement. A statement reads at its own in_txn_limit,
-- which is below every row it writes, so it never reads its own output back.

-- Case 1: a self-referencing INSERT. The SELECT must find only the 100000 rows that
-- existed when the statement started, so the INSERT adds exactly 100000 more rather
...
-- Case 2: an aggregate over the table being written. sum(balance) must cover only the
-- two rows the earlier statement wrote.
BEGIN;
...
-- Case 3: an UPDATE whose scan would otherwise re-read the rows it just wrote:
-- 1. Scan finds val=10. 10 < 100000 is TRUE.
-- 2. UPDATE writes val=11 directly to Regular DB (Skip Intents).
-- 3. If the scan doesn't avoid reading writes by the same statement, it might
--    find val=11 later.
-- 4. 11 < 100000 is still TRUE.
...
-- EXPECTED (a statement does not read its own writes): val = 11
-- ACTUAL (if it does): val = 100000 (or the query loops/times out)
SELECT * FROM increment_test;

COMMIT;

-- -----------------------------------------------------------------------------
-- TEST 3: Interleaved Modifying CTEs Visibility (YB vs PG deviation)
-- -----------------------------------------------------------------------------
BEGIN;
...
-- -----------------------------------------------------------------------------
-- TEST 4: Statement should not see own writes (Case 1) (GHI #32221)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- TEST 5: Statement should not see own writes (Case 2) (GHI #32221)
-- -----------------------------------------------------------------------------
...
ROLLBACK;
-- -----------------------------------------------------------------------------
-- TEST 6: Statement should not see own writes (Case 3) (GHI #32221)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- TEST 7: Statement should not see own writes (Case 4) (GHI #32221)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- TEST 8: Interleaved Nested Write to Existing Table Anomaly (GHI #32221)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- TEST 9: CTAS Nested Read Anomaly (GHI #32221)
-- -----------------------------------------------------------------------------
...
-- Top-level CTAS statement. It runs outside an explicit
...
-- -----------------------------------------------------------------------------
-- TEST 10: Nested statements see writes of earlier statements (GHI #32221)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- TEST 11: Cursor and savepoint visibility of skip intent writes
-- (GHI #32221, GHI #33543)
-- -----------------------------------------------------------------------------
-- The output of every case below is the same irrespective of whether the skip-intents
...
-- the regular db, at a hybrid time above the transaction read time. Reading at
-- in_txn_limit is what keeps them visible, and collapsing the uncertainty
...
-- Case 8: a savepoint disables the optimization for the rest of the transaction, so the
-- statements after it take the regular transactional path while the rows written before
-- it are already in the regular db. A write after the savepoint goes to the intents db,
-- and both must be visible.
BEGIN;
CREATE TABLE savepoint_reads (id INT PRIMARY KEY);
INSERT INTO savepoint_reads SELECT generate_series(1, 5);
SAVEPOINT sp;
SELECT count(*) FROM savepoint_reads;
INSERT INTO savepoint_reads VALUES (6);
SELECT id FROM savepoint_reads ORDER BY id;
COMMIT;
SELECT count(*) FROM savepoint_reads;

-- Case 9: ROLLBACK TO SAVEPOINT. Rows written before the savepoint went to the regular
-- db and cannot be rolled back, which is precisely why the savepoint disables the
-- optimization; the rows written after it were intents and are aborted.
BEGIN;
CREATE TABLE savepoint_rollback (id INT PRIMARY KEY);
INSERT INTO savepoint_rollback SELECT generate_series(1, 3);
SAVEPOINT sp;
INSERT INTO savepoint_rollback SELECT generate_series(4, 6);
ROLLBACK TO SAVEPOINT sp;
SELECT id FROM savepoint_rollback ORDER BY id;
COMMIT;
SELECT id FROM savepoint_rollback ORDER BY id;

DROP TABLE cursor_new_rel, cursor_stability, cursor_for_update, cursor_with_hold,
           cursor_rewrite, cursor_savepoint, savepoint_reads, savepoint_rollback;
