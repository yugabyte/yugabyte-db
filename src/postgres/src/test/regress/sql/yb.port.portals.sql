...
FETCH ALL FROM c1;
COMMIT;
/* YB: The below DROP TABLE is commented because the above FETCH ALL command fails
in YB, so the transaction aborts and the table 'cursor' is rolled back.
This isn't the case in PG.
DROP TABLE cursor;
*/ -- YB
