...
#define YB_GENERIC_CHECK_OVERFLOW(op, arg1, arg2, int_type, builtin_suffix, \
								  out_of_range_msg) \
	do { \
...
#define YB_CHECK_INT64_OVERFLOW(op, arg1, arg2) \
	YB_GENERIC_CHECK_OVERFLOW(op, arg1, arg2, int64_t, \
							  BOOST_PP_CAT(YB_INT64_OVERFLOW_CHECKING_FRAGMENT, \
										   _overflow), \
							  "bigint out of range")

#define YB_GENERIC_DISALLOW_VALUE(arg, disallowed_value, int_type, \
								  out_of_range_msg) \
	do { \
		YB_CHECK_OVERFLOW_MACRO_ARG_TYPE(arg, "First argument", int_type); \
		YB_CHECK_OVERFLOW_MACRO_ARG_TYPE(disallowed_value, "Second argument", \
										 int_type); \
