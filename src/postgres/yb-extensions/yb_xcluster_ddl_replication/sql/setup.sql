...
CREATE PROCEDURE TEST_reset()
  LANGUAGE SQL AS
$$
  SET yb_xcluster_ddl_replication.ddl_queue_primary_key_ddl_end_time = 0;
...
-- The consumer extension expects for ddl_queue_primary_key_ddl_end_time and
...
BEGIN
  ...
  BEGIN
    ddl_end_time := current_setting(
        'yb_xcluster_ddl_replication.ddl_queue_primary_key_ddl_end_time');
    ddl_end_time := ddl_end_time + 1;
    PERFORM set_config(
        'yb_xcluster_ddl_replication.ddl_queue_primary_key_ddl_end_time', ddl_end_time::text, false);
