...
BEGIN
  for rec in
    ...
    loop
        foreach map_key in array
            array['new_rel_map', 'sequence_info', 'type_info', 'enum_label_info']
        loop
            if rec.yb_data->map_key is null then
                continue;
            end if;
            new_array := '[]'::jsonb;
            for element in
                select * from jsonb_array_elements(rec.yb_data->map_key)
            loop
                for k, v in
                    select * from jsonb_each_text(element)
                loop
                    if k in ('relfile_oid', 'colocation_id', 'oid',
                             'enum_oid', 'label_oid') then
                        element := jsonb_set(element, ('{' || k || '}')::text[], '"***"');
                    end if;
                end loop;
                new_array := new_array || element;
            end loop;
            rec.yb_data := jsonb_set(rec.yb_data, ('{' || map_key || '}')::text[], new_array);
        end loop;
        return next rec;
    end loop;
END
