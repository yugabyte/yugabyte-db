 ...
 */
static inline int yb_od_relay_auth_server_to_client(od_server_t *server,
						    machine_msg_t *msg,
						    od_client_t *client,
						    od_instance_t *instance,
						    char *context)
		    ...
		    od_client_t *client)
{
	...
	if (client->yb_is_authenticating)
	{
		...
		assert(auth_type == OD_AUTH_CLEARTEXT ||
		       auth_type == OD_AUTH_MD5 || auth_type == OD_AUTH_SASL ||
		...
		rc = yb_od_relay_auth_server_to_client(
			server, msg, external_client, instance,
			yb_authtype_to_string(auth_type));
		if (rc == -1) {
			return -1;
		}
		...
		rc = yb_od_relay_client_to_auth_server(
			server, external_client, instance,
			yb_authtype_to_string(auth_type));
		if (rc == -1) {
			return -1;
		}
