...
// under the License.
= YugaByte Developer Documentation

== Building and installing YugaByte

All you need to build the code is:
[source,bash]
----
$ ./yb_build.sh
----

To see where the executables are built:
[source,bash]
----
$ ls build/debug/bin
----
...
To run the YB unit tests, you can use the `ctest` command from within the
...
_build/debug/bin_. Since YB uses the Google C++ Test Framework (gtest),
...
errors. The Jenkins setup for yb runs these tests automatically on a regular
...
ASAN above, but pass the `-DYB_USE_UBSAN=1` flag to the `cmake` invocation.
...
bugs. To enable TSAN, pass `-DYB_USE_TSAN=1` to the `cmake` invocation,
...
YB uses cpplint.py from Google to enforce coding style guidelines. You can run the
...
=== Building YB documentation

YB's documentation is written in asciidoc and lives in the _docs_ subdirectory.
...
The yb build is compatible with ccache. Simply install your distro's _ccache_ package,
...
One of the major time sinks in the YB build is linking. GNU ld is historically
...
=== Building YB with dynamic linking

YB can be built into shared objects, which, when used with ccache, can result in a
...
[source,bash]
----
$ cmake -DYB_LINK=dynamic ../..
----

Subsequent builds will create shared objects instead of archives and use them when
linking the yb binaries and unit tests. The full range of options for `YB_LINK` are
...
== Developing YB in Eclipse

Eclipse can be used as an IDE for YB. To generate Eclipse project files, run:
...
By default, the Eclipse CDT indexer will index everything under the _yb/_
