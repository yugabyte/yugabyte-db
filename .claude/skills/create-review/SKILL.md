...
- **PII** — real names, real emails, real phone numbers, real postal addresses, real IP addresses (public or private). In tests use only the documentation ranges: IPv4 `192.0.2.0/24` / `198.51.100.0/24` / `203.0.113.0/24` (RFC 5737), IPv6 `2001:db8::/32` (RFC 3849), hostnames `example.com` / `example.org` / `example.net` (RFC 2606).
...
- **Secrets / credentials** — API keys, tokens, passwords, certificates, license keys, kubeconfigs, production S3 buckets, internal Slack/JIRA/Linear URLs (other than referencing them by ticket key like `PLAT-20518`).
- **YugabyteDB-internal information not yet public** — unreleased roadmap, internal SLAs, embargoed security findings, unshipped gflag/code-path names.
