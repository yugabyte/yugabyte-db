...
CREATE OR REPLACE FUNCTION oracle.substr(str text, start int)
RETURNS text
...
CREATE OR REPLACE FUNCTION oracle.substr(str text, start int, len int)
RETURNS text
...
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION plvdate.use_easter(bool) IS 'Easter Sunday and easter monday will be holyday';

CREATE OR REPLACE FUNCTION plvdate.use_easter()
RETURNS bool
AS $$SELECT plvdate.use_easter(true); SELECT NULL::boolean;$$
LANGUAGE SQL VOLATILE STRICT;
COMMENT ON FUNCTION plvdate.use_easter() IS 'Easter Sunday and easter monday will be holyday';

CREATE OR REPLACE FUNCTION plvdate.unuse_easter()
RETURNS bool
AS $$SELECT plvdate.use_easter(false); SELECT NULL::boolean;$$
LANGUAGE SQL VOLATILE STRICT;
COMMENT ON FUNCTION plvdate.unuse_easter() IS 'Easter Sunday and easter monday will not be holyday';
...
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION plvdate.default_holydays(text) IS 'Load calendar for some nations';
...
CREATE OR REPLACE FUNCTION plvstr.swap(str text, replace text, start int, length int)
RETURNS text
...
CREATE OR REPLACE FUNCTION plvstr.betwn(str text, start int, _end int, inclusive bool)
RETURNS text
...
CREATE OR REPLACE FUNCTION plvstr.betwn(str text, start int, _end int)
RETURNS text
...
CREATE OR REPLACE FUNCTION plvstr.betwn(str text, start text, _end text, startnth int, endnth int, inclusive bool, gotoend bool)
RETURNS text
...
CREATE OR REPLACE FUNCTION plvstr.betwn(str text, start text, _end text, startnth int, endnth int)
RETURNS text
...
GRANT EXECUTE ON FUNCTION dbms_alert.defered_signal() TO POSTGRES;
...
RETURNS text
AS $$SELECT plvsubst.string($1, $2, $3, NULL); $$ 
LANGUAGE SQL STRICT STABLE;
COMMENT ON FUNCTION plvsubst.string(text, text, text) IS 'Scans a string for all instances of the substitution keyword and replace it with the next value in the substitution values list';
