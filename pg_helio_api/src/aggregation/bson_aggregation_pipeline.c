...
static Query * HandleSample(const bson_value_t *existingValue, Query *query,
							AggregationPipelineBuildContext *context);
static Query * HandleSearch(const bson_value_t *existingValue, Query *query,
							AggregationPipelineBuildContext *context);
							  ...
							  pgbson *searchParamPgbson)
{
	if (searchParamPgbson != NULL)
	{
		...
		List *args = list_make2(CreateDocumentVar(), searchParam);
			 ...
			 AggregationPipelineBuildContext *context)
{
		...
		!IsBsonValueEmptyDocument(&filterBson))
	{
		...
		query = HandleMatch(&filterBson, query, context);
	}
	...
	sortBy->location = -1;
	sortBy->sortby_dir = SORTBY_DEFAULT; /* reset later */
	sortBy->node = (Node *) processedSortExpr;

	bool resjunk = true;
	TargetEntry *sortEntry = makeTargetEntry((Expr *) processedSortExpr,
											 (AttrNumber) parseState->p_next_resno++,
											 "?sort?",
											 resjunk);
	query->targetList = lappend(query->targetList, sortEntry);
	...
	/* Create WHERE bson_extract_vector(document, path) IS NOT NULL and add it to the WHERE */

	Assert(IsA(processedSortExpr, OpExpr));
	FuncExpr *castFunctionExpr = (FuncExpr *) linitial(
		((OpExpr *) processedSortExpr)->args);
