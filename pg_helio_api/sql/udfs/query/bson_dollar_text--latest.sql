...
CREATE OR REPLACE FUNCTION helio_api_internal.bson_dollar_text(__CORE_SCHEMA__.bson, __CORE_SCHEMA__.bson)
 RETURNS bool
 LANGUAGE c
...
CREATE OR REPLACE FUNCTION helio_api_internal.bson_text_tsquery(__CORE_SCHEMA__.bson, tsquery)
 RETURNS bool
 LANGUAGE c
 IMMUTABLE PARALLEL SAFE STRICT
AS 'MODULE_PATHNAME', $function$bson_dollar_text$function$;
...
CREATE OR REPLACE FUNCTION helio_api_internal.bson_text_meta_qual(__CORE_SCHEMA__.bson, tsquery, bytea, bool)
 RETURNS bool
 LANGUAGE c
 IMMUTABLE PARALLEL SAFE STRICT
...
DROP FUNCTION IF EXISTS helio_api_internal.bson_query_to_tsquery;

-- This is a diagnostic query used to test translation of $text search clauses to TSQuery in Postgres.
CREATE OR REPLACE FUNCTION helio_api_internal.bson_query_to_tsquery(query __CORE_SCHEMA__.bson, textSearch text default NULL)
 RETURNS tsquery
 LANGUAGE c
