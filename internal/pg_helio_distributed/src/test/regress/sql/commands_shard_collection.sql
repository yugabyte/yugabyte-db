...
SET helio_api.next_collection_id TO 24000;
SET helio_api.next_collection_index_id TO 24000;
...
SELECT FORMAT('helio_data.documents_%s', :reshard_collection_id) AS db_shard_data_table_name \gset

-- make plans (more) deterministic
VACUUM (ANALYZE) :db_shard_data_table_name;

SELECT * FROM helio_distributed_test_helpers.get_collection_indexes('db', 'reshard') ORDER BY index_id;
...
SET LOCAL seq_page_cost TO 9999999;
EXPLAIN (COSTS OFF)
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"value":11}' ORDER BY object_id;
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"value":11}' ORDER BY object_id;
ROLLBACK;
...
SET LOCAL seq_page_cost TO 9999999;
EXPLAIN (COSTS OFF)
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"value":{"$numberLong" : "134311"}}' ORDER BY object_id;
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"value":{"$numberLong" : "134311"}}' ORDER BY object_id;
ROLLBACK;
...
SET LOCAL seq_page_cost TO 9999999;
EXPLAIN (COSTS OFF)
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"value":{"$numberInt" : "134311"}}' ORDER BY object_id;
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"value":{"$numberInt" : "134311"}}' ORDER BY object_id;
ROLLBACK;
...
SET LOCAL seq_page_cost TO 9999999;
EXPLAIN (COSTS OFF)
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"value":15}' ORDER BY object_id;
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"value":15}' ORDER BY object_id;
ROLLBACK;
...
SELECT collection_id AS collection_id, shard_key::text AS shard_key FROM helio_api_catalog.collections
  WHERE collection_name = 'reshard' AND database_name = 'db' \gset

DO $$
DECLARE
  v_collection helio_api_catalog.collections;
  v_compute_shard_key bigint;
BEGIN
  SELECT * INTO v_collection FROM helio_api_catalog.collections WHERE database_name = 'db' AND collection_name = 'reshard';
  SELECT helio_api_internal.get_shard_key_value(v_collection.shard_key, v_collection.collection_id, '{"value":11,"_id":"1"}') INTO v_compute_shard_key;
...
SET LOCAL seq_page_cost TO 9999999;
EXPLAIN (COSTS OFF)
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"a.b":22}' ORDER BY object_id;
SELECT document FROM helio_api.collection('db','reshard') WHERE document @@ '{"a.b":22}' ORDER BY object_id;
ROLLBACK;
...
-- create some indexes
SELECT helio_api_internal.create_indexes_non_concurrently('db', helio_distributed_test_helpers.generate_create_index_arg('reshardwithindexes2', 'idx1', '{ "value": 1 }'), true);
SELECT helio_api_internal.create_indexes_non_concurrently('db', helio_distributed_test_helpers.generate_create_index_arg('reshardwithindexes2', 'idx2', '{ "otherValue": 1 }'), true);
