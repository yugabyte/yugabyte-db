...
BEGIN
  ...
  IF create_index_response.ok THEN
    ...
    IF indexRequest IS NOT NULL THEN
      LOOP
          SELECT * INTO check_build_index_status FROM helio_api_internal.check_build_index_status(create_index_response.requests);
...
AS 'pg_helio_api', $$gin_bson_get_single_path_generated_terms$$;

CREATE OR REPLACE FUNCTION helio_distributed_test_helpers.gin_bson_get_wildcard_project_generated_terms(
    document helio_core.bson,
    pathSpec text,
    isExclusion bool,
    includeId bool,
