...
SELECT helio_api_internal.create_indexes_non_concurrently(p_arg=>'{"createIndexes": "collection_3", "indexes": [{"key": {"a": 1}, "name": "idx_1", "unique": true }]}', p_database_name=>'db', p_skip_check_collection_create=>true);
CALL helio_api.drop_indexes('db', '{"dropIndexes": "collection_3", "index": ["idx_1"]}');

-- test drop_collection

SELECT helio_api_internal.create_indexes_non_concurrently('db', '{"createIndexes": "drop_collection_test", "indexes": [{"key": {"a": 1}, "name": "my_idx_1"}]}', true);
