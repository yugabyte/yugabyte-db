...
SELECT helio_api_internal.create_indexes_non_concurrently('collection_management', '{"createIndexes": "drop_collection_test", "indexes": [{"key": {"a": 1}, "name": "my_idx_1"}]}', true);
...
PREPARE qcountNoneExistent(text, text, bson, text) AS WITH "stage0" as ( SELECT document FROM helio_api.collection($1, $2) WHERE document OPERATOR(helio_api_catalog.@@) $3 ) ,
            "stage1" as ( SELECT helio_core.bson_repath_and_build($4, BSONSUM('{ "": 1 }'::bson)) as document FROM "stage0" ) SELECT * FROM "stage1";
