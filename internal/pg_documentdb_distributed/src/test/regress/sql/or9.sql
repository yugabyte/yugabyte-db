-- Based on or9.js
CREATE SCHEMA or9;
SET search_path TO helio_core,helio_api,helio_api_catalog,helio_api_internal,public,or9;
...
SELECT helio_distributed_test_helpers.drop_primary_key('db', 'or9');
...
SELECT helio_api_internal.create_indexes_non_concurrently('db', helio_distributed_test_helpers.generate_create_index_arg('or9', 'index_1', '{"a.$**": 1}'), true);
SELECT helio_api_internal.create_indexes_non_concurrently('db', helio_distributed_test_helpers.generate_create_index_arg('or9', 'index_2', '{"b.$**": 1}'), true);
...
BEGIN
	...
	EXECUTE format('TRUNCATE helio_data.documents_%s', v_collection_id);
END
$$;
