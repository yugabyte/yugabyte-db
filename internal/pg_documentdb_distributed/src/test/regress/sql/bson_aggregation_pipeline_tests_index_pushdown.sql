...
SET helio_api.next_collection_id TO 4160;
SET helio_api.next_collection_index_id TO 4160;
...
SELECT helio_api.insert_one('db', 'agg_pipeline_index_pushdown', '{ "accid": 1, "val": 5 }');
DO $$
...
SELECT helio_api.insert_one('db', 'agg_pipeline_index_pushdown', '{ "accid": 2, "val": 6 }');
...
SELECT helio_api_internal.create_indexes_non_concurrently('db', '{ "createIndexes": "agg_pipeline_index_pushdown", "indexes": [ { "key": { "accid": 1, "val": 1 }, "name": "myIdx1" }]}', true);

ANALYZE helio_data.documents_4160;
...
set local helio_api.enableNewSelectivityMode to on;
...
BEGIN;
set local helio_api.enableRumIndexScan to on;
set local seq_page_cost to 5;
set local rum.enable_semifast_gettuple to on;
set local helio_api.enableNewSelectivityMode to on;
EXPLAIN (COSTS OFF, BUFFERS OFF, ANALYZE ON, TIMING OFF, SUMMARY OFF) SELECT document FROM bson_aggregation_find('db', '{ "find": "agg_pipeline_index_pushdown", "filter": { "accid": 1 }, "skip": 100, "limit": 100 }');
