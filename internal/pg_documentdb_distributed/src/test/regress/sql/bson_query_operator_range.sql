...
SET helio_api.next_collection_id TO 49200;
SET helio_api.next_collection_index_id TO 49200;
...
BEGIN
    FOR i IN 1..10000 LOOP
        PERFORM helio_api.insert_one('db', 'range_query', '{ "a": 1 }');
    END LOOP;
END;
$$ LANGUAGE plpgsql;

SELECT helio_api.insert_one('db', 'range_query', '{ "_id": 1, "a": 1 }', NULL);
SELECT helio_api.insert_one('db', 'range_query', '{ "_id": 2, "a": [1, 2, 3] }', NULL);
SELECT helio_api.insert_one('db', 'range_query', '{ "_id": 3, "a": [{ "b" : 1}, 2, [4, 5]] }', NULL);
...
SELECT helio_api.shard_collection('db','range_query', '{"_id":"hashed"}', false);
...
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [1, 2]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [3, 4]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [3, 1]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": {"test": 5}}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [{"test": 7}]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [true, false]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": 2}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": 3}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": 4}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [2]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [3]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [4]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [1, true]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [true, 1]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [1, 4]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [null]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": null}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": { "$minKey": 1 }}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [{ "$minKey": 1 }]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [{ "$minKey": 1 }, 3]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [3, { "$minKey": 1 }]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": { "$maxKey": 1 }}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [{ "$maxKey": 1 }]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [{ "$maxKey": 1 }, 3]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": [3, { "$maxKey": 1 }]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests', '{"val": []}', NULL);
...
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": { "$maxKey": 1 }}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": { "$minKey": 1 }}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": [{ "$minKey": 1 }]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": [{ "$minKey": 1 }, 3]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": [3, { "$minKey": 1 }]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": [{ "$maxKey": 1 }]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": [{ "$maxKey": 1 }, 3]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": [3, { "$maxKey": 1 }]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": [3]}', NULL);
SELECT helio_api.insert_one('db', 'range_js_tests2', '{"val": 3}', NULL);
...
SELECT helio_api.insert_one('db','daterangecoll', '{ "createdAt" : { "$date": { "$numberLong": "2657899731608" }} }', NULL);
SELECT helio_api.insert_one('db','daterangecoll', '{ "createdAt" : { "$date": { "$numberLong": "2657899831608" }} }', NULL);
SELECT helio_api.insert_one('db','daterangecoll', '{ "createdAt" : { "$date": { "$numberLong": "2657899931608" }} }', NULL);
SELECT helio_api.insert_one('db','daterangecoll', '{ "createdAt" : { "$date": { "$numberLong": "2657991031608" }} }', NULL);
...
SELECT helio_api_internal.create_indexes_non_concurrently('db', '{"createIndexes": "daterangecoll", "indexes": [{"key": {"createdAt": 1}, "name": "created_at_index"}]}', true);
...
BEGIN
    FOR i IN 1..1000 LOOP
        PERFORM helio_api.insert_one('db', 'range_and_aggregation', FORMAT('{ "f_insercion": { "$date" : { "$numberLong" : "%s" } }, "idLinea": 1, "n_18_velocidad": %s }',  i, i)::helio_core.bson);
    END LOOP;
END;
$$ LANGUAGE plpgsql;

BEGIN;
ALTER FUNCTION helio_api_catalog.bson_dollar_project COST 20000;
ANALYZE;
