...
-- Insert data
SELECT helio_api.insert_one('db','sample',' { "_id" : 1, "item" : "almonds", "price" : 1, "quantity" : 1 }', NULL);
SELECT helio_api.insert_one('db','sample',' { "_id" : 2, "item" : "almonds", "price" : 1, "quantity" : 1 }', NULL);
SELECT helio_api.insert_one('db','sample',' { "_id" : 3, "item" : "almonds", "price" : 1, "quantity" : 1 }', NULL);
SELECT helio_api.insert_one('db','sample',' { "_id" : 4, "item" : "almonds", "price" : 1, "quantity" : 1 }', NULL);
SELECT helio_api.insert_one('db','sample',' { "_id" : 5, "item" : "almonds", "price" : 1, "quantity" : 1 }', NULL);
SELECT helio_api.insert_one('db','sample',' { "_id" : 6, "item" : "almonds", "price" : 1, "quantity" : 1 }', NULL);
...
SELECT * FROM helio_api.aggregate_cursor_first_page(database => 'db', commandSpec => '{ "aggregate": "sample", "pipeline": [ { "$sample": { "size": 3 } }, { "$project": { "_id": 0 } } ], "cursor": { "batchSize": 1 } }', cursorId => 4294967294);

-- Shard orders collection on item 
SELECT helio_api.shard_collection('db','sample', '{"item":"hashed"}', false);
