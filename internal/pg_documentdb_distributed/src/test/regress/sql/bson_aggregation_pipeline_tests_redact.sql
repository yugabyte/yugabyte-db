...
SET helio_api.next_collection_id TO 51500;
SET helio_api.next_collection_index_id TO 51500;

/* Insert data, normal case*/
SELECT helio_api.insert_one('db','redactTest1','{ "_id": 1, "level": "public", "content": "content 1", "details": { "level": "public", "value": "content 1.1", "moreDetails": { "level": "restricted", "info": "content 1.1.1" } } }', NULL);
SELECT helio_api.insert_one('db','redactTest1','{ "_id": 2, "level": "restricted", "content": "content 2", "details": { "level": "public", "value": "content 2.1", "moreDetails": { "level": "restricted", "info": "content 2.1.1" } } }', NULL);
SELECT helio_api.insert_one('db','redactTest1','{ "_id": 3, "level": "public", "content": "content 3", "details": { "level": "restricted", "value": "content 3.1", "moreDetails": { "level": "public", "info": "content 3.1.1" } } }', NULL);
SELECT helio_api.insert_one('db','redactTest1','{ "_id": 4, "content": "content 4", "details": { "level": "public", "value": "content 4.1" } }', NULL);
SELECT helio_api.insert_one('db','redactTest1','{ "_id": 5, "level": "public", "content": "content 5", "details": { "level": "public", "value": "content 5.1", "moreDetails": [{ "level": "restricted", "info": "content 5.1.1" }, { "level": "public", "info": "content 5.1.2" }] } }', NULL);
...
SELECT helio_api.insert_one('db','redactTest2','{ "_id": 1, "level": 1, "a": { "level": 2, "b": { "level": 4, "c": { "level": 1, "d": 8 } } }, "e": 20 }', NULL);
SELECT helio_api.insert_one('db','redactTest2','{ "_id": 2, "level": 4, "a": { "level": 3, "b": [{ "level": 1, "e": 4 }] } }', NULL);
SELECT helio_api.insert_one('db','redactTest2','{ "_id": 3, "level": 2, "a": { "b": { "level": 3, "n": 12 } } }', NULL);
SELECT helio_api.insert_one('db','redactTest2','{ "_id": 4, "level": 3, "a": [{ "level": 5, "b": 20 }] }', NULL);
SELECT helio_api.insert_one('db','redactTest2','{ "_id": 5, "level": 3, "b": { "level": 3, "d": [ { "level": 1, "e": 4 }, { "level": 5, "g": 9 }, [{ "level": 1, "r": 11 }, 52, 42, 75, { "level": 5, "s": 3 }]] } }', NULL);
...
SELECT helio_api.insert_one('db','redactTest3','{ "_id": 1, "level": 1, "a": { "level": 1, "b": 1 }, "c": { "level": 1, "d": 1 }}', NULL);
SELECT helio_api.insert_one('db','redactTest3','{ "_id": 2, "level": 2, "a": { "level": 2, "b": 2 }, "c": { "level": 2, "d": 2 }}', NULL);
SELECT helio_api.insert_one('db','redactTest3','{ "_id": 3, "level": 3, "a": { "level": 3, "b": 3 }, "c": { "level": 3, "d": 3 }}', NULL);
...
/*switch case*/
SELECT helio_api.insert_one('db','redactTestSwitch','{ "_id": 1, "title": "Document 1", "classification": "confidential", "content": "This is confidential content.", "metadata": { "author": "Alice", "revision": 3 } }', NULL);
SELECT helio_api.insert_one('db','redactTestSwitch','{ "_id": 2, "title": "Document 2", "classification": "public", "content": "This is public content.", "metadata": { "author": "Bob", "revision": 1 } }', NULL);
SELECT helio_api.insert_one('db','redactTestSwitch','{ "_id": 3, "title": "Document 3", "classification": "restricted", "content": "This content is restricted.", "metadata": { "author": "Charlie", "revision": 2 } }', NULL);
...
SELECT helio_api.insert_one('db','redactUsers','{ "_id": 1, "name": "Alice"}', NULL);
SELECT helio_api.insert_one('db','redactUsers','{ "_id": 2, "name": "Bob"}', NULL);
SELECT helio_api.insert_one('db','redactUsers','{ "_id": 3, "name": "Charlie"}', NULL);

SELECT helio_api.insert_one('db','redactOrders','{ "_id": 1, "user_id": 1, "order_amount":150, "status": "completed"}', NULL);
SELECT helio_api.insert_one('db','redactOrders','{ "_id": 2, "user_id": 1, "order_amount":200, "status": "pending"}', NULL);
SELECT helio_api.insert_one('db','redactOrders','{ "_id": 3, "user_id": 2, "order_amount":500, "status": "completed"}', NULL);
SELECT helio_api.insert_one('db','redactOrders','{ "_id": 4, "user_id": 2, "order_amount":50, "status": "completed"}', NULL);
SELECT helio_api.insert_one('db','redactOrders','{ "_id": 5, "user_id": 3, "order_amount":300, "status": "pending"}', NULL);
...
SELECT helio_api.insert_one('db', 'facetUsers', '{ "_id": 1, "name": "Alice", "email": "alice@example.com", "age": 25 }', NULL);
SELECT helio_api.insert_one('db', 'facetUsers', '{ "_id": 2, "name": "Bob", "email": "bob@example.com", "age": 25 }', NULL);
SELECT helio_api.insert_one('db', 'facetUsers', '{ "_id": 3, "name": "Charlie", "email": "charlie@example.com", "age": 30 }', NULL);
...
SELECT helio_api.insert_one('db', 'graphLookupEmployee', '{ "_id": 1, "employee_id": 1, "name": "Alice", "manager_id": null, "email": "alice@example.com" }', NULL);
SELECT helio_api.insert_one('db', 'graphLookupEmployee', '{ "_id": 2, "employee_id": 2, "name": "Bob", "manager_id": 1, "email": "bob@example.com" }', NULL);
SELECT helio_api.insert_one('db', 'graphLookupEmployee', '{ "_id": 3, "employee_id": 3, "name": "Charlie", "manager_id": 1, "email": "charlie@example.com" }', NULL);
SELECT helio_api.insert_one('db', 'graphLookupEmployee', '{ "_id": 4, "employee_id": 4, "name": "David", "manager_id": 2, "email": "david@example.com" }', NULL);
SELECT helio_api.insert_one('db', 'graphLookupEmployee', '{ "_id": 5, "employee_id": 5, "name": "Eve", "manager_id": 2, "email": "eve@example.com" }', NULL);
...
SELECT helio_api.insert_one('db', 'unionWithUsers', '{ "_id": 1, "name": "Alice", "role": "admin", "address": { "city": "New York", "zip": "10001" } }', NULL);
SELECT helio_api.insert_one('db', 'unionWithUsers', '{ "_id": 2, "name": "Bob", "role": "user", "address": { "city": "Los Angeles", "zip": "90001" } }', NULL);
SELECT helio_api.insert_one('db', 'unionWithEmployees', '{ "_id": 101, "name": "Charlie", "role": "admin", "department": "HR", "address": { "city": "San Francisco", "zip": "94101" } }', NULL);
SELECT helio_api.insert_one('db', 'unionWithEmployees', '{ "_id": 102, "name": "David", "role": "user", "department": "IT", "address": { "city": "Chicago", "zip": "60601" } }', NULL);
...
/*EXPLAIN*/
EXPLAIN (VERBOSE ON, COSTS OFF) SELECT document FROM helio_api_catalog.bson_aggregation_pipeline('db', '{ "aggregate": "redactTest3", "pipeline": [ { "$redact": "$$KEEP" } ] }');
EXPLAIN (VERBOSE ON, COSTS OFF) SELECT document FROM helio_api_catalog.bson_aggregation_pipeline('db', '{ "aggregate": "redactTest3", "pipeline": [ { "$redact": { "$cond": {"if": {"$eq": ["$level", 1]}, "then": "$$KEEP", "else": "$$PRUNE"} } } ] }');
