SET citus.next_shard_id TO 540000;
SET helio_api.next_collection_id TO 5400;
SET helio_api.next_collection_index_id TO 5400;
...
PERFORM helio_api.insert_one('db', 'findAndModify', FORMAT('{"_id": %s, "a": "%s", "b": "%s", "c": "%s_1"}', r, (select string_agg( substr(md5(random()::text), 1, 280), '') FROM generate_series(1, 50)), (select string_agg( substr(md5(random()::text), 1, 280), '') FROM generate_series(1, 80)),r)::helio_core.bson);
end loop;
end;
$$;

SELECT helio_api.find_and_modify('db', '{"findAndModify": "findAndModify", "remove": false, "query": {"_id": 549}, "upsert": false, "update": {"$set": {"c": "foo"}}, "new": false, "fields": { "_id": 1 }}');
SELECT helio_api.find_and_modify('db', '{"findAndModify": "findAndModify", "remove": false, "query": {"_id": 549}, "upsert": false, "update": {"$set": {"c": "foo"}}, "new": false, "fields": { "_id": 1 }}');