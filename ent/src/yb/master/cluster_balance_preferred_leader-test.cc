...
class TestLoadBalancerPreferredLeader : public TestLoadBalancerBase<ClusterLoadBalancerMocked> {
 ...
 private:
  ...
  std::shared_ptr<ClusterLoadBalancerMocked> cb;

 public:
  void TestAlgorithm() { TestLoadBalancerBase<ClusterLoadBalancerMocked>::TestAlgorithm(); }
  ...
  void TestAlreadyBalancedMultiAffinitizedLeaders() {
  ...
  void TestBalancingOneMultiAffinitizedLeader() {
    TSDescriptorVector ts_descs(Generate5NodeTsDesc());
    PrepareTestState(ts_descs);
  ...
  void TestBalancingMultiPrimaryAffinitizedLeader() {
    TSDescriptorVector ts_descs(Generate5NodeTsDesc());
    PrepareTestState(ts_descs);
  ...
  void TestBalancingMultiSecondaryAffinitizedLeader() {
    TSDescriptorVector ts_descs(Generate5NodeTsDesc());
    PrepareTestState(ts_descs);
...
TEST(TestLoadBalancerPreferredLeader, TestLoadBalancerAlgorithm) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())->TestAlgorithm();
}

TEST(TestLoadBalancerPreferredLeader, TestBalancingOneAffinitizedLeader) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
      ->TestBalancingOneAffinitizedLeader();
}

TEST(TestLoadBalancerPreferredLeader, TestBalancingTwoAffinitizedLeaders) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
      ->TestBalancingTwoAffinitizedLeaders();
}

TEST(TestLoadBalancerPreferredLeader, TestReadOnlyLoadBalancing) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())->TestReadOnlyLoadBalancing();
}

TEST(TestLoadBalancerPreferredLeader, TestLeaderBalancingWithReadOnly) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
      ->TestLeaderBalancingWithReadOnly();
}

TEST(TestLoadBalancerPreferredLeader, TestAlreadyBalancedMultiAffinitizedLeaders) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
      ->TestAlreadyBalancedMultiAffinitizedLeaders();
}

TEST(TestLoadBalancerPreferredLeader, TestBalancingOneMultiAffinitizedLeader) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
      ->TestBalancingOneMultiAffinitizedLeader();
}

TEST(TestLoadBalancerPreferredLeader, TestBalancingMultiPrimaryAffinitizedLeader) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
      ->TestBalancingMultiPrimaryAffinitizedLeader();
}

TEST(TestLoadBalancerPreferredLeader, TestBalancingMultiSecondaryAffinitizedLeader) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
      ->TestBalancingMultiSecondaryAffinitizedLeader();
}

TEST(TestLoadBalancerPreferredLeader, TestBalancingMultiNodeAffinitizedLeader) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
      ->TestBalancingMultiNodeAffinitizedLeader();
}
TEST(TestLoadBalancerPreferredLeader, TestBalancingWithLeaderBlacklist) {
  TestLoadBalancerPreferredLeader::CreateInstance(CURRENT_TEST_NAME())
