...
Status CatalogManager::UpdateCDCStream(const UpdateCDCStreamRequestPB *req,
  ...
  RETURN_NOT_OK(sys_catalog_->UpdateItem(stream.get(), leader_ready_term()));
...
Status CatalogManager::DeleteUniverseReplicationUnlocked(
  ...
  RETURN_NOT_OK_PREPEND(
      sys_catalog_->DeleteItem(universe.get(), leader_ready_term()),
...
Status CatalogManager::RenameUniverseReplication(
...

  {
...

    {
      ...
      RETURN_NOT_OK(w->MutateItem(universe.get(), QLWriteRequestPB::QL_STMT_DELETE));
      RETURN_NOT_OK(w->MutateItem(new_ri.get(), QLWriteRequestPB::QL_STMT_UPDATE));
      RETURN_NOT_OK(w->MutateItem(cluster_config_.get(), QLWriteRequestPB::QL_STMT_UPDATE));
