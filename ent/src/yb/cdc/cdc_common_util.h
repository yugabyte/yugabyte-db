...
void AddProtoRecordColumnToMap(const ColumnSchema &col_schema,
                               const docdb::PrimitiveValue &col,
                               cdc::KeyValuePairPB *kv_pair,
                               bool is_proto_record,
                               DatumMessagePB *cdc_datum_message = nullptr);

Result<bool> SetCommittedRecordIndexForReplicateMsg(const consensus::ReplicateMsgPtr& msg,
                                                    size_t index,
                                                    const TxnStatusMap& txn_map,
                                                    ReplicateIntents replicate_intents,
                                                    std::vector<RecordTimeIndex>* records);

Result<std::vector<RecordTimeIndex>> GetCommittedRecordIndexes(const consensus::ReplicateMsgs &msgs,
                                                               const TxnStatusMap &txn_map,
                                                               ReplicateIntents replicate_intents,
                                                               OpId *checkpoint);

Result<consensus::ReplicateMsgs> FilterAndSortWrites(const consensus::ReplicateMsgs &msgs,
                                                     const TxnStatusMap &txn_map,
                                                     ReplicateIntents replicate_intents,
                                                     OpId *checkpoint);

Result<TransactionStatusResult> GetTransactionStatus(const TransactionId &txn_id,
