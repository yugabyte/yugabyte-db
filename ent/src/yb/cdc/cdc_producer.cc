    ...
    if (prev_key != primary_key) {
      ...
      if (batch.has_transaction()) {
        if (!replicate_intents) {
          ...
          transaction_state->set_transaction_id(batch.transaction().transaction_id());
          transaction_state->add_tablets(tablet_peer->tablet_id());
          transaction_state->set_external_status_tablet_id(
              batch.transaction().status_tablet());
        }
      }
    }
...
Status PopulateTransactionRecord(const ReplicateMsgPtr& msg,
  ...
  txn_state->set_transaction_id(transaction_state.transaction_id());
