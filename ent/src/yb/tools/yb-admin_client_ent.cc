...
Status ClusterAdminClient::ListSnapshots(const ListSnapshotsFlags& flags) {
  ...
  if (!json) {
    if (resp.snapshots_size()) {
      cout << RightPadToUuidWidth("Snapshot UUID") << kColumnSep << ightPadToWidth("State", kSmallColWidth) << kColumnSep << "Creation Time" << endl;
    } else {
  ...
  for (SnapshotInfoPB& snapshot : *resp.mutable_snapshots()) {
    ...
    if (json) {
      ...
      cout << SnapshotIdToString(snapshot.id()) << kColumnSep << snapshot.entry().state() << 
        kColumnSep << HybridTimeToString(HybridTime::FromPB(snapshot.entry().snapshot_hybrid_time())) << endl;
    }
