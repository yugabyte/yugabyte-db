...
TEST_F_EX(YBBackupTest,
  ...
  // 0x7ffe-0x7fff | 98188 | 0x7ffe | 2                     | 3                   | Y
  // 0x7fff-0xaaaa | 4     | 0x9eaf | 3                     | 3                   | N
  ...
  ASSERT_NO_FATALS(InsertOneRow(Format("INSERT INTO $0 VALUES (98188)", table_name)));
  string select_query = Format("SELECT k, to_hex(yb_hash_code(k)) AS hash FROM $0"
                               " WHERE k IN (1, 2, 4, 6, 9, 27, 98188) ORDER BY hash",
                               table_name);
  ...
  // - 0x7fff-0xaaaa
  // - 0xaaaa-
  // If commit f9c8d2ff443c06d41074c7115b85d128779c4597 is backported to 2.8, the split point 0x7fff
  // will instead be 0x9c76.
  ...
  ASSERT_TRUE(ASSERT_RESULT(CheckPartitions(tablets, {"\x55\x55", "\x7f\xff", "\xaa\xaa"})));
