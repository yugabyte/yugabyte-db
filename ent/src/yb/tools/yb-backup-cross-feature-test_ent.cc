...
TEST_F(YBBackupTest, YB_DISABLE_TEST_IN_TSAN(DeleteSnapshotAfterTabletSplitting)) {
  const string table_name = "mytbl";
...
TEST_F_EX(
    YBBackupTest, YB_DISABLE_TEST_IN_TSAN(DeleteSnapshotAfterTabletSplittingAndMasterFailover),
...
TEST_F_EX(YBBackupTest,
  ...
  auto tablets = ASSERT_RESULT(GetTablets(table_name, "pre-backup"));
  LogTabletsInfo(tablets);
  ASSERT_EQ(tablets.size(), 3);
...
TEST_F_EX(YBBackupTest,
  ...
  tablets = ASSERT_RESULT(GetTablets(index_name, "pre-backup"));
  LogTabletsInfo(tablets);
  ASSERT_EQ(tablets.size(), 3);
  // 'S' represents the KeyEntryType for String.
...
TEST_F_EX(YBBackupTest,
  ...
  auto tablets = ASSERT_RESULT(GetTablets(index_name, "pre-split"));
  LogTabletsInfo(tablets);
...
class YBBackupCrossColocation : public YBBackupTest,
 ...
 protected:
  void SetUp() override {
    ...
    if (GetParam()) {
      ASSERT_NO_FATALS(RunPsqlCommand(
          Format("CREATE DATABASE $0 WITH COLOCATED=TRUE", backup_db_name), "CREATE DATABASE"));
