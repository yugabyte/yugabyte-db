...
except when setting timestamp on a `YBScanner`.
...
and is exposed through the `AsyncYBClient#getLastPropagatedTimestamp()` and
`AsyncYBClient#setLastPropagatedTimestamp()` methods.
...
in the java client, by setting `YBSession#setExternalConsistencyMode()`
...
the user by means of the `YBScanner::SetReadMode()` method.
...
explicitly by the user through `YBScanner::SetSnapshotMicros()`. Explicitly setting
