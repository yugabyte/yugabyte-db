...
This page includes an overview about the YugabyteDB support for 2-data center (2DC) deployments, supported scenarios, and the life cycle of replication.
...
The terms "cluster" and "universe" will be used interchangeably. For simplicity in the following sections, each YugabyteDB universe is assumed to be deployed in a single data center.

{{< /note >}}

YugabyteDB supports two data center (2DC) deployments, which is built on top of [Change Data Capture (CDC)](../cdc). YugabyteDB support for two data center (2DC) deployments includes the following:
...
YugabyteDB supports the following two scenarios for 2DC deployments.
...
A 2DC one-way sync replication in YugabyteDB follows these four life cycle phases:
