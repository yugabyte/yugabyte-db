...
This section describes how to use triggers when performing data manipulation. 

## Understanding Triggers

In YSQL, a function invoked automatically when an event associated with a table occurs is called a trigger. The event is typically caused by data manipulation during `INSERT`, `UPDATE`, and `DELETE`.
...
While YSQL implements SQL standard, YSQL triggers have the following features that are YSQL-specific:
