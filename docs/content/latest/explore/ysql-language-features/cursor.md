...
You can create a function that returns a reference to a cursor, therefore returning a large result set ready to be processed by the caller of the function.

The following commands, in order, are associated with YSQL cursors:

1. `DECLARE` – Allows you to create and execute the cursor inside a transaction.
2. `OPEN` – Allows you to open the cursor.
3. `FETCH` – Allows you to retrieve rows from an open cursor.
4. `MOVE` – Allows you to move the current position of the cursor.
5. `CLOSE` – Allows you to close the cursor and release memory used during the process.
...
- As a variable of type `refcursor` placed within the YSQL block's declaration section, as follows:
  ...
  ```sql
  DECLARE 
    new_cursor refcursor;
  ```

- As an element bounded to a query, based on the following syntax:

  ```sql
  cursor_name [ [ NO ] SCROLL ] CURSOR [( arguments )] FOR a_query;
  ```

  *cursor_name* represents a variable name for the cursor. By using an optional setting of `SCROLL` or `NO SCROLL`, you can define whether or not the cursor can be scrolled backward. *arguments* represents a  comma-separated list of query parameters that are substituted by values when the cursor is opened. *a_query* can be any `SELECT` statement.
...
DECLARE 
  ...
  employees_cursor_2 CURSOR 
    FOR SELECT * FROM employees;
  employees_cursor_3 CURSOR (section text) 
    FOR SELECT * FROM employees WHERE department = section;
```

`employees_cursor_1` is not bound to a query so it can be used with any query. `employees_cursor_2` encapsulates all rows in the `employees` table and is bound to s specific query, whereas the `employees_cursor_3` encapsulates rows containing employees of a specific department and is bound to a query with parameters.

```sql


BEGIN;
  DECLARE employees_cursor_1 FOR select * from employees;
  FETCH ALL FROM employees_cursor_1;
END;
```
...
```sql
OPEN employees_cursor_1 
  FOR SELECT * FROM employees	WHERE department = section;
```

The following examples show how to open a cursor that was already bound to a query at the time of its declaration. In this case, if there are any query parameters, you have to pass them.
...
```sql
OPEN employees_cursor_3 (section:='Sales');
```

## Using a Cursor

You can use an open cursor with the YSQL data manipulation statements such as `FETCH`, `MOVE`, `UPDATE`, `DELETE`.
...
The next row is fetched by default. You can specify one of the following fetch directions to override `NEXT`:
