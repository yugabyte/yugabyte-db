...
The point-in-time recovery feature allows you to restore the state of your cluster's data (and some types of metadata) from a specific point in time. This can be relative, such as "three hours ago", or an absolute timestamp.
...
1. Create a snapshot schedule for the database from a shell prompt. In this example, the schedule is on the default database yugabyte, one snapshot every minute, and each snapshot is retained for ten minutes.
...
1. In order to restore from an absolute time, get a timestamp from the command prompt. We'll create a table and then restore to this time in order to undo the table creation.
...
1. In order to restore from an absolute time, get a timestamp from the command prompt. We'll delete the table and then restore to this time in order to undo the delete.
...
1. In order to restore from an absolute time, get a timestamp from the command prompt. We'll add a column to the table and then restore to this time in order to undo the added column.
...
1. Restore the snapshot schedule to the timestamp you obtained before you added the column, at a terminal prompt.
...
1. In order to restore from an absolute time, get a timestamp from the command prompt. We'll remove a column from the table and then restore to this time in order to get the column back.
...
1. In order to restore from an absolute time, get a timestamp from the command prompt. We'll create an index on the table and then restore to this time in order to undo the index creation.
