...
This client authentication is managed in YugabyteDB by the YB-TServer's configuration option [`--ysql_hba_conf`](../../../reference/configuration/yb-tserver/#ysql-options), which works similar to the `pg_hb.conf` file in PostgreSQL. The values include records that specify allowed connection types, users, client IP addresses, and the authentication method.
