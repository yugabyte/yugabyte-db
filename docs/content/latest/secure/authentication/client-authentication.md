...
This client authentication is managed in YugabyteDB by the YB-TServer configuration flag [`--ysql_hba_conf`](../../../reference/configuration/yb-tserver/#ysql-flags), which works similar to the `pg_hb.conf` file in PostgreSQL. The values include records that specify allowed connection types, users, client IP addresses, and the authentication method.
...
Records in YugabyteDB's `yb_hba.conf` are autogenerated based on the values included in the `--ysql_hba_conf` flag. For example, starting a YB-TServer with the following `--ysql_hba_conf` flag will enable authorization for all users except `yugabyte`:
...
To display the current values in the `yb_hba.conf` file, run the following `SHOW` statement to get the file location:
...
In the `yb_hba.conf` are the records autogenerated based on your `--ysql_hba_conf` flag. Here is an example.
...
The `--pg_hba_conf` flag is read on start-up and when the main server process receives a SIGHUP signal. If you edit the file on an active system, you will need to signal the postmaster (using `pg_ctl` reload, calling the SQL function `pg_reload_conf()`, or using `kill -HUP`) to make it re-read the file.
