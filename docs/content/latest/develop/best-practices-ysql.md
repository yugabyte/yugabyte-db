...
##### Pros:
This sharding strategy is ideal for massively scalable workloads because it distributes data evenly across all the nodes in the cluster, 
...
##### Cons:
Performing range queries could be inefficient. Examples of range queries are finding rows greater than a lower bound 
...
##### Pros:
This type of sharding allows efficiently querying a range of rows by the primary key values. 
