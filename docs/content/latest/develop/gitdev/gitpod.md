...
The easy way to get started with Gitpod is to simply fork this [source repo](https://github.com/yugabyte/yb-todo-app.git) 
...
{{< note title="Note" >}}
The application uses non-blocking reactive APIs to connect to the YugabyteDB.
{{< /note >}}
...
You need to customize the default universal image to include the YugabyteDB binary. This is done by defining your own `Dockerfile` at `.gitpodcontainer/Dockerfile`. Refer to the [source repo](https://github.com/yugabyte/yb-todo-app.git) for the complete file.
