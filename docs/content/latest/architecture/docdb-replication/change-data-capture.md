...
To consume the events generated by CDC, we will be using Debezium as the connector. It is an open source distributed platform, you just need to point it at the database with the proper stream ID and the rest will be taken care of by Debezium itself. Take a look at the steps how to setup debezium [here](https://github.com/vaibhav-yb/yugabyte-db/blob/cdc-docs/docs/content/latest/cdc/running_cdc_with_debezium.md#running-cdc-with-debezium-locally).

### CDC yb-admin commands

There are a few yb-admin commands for the manipulation of the Change Data streams. They are:
