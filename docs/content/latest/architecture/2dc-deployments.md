...
linkTitle: Two data center deployments
...
YugabyteDB provides synchronous replication of data in clusters dispersed across multiple (three or more) data centers, using the Raft consensus algorithm to enhance high availability and performance. But what if you only need to replicate data between two data centers? Many use cases and applications do not require synchronous replication, but can use asynchronous data replication to meet their requirements.

YugaByte DB supports two data center (2DC) deployments that leverages [change data capture (CDC)](../cdc-architecture) to provide support 
...
## Support for two data center (2DC) deployments
...
For two data center deployments, Yugabyte DB supports the following scenarios.
...
Since this feature builds on top of [Change Data Capture (CDC)](../cdc), the change stream has the following guarantees:
