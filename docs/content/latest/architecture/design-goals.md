...
Split-brain is a computing scenario that represents data and availability inconsistencies which arise when a distributed system incurs a network partition. For YugabyteDB, when a network partition occurs, the remaining ( majority for write acknowledgement purposes ) RAFT group peers elect a new tablet leader. YugabyteDB makes use of a concept known as leader leases which ensures a single tablet leader exists throughout the entire distribured system including when network partitions occur. Leader leases have a default value of two seconds and can be configured to use a different value. Because of YugabyteDB's architecture, the distributed database is not suspectible to the split-brain condition.
=======
## Theorem & Split-Brain
