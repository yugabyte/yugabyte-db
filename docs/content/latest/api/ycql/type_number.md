...
```sql
example> CREATE TABLE sensor_data (sensor_id INT PRIMARY KEY, float_val FLOAT, dbl_val DOUBLE, dec_val DECIMAL);
```

```sql
example> INSERT INTO sensor_data(sensor_id, float_val, dbl_val, dec_val) 
...
```sql
example> UPDATE sensor_data SET float_val = 1, dbl_val = 1, dec_val = 1 WHERE sensor_id = 2;
```

```sql
example> SELECT * FROM sensor_data;
```
