...
- No high availability - During maintenance, one node is always offline. In a cluster with a replication factor (RF) of 3, if one of the remaining nodes goes down, you lose access to the database. For clusters with a higher RF, there is less risk.
