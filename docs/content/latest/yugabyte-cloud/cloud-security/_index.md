...
Yugabyte leverages public cloud provider PAAS and SAAS offerings to deliver the Yugabyte Cloud service. Under the shared responsibility model, the public cloud providers are responsible for providing and protecting the infrastructure (that is, the hardware, software, networking, and facilities) that runs all of the cloud services that Yugabyte provides in Yugabyte Cloud. Configuration related to infrastructure managed and operated by the public cloud providers is generally not available for Yugabyte or Yugabyte customers to audit or modify in Yugabyte Cloud. 
