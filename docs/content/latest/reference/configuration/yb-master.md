...
Default: `-1` (server internally sets default value). For servers with two or less CPU cores, then the default value is `4`. For three or more CPU cores, the default value is `8`. Local cluster installations, created with `yb-ctl` and `yb-docker-ctl`, use a value of `2` for this flag. Clusters created with `yugabyted` use a value of 1.
...
Default: `-1` (server internally sets default value). For servers with two or less CPU cores, then the default value is `2`. For servers with three or four CPU cores, the default value is 4. Beyond 4 cores, the default value is `8`. Local cluster installations, created with `yb-ctl` and `yb-docker-ctl`, use a value of `2` for this flag. Clusters created with `yugabyted` use a value of 1.
