...
- HashiCorp Vault URL is accessible by Yugabyte Platform. 
- Since HashiCorp Vault is accessed via an authentication token mechanism, a token must be created beforehand while creating a key provider with appropriate permissions. 
- HashiCorp Vault needs to be running and always accessible to Yugabyte Platform.

- HashiCorp PKI certificate revocation list (CRL) or CA URLs must be accessible from each node server.
- Appropriate certificates and roles have been created for Yugabyte Platform usage.
...
For details, see [How to configure HashiCorp Vault](#how-to-configure-hashicorp-vault).
...
### How to use HashiCorp Vault-provided certificates to enable TLS
...
6. In the **Secret Token** field, specify the secret tocken for the vault.
7. In the **Role** field, specify the role used for creating certificates.
8. Optionally, provide the secret engine path on which the PKI is mounted. If you do not supply this informatioin, `pki/` will be used.
