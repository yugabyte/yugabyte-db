...
You can enable TLS encryption during universe creation and change these settings after a universe has been created.  
...
- Click **Actions > Edit Security > Encryption in-Transit** to open the **TLS Configuration** dialog and then proceed as follows:
  ...
  - If encryption in-transit is currently disabled for the universe, enable it via the **Encryption in Transit for this Universe** field, as per the following illustration:<br><br>
    ...
    <br><br>Use the expanded **TLS Configuration** dialog shown in the following illustration to change the settings to meet your requirements:

    ![TLS Configuration Expanded](/images/yp/encryption-in-transit/tls-config2.png)

    <br><br>Since modifying certificates requires restart of Master and T-Server processes, you can avoid downtime by using certificate rotation: after enabling node to node encryption, client to node encryption, or both, you can enable rotation of the corresponding certificates by accepting the default value (enabled) for the **Rolling Upgrade** field and specifying the delay between node upgrades.
...
- In the **Expiration Date** field, specify the expiration date of the root certificate. To find this information, execute the `openssl x509 -in <root-crt-file-path> -text -noout` command and note the **Validity Not After** date.
...
- Complete the **TLS Configuration** dialog shown in the following illustration:
  ...
  - Since modifying certificates requires restart of Master and T-Server processes, you can avoid downtime by using certificate rotation: after enabling node to node encryption, client to node encryption, or both, you can enable rotation of the corresponding certificates by accepting the default value (enabled) for the **Rolling Upgrade** field and specifying the delay between node upgrades.
