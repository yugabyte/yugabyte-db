...
3. Complete the following fields:
    ...
    - **KMS Configuration**: (optional) If the backup to be restored is from a universe that has [encryption at rest enabled](../secure/enable-encryption-at-rest), then you need to select the KMS configuration to use to retrieve the universe keys referenced in the metadata file. These retrieved keys are used to build and augment the key registry on the restore universe with the required universe keys. The universe data files are restored as normal afterwards.
