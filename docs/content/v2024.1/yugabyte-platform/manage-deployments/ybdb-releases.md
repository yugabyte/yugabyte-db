...
- Direct download. You can download a release using the following URL format:
    ...
    ```text
    https://software.yugabyte.com/releases/{{< yb-version version="v2024.1">}}/yugabyte-{{< yb-version version="v2024.1" format="build">}}-linux-x86_64.tar.gz
    ```
...
```text.nocopy
yugabyte-{{< yb-version version="v2024.1" format="build">}}-linux-x86_64.tar.gz
```
...
1. Choose whether to upload the release as a file, or download the release via a URL.
    ...
    - If uploading a file, click **Upload** and select the release file. Upload the package in tar.gz format. For example:
        ...
        `yugabyte-{{< yb-version version="v2024.1" format="build">}}-el8-aarch64.tar.gz`
