...
Running the above command is an online operation and doesn't require stopping a running cluster. Note that `upgrade_ysql` is a cluster-level operation; you don't need to run it on every node.
