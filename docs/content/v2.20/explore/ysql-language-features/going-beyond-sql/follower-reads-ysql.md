...
- `yb_follower_read_staleness_ms` sets the exact staleness to read at (the read executes at hybrid time ~`now - yb_follower_read_staleness_ms`). The default value is 30000 (30 seconds). Unlike YCQL follower reads which support a [looser consistency model](../follower-reads-ycql/#consistency-level), YSQL follower reads do not return fresher data than this value, even if available at a local replica, in order to ensure consistency.
