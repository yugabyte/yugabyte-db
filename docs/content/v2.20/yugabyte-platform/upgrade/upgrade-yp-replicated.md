...
YugabyteDB Anywhere will end support for Replicated installation at the end of 2024. You can migrate existing Replicated YugabyteDB Anywhere installations using YBA Installer. See [Migrate from Replicated](../../install-yugabyte-platform/migrate-replicated/).
...
1. Manually obtain and move the YugabyteDB Anywhere airgapped package to the `/opt/yugabyte/releases/<new_version_dir>` directory.
   ...
   ```sh
   wget https://software.yugabyte.com/releases/{{<yb-version version="v2.20">}}/yugaware-{{<yb-version version="v2.20" format="build">}}-linux-x86_64.airgap
   ```
