...
```sh
helm install yugabyte --version {{<yb-version version="v2.20" format="short">}} --namespace yb-demo --name yb-demo --set storage.master.storageClass=<desired storage class>,storage.tserver.storageClass=<desired storage class> --wait
```
