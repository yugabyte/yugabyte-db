...
A simpler explanation is that the time is determined by the [tablet leader](../../../architecture/key-concepts/#tablet-leader) of the table and this is the time used by all followers of the leader. Therefore the UTC timestamp of the underlying server can differ from the current timestamp that is used for a transaction on a particular table.
