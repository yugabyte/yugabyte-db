...
{{<tags/feature/tp>}} [ClockBound](https://github.com/aws/clock-bound) is an open source daemon that allows you to compare timestamps to determine order for events and transactions, independent from the instances’ respective geographic locations. ClockBound provides a strict interval in which the reference time (true time) does exist.
