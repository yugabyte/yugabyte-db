...
1. Once you select a location for the key, you’ll be prompted to enter an optional passphrase which encrypts the private key file on disk.
...
1. After this, you will have a public and private key that you can use to authenticate YugabyteDB VMs.
...
1. Once the deployment creation is complete, you can describe it as shown below.
...
1. First create a resource group, to create a new resource group, select **Resource groups** from the [Azure portal](https://portal.azure.com/).
...
1. Now click **Build your own template in the editor**.
1. Click **Load file** button in specify template section and upload the `yugabyte_deployment.json` file from cloned repo.
1. Click **Save** at the bottom of the window.
1. Now provide the required details.
1. Once details are provided, then check the **Terms and Condition** checkbox and click **Purchase**.
1. Once deployments get completed, you can access the YugabyteDB Admin from the URL you get in the deployment output section.
