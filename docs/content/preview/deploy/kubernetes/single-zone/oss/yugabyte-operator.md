...
The Kubernetes Operator (legacy) for YugabyteDB v0.0.1 is no longer maintained. It is recommended that you use the actively maintained [helm charts](../helm-chart/) for YugabyteDB to install YugabyteDB on Kubernetes. For documentation on using the Kubernetes Operator (legacy) for YugabyteDB, refer to the [GitHub repository](https://github.com/yugabyte/yugabyte-operator).
