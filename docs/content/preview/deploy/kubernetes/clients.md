...
The following example shows a client that uses the YSQL shell ([`ysqlsh`](../../../yugabyte-clients/ysqlsh/)) to connect:
...
When a client uses the `YSQL shell` ([`ysqlsh`](../../../yugabyte-clients/ysqlsh/)) to connect, you can execute the following command to verify the connection:
...
When a client that uses the `YSQL shell` ([`ysqlsh`](../../../yugabyte-clients/ysqlsh/)) to connect, the command to execute specifies the external LoadBalancer IP of the `yb-tserver-service`, as described in [Connect using external clients](../single-zone/oss/helm-chart/#connect-using-external-clients). You can verify the connection via the following command:
