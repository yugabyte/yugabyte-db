...
1. Verify that YugabyteDB is running: you need to ensure that the expected YugabyteDB processes are running on the current node. At a minimum, the YB-TServer process must be running to be able to connect to the node with a client or application.
    ...
    For more information, see [Check processes](/preview/troubleshoot/nodes/check-processes/).

2. Check [cluster-level issues](/preview/troubleshoot/cluster/) and their solutions.

3. Check logs: you should inspect the YugabyteDB logs for additional details on your issue. For more information, see [Inspect logs](/preview/troubleshoot/nodes/check-logs/).
