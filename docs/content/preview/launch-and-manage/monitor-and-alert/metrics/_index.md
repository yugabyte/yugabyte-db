...
| `reset_histograms` | 2.17.1.0 | `true`, `false`. Default is `true` | If set to `false`, percentiles will not be reset each time `/prometheus-metrics` is fetched. Conversely, if set to `true`, percentiles reset with each fetch. |
