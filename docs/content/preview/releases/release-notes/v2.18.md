...
* Balances Out-Of-Memory (OOM) score between the webserver and PG backends to 900, reducing unnecessary webserver kills while prioritizing its termination when it significantly consumes memory. {{<issue 20028>}}
...
* Introduces a GFlag: `enable_tablet_split_of_cdcsdk_streamed_tables` (default:`false`) to toggle automatic tablet splitting for tables within a CDCSDK stream, enhancing user control over replication processes. {{<issue 19482>}}
...
* Introduces a per-database Postgres new object identifier (OID) allocation option, enabling unique OID within each database and supporting increased scalability for multi-node or multi-tenant environments. New config flag `ysql_enable_pg_per_database_oid_allocator` added to toggle between old and new allocator behavior. {{<issue 16130>}}
...
* Enhances debugging capabilities in YSQL Webserver process by introducing two new flags for endpoint access logging and tcmalloc stats printing. Also, values can now be updated via a SIGHUP signal, with `yb_pg_metrics` enabled to handle this signal. {{<issue 20157>}}
