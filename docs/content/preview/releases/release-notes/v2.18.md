...
* Improvements to xCluster replication, including:
  ...
  * Set up replication even if matching tables do not exist on the target universe
...
* View latency histograms for queries using the yb_latency_histogram column in pg_stat_statements. See [yb_latency_histogram column](/stable/explore/query-1-performance/pg-stat-statements/#yb-latency-histogram-column) for more details.
* A new join execution method referenced as Batched Nested Loop, derived from classic Nested Loop Joins.
