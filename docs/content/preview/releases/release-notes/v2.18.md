...
* Mitigates potential security issues by importing upstream PostgreSQL commit from REL_11_STABLE for future support of `ALTER <object> DEPENDS ON EXTENSION`, although it's not currently available in YugabyteDB. {{<issue 14419>}}
...
* Adjusts the 'yugabyted start' command to interpret '0.0.0.0' as '127.0.0.1' in the `advertise_address`, aligning with the IP use in master, YB-TServer, and yugabyted-UI. {{<issue 18580>}}
...
* Introduces a per-database PostgreSQL new object identifier (OID) allocation option, enabling unique OID in each database and supporting increased scalability for multi-node or multi-tenant environments. New flag `ysql_enable_pg_per_database_oid_allocator` added to toggle between old and new allocator behavior. {{<issue 16130>}}
