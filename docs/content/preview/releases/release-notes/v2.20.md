...
* Adjusts YSQLDump to create index NONCONCURRENTLY and circumvent automatic index backfilling during backup-restore, making the backup pipeline faster and more efficient. {{<issue 19457>}}
* Facilitates OpenSSL 3.0+ upgrade by importing the `pgcrypto: Check for error return of px_cipher_decrypt` upstream PostgreSQL commit as a prerequisite. {{<issue 19732>}}
* Enables the upgrade to OpenSSL 3.0+ by importing the upstream PG commit `Disable OpenSSL EVP digest padding in pgcrypto`. {{<issue 19733>}}
* Facilitates OpenSSL 3.0+ upgrade by importing an upstream PostgreSQL commit that adds an alternative output when OpenSSL 3 doesn't load legacy modules. {{<issue 19734>}}
...
* Mitigates CVE-2020-1720 by importing upstream Postgres commit from REL_11_STABLE, enabling support for `ALTER <object>DEPENDS ON EXTENSION` in future for objects like function, procedure, routine, trigger, materialized view, and index. {{<issue 14419>}}
* Modifies the planner to allow ordered index scans with IN conditions on lower columns, benefiting from the YB LSM indexes, which maintains index order despite the conditions. {{<issue 19576>}}
* Sets the Out of Memory (OOM) score of the webserver to the value specified by the `yb_webserver_oom_score_adj` flag (defaults to 900), preventing unnecessary killing of the webserver. {{<issue 20028>}}
* Bans the use of advisory locks in YSQL, providing a meaningful error message when such locks are requested. {{<issue 18954>}}
* Introduces `yb_silence_advisory_locks_not_supported_error` as a temporary solution, allowing users to avoid disruption while transitioning from the use of advisory locks. {{<issue 19974>}}

#### DocDB

* Introduces a new flag `rocksdb_check_sst_file_tail_for_zeros` to enable checks for trailing zeros in SST data files to help identify potential corruption and log necessary information for further analysis. {{<issue 19691>}}
...
* Enables printing of long trace logs by incorporating a continuation marker when a trace is split into multiple LOG(INFO) outputs, circumventing GLog's message length limits, and enhancing trace log readability. {{<issue 19532>}},{{<issue 19808>}}
* Enhances log differentiation by printing the verbose level for VLOG macros, facilitating debugging and analysis. The change replaces `include <glog/>` with `include "yb/util/"` across all files, eliminating duplicate includes. {{<issue 15273>}}
* Upgrades OpenSSL to version 3.0.8 ensuring better security and compatibility as the prior version 1.1.1 reaches its end of life, along with disabling Linuxbrew builds and updating glog for stack unwinding based on the backtrace function. {{<issue 19736>}}
...
* Replaces all instances of `include <glog/>` with `include "yb/util/"` in all files, eliminating duplicate includes and enhancing the distinctness between INFO and VERBOSE logs. {{<issue 15273>}}
* Enables SST write retries on detected corruption with the `rocksdb_max_sst_write_retries` flag, now offering multiple write attempts for both flushes and compactions. {{<issue 19730>}}
* Balances load during tablet creation across all drives, preventing bottlenecks and underutilized drives during remote-bootstrapping, by using the total number of tablets assigned for tie-breaking. {{<issue 19846>}}
* Refines UI by adding a tserver page to display all ongoing Remote Bootstrap sessions for enhanced visibility, and shows the source of bootstrap sessions in the `Last Status` field. {{<issue 19568>}}
...
* Enables the `TEST_SYNC_POINT` macro in release and retail builds, adding a `FLAGS_TEST_enable_sync_points` check to mitigate performance impact, while new `DEBUG_ONLY_TEST_SYNC_POINT` and `DEBUG_ONLY_TEST_SYNC_POINT_CALLBACK` macros maintain existing behaviors in critical code paths. {{<issue 19379>}}
...
* Introduces a new `get_auto_flags_config` yb-admin command, delivering immediate access to the current AutoFlags config, thereby simplifying the debugging of xCluster replication failures. {{<issue 20046>}}
* Enables automatic recovery of index tables affected by a bug, improving performance and disk size management by removing excess tombstones in the SST files. {{<issue 19731>}}
...
* Allows the catalog manager to eliminate erroneous entries from the `cdc_state` table for newly split tablets by reversing the order of operations during the `CleanUpCDCStreamsMetadata` process to prevent race conditions. {{<issue 19746>}}
...
* Eliminates the deprecated `use_initial_sys_catalog_snapshot` gflag in yugabyted, reducing log warning messages. {{<issue 20056>}}

#### Other

* Enhances readability of remotely fetched multi-line trace entries by ensuring proper nesting levels, which eases interpretation of output data. {{<issue 19758>}}
...
* Allows the postprocess script in `pg_regress` to run on alternative expected files alongside `default_expectfile` and `platform_expectfile`, rectifying unintended mismatches. {{<issue 19737>}}
* Allows the release of LWLocks during AbortTransaction, preventing stuck PostgreSQL processes and ensuring successful acquisition of locks in the future. This enhancement particularly aids in preventing deadlocks when creating replication slots with duplicate names. {{<issue 19509>}}
* Introduces a per-database PG new OID allocator mechanism that enhances horizontal scalability by ensuring OID uniqueness within a database in YB environment without risk of collisions across multiple nodes or tenants. This change is safe for upgrades and rollbacks, enabled by a new gflag `ysql_enable_pg_per_database_oid_allocator`. {{<issue 16130>}}
* Prevents postmaster crashes during connection termination cleanup by using the killed process's `ProcStruct` to wait on a lock, maintaining database stability and reliability. {{<issue 18000>}}
* Corrects the distinct iteration process to now recognize between deleted and live keys to prevent missing live rows during distinct queries on tables with dead tuples, enhancing accuracy of data retrieved in these circumstances. {{<issue 19911>}}
* Rectifies a bug in type-checking bound tuple IN conditions involving binary columns like UUID, and assures compatibility from releases 2.17.1 onwards. {{<issue 19753>}}
* Corrects an issue during table rewrite which caused `relforcerowsecurity` setting to be neglected, now ensuring the Row-Level Security (RLS) behavior is properly preserved. {{<issue 19815>}}
* Revamps the system to restart the postmaster if a process holding a spinlock is killed during initialization or if it dies before obtaining a `MyProc`. However, if the process exits with a 0 or 1 code, or if the database is still initializing, the postmaster will not restart to avoid an endless restart loop due to premature connection attempts. Also, reduces the spinlock deadlock detection time by 75%, ensuring better system resilience and improving user experience. {{<issue 18272>}},{{<issue 18265>}}
* Facilitates clean recovery in the rare event a process termination occurs during its initiation or cleanup by restarting the postmaster, negating the need to determine what subset of shared memory items to clean. {{<issue 19945>}}
* Fixes the derivation of `element_typeid` in Batched Nested Loop (BNL) queries, preventing unexpected crashes during plan creation when joining two varchar columns, therefore enhancing query reliability. {{<issue 20003>}}
...
* Introduces two new boolean flags for heightened webserver debugging and failure identification, logging every endpoint access and its argument list, and printing basic tcmalloc stats post-path handler and garbage collection run. {{<issue 20157>}}
* Rectifies an issue causing segmentation faults when the postmaster acquires already owned LWLock, enhancing stability during process cleanup. {{<issue 20166>}}
* Introduces `:13000/memz` and `:13000/webserver-heap-prof` for enhanced insight into webserver memory usage, which print tcmalloc stats and display currents or peak allocations respectively. {{<issue 20157>}}
* Resolves a backend process death in a critical section that used to result in an uncaught odd value of `st_changecount`, creating a CPU consuming infinite loop, by enabling a postmaster restart. {{<issue 20255>}}
...
* Eliminates segmentation faults in the webserver SIGHUP handler at cleanup by using `MyLatch` instead of `MyProc->procLatch`, ensuring safe process life cycles. {{<issue 20309>}}
...
* Enables Masters to decrypt universe key registry correctly when Master turns into a leader following Remote Bootstrap. {{<issue 19513>}}
* Fixes testing failure with 2.20 by ensuring two backports use Jenkins, and aggregates retryable requests mem-tracker metric at table-level for Prometheus by adjusting metric and tablet entities. {{<issue 19301>}}
* Adjusts TransactionParticipant's Add method to prevent the removal of large transactions still applying intents during bootstrap, ensuring data consistency. {{<issue 19359>}}
* Alters `RefinedStream::Connected` to return status instead of calling `context_->Destroy(status)`, preventing a write to freed memory issue. {{<issue 19727>}}
* Allows serving remote bootstrap from a follower peer with log anchoring at the follower's last logged operation ID index, reducing fallback to bootstrapping from the leader and boosting system efficiency under high load. {{<issue 19536>}}
...
* Resolves intermittent index creation failure for empty YCQL tables by validating the `is_running` state instead of checking the index state directly, preventing inaccurate values in old indexes and ensuring `retain_delete_markers` are correctly set to false. {{<issue 19933>}}
...
* Deprecates the `enable_process_lifetime_heap_sampling` flag in favor of directly setting `profiler_sample_freq_bytes` for controlling tcmalloc sampling, enhancing control over sampling process. {{<issue 20236>}}
...
* Eliminates startup failure for the second local RF-1 YugabyteDB instance on Mac, by adding a check for an empty join flag. {{<issue 20018>}}
