...
* Added support for read-only mode for DR replica, currently supported for YSQL tables. {{<badge/ga>}}

* Tablet splitting now can be enabled on tables with CDC configured. {{<badge/ga>}} {{<issue 18479>}}

* Transactional CDC now supports consistent snapshots, ensuring data integrity during replication to a sink. Snapshots, ordered by commit time across all tables and tablets, establish a reliable replication order. {{<badge/ga>}}, {{<issue 19682>}}
...
* Shifts from the test gflag "FLAGS_TEST_enable_db_catalog_version_mode" to the tech preview gflag "FLAGS_ysql_enable_db_catalog_version_mode", enhancing user control over concurrent DDL execution across different databases. {{<issue 12417>}}
