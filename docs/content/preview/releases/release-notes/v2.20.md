...
{{< note title="LTS - Feature availability" >}}
All features in this release are considered to be {{<badge/ga>}} unless marked otherwise.
{{< /note >}}
...
* Support for ALTER COLUMN TYPE with on-disk changes. Expand ALTER TABLE capabilities with a broader range of data type modifications, including changes to the type itself, not just size increments. {{<badge/ga>}}
