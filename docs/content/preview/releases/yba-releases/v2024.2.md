...
The use of [cron to start YB services](/preview/yugabyte-platform/upgrade/prepare-to-upgrade/#node-provisioning) on DB nodes has been deprecated.  Instead, use systemd.  YBA now flags any existing DB nodes using cron with a warning containing remediation instructions.  <!-- IDEA-1805 -->
