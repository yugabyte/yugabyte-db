...
For Kubernetes-based universes using encryption-in-transit, this release adds support for `aws-privateca-issuer`, simplifying [certificate management with AWS Private CA](/preview/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/#including-the-common-name).
...
The use of [cron to start YB services](/preview/yugabyte-platform/upgrade/prepare-to-upgrade/#cron-based-universes) on DB nodes has been deprecated.  Instead, use systemd.  YBA now flags any existing DB nodes using cron with a warning containing remediation instructions.  <!-- IDEA-1805 -->
