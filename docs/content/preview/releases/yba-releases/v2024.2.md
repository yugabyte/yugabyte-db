...
* [Deprecation of cron on DB nodes](/preview/yugabyte-platform/upgrade/prepare-to-upgrade/#node-provisioning). The use of cron to start YB services on DB nodes has been deprecated.  Instead, use systemd.  YBA now flags any existing DB nodes using cron with a warning containing remediation instructions. {{<tags/feature/ga idea="1805">}} <!-- IDEA-1805 -->
