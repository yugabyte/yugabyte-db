...
{{<warning title="Migrate existing YBA installations from Replicated before upgrading YBA">}}
YugabyteDB Anywhere (YBA) will end support for Replicated installation at the end of 2024. You can migrate existing Replicated YugabyteDB Anywhere installations using [YBA Installer](../../../yugabyte-platform/install-yugabyte-platform/install-software/installer/).<br>
Note that you must migrate from Replicated to YBA Installer before [upgrading YBA](../../../yugabyte-platform/upgrade/) to v2024.1. See [Migrate from Replicated](../../../yugabyte-platform/install-yugabyte-platform/migrate-replicated/).
{{</warning>}}
...
You can enable the emulation mode by setting the following flags on a universe:
...
- [Kubernetes Resource Specification](../../../yugabyte-platform/create-deployments/create-universe-multi-zone-kubernetes/#instance-configuration). You can configure CPU and memory needs for YB-TServer and YB-Master pods natively, instead of using the instance types API. {{<badge/ea>}}
...
- [Database registration simplification](../../../yugabyte-platform/manage-deployments/ybdb-releases/). Simplification of process for registering database versions (and their binaries) with YBA. The UX has been dramatically simplified. {{<badge/ea>}}
...
- [Support for OIDC groups in YBA](../../../yugabyte-platform/administer-yugabyte-platform/oidc-authentication/#use-oidc-groups-with-yba-roles). In support of role-based access control (RBAC), ability to assign YBA roles to groups from an OIDC-based Identity Provider. {{<badge/ea>}}
...
- [YBA HA enhancements](../../../yugabyte-platform/administer-yugabyte-platform/high-availability/). Simplified user experience for active/passive YBA deployments.
