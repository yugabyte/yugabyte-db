...
* [Validation of Certificate Authority settings](/stable/yugabyte-platform/security/authentication/oidc-authentication-aad/). When configuring an external Certificate Authority to manage SSL certs used in Encryption in Transit (EIT), YugabyteDB Anywhere now validates the settings up front, thereby improving usability.
