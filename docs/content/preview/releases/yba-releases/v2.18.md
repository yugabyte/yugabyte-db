...
## v2.18.1.0 - June 23, 2023 {#v2.18.1.0}

**Build:** `2.18.1.0-b84`

**Third-party licenses:** [YugabyteDB](https://downloads.yugabyte.com/releases/2.18.0.1/yugabytedb-2.18.1.0-b84-third-party-licenses.html), [YugabyteDB Anywhere](https://downloads.yugabyte.com/releases/2.18.1.0/yugabytedb-anywhere-2.18.1.0-b84-third-party-licenses.html)
...
* Deploy multi-region stretched universes in Kubernetes. In particular, when such universes span 2+ K8s clusters and use Multi-cluster Services (MCS) to address the challenges with cross-k8s-cluster network addressing.
* Perform single-table restores in YCQL (as long as the keyspace was made using a table-by-table backup) in combination with incremental backups. Previously these features did not work together.
* Dynamically change the number of replicas in a universe's read-replica cluster. This is accessible via the Universe -> Edit Universe action
* Assign independent flags to read replica clusters. (By default, read replicas are assigned the same flags as the primary cluster.)
* Enhanced LDAP support. Account administrators can now map roles to Active Directory groups. Members of these groups will inherit role permissions automatically.
...
* [PLAT-9177] Add a separate field for base DN when using group search filter
* [PLAT-9178] [LDAP] [UI] Add a field for asking the base DN for running group search filter query
...
* [PLAT-7252] Decoupling base dn from service account config on YBA LDAP
...
* [PLAT-8948] Change runtime config for boolean fields from dropdown to toggle
* [PLAT-8950] Update bootstrapParams.table logic for create xCluster config
* [PLAT-9019] [PLAT-9210] Add --username and --password flags to Run Sample Apps dialog, Enable feature flag for Add UI support for seperate gflags for RR

### Bug fixes

* [PLAT-7289] Fix the sliding of incremental backup schedule tasks on each run
* [PLAT-7392] [xcluster] Add alert when new tables are added to YSQL dbs in producer xcluster config
...
* [PLAT-8766] Add check for existing repl config to enable txn atomicity checkbox
...
* [PLAT-8922] [PLAT-8923] Fix recommendation content being same in header and body and change deeplink of learnMore tutorials to YBA rather than YBM
...
* [PLAT-9044] [xCluster] Return true on need_boostrap API when a table does not exist on the target unvierse
* [PLAT-9098] Disable KMS API should not allow updating kmsConfigUUID
* [PLAT-9116] Handle colocated parent table name when constructing table indentifier for xCluster
...
* [PLAT-9151] Fix non-rolling gflags uprgade check
