...
* [Nutanix Ready](../../../integrations/nutanix-ahv/). YugabyteDB and YugabyteDB Anywhere has been certified to be compatible as part of the Nutanix Ready AHV program. {{<badge/ga>}}

* Automated [Linux OS patching](../../../yugabyte-platform/manage-deployments/upgrade-nodes/) and upgrades for on-prem. You can now utilize safety-improved YugabyteDB  Anywhere APIs when automating you rolling patch scripts for Linux OSs (typically, for OS security, and minor or major OS upgrades). {{<badge/ga>}} PLAT-10591, PLAT-10988

* REST API versioning and clean-up. REST API simplification and streamlining for most important APIs {{<badge/ga>}} PLAT-11133

* YugabyteDB Anywhere (YBA) and YugabyteDB version compatibility list. You can now independently upgrade YBA without upgrading the YugabyteDB universes. All supported YugabyteDB versions are compatible. {{<badge/ga>}} PLAT-11260

* Improved Auth when using [GCS for backups for Kubernetes universes](../../../yugabyte-platform/back-up-restore-universes/configure-backup-storage/#google-cloud-storage). When taking backups of a Kubernetes universe to Google GCP, allow use of the Google GKE's Service Account for authentication to the Google Storage. {{<badge/ga>}} PLAT-10571

* Asymmetric nodes in each region (API support only). In a stretched cluster, nodes in each region can now have different instance types (that is, different number of CPU cores, memory, and so on.) {{<badge/ga>}}

* [Task Retryability](/stable/yugabyte-platform/manage-deployments/retry-failed-task/) for VM-based universes. You can now address the cause of a failure for a complex operation fails and retry. For example, if adding nodes to a universe fails ( due to a cloud quota limit), you can now address the cloud issue and retry. {{<badge/ea>}} PLAT-6141

* [Backup portability](../../../yugabyte-platform/back-up-restore-universes/restore-universe-data/#advanced-restore-procedure). After sending backups to a cloud object store (For example, S3, GCS, Azure blob storage), you can now copy or transfer those backup sets to other cloud storage buckets and still restore successfully. {{<badge/ea>}} PLAT-10179

* [xCluster DR](../../../yugabyte-platform/back-up-restore-universes/disaster-recovery/). A turnkey solution for Business Continuity and Disaster Recovery to recover from an unplanned outage and planned maintenance. Replication occurs asynchronously (using [xCluster Replication](../../../architecture/docdb-replication/async-replication/)) from one universe to another. {{<badge/ea>}}

* [Edit universe's provider](../../../yugabyte-platform/configure-yugabyte-platform/set-up-cloud-provider/gcp/): add/remove regions, change service account. Expanded support for reconfiguring a universe. You can now add or remove an availability zone/region/or an AMI, and modify CSP credentials when a universe's provider is actively in-use.
