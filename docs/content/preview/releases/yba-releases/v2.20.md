...
1. **Use database version 2.20.1.3 or later**
...
1. **Correctness issue from version 2.20.1.0 to 2.20.1.3**

    YugabyteDB releases in the v2.20 series up to v2.20.1.3 contains a correctness issue for queries using SELECT DISTINCT clause. To encounter this issue, the following conditions needs to be true.

    - The query contains a DISTINCT clause.
    - The query plan contains `BatchNestedLoopJoin`, and the DISTINCT clause is pushed down to the [DocDB](../../../architecture/transactions/).
