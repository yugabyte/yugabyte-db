...
The release notes for YugabyteDB are on [their own page](../../ybdb-releases/v2.20/).
{{< /tip >}}
...
**Third-party licenses:** [YugabyteDB](https://downloads.yugabyte.com/releases/2.20.3.0/yugabytedb-2.20.3.0-67-third-party-licenses.html), [YugabyteDB Anywhere](https://downloads.yugabyte.com/releases/2.20.3.0/yugabytedb-anywhere-2.20.3.0-67-third-party-licenses.html)

### Download

<ul class="nav yb-pills">
 <li>
   <a href="https://downloads.yugabyte.com/releases/2.20.3.0/yba_installer_full-2.20.3.0-67-linux-x86_64.tar.gz">
...
* [Automated OS Patching and upgrades for public cloud](/stable/yugabyte-platform/manage-deployments/upgrade-nodes-csp/). With just a few clicks, you can non-disruptively upgrade your Linux OSs to the latest security patch levels, or even to newer or different Linux OSs. PLAT-9012
* Asymmetric Network proxy settings. For a given universe, network proxy settings can now be specified at per region granularity (rather than per universe granularity). For example, beneficial for multi-cloud universes sending their backups to a single cloud bucket. {{<badge/ea>}} PLAT-12604

### Improvements

* TESCO support to directly specify images in Azure provider. PLAT-12814
...
* Now allows for custom replication lag alert configurations outside of modal with the user-set maximal threshold, eliminating risk of UI page blanking. Removes the legacy modal for setting replication slack alerts as users can now utilize the alert config page. PLAT-12463
...
* Ensures correct setting of `installRoot` config values for a smooth replicated migration process by adding a validation check before starting the migration. The check prevents `installRoot` from equating `storage_path` and considers the `precheck type` while creating the config. PLAT-12785,PLAT-12784
...
* Enables index table support in the non-transactional xCluster edit table which eliminates the need for `restart replication` when adding tables. It offers automatic selection of new index tables if the main table is selected and allows table removal from the table details list. Updates also integrate the edit table modal into the xCluster replication tab. PLAT-12472,PLAT-11801,PLAT-11853
...
* Enables re-execution of similar rolling operation tasks, including changes in Gflags, instance type, and AMI restarts. This is beneficial in cases of unrecoverable errors caused by incorrect gflags that lead to master/tserver crashes. PLAT-12797
...
* Allows individual index table selection for xCluster replication and xCluster DR. If `autoIncludeIndexTables` is enabled, relevant index tables are added to user's table selection. Mostly useful when wanting to exclude an index table from replication without removing the main table. Also includes some minor text updates and stricter block on create DR UI action. PLAT-12985
...
* [xCluster DR](/stable/yugabyte-platform/back-up-restore-universes/disaster-recovery/). A turnkey solution for business continuity and disaster recovery, allowing you to recover from an unplanned outage (failover) or to perform a planned switchover. {{<badge/ea>}}
