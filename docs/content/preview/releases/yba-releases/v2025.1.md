...
* *Faster software upgrades (and gFlag changes, rolling restarts, etc.) (for Kubernetes)* -  K8s support for rolling operations performed multiple nodes at a time (rather than one at a time), speeding up the operation by 2x, 3x, or more. {{<tags/feature/ga idea="444">}}
