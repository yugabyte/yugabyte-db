...
## v2025.1.0.0 - July 23, 2024 {#v2025.1.0.0}
...
Older clients that perform strict JSON deserialization ( clients that ignore unknown fields won't be affected) on the response from `getXClusterConfig` may fail. These clients are not expecting the `backupUuid`, `restoreUuid`, and `replicationSetupTime` fields and will likely throw an "unrecognized property" error.

**Action required**: Users of client applications must update their data models for `XClusterNamespaceConfig` to include the three new fields. It is crucial that the client code can correctly handle null values for these properties, as they are not always populated.
