...
## v2025.1.0.0 - July 21, 2024 {#v2025.1.0.0}
...
* You cannot [upgrade from PostgreSQL 11 to PostgreSQL 15](/stable/yugabyte-platform/manage-deployments/ysql-major-upgrade-yba/) for Kubernetes and OpenShift universes if either YugabyteDB Anywhere or database pods are deployed on different clusters, or use different service accounts.
...
Before upgrading from PostgreSQL 11 to PostgreSQL 15, increase the catalog upgrade timeout by setting the YBA runtime configuration parameter yb.upgrade.wait_attempts_for_major_catalog_upgrade to 60 or higher at the universe scope.
This prevents timeout issues during the PostgreSQL 15 catalog upgrade process.
