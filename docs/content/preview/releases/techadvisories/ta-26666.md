...
A typical customer facing symptom due to this issue is likely to be that the system reaches its soft-memory limit, and this'll cause the system to reject requests. If there is a more rapid memory spike, it can also result in an OOM Killer and the YB-TServer process will get restarted.
...
The transaction inserts the row to the child table, and the insert generates intents on the child table's tablet. As this insert references the parent tablets row, a lock record is generated corresponding to the row on the parent tablet's row (in order to maintain the foreign key referential integrity). After the transaction commits, these intents can be applied to regular RocksDB and no longer be maintained in the system. In every tablet (parent and child), DocDB apply process adds the transaction that generates intents to a map. This map is used to determine the set of transactions that the transaction loader loads during the tablet bootstrap process and is necessary to ensure that all the transactions necessary for bootstrap are loaded. This map (consisting of transaction IDs), is persisted along with retryable requests structure at the time of transaction log (WAL) rollover, that is used by tablet bootstrap.
...
- If the workload only generates a lot of write traffic to the child table, then the intents generated for the parent table's tablets do not have any write_batches, as a result the existing trimming logic that relies on checking `max_replicated_op_id` doesn't qualify as `max_replicated_op_id` does not take effect, rendering the existing cleanup logic useless causing the memory of the map to grow unbounded.
...
The metric `wal_replayable_applied_transactions` can be used to determine the current size of the map in question.
...
Heap snapshot URL on the TServer, would show a lot of allocations from the `TransactionParticipant::Impl::RemoveTransaction()` as per the following illustration:
...
This issue is a regression introduced by fix for issue [23890](https://github.com/yugabyte/yugabyte-db/issues/23890) (commit [d6bbf59](https://github.com/yugabyte/yugabyte-db/commit/d6bbf59db8b502f790fc160ca9eef72cf5791b7b)), and it affects all builds from v2024.2 and v2.25.
...
The fix was implemented in two parts, tracked by [26860](https://github.com/yugabyte/yugabyte-db/issues/26860) and [26666](https://github.com/yugabyte/yugabyte-db/issues/26666), and is available in all builds starting from version 2024.2.3 and later.
