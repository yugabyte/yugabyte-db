...
The issue due to the logic that checked if the hybrid time-based retention barrier could be moved was only based on whether the before image mode was set to ALL or the replica identity was set to FULL. For all other before-image modes and replica identities that required further retention, these barriers were lifted prematurely.
