...
The following example demonstrates the same TServer cache entry being used across two different sequences in two different databases. To reproduce the following issue, set the cluster's flag `ysql_sequence_cache_method = server`.
...
1. Connect to database seqtest1 and create sequence foo in seqtest1. Allocate three values from foo by calling the function nextval three times as follows:
...
1. Connect to database seqtest2, create a new sequence bar in database seqtest2 and allocate value by calling the function nextval. In this scenario, sequence bar has the same OID as sequence foo, so it results in using the same cache on TServer. This result in nextval function returns 4 which is the next available sequence value in the cache. The correct behavior would be to return 1, as this is first time values are allocated from the sequence bar and default for an sequence minvalue is 1.
    ...
    TServer sequence cache entries are never invalidated and reside in memory until TServer shuts down as described in the following steps.

1. Drop the database seqtest1 and seqtest2 which drops all the objects including sequences foo and bar. Create a new database seqtest3 and connect to database seqtest3.
...
1. Create a sequence baz in seqtest3 and allocate value from sequence baz using nextval. In this scenario, the nextval function returns 5 because the sequence baz shares the same cache entry on the TServer as foo and bar, due to having the same OID.
