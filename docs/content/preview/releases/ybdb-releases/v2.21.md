...
* [Backup and restore support in yugabyted](../../../stable/reference/configuration/yugabyted/). yugabyted CLI enabled support for taking database/keyspace backups. You now have the capability to upload backups to public clouds, including AWS and GCP.
...
* YSQL: DDL atomicity. Ensures that YSQL DDLs are fully atomic between YSQL and DocDB layers, that is in case of any errors, they are fully rolled back, and in case of success they are applied fully. Currently, such inconsistencies are rare but can happen.

* YSQL: Lower latency for large scans with size-based fetching. A static size based fetch limit value to control how many rows can be returned in one request from DocDB.

* YSQL: ALTER TABLE support. Adds support for the following variants of ALTER TABLE ADD COLUMN:
