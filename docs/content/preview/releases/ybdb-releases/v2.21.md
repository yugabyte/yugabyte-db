...
* Support for isolating DDLs per database. Specifically, a DDL in one database does not cause catalog cache refreshes or aborts transactions due to breaking change in another database. IDEA-573
* Ensures that YSQL DDLs are fully atomic between YSQL and DocDB layers – i.e. in case of any error they are fully rolled back, and in case of success they are applied fully. Such inconsistencies are rare but can happen today. IDEA-574
* A static size based fetch limit value  to control how many rows can be returned in one request from DocDB IDEA-921
* Adds supports for the following variants of ALTER TABLE ADD COLUMN: a) with a SERIAL data type, b) with a volatile DEFAULT, c) with a PRIMARY KEY. IDEA-976
...
* yugabyted CLI enabled support for taking database/keyspace backups. Users will have the capability for uploading backups to public clouds - AWS and GCP. IDEA-1072
