...
- [Shared memory](../../../reference/configuration/yb-master/#memory-division-flags). Implement shared memory for PostgreSQL to TServer communication to minimize latency and enhance query performance by reducing communication overhead. {{<badge/tp>}}
...
* If you are upgrading your clusters from v2.16 to v2024.1, some transactions may fail when a YSQL with the flag `enable_wait_queues` is set to true and sends an explicit row lock RPC with WAIT_BLOCK to a TServer still with the value set as false (and hence not having wait queues). {{<issue 14935>}}
