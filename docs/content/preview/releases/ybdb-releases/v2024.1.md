...
We're excited to announce support for the PostgreSQL Logical Replication Protocol for Change Data Capture (CDC) in the 2024.1.1.0 release, in addition to the existing native [gRPC Replication protocol](/preview/explore/change-data-capture/using-yugabytedb-grpc-replication/).
This feature allows you to manage CDC streams using [Publications](https://www.postgresql.org/docs/11/sql-createpublication.html), similar to native PostgreSQL. Additionally, a [new connector](/preview/explore/change-data-capture/using-logical-replication/get-started/#get-started-with-yugabytedb-connector) is introduced that uses the logical replication protocol to consume the CDC streams via [Replication slots](https://www.postgresql.org/docs/current/logicaldecoding-explanation.html#LOGICALDECODING-REPLICATION-SLOTS).
