...
* Added support for read-only mode for DR replica, currently supported for YSQL tables. {{<link "/:version/deploy/multi-dc/async-replication/async-transactional-failover/#restore-the-cluster-to-a-consistent-state">}}

* Tablet splitting now can be enabled on tables with CDC configured. {{<issue 18479>}} {{<link "/:version/deploy/multi-dc/async-replication/async-transactional-failover/#restore-the-cluster-to-a-consistent-state">}}

* Transactional CDC now supports consistent snapshots, ensuring data integrity during replication to a sink. Snapshots, ordered by commit time across all tables and tablets, establish a reliable replication order. {{<issue 19682>}} {{<link "/:version/deploy/multi-dc/async-replication/async-transactional-failover/#restore-the-cluster-to-a-consistent-state">}}
