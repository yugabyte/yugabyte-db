...
## v2.20.3.0 - April 8, 2024 {#v2.20.3.0}

**Build:** `2.20.3.0-b67`

**Third-party licenses:** [YugabyteDB](https://downloads.yugabyte.com/releases/2.20.3.0/yugabytedb-2.20.3.0-b67-third-party-licenses.html), [YugabyteDB Anywhere](https://downloads.yugabyte.com/releases/2.20.3.0/yugabytedb-anywhere-2.20.3.0-b67-third-party-licenses.html)

### Downloads

<ul class="nav yb-pills">
 <li>
   <a href="https://downloads.yugabyte.com/releases/2.20.3.0/yugabyte-2.20.3.0-b67-darwin-x86_64.tar.gz">
     <i class="fa-brands fa-apple"></i>
     <span>macOS</span>
   </a>
 </li>
 <li>
   <a href="https://downloads.yugabyte.com/releases/2.20.3.0/yugabyte-2.20.3.0-b67-linux-x86_64.tar.gz">
     <i class="fa-brands fa-linux"></i>
     <span>Linux x86</span>
   </a>
 </li>
 <li>
   <a href="https://downloads.yugabyte.com/releases/2.20.3.0/yugabyte-2.20.3.0-b67-el8-aarch64.tar.gz">
     <i class="fa-brands fa-linux"></i>
     <span>Linux ARM</span>
   </a>
 </li>
...
```sh
docker pull yugabytedb/yugabyte:2.20.3.0-b67
```

### New feature

* [Roll back upgrades](../../../manage/upgrade-deployment/#b-rollback-phase). Ability to roll back an upgrade to the pre-upgrade release. (The release from which the database was upgraded). You can roll back a database upgrade _only_ to the pre-upgrade release. {{<badge/ea>}}
...
* Incorporates checks of inequality filters in the YSQL layer to avoid transmitting trivially false inequalities, preventing undesired behavior from DocDB iterators. Requires backports to 2.20 and 2.18. {{<issue 21383>}}
...
* Incorporates detection of recently aborted transactions into the transaction coordinator. Now, deadlock detectors constantly send RPCs to abort a deadlocked transaction, streamlining the behavior for local and remote deadlock detectors. The new flag `clear_deadlocked_txns_info_older_than_seconds` is introduced to store details of aborted transactions due to deadlocking. The detector only aborts the youngest transaction in a deadlock, ensuring a minimized rollback, hence more efficient transaction handling. {{<issue 14165>}},{{<issue 19257>}}
...
* :[CDCSDK]Fix for tablet split data loss /In the current implementation,when explicit checkpointing is enabled,due to buggy implementation the `cdc_sdk_safe_time`is moved forward to the `safe_hybrid_time`with every get changes request.This means that we would update the `cdc_sdk_safe_time`without receiving an explicit checkpoint confirming the consumption of records corresponding to previous safe time.This behaviour could lead to data loss when a tablet is split.The `getTabletListToPollForCDC `returns an Op Id corresponding to the last checkpointed record on the parent tablet.However the checkpointed safe time is incorrect and could lead to filtering of records.This diff introduces a fix for this problem by increasing the `cdc_sdk_safe_time`only when an acknowledgement is received from the GetChanges request explicit checkpoint.Backport Description The test CDCSDKBeforeImageTest.TestCompactionWithConsistentSnapshotAndNoBeforeImage failed in 2.20 and 2.20.2 but not on master.This was because the changes introduced by are not present in these branches.As a result when the test calls GetChanges by providing an explicit checkpoint with snapshot time set to zero,the cdc_sdk_safe_time in the cdc_state table was being reset to Invalid.To fix this,if an empty optional value is passed for safe_time to the method UpdateCheckpointAndActiveTime ,then we get the checkpoint from the state table and rewrite the safe time with the safe time from the checkpoint.Since the connector is not expected to send an explicit checkpoint without snapshot_time this code is not expected to be hit in normal scenarios.[CDC +LRU]Data loss seen in tablet split LRU,no error in connector,server side issue Jira Link:Description Logs:`AssertionError:Change is the same for 30 minutes test_cdc_a6807a yb=367195 sink=366945.Replication stopped at 2023-01-16T20:05:47.262923 `![Profile (7)]:In the current implementation,when explicit checkpointing is enabled,due to buggy implementation the `cdc_sdk_safe_time`is moved forward to the `safe_hybrid_time`with every get changes request.This means that we would update the `cdc_sdk_safe_time`without receiving an explicit checkpoint confirming the consumption of records corresponding to previous safe time.This behaviour could lead to data loss when a tablet is split.The `getTabletListToPollForCDC `returns an Op Id corresponding to the last checkpointed record on the parent tablet.However the checkpointed safe time is incorrect and could lead to filtering of records.This diff introduces a fix for this problem by increasing the `cdc_sdk_safe_time`only when an acknowledgement is received from the GetChanges request explicit checkpoint. {{<issue 15718>}}
