...
* Introduces a dedicated auth-backend for YSQL to handle connection authentication, enhancing security and streamlining the process. {{<issue 24147>}},{{<issue 24427>}}
...
* Sets `yb_bnl_batch_size` to 1024 and `yb_prefer_bnl` to true by default, ensuring BNLs replace nested loop joins without altering non-NL join plans. {{<issue 19273>}}
