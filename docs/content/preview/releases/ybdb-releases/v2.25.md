...
YugabyteDB’s latest release introduces support for [**PostgreSQL advisory locks**](../../../explore/transactions/explicit-locking/#advisory-locks), enabling efficient application-level locking mechanisms. These locks allow developers to coordinate concurrent processes without blocking transactions, improving **workload management and resource synchronization** in distributed environments. More details on Postgres advisory locks can be found [here](https://www.postgresql.org/docs/current/explicit-locking.html#ADVISORY-LOCKS)

## Time-travel Queries {{<tags/feature/tp idea="1110">}}

YugabyteDB now supports [**time-travel queries**](../../../../manage/backup-restore/point-in-time-query/), allowing users to run queries as if they were executed at a specified point in the past. This capability enables historical data analysis, auditing, and debugging without requiring separate backups or snapshots, enhancing data visibility in a running database

## PgVector HNSW Index Support {{<tags/feature/tp idea="1111">}}

YugabyteDB now supports [**HNSW (Hierarchical Navigable Small World) indexes**](../../../explore/ysql-language-features/pg-extensions/extension-pgvector/#vector-indexing) through the **pgvector** extension, enabling efficient approximate nearest neighbor (ANN) search for high-dimensional data. This addition expands YugabyteDB’s vector indexing capabilities, providing another option alongside existing indexing methods for vector similarity search.

## Upgrade from PostgreSQL11 to PostgreSQL15 {{<tags/feature/tp idea="1746">}}

YugabyteDB now supports [in-place zero downtime upgrade  from PostgreSQL 11](../../../../manage/upgrade-deployment/)  compatible   2024.2.2  release  to PostgreSQL 15  compatible  2.25.1 release. This capability allows PostgreSQL major version upgrade without  any downtime and interruption to  the application.

## Extended Statistics {{<tags/feature/tp>}}

YugabyteDB now supports  the ability to compute multivariate statistics using the [***CREATE STATISTICS***](../../../develop/pg15-features) command. This feature allows the query planner to use cross-column correlation statistics to generate better query plans.

### New features

#### Core Database

* [PostgreSQL Anonymizer](../../../explore/ysql-language-features/pg-extensions/extension-pganon/), which masks or replaces personally identifiable information (PII) or commercially sensitive data, is now bundled with Yugabyte {{<tags/feature/tp idea="1497">}} <!-- IDEA-1497 --> 
