...
| pgaudit.log_catalog | Enable session logging for statements in the PostgreSQL system catalog relations in `pg_catalog`. These system catalog tables record activity such as metadata lookups and activity from third-party tools performing lookups.<br>Disable this setting to reduce noise in the log. | ON |
