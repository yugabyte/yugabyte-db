...
| pgaudit.log_catalog | Enables session logging for statements in the postgres system catalog relations in `pg_catalog`. These system catalog tables record activity such as metadata lookups and activity from third-party tools performing lookups.<br>Disable this setting to reduce noise in the log. | ON |
