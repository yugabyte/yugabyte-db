...
- Set the `log_line_prefix` PostgreSQL server option to log timestamp, PostgreSQL PID, and session identifier.
    ...
    For information on setting server options in YugabyteDB, refer to [Server configuration parameters](../../../reference/configuration/yb-tserver/#server-configuration-parameters).
