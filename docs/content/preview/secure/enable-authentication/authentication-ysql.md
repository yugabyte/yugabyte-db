...
A YugabyteDB cluster with authentication enabled starts with the default admin user of `yugabyte` and the default database of `yugabyte`. You can connect to the cluster and use the [YSQL shell](../../../yugabyte-clients/ysqlsh/) by running the following `ysqlsh` command from the YugabyteDB home directory:
...
As an easier alternative, you can run the `\du` [meta-command](../../../yugabyte-clients/ysqlsh-meta-commands/) to see this information in a simpler format:
