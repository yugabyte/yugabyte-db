...
You can list all privileges granted to the various roles with the [\du meta-command](../../../yugabyte-clients/ysqlsh-meta-commands/#du-s-pattern-patterns):
...
Verify that the `engineering` role has `SELECT` privilege on the table using the [\z meta-command](../../../yugabyte-clients/ysqlsh-meta-commands/#z-pattern-patterns), which lists tables with their associated access privileges:
