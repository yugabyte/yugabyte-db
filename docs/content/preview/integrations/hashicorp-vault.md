...
- To run the vault in development mode, add the default vault address and vault token as follows:
...
The vault automatically registers the plugin if the directory of the binary of the plugin is provided as an input with the `dev-plugin-dir` flag. To run the vault server in development mode, use the `dev` flag. Set the `dev-root-token` flag which informs the vault to use the default vault token of root to login (this token is required in production mode).
