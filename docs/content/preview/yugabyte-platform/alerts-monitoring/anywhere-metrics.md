...
| Catalog Cache Misses | During YSQL query processing, system catalog (pg_catalog) tables that live on the yb-master are cached on the local YSQL backend. This metric counts the total number of such YSQL catalog cache misses that needed to be fetched from the yb-master. It can be further broken down by the specific names of the pg_catalog tables that triggered these misses. | This metric is informational and should not be subject to alerting. You may consider this information while examining alerts on other metrics. | ![Catalog Cache Misses](/images/yp/metrics114.png) 
