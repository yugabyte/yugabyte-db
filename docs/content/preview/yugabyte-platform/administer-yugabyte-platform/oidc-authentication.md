...
- For information on how to add users, see [Manage YugabyteDB Anywhere users](../anywhere-rbac/). The email ID that you enter in the **Add User** dialog must be registered with the identity provider, and the role must reflect the user's role on YugabyteDB Anywhere.
