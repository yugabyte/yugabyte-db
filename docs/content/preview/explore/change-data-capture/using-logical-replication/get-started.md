...
1. Load the schema of the sample tables. The file `share/schema.sql` is bundled along with the `yugabyte-db` binary when downloaded from [download.yugabyte.com](https://download.yugabyte.com/) and no external file is needed:
