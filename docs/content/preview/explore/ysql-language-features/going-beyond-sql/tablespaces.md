...
In the following example our tablespace is setup to have replicas in us-east-1, us-east-2 and us-west-1. This enables it to survive the loss of an entire Region. The clients are located in us-east-1.
...
```
Setting `leader_preference` of us-east-1a to 1 (most preferred), the YugabyteDB load balancer will place all associated tablet leaders in this zone. If all the nodes in us-east-1a are unavailable, it we will fallback to the next preferred zone us-east2.
