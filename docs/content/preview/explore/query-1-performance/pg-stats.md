...
Similar to how pg_stats reports common values, it also reports commonly occuring elements and their respective frequencies within array data types in the `most_common_elems` column. For example, consider a table that stores the labels that movies are tagged with:
