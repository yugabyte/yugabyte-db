...
### View execution latencies for specific time spans

Use the `yb_latency_histogram` column to view query execution latencies falling in a particular time span.

Each row in the `pg_stat_statement` view in YugabyteDB includes the `yb_latency_histogram` column, of type JSONB.
...
Record 1 shows that the `insert` query took between 0.1 to 0.2 ms 4 times and 0.2 to 0.3 ms 2 times.

Record 2 shows that the `pg_sleep`  query took between 0.8 to 0.9 ms 6 times and 3 times it took more than 1677721.6 ms.
