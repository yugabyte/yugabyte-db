...
headerTitle: Running the examples
linkTitle: Setup
description: Set up clusters to run Explore examples.
headcontent: Set up clusters to run Explore examples
menu:
  preview:
    identifier: cluster-setup
    parent: explore
    weight: 5
type: docs
---

You can run most of the examples in Explore, in particular those demonstrating database features, such as API compatibility, using a single-node cluster either on your computer, using the free Sandbox cluster in YugabyteDB Aeon, or a universe created via YugabyteDB Anywhere. Some more advanced scenarios use a multi-node deployment.
