...
To check if a table is colocated or not, you can use the [`\d`](../../yugabyte-clients/ysqlsh-meta-commands/#d-s-pattern-patterns) meta-command in [ysqlsh](../../yugabyte-clients/ysqlsh/). You can also retrieve the same information using the `yb_table_properties` function as follows:
