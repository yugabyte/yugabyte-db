...
`pg_stat_statements` is an extension that tracks and aggregates statistics for SQL queries executed on the database. It helps monitor query performance by recording execution counts, total and average execution times, rows processed, and resource usage (e.g., shared buffer hits and disk I/O). The extension groups queries with similar structures (normalized queries) to provide a concise and meaningful view of query behavior.

By analyzing the `pg_stat_statements` view, database administrators can identify slow, frequently executed, or resource-intensive queries, making it a powerful tool for performance tuning. It is easy to use—simply enable the extension and query the view to gain insights into query patterns and optimize database performance.
...
`pg_stat_activity` is a system view that provides real-time information about the currently active database sessions. It helps monitor user connections, query execution, and session states, making it a vital tool for understanding database activity and diagnosing performance issues.
...
The `yb_local_tablets` provides information about the how your table data is distributed across the different tablets in your cluster
...
The COPY command is used to transfer data in and out out a database. This could be a long runnig operation depending on teh size of data.
...
By querying pg_locks, you can identify which processes are holding locks, waiting for locks, and the types of locks involved (e.g., row-level, table-level). This information is invaluable for diagnosing performance bottlenecks caused by lock contention, optimizing query execution, and ensuring smooth database operation.
