...
YugabyteDB offers 2 schemes for [data sharding](../../../explore/going-beyond-sql/data-sharding), namely [Hash](../../../explore/going-beyond-sql/data-sharding#hash-sharding) and [Range](../../../explore/going-beyond-sql/data-sharding#range-sharding). Here is a quick comparison of the 2 schemes.
...
| Ordering of data within a partition            | Actual value of the Primary key | Hash of the Primary key     |
...
Now to see how point lookups perform in a range sharded table, let us fetch the row for a specific row.
...
This shows that only one row was scanned (via `Storage Rows Scanned: 1`) to retrieve one row (via `rows=1`) and hence point lookups are very efficient in a range-sharded table.

Now to understand how range scans are efficient in a range-sharded table, let us fetch the rows with `id > 2 and id < 6`,
...
Note that exactly 3 rows were scanned (via `Storage Rows Scanned: 3`) to retrieve 3 rows. This is because the data is ordered on the primary key and hence an index scan was done for the range query (via `Index Scan using user_range_pkey`).
...
To see how efficient point lookups are in a hash-sharded table, let us execute,
...
The primary key index, `user_hash_pkey` was used to lookup `id =  5` and only one row was scanned to retrieve on row. This shows that point lookups are very efficient on a hash-sharded table.

Now let us see how the performance of a  range scan on a hash-sharded table. For this let us fetch the rows with `id > 2 and id < 6`,
