...
| SCRAM-sha256              | A secure password-based authentication that protects credentials using hashing, salting and challenge-response.                                           | ✅ Supported (On 2.27.0.0, coming in 2024.2, 2025.1 as well)                |
| MD5                       | Any password of a user is by default stored as md5 encryption format in the database.                                                                     | ✅ Supported                                   |
| Cert                      | Cert based authentication requires the client to provide certificates to the server over TLS connection for authentication. 
