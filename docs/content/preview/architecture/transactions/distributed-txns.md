...
Provisional records are stored in a separate RocksDB instance in the same tablet peer, which we refer to as IntentsDB (as opposed to RegularDB, for regular records).. Compared to other possible design options, such as storing provisional records inline with the regular records, or putting them in the same RocksDB instance together with regular records, the chosen approach has the following benefits:
