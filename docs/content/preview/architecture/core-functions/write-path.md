...
For information on more complex cases, such as distributed transactions with multiple keys requiring atomical updates, see [Distributed transactions I/O path](../../transactions/distributed-txns/).

## Write operation processing by YQL layer

The user-issued write request interacts with the YQL query layer via a port with the appropriate API (either YSQL or YCQL). This user request is translated by the YQL layer into an internal key. As described in [Sharding](../../concepts/sharding/), each key is owned by one tablet which, along with the YB-TServers hosting it, can be determined by the YQL layer making an RPC call to the YB-Master. The response is cached for future uses.

YugabyteDB has a [smart client](../../../develop/client-drivers/java/) that can cache the location of the tablet directly and can therefore save the extra network hop, therefore allowing it to send the request directly to the YQL layer of the appropriate YB-TServer hosting the tablet leader. If the YQL layer finds that the tablet leader is hosted on the local node, the RPC call becomes a local function call and saves the time needed to serialize and deserialize the request, and then send it over the network.
...
The following diagram depicts the process of the preparation of the operation for replication by the tablet leader:
...
* The leader appends the batch to its Raft log and chooses a so-called hybrid timestamp to be used as the timestamp of the write operation.
...
it is known to have been committed. The leader piggybacks the advancement of the commit point in subsequent RPC requests, as follows:
...
* In the next update that the leader sends to followers, it also notifies followers that the entry has been committed, and the followers apply the write batch to their RocksDB instances.
...
Specifically for YCQL, the use of the YugabyteDB smart client is recommended for removing the extra network hop.