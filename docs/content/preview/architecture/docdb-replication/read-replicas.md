...
Since read replicas are a Raft replication-level extension, the schema changes are transparently apply to these replicas. There is no need to execute DDL operations separately on the read replica cluster.
