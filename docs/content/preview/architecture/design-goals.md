...
- The [YSQL API](../../api/ysql/) supports Serializable, Snapshot (default), and Read Committed {{<badge/ea>}} isolation using the PostgreSQL isolation level syntax of `SERIALIZABLE`, `REPEATABLE READ`, and `READ COMMITTED` respectively. For more details, see [YSQL vs. PostgreSQL isolation levels](#ysql-vs-postgresql-isolation-levels).
...
YugabyteDB splits table data into smaller pieces called tablets and distributes these tablets across different nodes in the cluster for better performance, availability, and fault tolerance. YugabyteDB automatically rebalances the no.of tablets on each node as the cluster scales.

## Load Balancing

YugabyteDB monitors and automatically rebalances the no.of tablet leaders and followers on each node continuously. This leads to the distribution of reads and writes across multiple avoiding hotspots and ensuring efficient resource utilization.
...
[YCQL](../../api/ycql/) is a [semi-relational SQL API](../../explore/ycql-language/) that is best suited for internet-scale OLTP and HTAP applications needing massive write scalability and fast queries. YCQL supports distributed transactions, strongly-consistent secondary indexes, and a native JSON column type. YCQL has its roots in the Cassandra Query Language. {{<link "../query-layer">}}

## Performance

Written in C++ to ensure high performance and the ability to use large memory heaps (RAM) as an internal database cache, YugabyteDB is optimized primarily to run on SSDs and Non-Volatile Memory Express (NVMe) drives. YugabyteDBto handle ever-growing event dataorkload characteristics in mind:
...
YugabyteDB supports [Row Level Geo-Partitioning](../../explore/multi-region-deployments/row-level-geo-partitioning) of tables. This enables a specific row to be placed in a specific geography. It leverages two Postgres features - [Table Partitioning](../../explore/ysql-language-features/advanced-features/partitions/) and [TableSpaces](../../explore/ysql-language-features/going-beyond-sql/tablespaces/) to accomplish this elegantly.

## Cloud-ready

YugabyteDB is a cloud-native database, can easily be deployed out of the box in most public cloud services like [AWS, GCP, Azure](../../deploy/public-clouds/). YugabyteDB also supports a [mult-cloud deployement](../../develop/multi-cloud/multicloud-setup/) which enables a cluster to be spread across different cloud providers.
