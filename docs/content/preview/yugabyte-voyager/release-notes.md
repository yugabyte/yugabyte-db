...
- Enhanced the `assess-migration` and `analyze-schema` commands to report issues in Functions or Procedures for variables declared with reference types (%TYPE) under the Unsupported PL/pgSQL Objects section.
...
- Allow yb-voyager upgrades during migration from the recent breaking release (`1.8.5`) to the higher versions in the same migration.
...
- Fixed formatting issues in the assessment HTML report, where extra spaces or characters appeared after the "Unsupported PL/pgSQL Objects" section heading, depending on the browser used for viewing.
