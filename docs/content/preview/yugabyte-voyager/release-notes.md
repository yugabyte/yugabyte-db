...
- Introduced a flag `--version` in the voyager installer script which can be used to specify the version to install.
...
- Fixed a [bug](https://github.com/yugabyte/yb-voyager/issues/1920) where export-data errors out if background metadata queries (count*) are still running after pg_dump completes.
