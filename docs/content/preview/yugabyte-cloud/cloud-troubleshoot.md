...
By default, multi-region clusters (which are deployed in VPCs) do not expose any publicly-accessible IP addresses. If you are trying to connect from a public IP, enable Public Access for the cluster as well as add the IP to the cluster allow list.
