...
By default, multi-region clusters, which must be deployed in a VPC, do not expose any publicly-accessible IP addresses. To add public IP addresses, enable Public Access on the cluster **Settings** tab. Alternatively, use the [Cloud shell](../connect-cloud-shell/) instead.
