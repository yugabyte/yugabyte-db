...
Multi-region clusters are deployed in a VPC and do not expose any publicly-accessible IP addresses. As a result, you can only connect to multi-region clusters from applications that reside on a peered network, and the [peering connection](../../cloud-basics/cloud-vpcs/cloud-add-peering/) must be Active.

In addition, if your cluster is deployed in a [VPC](../../cloud-basics/cloud-vpcs/), you need to add the IP addresses of the peered application VPC to the cluster IP allow list.
