...
| **Tablet Server** | | |
| WAL Bytes Written / Sec / Node | The number of bytes written to the write-ahead logging (WAL) after the tablet start. | A low-level metric related to the storage layer. This can be helpful when debugging certain latency or throughput issues by isolating where the bottleneck happens. |
| **DocDB** | | |
...
| Average SSTables / Node | The average number of SSTable (SST) files across nodes. | A low-level metric related to the storage layer. This can be helpful when debugging certain latency or throughput issues by isolating where the bottleneck happens. |
