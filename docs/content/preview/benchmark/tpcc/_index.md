...
{{<note>}}
All benchmarks were run on a single region YugabyteDB cluster running on {{<release "2.18.1">}}, except 150K warehouses, which was run on 2.11.
{{</note>}}
...
Once you've identified a cluster specification that matches your needs, apply the TPC-C workload recommended for that particular setup. The goal is to validate that the cluster can sustain the expected transaction throughput—measured in tpmC with a high degree of efficiency, typically exceeding 99.5%. This high-efficiency rate ensures that the cluster is not just meeting the benchmark's demands but doing so with minimal resource overhead, indicating its readiness to handle your real-world, high-volume transactional workloads

{{<lead link="running-tpcc/">}}
For more details on cluster spec/workload and how to run the TPC-C against a local or a YB Managed cluster, see [Running TPC-C](running-tpcc/)
{{</lead>}}
