...
`yugabyted` by default uses `/home/user/var` directory. When you start `yugabyted` with a non-default `--base_dir` value, you must set the `--base_dir` flag for every subsequent `yugabyted` command.
