...
- `load_balance` - enable cluster-aware load balancing by setting this property to one of the allowed values described below other than `false`; disabled by default.
...
If your cluster has read replicas, see [Node type-aware load balancing](../../smart-drivers/#node-type-aware-load-balancing) to distribute connections with the `load_balance` property and its additional values.
