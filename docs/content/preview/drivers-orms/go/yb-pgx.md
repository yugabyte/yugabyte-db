...
Learn how to establish a connection to YugabyteDB database and begin basic CRUD operations using the steps in the [Build an application](../../../develop/build-apps/additional-examples/go/ysql-yb-pgx) page.
...
For a YugabyteDB Managed cluster, or a YugabyteDB cluster with SSL/TLS enabled, set the following SSL-related environment variables at the client side. SSL/TLS is enabled by default for client-side authentication. Refer to [Configure SSL/TLS](../../../develop/build-apps/additional-examples/go/ysql-pgx/#ssl-tls-configuration) for the default and supported modes.
