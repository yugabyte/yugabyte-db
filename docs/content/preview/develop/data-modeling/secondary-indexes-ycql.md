...
In the prior example, to retrieve 4 rows the index was first looked up, and then more columns (e.g, `name`) were fetched for the same rows from the table. This additional round trip to the table is needed because the columns are not present in the index. To avoid this, you can store the column along with the index as follows:
...
The `DESC TABLE <table>` meta command lists the indexes associated with a table along with the schema details.
...
gives the description of just the index as follows:
