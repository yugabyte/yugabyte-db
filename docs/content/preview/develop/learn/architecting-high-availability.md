...
As failures are inevitable, design your applications to take appropriate actions on the failed statements to ensure they are highly available. YugabyteDB returns various error codes for errors that occur during transaction processing.

YugabyteDB supports different [isolation levels](../../../explore/transactions/isolation-levels/) during a transaction. Although some errors are very specific to certain transaction isolation levels, most errors are common across multiple isolation levels. In general, the error codes can be classified into three types.
...
Here, through some simple examples, we will understand the scenarios which cause them and the methods on how they should be handled by the applications.

## Prerequisites

Follow the [setup instructions](../../../explore#tabs-00-00) to start a single YugabytedDB instance on your machine. Consider the following table format with some data for the following scenarios:
...
The server will retry failed transactions automatically whenever possible. There would be scenarios where a server side retry is not fit. eg. The retry limit has been reached or the transaction is not in a valid state. At this juncture it would be the client's responsibility to retry the transaction at the application layer. Most transaction errors that happen due to conflicts, deadlocks can be restarted by the client. The following scenarios describe the causes for failures, and the required methods to be handled by the applications.

### Client Side Retry

The basic idea is to execute the transaction within a `try..catch` block in a loop. When a retriable failure happens, issue `ROLLBACK` and then retry the transaction. To avoid overloading the server, we would wait for some time and limit the number of retries, so that we are not stuck in an indefinite loop. A typical client-side retry implementation would be:
...
We are breaking out the loop if the `COMMIT` was successful. `attempt < max_attempts` limits the no.of retries to `max_attempts` and the amount of time the code waits before the next retry also increases with `sleep_time *= backoff`. Feel free to choose values appropriately for your application.


### 40001 - SerializationFailure

Serialization Failure happens when multiple transactions are updating the same set of keys (conflict) or when transactions are waiting on each other (deadlock). The error messages could be one of the following types:
...
The correct way to handle this error is with a retry loop with exponential backoff as mentioned in the [client side retry](#client-side-retry) section as follows:
...
When the `UPDATE` or `COMMIT` fails because of `SerializationFailure`, the code will retry after waiting for `sleep_time` seconds upto `max_attempts`.
...
The only valid statements at this point would be `ROLLBACK` or `COMMIT`. The correct way to handle this is to resolve the actual error and then issue a rollback. Consider this scenario:
...
To avoid getting stuck in a wait loop because of starvation, it is recommended to use a reasonable timeout for the statements imialr to the following:

```plpgsql
SET statement_timeout = '10s';
```

This will ensure that the transaction would not be blocked for more than `10` seconds.

### Idle Timeout

When an application takes a long time between two statements within a transaction or just hangs, it could be still holding the locks on the [provisional records](../../../architecture/transactions/distributed-txns/#provisional-records) during that period. It would hit a timeout if the `idle_in_transaction_session_timeout` is set accordingly. Once that timeout is reached, the connection is disconnected and the client would have to reconnect. The typical error message would be:

```output.plpgsql
FATAL:  25P03: terminating connection due to idle-in-transaction timeout
```

By default the `idle_in_transaction_session_timeout` is set to `0`. It could be set to a specific value in [ysqlsh](../../../admin/ysqlsh/#starting-ysqlsh) by this command:

```plpgsql
SET idle_in_transaction_session_timeout = '10s';
```

The current value can be viewed in [ysqlsh](../../../admin/ysqlsh/#starting-ysqlsh) by this command:
...
Setting this timeout is useful to avoid deadlock scenarios where applications acquire locks and then hang unintentionally.

### Defferable Property

When a transaction is in `SERIALIZABLE` isolation level and `READ ONLY` mode, if the transaction property `DEFERRABLE` is set, then that transaction executes with much lower overhead and is never canceled because of a serialization failure. This can be used for batch or long-running jobs. Eg.
...
As mentioned before, all transactions are dynamically assigned a priority. This is a value in the range of `[0.0, 1.0]`. The current priority can be fetched using the `yb_transaction_priority` setting like this:
...
This would ensure that the priority assigned to your transaction is in the range : `[0.9-1.0]`.
