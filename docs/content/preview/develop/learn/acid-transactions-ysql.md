...
YugabytedDB provides strong [ACID](#ACID) guarantees on three categories of transactions, for each of which, YugabyteDB has separate optimizations to make them faster.

1. [**Single-row**](../../../architecture/transactions/single-row-transactions/) transactions are transactions where all operations impact a single row (i.e., a key). Single-row ACID is easier to achieve as, in most distributed databases, the data for a single row doesn’t cross into other nodes. 

1. [**Single-shard**](../../../architecture/transactions/single-row-transactions/) transactions occur when all rows involved in the transaction’s operations exist within a single shard (which lies within a single node) of a distributed database. 
...
These are the commands typically involved in a transaction flow.

- [BEGIN](../../../api/ysql/the-sql-language/statements/txn_begin) : Start a transaction.
- [SET](../../../api/ysql/the-sql-language/statements/cmd_set) : Set config values
- [SET TRANSACTION](../../../api/ysql/the-sql-language/statements/txn_set) : Set the isolation level.
- [SHOW](../../../api/ysql/the-sql-language/statements/cmd_show) : Display config values.
...
These are some session level settings affecting transactions that can be configured to your needs. `NOTE:` These will impact all tranctions in the current session.

-   `transaction_read_only` = `[on|off]` - Set this to `on` to make all transactions in the current session to be read-only.
...
-   `transaction_deferrable` = `[on|off]` - Set this to `on` to make all transacations in the current session to [deferrable](#deferrable-property)
-   `idle_in_transaction_session_timeout` - Set this to time duration eg.`'10s or 1000'` to limit delays within transaction statements. Default time units is millisecconds. Ref: [Idle timeout](#idle-timeout).
-   `yb_transaction_priority_lower_bound` = `[0.0-1.0]` - Set the lower bound of the dynamic priority assigment.
-   `yb_transaction_priority_upper_bound` = `[0.0-1.0]` - Set the upper bound of the dynamic priority assigment.

## Isolation Levels

Isolation level defines the level of data visibility to the transaction.YugabytedDB supports [MVCC](../../../architecture/transactions/transactions-overview/#multi-version-concurrency-control) which enables isolation to concurrent transactions without the need for locking. YugabyteDB support three kinds of isolation levels to support different application needs.
...
Here is the list of error codes that typically occur during transaction processing and their meaning.
...
This error occurs when certain statements are executed outside of a transaction but should be executed only within a transaction. For example, issuing a `ROLLBACK` outside a transaction.
...
Although some errors are very specific to certain [transaction isolation levels](../../../explore/transactions/isolation-levels/), most errors are common across multiple isolation levels. In general, the error codes can be classified into three types. 
...
Consider the example below that inserts a row `[k=1, v=30]`. 
...
If the row `[k=1]` already exists in the table, the `insert` would result in a UniqueViolation exception. Techically the transaction would be in an error state and further statements would result in a [25P02: In failed sql transaction](#25P02-in-failed-sql-transaction) error. You have to catch the exception and rollback. But instead of rolling back the entire transaction, you can rollback to the previously declared savepoint, `before_insert` and update the value of the row with `k=1`. Now you can continue with other statements in the transaction.
...
The [insert]() statement has an optional [`ON CONFLICT`](../../../api/ysql/the-sql-language/statements/dml_insert/#on-conflict-clause) clause that can be helpful to circumvent certain errors and avoid multiple statements. For example, if concurrent transactions are inserting the same row which could cause a UniqueViolation, instead of letting the server throw an error and you handling it in code, you could just ask the server to ignore it like:

```plpgsql
INSERT INTO txndemo VALUES (1,10) DO NOTHING;
```

With [`DO NOTHING`](../../../api/ysql/the-sql-language/statements/dml_insert/#conflict-action-1), the server does not throw an error, one less round-trip between the application and the server. You can also simulate an `upsert` instead of a `insert`-> fail -> `update` scenario via `DO UPDATE SET` like:
...
Now, the server automatically updates the row when it fails to insert. Again, one less round-trip between the application and the server.


### Returning on insert/update

You can fetch the row data inserted or update in a single statement instead of `update` and then a `select` using the `RETURNING` clause like:
...
This would help you save one round trip and immediately fetch the updated value.
