...
The following table summarizes the arguments and options you can pass to the yb_migrate tool to establish an SSL connection.

| Database | Arguments | Options | Description |
| :------- | :-------- | :------ | :---------- |
| PostgreSQL <br /> MySQL | `--source-ssl-mode`| <br /> <br /> disable <br /> allow <br /> prefer(default) <br/> require <br/> verify-ca <br/> verify-full <br/> |  Value of this argument determines whether an encrypted connection is established between yb_migrate and the database server; and whether the certificate of the database server is verified from a CA. <br /> Only try a non-SSL connection. <br/> First try a non-SSL connection; if that fails, try an SSL connection. (Not supported for MySQL.) <br /> First try an SSL connection; if that fails, try a non-SSL connection. <br/> Only try an SSL connection. If a root CA file is present, verify the certificate in the same way as if verify-ca was specified. <br/> Only try an SSL connection, and verify that the server certificate is issued by a trusted certificate authority (CA). <br/> Only try an SSL connection, verify that the server certificate is issued by a trusted CA and that the requested server host name matches that in the certificate. |
...
| YugabyteDB | `--target-ssl-mode` | <br /> <br /> disable <br /> allow <br /> prefer(default) <br/> require <br/> verify-ca <br/> verify-full <br/> |  Value of this argument determines whether an encrypted connection is established between yb_migrate and the database server; and whether the certificate of the database server is verified from a CA. <br /> Only try a non-SSL connection. <br/> First try a non-SSL connection; if that fails, try an SSL connection. (Not supported for MySQL.) <br /> First try an SSL connection; if that fails, try a non-SSL connection. <br/> Only try an SSL connection. If a root CA file is present, verify the certificate in the same way as if verify-ca was specified. <br/> Only try an SSL connection, and verify that the server certificate is issued by a trusted certificate authority (CA). <br/> Only try an SSL connection, verify that the server certificate is issued by a trusted CA and that the requested server host name matches that in the certificate. |
