...
| Oracle | `--oracle-tns-alias` | A TNS alias that is configured to establish a secure connection with the server is passed to yb-voyager. When you pass this argument, you cannot use any other arguments to connect to your Oracle instance. To connect to Oracle instances you can use only one from the following arguments: <br/> [`--source-db-schema`](../yb-voyager-cli/#source-db-schema) <br/> [`--oracle-db-sid`](../yb-voyager-cli/#oracle-db-sid)<br/>[`--oracle-tns-alias`](../yb-voyager-cli/##ssl-connectivity)|
