...
- **Enable packed rows** to increase the throughput by more than two times. Enable packed rows on the YugabyteDB cluster by setting the YB-TServer GFlag [`ysql_enable_packed_row`](../../reference/configuration/yb-tserver/#ysql-enable-packed-row) to true.
...
| Enabling packed rows | 3 node RF 3 cluster,<br> c5.4x large (16 cores 32 GB) <br> 1 EBS Type gp3 disk per node,<br> 10000 IOPS,<br> 500 MiB bandwidth | batch-size=20k<br>parallel-jobs=48<br>YB-TServer GFlag: [`ysql_enable_packed_row`](../../reference/configuration/yb-tserver/#ysql-enable-packed-row) = `true` | ~95% | 134048 rows/sec |
