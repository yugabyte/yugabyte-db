...
For a typical deployment, values used for Raft and the write ahead log (WAL) in `yb-tserver` configurations should match the values in [yb-master](../yb-master/#raft-flags) configurations.
