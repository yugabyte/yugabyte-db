...
: When authentication is enabled, the default user is `yugabyte` in YSQL, and `cassandra` in YCQL. `yugabyted` will print a message with the credentials file location in the form of `Credentials File is stored at <credentials_file_path.txt>` when the cluster is started.
