...
Learn how to establish a connection to YugabyteDB database and begin CRUD operations using the steps in [Build an Application](/preview/develop/build-apps/additional-examples/java/ysql-yb-jdbc) in the Quick Start section.
...
This tutorial shows how to use the Yugabyte Psycopg2 driver with YugabyteDB. Start by creating a 3 node cluster with a replication factor of 3. This tutorial uses the [yb-ctl](/preview/admin/yb-ctl/#root) utility.

Next, use Python shell terminal to demonstrate the driver's load balancing features by running few python scripts.
