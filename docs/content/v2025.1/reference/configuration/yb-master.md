...
Enables YSQL backends to generate and consume invalidation messages incrementally for schema changes. When this knob is enabled, invalidation messages are propagated via the `pg_yb_invalidation_messages` per-database catalog table. Details of the invalidation messages generated by a DDL are also logged to the YSQL log when the GUC `log_min_messages` is set to `DEBUG1` or when the GUC `yb_debug_log_catcache_events` is set to true. When this knob is disabled, schema changes cause a full catalog cache refresh on existing backends, that can result in a latency and memory spike on existing YSQL backends. 
