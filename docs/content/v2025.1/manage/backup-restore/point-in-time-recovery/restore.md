...
On a manually managed cluster, step 1 is the [off-cluster restore procedure](#restore-on-another-cluster). In YugabyteDB Anywhere, restore the PITR-enabled backup to another universe instead; see [Restore a PITR-enabled backup](../../../../yugabyte-platform/back-up-restore-universes/restore-universe-data/#restore-a-pitr-enabled-backup).
