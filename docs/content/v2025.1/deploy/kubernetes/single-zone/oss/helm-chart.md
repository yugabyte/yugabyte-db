...
```sh
helm install yugabyte --version {{<yb-version version="v2025.1" format="short">}} --namespace yb-demo --name yb-demo --set storage.master.storageClass=<desired storage class>,storage.tserver.storageClass=<desired storage class> --wait
```
...
```sh
helm repo update
helm upgrade yb-demo yugabytedb/yugabyte --version {{<yb-version version="v2025.1" format="short">}} --wait -n yb-demo
```
