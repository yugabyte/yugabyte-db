...
Note that the `config.json` in the `yugastore` directory stores the connection parameters for YugabyteDB. If you are running YugabyteDB in a different configuration, then you will need to change these parameters.
...
Load the initial data of the Yugastore app into YugabyteDB.
```sh
...
Now you can observe the effects of the read/write operations generated by the load tester by simply going to the YugabyteDB Admin UI at http://127.0.0.1:7000.
