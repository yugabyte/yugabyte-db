---
title: Interval multiplication [YSQL]
headerTitle: Multiplying or dividing an interval value by a real or integral number
linkTitle: interval-number multiplication
description: Explains the semantics of multiplying or dividing an interval value by a number. [YSQL]
menu:
  v2.6:
...
```plpgsql
drop function if exists interval_multiplication_result(interval, numeric) cascade;

create function interval_multiplication_result(i in interval, f in numeric)
  returns table(x text)
  language plpgsql
as $body$
declare
  mm_dd_ss_in      constant interval_mm_dd_ss_t := interval_mm_dd_ss(i);

  mm_model         constant numeric := mm_dd_ss_in.mm*f;
  dd_model         constant numeric := mm_dd_ss_in.dd*f;
  ss_model         constant numeric := mm_dd_ss_in.ss*f;
  ...
  i_actual         constant interval := i*f;
...
```plpgsql
\t on
select x from interval_multiplication_result('2 months'::interval, 1.2345678);
select x from interval_multiplication_result('2 months'::interval, 0.9876543);
\t off
```
...
Notice that because the internal representation for the input has a non-zero value for only the _mm_ field. Then, after both multiplication and division, the intermediate result, of course, still has a non-zero value for only the _mm_ field. However, this non-zero value is now a real number and not an integer. This means that when the ultimate result is constructed, the hard-to-grasp spill-down rules that the section [Modeling the internal representation and comparing the model with the actual implementation](../../interval-representation/internal-representation-model/)) explains kick in. This explains the fact that the ultimate result has non-zero values for the _dd_ and _ss_ fields as well as for the _mm_ field.

This shows that a practice that the user might adopt to use only _interval_ values that have just a single non-zero internal representation field can easily be thwarted by _interval_ multiplication or division. The section [Defining and using custom domain types to specialize the native interval functionality](../../custom-interval-domains/) shows how you can guarantee that you avoid this problem in a way that is consistent with what you can reasonably expect _interval_ multiplication and division to mean.
...
```plpgsql
drop procedure if exists assert_interval_multiplication_model_ok(interval, numeric);

create procedure assert_interval_multiplication_model_ok(i in interval, f in numeric)
  language plpgsql
as $body$
declare
  mm_dd_ss_in      constant interval_mm_dd_ss_t := interval_mm_dd_ss(i);
...
/*
This test causes an assert failure.
...
```plpgsql
select x from interval_multiplication_result('2 months 2 days 2 hours'::interval, 0.96);
```
...
And next, a test where the model result _disagrees_ with the actual result:
...
There's every reason to thing that the model implements a sensible requirements specification and that the actual behavior is an emergent result of what the C PostgreSQL implementation happens to do. Here's a good way to choose the winner. Consider this ordinary principle of algebra and arithmetic:
