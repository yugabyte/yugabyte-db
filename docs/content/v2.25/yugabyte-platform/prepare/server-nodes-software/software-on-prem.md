...
```sh
tar -xvzf node-agent.tar.gz && cd {{<yb-version version="v2.25" format="build">}}/scripts/
```
...
```sh
tar -xf yugabundle-{{<yb-version version="v2.25" format="build">}}-centos-x86_64.tar.gz
cd yugabyte-{{<yb-version version="v2.25" format="build">}}/
```

Extract the node agent package and go to the `scripts` directory:

```sh
tar -xf node_agent-{{<yb-version version="v2.25" format="build">}}-linux-amd64.tar.gz && cd {{<yb-version version="v2.25" format="build">}}/scripts/
```

or

```sh
tar -xf node_agent-{{<yb-version version="v2.25" format="build">}}-linux-arm64.tar.gz && cd {{<yb-version version="v2.25" format="build">}}/scripts/
```
...
The following table describes options that are changed for a typical installation. The file is commented; you can [review the file](https://github.com/yugabyte/yugabyte-db/blob/{{< yb-version version="v2.25" format="short">}}/managed/node-agent/resources/node-agent-provision.yaml) and its default settings on GitHub.
