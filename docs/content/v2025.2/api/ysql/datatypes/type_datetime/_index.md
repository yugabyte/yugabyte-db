...
The "Size" column shows the size of a value's internal representation—the size that PostgreSQL defines, that YSQL inherits, and that _pg_column_size()_ reports. It isn't the size that a value occupies on disk in YugabyteDB, because DocDB adds its own per-value and per-row overhead. The sizes, and the values in the "Min", "Max", and "Resolution" columns, are as PostgreSQL specifies them in [Table 8.9. Date/Time Types](https://www.postgresql.org/docs/15/datatype-datetime.html#DATATYPE-DATETIME-TABLE).
