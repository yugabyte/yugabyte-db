...
Records in the YugabyteDB `ysql_hba.conf` file are autogenerated based on the values included in the `--ysql_hba_conf_csv` flag. For example, starting a YB-TServer with the following `--ysql_hba_conf_csv` flag will enable SCRAM-SHA-256 password authentication for all users except `yugabyte`, which can use trust when connecting from localhost:

```sh
--ysql_hba_conf_csv="host all yugabyte 127.0.0.1/0 trust, host all all 0.0.0.0/0 scram-sha-256, host all yugabyte ::1/128 trust, host all all ::0/0 scram-sha-256"
```
...
Specifies the authentication method to use when a connection matches this record. For password-based authentication, use `scram-sha-256` (the default); avoid `md5`, which is deprecated.
...
Use SCRAM-SHA-256 password authentication. This is the default and recommended method for password-based authentication. For more information, refer to [Password authentication](../password-authentication/).

#### md5

Use MD5 password encryption. Prefer `scram-sha-256` for password authentication. For more information, refer to [Password authentication](../password-authentication/).
...
The following record allows SSL connections to any database as any user from any address using SCRAM-SHA-256 password authentication.
