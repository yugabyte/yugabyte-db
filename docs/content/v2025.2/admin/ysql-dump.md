...
Restoring a plain-text dump also depends on the version of ysqlsh used to replay it, separately from the server version. Dumps taken with ysql_dump in v2025.2.7.0+ and 2026.1.2.0+ begin with the [`\restrict`](../../api/ysqlsh-meta-commands/#restrict-restrict) meta-command, which earlier versions of ysqlsh do not recognize. Replay these dumps with ysqlsh from the same or a later version.
