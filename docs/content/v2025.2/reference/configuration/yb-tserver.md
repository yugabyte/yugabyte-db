...
This flag supports dynamic updates: when set at runtime, it sends a `SIGHUP` signal to the postmaster process to reload the configuration without requiring a server restart.
...
Specifies a comma-separated list of PostgreSQL user name mapping rules that is written to the `ysql_ident.conf` file. These rules map PostgreSQL user names to database user names, typically used for authentication methods like LDAP or Kerberos. To see the current values in the `ysql_ident.conf` file, run the `SHOW ident_file;` statement and then view the file. Because the file is autogenerated, direct edits are overwritten by the autogenerated content.
...
```sh
--ysql_ident_conf_csv='map_x user_foo yb_user_foo'
```

For details on using `--ysql_ident_conf_csv` to configure user name mapping, refer to [User name maps](../../../secure/authentication/host-based-authentication/#user-name-maps) in the Host-based authentication documentation.
