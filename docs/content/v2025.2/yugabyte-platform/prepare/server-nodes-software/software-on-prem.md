...
```sh
tar -xvzf node-agent.tar.gz && cd {{<yb-version version="v2025.2" format="build">}}/scripts/
```
...
```sh
wget https://downloads.yugabyte.com/releases/{{<yb-version version="v2025.2" format="long">}}/yba_installer_full-{{<yb-version version="v2025.2" format="build">}}-linux-x86_64.tar.gz
tar -xf yba_installer_full-{{<yb-version version="v2025.2" format="build">}}-linux-x86_64.tar.gz
cd yba_installer_full-{{<yb-version version="v2025.2" format="build">}}/
```

Extract the yugabundle package:

```sh
tar -xf yugabundle-{{<yb-version version="v2025.2" format="build">}}-centos-x86_64.tar.gz
cd yugabyte-{{<yb-version version="v2025.2" format="build">}}/
```

Extract the node agent package and go to the `scripts` directory:

```sh
tar -xf node_agent-{{<yb-version version="v2025.2" format="build">}}-linux-amd64.tar.gz && cd {{<yb-version version="v2025.2" format="build">}}/scripts/
```

or

```sh
tar -xf node_agent-{{<yb-version version="v2025.2" format="build">}}-linux-arm64.tar.gz && cd {{<yb-version version="v2025.2" format="build">}}/scripts/
```
...
The following table describes options that are changed for a typical installation. The file is commented; you can [review the file](https://github.com/yugabyte/yugabyte-db/blob/{{< yb-version version="v2025.2" format="short">}}/managed/node-agent/resources/node-agent-provision.yaml) and its default settings on GitHub.
...
| `is_configure_clockbound` | [ClockBound](https://github.com/aws/clock-bound) improves clock accuracy and reduces read-restart errors in YSQL. ClockBound requires [chrony](https://chrony-project.org/) to be configured. <br/>The provisioning script installs ClockBound as a systemd unit and configures it to communicate with chronyd. <br/>The script also configures the on-premises provider configuration (see the following options) so that universes created using the provider automatically have the [time_source](../../../../reference/configuration/yb-master/#time-source) flag set to `clockbound`. |
...
| `node_external_fqdn` | The fully qualified domain name (FQDN) or IP address of the node, must be accessible from the YugabyteDB Anywhere server. |
...
```sh
./node-agent-provision.sh  --generate_and_run --provision
```

To do a dry run with the generated configuration file:
