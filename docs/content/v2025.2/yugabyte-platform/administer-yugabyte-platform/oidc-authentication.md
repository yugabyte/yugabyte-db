...
By default, you can't assign the SuperAdmin role to an OIDC group or via role bindings to OIDC users. To allow this, set the **Allow SuperAdmin in LDAP/OIDC group and user role mappings** Global Runtime Configuration option (config key `yb.security.allow_superadmin_user_group_mapping`) to true. Refer to [Manage runtime configuration settings](../manage-runtime-config/). Only a Super Admin can change this global setting, and only a Super Admin can assign the SuperAdmin role when the setting is enabled.

Keep this setting disabled unless you require SuperAdmin access for OIDC users. Prefer mapping a dedicated OIDC group to SuperAdmin rather than assigning the role broadly.

Local Super Admin users can't sign in using SSO. OIDC-provisioned Super Admin users can sign in through SSO as usual.

If you remove all local users and rely only on OIDC Super Admins, an IdP outage can lock you out of YugabyteDB Anywhere. Keep at least one local Super Admin, or recover access as described in [Recover Super Admin access](../../install-yugabyte-platform/create-admin-user/#recover-super-admin-access).
