---
title: Test the date-time multiplication overloads [YSQL]
headerTitle: Test the date-time multiplication overloads
...
```plpgsql
drop procedure if exists test_date_time_multiplication_overloads(text) cascade;

create procedure test_date_time_multiplication_overloads(i in text)
  language plpgsql
as $body$
declare
...
as $body$
begin
  call test_date_time_multiplication_overloads(i);
...
The final anonymous block finishes silently, confirming that multiplication is not supported between a pair of _date-time_ data types.
