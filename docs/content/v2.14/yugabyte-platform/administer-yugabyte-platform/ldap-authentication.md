...
- Prefix to the common name (CN) of the user `yb.security.ldap.ldap_dn_prefix`, as follows:
  ...
  <br>Note that YugabyteDB Anywhere combines `ldap_basedn` and `ldap_dn_prefix` with the username provided during log in to query the LDAP server. `ldap_basedn` and `ldap_dn_prefix` should be set accordingly.
