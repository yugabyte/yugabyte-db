...
Learn how to establish a connection to YugabyteDB database and begin CRUD operations using the steps from [Build a Node.js Application](/preview/tutorials/quick-start/build-apps/nodejs/ysql-pg) in the Quick start section.
...
Because a YugabyteDB Managed cluster is always configured with SSL/TLS, you don't have to generate any certificate but only set the client-side SSL configuration. To fetch your root certificate, refer to [CA certificate](/preview/tutorials/quick-start/build-apps/go/ysql-pgx/#ca-certificate).
