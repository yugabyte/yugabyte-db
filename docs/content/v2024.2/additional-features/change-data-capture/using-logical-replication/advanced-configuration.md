...
If CDC retention barriers remain stuck on a tablet after dropping a replication slot (for example, on index tablets that are not part of the publication), use the [cdc_release_barriers_on_tablet](../../../../admin/yb-ts-cli/#cdc-release-barriers-on-tablet) `yb-ts-cli` command on each YB-TServer that hosts the peer of the tablet.
{{< /warning >}}
