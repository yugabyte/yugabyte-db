...
If [`Restart read required error`](https://docs.yugabyte.com/stable/architecture/transactions/read-restart-error/) is encountered while running `yb_index_check()`, set the following parameter and then re-run `yb_index_check()`:
