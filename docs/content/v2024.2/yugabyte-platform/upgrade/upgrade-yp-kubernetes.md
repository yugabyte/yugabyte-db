...
1. Run the following `helm upgrade` command to enable the YBA upgrade:
    ...
    ```sh
    helm upgrade yba yugabytedb/yugaware --version 2024.1.0 --set kubernetesOperatorEnabled=true,yugaware.yugaware.kubernetesOperatorNamespace=yb-platform-test
    ```
