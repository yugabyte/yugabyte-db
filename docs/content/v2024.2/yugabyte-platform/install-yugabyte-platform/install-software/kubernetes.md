...
1. Run the following `helm repo add` command to clone the [YugabyteDB charts repository](https://charts.yugabyte.com/):
    ...
    ```sh
    helm search repo yugabytedb/yugaware --version {{<yb-version version="v2024.2" format="short">}}
    ```
    ...
    yugabytedb/yugaware {{<yb-version version="v2024.2" format="short">}}          {{<yb-version version="v2024.2" format="build">}}  YugaWare is YugaByte Database's Orchestration a...
    ```

1. Run the following `helm install` command to install the YugabyteDB Anywhere (`yugaware`) Helm chart:

    ```sh
    helm install yw-test yugabytedb/yugaware --version {{<yb-version version="v2024.2" format="short">}} -n yb-platform --wait
    ```
...
By default, the Helm chart will attempt to create a service account that has certain ClusterRoles listed [here](https://github.com/yugabyte/charts/blob/master/v2024.2/yugaware/templates/rbac.yaml#L166). These roles are used to do the following:
