...
If you want to change the defaults, you can use the following command:

```sh
helm upgrade --set resource.tserver.requests.cpu=8,resource.tserver.requests.memory=15Gi yb-demo ./yugabyte
```

You can even do `helm install` instead of `helm upgrade` when you are installing on a Kubernetes cluster with configuration different than the defaults.

You can change replica count using the following command:
...
```sh
helm install yugabyte --version {{<yb-version version="v2024.2" format="short">}} --namespace yb-demo --name yb-demo --set storage.master.storageClass=<desired storage class>,storage.tserver.storageClass=<desired storage class> --wait
```
...
You can upgrade the software on the YugabyteDB cluster with the following command. By default, this performs a [rolling update](https://github.com/yugabyte/charts/blob/853d7ac744cf6d637b5877f4681940825beda8f6/v2024.2/yugabyte/values.yaml#L60) of the pods.

```sh
helm repo update
helm upgrade yb-demo yugabytedb/yugabyte --version {{<yb-version version="v2024.2" format="short">}} --wait -n yb-demo
```
...
You can update most settings in the helm chart by running a `helm upgrade` with the new values. By default, this performs a [rolling update](https://github.com/yugabyte/charts/blob/853d7ac744cf6d637b5877f4681940825beda8f6/v2024.2/yugabyte/values.yaml#L60) of the pods.
