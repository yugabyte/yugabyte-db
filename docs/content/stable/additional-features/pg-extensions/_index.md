...
| [pg_trgm](/stable/develop/learn/text-search/similarity-matching/#trigrams) | Provides functions and operators for determining the similarity of alphanumeric text based on trigram matching, as well as index operator classes that support fast searching for similar strings.<br/>For more information, see [pg_trgm](https://www.postgresql.org/docs/15/pgtrgm.html) in the PostgreSQL documentation. |
| [postgres_fdw](extension-postgres-fdw/) | Provides the foreign-data wrapper postgres_fdw, which can be used to access data stored in external PostgreSQL or YugabyteDB servers, and to query [cluster-wide database views](../../explore/observability/cluster-wide-db-views/) across the local cluster. |
...
| [pg_dist_rag](/stable/develop/AI/distributed-rag/){{<tags/feature/tp>}} | Pre-bundled | Manages distributed RAG pipelines with integrated document preprocessing and embedding generation. |
