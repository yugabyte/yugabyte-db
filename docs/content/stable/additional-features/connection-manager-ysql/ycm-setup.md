...
| ysql_enable_read_request_cache_for_connection_auth | Uses the YB-TServer response cache during connection authentication to prefetch authorization catalog metadata (for example, role and database ACL information), reducing YB-Master RPC overhead and improving connection acquisition latency when YSQL Connection Manager is enabled. Requires `ysql_enable_read_request_caching` to be enabled (default: true). Not supported when login profiles are enabled (`ysql_enable_profile`). Auth cache entries are refreshed according to `pg_cache_response_trust_auth_lifetime_limit_ms` (default: 60000 ms).<br>Default: false|
