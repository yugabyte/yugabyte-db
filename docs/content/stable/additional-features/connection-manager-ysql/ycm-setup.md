...
| ysql_conn_mgr_use_auth_backend | In "auth backend" mode, each incoming authentication request is handled by a freshly spawned postgres backend. When disabled, the "auth passthrough" mechanism is used instead, wherein backends from a pool of "control backends" are reused for all authentication requests, potentially reducing new connection acquisition latency. <br>Default: true |
