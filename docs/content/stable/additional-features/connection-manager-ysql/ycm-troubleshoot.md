...
- HBA authentication. Connection Manager does not support the _server-side_ HBA auth method `cert`, where the server authenticates the user via the client's certificate (CN/DN mapping). Client SSL modes `verify-ca` and `verify-full` (where the _client_ verifies the _server's_ certificate) are supported when connecting through the YSQL Connection Manager. Supporting the server-side cert-based authentication would require forwarding client certificates to the database over Unix socket backend connections, which is not yet implemented.
