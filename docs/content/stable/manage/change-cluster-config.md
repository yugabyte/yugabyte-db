...
`DEAD` servers will show up in the yb-tserver list in yb-master as an artifact of the cached state in the current yb-master leader's memory. 
This cache is cleared after 24 hours by default (see [hide_dead_node_threshold_mins](../../reference/configuration/yb-master/#hide-dead-node-threshold-mins)). The quickest way to refresh this is to elect a new yb-master leader. 

The `yb-admin` cli provides us a subroutine to do exactly that with [`master_leader_stepdown`](../../admin/yb-admin/#master-leader-stepdown). 
