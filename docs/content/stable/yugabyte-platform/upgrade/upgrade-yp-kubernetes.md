...
Starting in YugabyteDB Anywhere v2026.1.2, Docker images are STIG-compliant and hardened, and run as a non-root user by default. If you haven't pinned a user in `securityContext` on a non-OpenShift installation, upgrading to v2026.1.2 or later migrates the pods to a non-root user. To keep running as root on a non-OpenShift installation, see [Run containers as non-root](../../install-yugabyte-platform/install-software/kubernetes/#run-containers-as-non-root). This does not apply to OpenShift, which always runs containers as non-root.
