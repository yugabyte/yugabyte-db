...
The Metrics log export feature is {{<tags/feature/ea idea="441">}}. To enable the feature in YugabyteDB Anywhere, set the **Enable Metrics Logging** Global Configuration option (config key `yb.universe.metrics_export_enabled`) to true. Refer to [Manage runtime configuration settings](../../administer-yugabyte-platform/manage-runtime-config/). Note that only a Super Admin user can modify Global configuration settings. The flag can't be turned off if audit logging is enabled on a universe.

Create an export configuration. A configuration defines the sign in credentials and settings for the tool that you want to export your logs to. Refer to [Integrations](../anywhere-export-configuration/).
