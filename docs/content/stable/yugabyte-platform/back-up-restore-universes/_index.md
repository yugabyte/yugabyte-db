...
- {{<tags/feature/ea idea="1484">}} Backups and high DDL activity. Starting with YugabyteDB version 2025.1.1, taking YSQL backups during DDLs are supported. While in Early Access, this feature is not available in YugabyteDB Anywhere by default. To enable it, set the **Enable backups during DDL** Global Runtime Configuration option (config key `yb.backup.enable_backups_during_ddl`) to true. Refer to [Manage runtime configuration settings](../administer-yugabyte-platform/manage-runtime-config/). You must be a Super Admin to set global runtime configuration flags.
