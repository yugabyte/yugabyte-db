...
If `yb.xcluster.db_scoped.automatic_ddl.creationEnabled` is `false` on the DR primary while `yb.xcluster.db_scoped.creationEnabled` remains `true`, new configurations use semi-automatic mode for supported versions. For earlier YugabyteDB versions than {{<release "2025.2.1">}}, semi-automatic or manual mode applies as described in this section. To learn where to set these keys and their default behavior, refer to [Universe runtime configuration for DB-scoped DR](./disaster-recovery-setup/#universe-runtime-configuration-for-db-scoped-dr) and [Manage runtime configuration settings](../../administer-yugabyte-platform/manage-runtime-config/).
