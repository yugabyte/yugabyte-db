...
headContent: Restore from full or incremental backups, or to a point in time
aliases:
...
Using YugabyteDB Anywhere, you can restore from a backup to a target universe.

You can perform the following types of restore:

- Restore the full backup, or a specific increment if the backup is incremental.
- Restore to a point in time, if the backup was created with a [PITR-enabled scheduled backup policy](../schedule-data-backups/#create-a-scheduled-backup-policy). You cannot restore to a time earlier than the most recent DDL change.
...
To access backups from a specific universe, navigate to the universe and choose **Backups > Backups**.

To access all backups for all universes, navigate to **Backups**.

To restore YugabyteDB universe data from a backup, do the following:
...
1. Choose the scope for the restore:

    - Click **Restore Entire Backup** to restore from the selected backup (the latest in an incremental chain).
    - If the backup includes incremental backups, click **Restore to this point** for the increment at which you want to restore.

    This displays the **Restore Backup** dialog.

    ![Restore backup](/images/yp/restore-backup-pitr.png)

1. Select the Keyspaces/Databases you want to restore. You can restore all databases/keyspaces, or a single database/keyspace.
...
1. If you are restoring a single YCQL keyspace, you can choose the tables in the keyspace you want to restore. (For instructions on restoring a single table in YSQL, refer to [Restore a single table in YSQL](../restore-ysql-single-table).)

1. If the backup is [configured for point-in-time recovery](../schedule-data-backups/#create-a-scheduled-backup-policy), you can restore to the backup time, or to any earlier point in time.

    Select the **An earlier point in time** option to show the available restore window (start and end times) for the restoration.
...
1. If you are restoring data from a universe that has, or previously had, [encryption at rest enabled](../../security/enable-encryption-at-rest), select the KMS configuration to use so that the master keys referenced in the metadata file can be retrieved.
