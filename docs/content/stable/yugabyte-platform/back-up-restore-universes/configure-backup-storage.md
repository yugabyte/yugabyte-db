...
1. Attach the KSA to database pods using provider or universe Helm overrides. For example:
    ...
    For details, see [EKS service account](../../scale-deployments/edit-helm-overrides/#eks-service-account). If you use the [YugabyteDB Kubernetes Operator](../../anywhere-automation/yb-kubernetes-operator/), set the same override under `spec.kubernetesOverrides`.
...
Saving these overrides in YBA and applying them to the universe is what makes the pod identity persistent. See [Helm overrides](../../scale-deployments/edit-helm-overrides/#configure-overrides).
...
1. Attach the KSA to database pods using provider or universe Helm overrides:
    ...
    For details, see [GKE service account](../../scale-deployments/edit-helm-overrides/#gke-service-account). To upgrade an existing universe, see [Upgrade universes for GKE service account-based IAM](../../scale-deployments/edit-helm-overrides/#upgrade-universes-for-gke-service-account-based-iam).
