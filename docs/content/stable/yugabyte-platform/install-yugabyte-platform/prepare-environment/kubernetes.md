...
1. Prefer a storage class based on remote volumes like cloud provider disks instead of [local storage volumes attached directly to the kubernetes node](https://kubernetes.io/docs/concepts/storage/volumes/#local) or [local ephemeral volumes](https://kubernetes.io/docs/concepts/storage/ephemeral-volumes/). Local storage provides great performance, but the data is not replicated and can be lost if the node fails or undergoes maintenance. This can cause a full remote bootstrap of YugabyteDB data within a pod when the pod is moved to a different node during common maintenance operations like cluster node pool upgrades or maintenance operations like rolling software upgrades. [It is not recommended to use local storage volumes for production use cases](../../../deploy/kubernetes/best-practices/#local-versus-remote-ssds).
