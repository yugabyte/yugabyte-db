...
- Using auth-backends (`ysql_conn_mgr_use_auth_backend=true`) for the authentication of logical connections results in higher connection acquisition latencies. It is still recommended to consult the YSQL Connection Manager Dev team before using the auth-passthrough (`ysql_conn_mgr_use_auth_backend=false`) implementation. [#25313](https://github.com/yugabyte/yugabyte-db/issues/25313)
