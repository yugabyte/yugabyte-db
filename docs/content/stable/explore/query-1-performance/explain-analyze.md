...
- **Timings**: At the end of the plan tree, YugabyteDB will add multiple time taken metrics when the [DIST](../../../api/ysql/the-sql-language/statements/perf_explain/#dist) option is specified. These are aggregate times across all plan nodes. Some of them are,
    ...
    - _Storage Read Execution Time_: The total time spent by the YSQL query layer in waiting to receive data from the DocDB storage layer. This is measured as the sum of wait times for individual RPCs to read tables and indexes. The YSQL query layer often pipelines requests to read data from the storage layer, and so the _Storage Read Execution Time_ is often lower than the sum of round-trips times to read tables and indexes.
    - _Storage Write Execution Time_: The sum of all round-trip times taken to flush buffered write requests to tables and indexes.
