...
Live migration for PostgreSQL source databases (using YugabyteDB Connector) is {{<tags/feature/ga>}}.
...
To speed up the CDC phase on write-heavy tables with unique indexes, see [Improve live-migration streaming (CDC) performance](../../reference/performance/#improve-live-migration-streaming-cdc-performance).
