...
1. Install PostgreSQL 18 (required for `pg_dump` and `pg_restore`) using Homebrew:

    ```sh
    brew install postgresql@18
    ```

    After installing PostgreSQL 18, Homebrew displays instructions on how to add the PostgreSQL 18 binaries to your shell's PATH.
    These steps vary depending on your system's Homebrew installation path (for example: `/opt/homebrew` or `/usr/local`) and the shell you are using.
      ...
      1. After the update, restart your terminal and verify that both report to PostgreSQL 18:
