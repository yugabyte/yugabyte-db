...
- Fixed an issue in live migration (fall-back or fall-forward streaming) where INTERVAL columns were not being handled correctly during value conversion, causing import-data to error out.

- Fixed an issue in live migration where prepared statement name collisions occurred when schema or table identifiers exceeded PostgreSQL's 63-character limit. The fix ensures distinct statement names are generated for different tables and operations even with long identifiers.
