...
| Behavior when the queue is full | New `NOTIFY` calls fail (return an error) until the queue drains. | The _slowest listener_ on that TServer is terminated so the queue tail can advance. Applications must handle disconnections by reconnecting and re-issuing `LISTEN`. |
