...
- If the name refers to a _procedure_, `ALTER ROUTINE` has the same effect as [`ALTER PROCEDURE`](../ddl_alter_procedure).
- If the name refers to a _function_, `ALTER ROUTINE` has the same effect as [`ALTER FUNCTION`](../ddl_alter_function).
...
For additional detail, see the [PostgreSQL documentation](https://www.postgresql.org/docs/15/sql-alterroutine.html).
