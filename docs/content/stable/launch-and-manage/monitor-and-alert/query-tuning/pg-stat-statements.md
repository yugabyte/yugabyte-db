...
| `yb_enable_pg_stat_statements_rpc_stats` | boolean | false | Enables RPC execution time stats for pg_stat_statements. When enabled, populates the DocDB-related columns, `docdb_wait_time` and  `catalog_wait_time`. This is a runtime flag that can be changed without restarting the server. |
