...
1. Enable PITR on all replicating databases on both Primary and Standby universes by following the steps in [Set up replication](../async-transactional-setup-semi-automatic/#set-up-replication).

1. Set up xCluster Replication from the ACTIVE to STANDBY universe (B to A) by following the steps in [Set up replication](../async-transactional-setup-semi-automatic/#set-up-replication).
...
To verify replication, see [Verify replication](../async-transactional-setup-semi-automatic/#verify-replication).
