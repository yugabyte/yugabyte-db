...
Monitor your cluster load using the **Load** dashboard. The Load dashboard displays data from the [Active Session History](../../../launch-and-manage/monitor-and-alert/active-session-history-monitor/) in the cluster load chart. (This is also the primary data source used for the [automatic anomaly detection](../aeon-anomalies/).) Use this view to answer the question _Was the system overloaded, and why_.
...
Load monitoring is only available for clusters running v2024.2 and later.
...
To view cluster load, navigate to the cluster **Performance** tab and choose **Load**. You can choose between **Overall Load** and **Load by Database**.
...
The top section shows the cluster load chart. The bars show the number of active connections to the cluster over time. These connections can be YSQL (Postgres) processes or TServer threads. The black line represents the number of running queries over time.
...
Select a query in the list to view details. This includes cluster load for the query, performance metrics, and tables and tablets accessed.

## Load by Database

The top section shows the Activity by Database chart, showing the number of active sessions for each cluster database through time.
