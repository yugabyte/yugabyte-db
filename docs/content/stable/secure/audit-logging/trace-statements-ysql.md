...
- Set the `log_line_prefix` PostgreSQL server option to log timestamp, PostgreSQL PID, and session identifier.
    ...
    For information on setting server options in YugabyteDB, refer to [Configuration parameters](../../../reference/configuration/yb-tserver/#postgresql-configuration-parameters).

## Review logs

Session information is written to the PostgreSQL logs, located in the YugabyteDB base folder in the `yb-data/tserver/logs` directory. For more information on logs, refer to [Understand YugabyteDB logging](../../../explore/observability/logging/).
