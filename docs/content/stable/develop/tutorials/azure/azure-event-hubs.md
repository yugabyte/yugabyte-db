...
1. Review the Node.js sample application.
    ...
    This application initializes a connection pool to connect to the YugabyteDB cluster using the [YugabyteDB node-postgres smart driver](/stable/develop/drivers-orms/nodejs/yugabyte-node-driver/). It then randomly inserts records into the _orders_ table at a regular interval.
