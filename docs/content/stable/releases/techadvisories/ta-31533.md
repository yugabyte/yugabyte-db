...
Use the [verification script](https://gist.github.com/hulien22/5dafa71ee9aeb805f9225aeac9db93c9) to check whether your universe (and some particular tables) are in a problematic state. You can run this script either before or after upgrading; running it before upgrading is _strongly_ recommended.
...
* For each affected table, do the following:
    ...
    1. Run the following `ADD CONSTRAINT` on the source and on the target. For bidirectional setups, run on both sides:

        ```sql
        ALTER TABLE <table> ADD CONSTRAINT xcluster_schema_verification_fix CHECK (true) NOT VALID;
        ```

    1. After step 1 is complete, run the following `DROP CONSTRAINT` on the source and on the target. For bidirectional setups, run on both sides:
    ...
    1. Upgrade (to any version). After upgrading, you will never encounter this problem again, even with additional DDLs.

### After you upgrade

If, after upgrading, your universe is hitting the crash loop:

1. Set the YB-Master flag `xcluster_skip_schema_compatibility_checks_on_alter` to `true` on the YB-Masters to stop the crash looping.
1. Delete and recreate your xCluster setup.
1. Set the `xcluster_skip_schema_compatibility_checks_on_alter` flag back to `false` on YB-Masters.
