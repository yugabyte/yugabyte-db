...
1. Using the `yb_index_check`, check each expression index.
...
- Disable the optimization to skip index updates by setting the `yb_skip_redundant_update_ops` TServer flag to false for the duration of the expression index creation.
...
When the attribute `rd_indexprs` is empty at the time of examination, the planner concludes that the given index has no expressions. The bug causes `rd_indexprs` to be not populated, leading to an incorrect assumption by the planner that the given index has no expressions, which then causes the updates to the Index to be missed. This state occurs during concurrent index creation, where the index is ready for inserts (indisready=true) but not yet valid (indisvalid=false).
