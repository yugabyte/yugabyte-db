...
If you use YugabyteDB Anywhere (YBA), run the [unified mitigation script](https://github.com/yugabyte/yb-tools/blob/add/cdc-index-barrier/cdc-index-barrier/mitigate-cdc-barriers.sh) from the YBA host machine. This script wraps the detection and cleanup scripts described in [Example](#example) and runs them on every YB-TServer, detecting affected tablets and releasing CDC barriers to mitigate the problem.
...
To detect this issue, run the [detection script](https://github.com/yugabyte/yb-tools/blob/add/cdc-index-barrier/cdc-index-barrier/detect_cdc_barrier_on_indexes.sh) on every YB-TServer. The script takes Master addresses and the names of indexes as input. It analyzes the tablets belonging to these indexes and prints the tablet IDs on which CDC has set retention barriers in the output.
