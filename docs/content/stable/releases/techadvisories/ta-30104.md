...
- In case of a partial index, rows that satisfy the partial index predicate are updated by the query so they no longer satisfy the predicate, or vice versa.
- In case of an expression index, the query updates a column that is included in the index.
...
- If the YugabyteDB version does not include a fix for this issue, then avoid `INSERT … ON CONFLICT … DO UPDATE` queries that affect partial or expression indexes.
