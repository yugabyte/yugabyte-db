...
To prepare for v2025.2, update your node provisioning automation and workflows to support the [node agent script](/stable/yugabyte-platform/prepare/server-nodes-software/software-on-prem/). To migrate universe nodes to the new automated provisioning, you can follow the [node patching procedure](/stable/yugabyte-platform/manage-deployments/upgrade-nodes/).
...
* Enables pre-check to ensure the Yugabyte home directory in the on-prem provider matches that on the DB node. PLAT-18366
...
Adds YugabyteDB Anywhere support for [Automatic transactional xCluster replication](/stable/deploy/multi-dc/async-replication/async-transactional-setup-automatic/) enabling seamless replication of schema changes across clusters in a DR setup.
For more information, refer to [xCluster Disaster Recovery](/stable/yugabyte-platform/back-up-restore-universes/disaster-recovery/).

**UI updates to automatically provision on-premises nodes** {{<tags/feature/ga idea="2081">}}.

To improve the out-of-box experience, the UI for creating on-premises providers has been modified to direct users to automatic provisioning. For more information, refer to [Automatically provision on-premises nodes](/stable/yugabyte-platform/prepare/server-nodes-software/software-on-prem/).

Note: Support for [legacy node provisioning](/stable/yugabyte-platform/prepare/server-nodes-software/software-on-prem-legacy/) will be dropped in v2025.2 (available late 2025). Before upgrading to v2025.2, be sure to update your node provisioning workflows to support automatic provisioning.

### New features

* [Backups with DDL](/stable/yugabyte-platform/back-up-restore-universes/). Backups succeed even when DDL changes are occurring during the backup process. {{<tags/feature/tp idea="1484">}}

* [Filter personally identifiable information (PII) in support bundles](/stable/yugabyte-platform/troubleshoot/universe-issues/#use-support-bundles). When creating support bundles, and the option to collect pgAudit logs is selected, sensitive PII data is now masked, reducing risk of data privacy violations and improving data security stance. {{<tags/feature/ga idea="2170">}}

* [Support for AWS's EBS disk encryption](/stable/yugabyte-platform/create-deployments/create-universe-multi-zone/#additional-aws-fields), complementing (or offering an alternative to) YugabyteDB's encryption-at-rest. {{<tags/feature/ea idea="2329">}}

* [Batching of rolling operations](/stable/yugabyte-platform/manage-deployments/edit-config-flags/#batched-rolling-restart). During a rolling restart, such as when applying software upgrades and flag changes, YugabyteDB Anywhere can now process multiple YB-TServer nodes in each availability zone simultaneously. This can reduce the time required to perform rolling operations on large clusters by 2x, 3x, or more, depending on the (configurable) batch size. {{<tags/feature/ga idea="444">}}
<!-- {{<tags/feature/ga idea="1710">}} -->

### Known issue

- [YSQL major upgrade](/stable/yugabyte-platform/manage-deployments/ysql-major-upgrade-yba/) (PostgreSQL 15 upgrade) does not work when PITR is enabled on a universe. You need to disable PITR before the upgrade and re-enable it after finalizing or rolling back the upgrade. See [limitations](/stable/yugabyte-platform/manage-deployments/ysql-major-upgrade-yba/#limitations).
