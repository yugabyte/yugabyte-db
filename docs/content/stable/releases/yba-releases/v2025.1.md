...
To prepare for v2025.2, update your node provisioning automation and workflows to support the [node agent script](/v2025.1/yugabyte-platform/prepare/server-nodes-software/software-on-prem/).

To migrate universe nodes to the new automated provisioning, you can follow the [node patching procedure](/v2025.1/yugabyte-platform/manage-deployments/upgrade-nodes/).
...
* Hides LDAP passwords in flags in support bundles. PLAT-18363
...
* Allows editing PITR retention period directly from the DR configuration modal in YBA UI. PLAT-15144
...
**Automatic provisioning of on-premesis VMs via new node agent script** {{<tags/feature/ga idea="1803">}}

Provisioning of VMs for on-premises providers and their universes is radically simplified using the `node-agent-provision.sh` script. The script:

- sets ulimits and transparent hugepages;
- prepares the node for YugabyteDB, including installing node agent;
- creates an on-premises provider; and
- adds the node to the provider free pool.

Automatic provisioning via the node agent script replaces all legacy provisioning methods for on-premises providers. For more information, refer to [Automatically provision on-premises nodes](/v2025.1/yugabyte-platform/prepare/server-nodes-software/software-on-prem/#run-the-provisioning-script).
...
* Before [upgrading from PostgreSQL 11 to PostgreSQL 15](/v2025.1/yugabyte-platform/manage-deployments/ysql-major-upgrade-yba/) for a multi-region universe, or a universe with more than 10,000 tables and indexes, increase the catalog upgrade timeout by setting the YugabyteDB Anywhere runtime configuration parameter `yb.upgrade.wait_attempts_for_major_catalog_upgrade` to 60 or higher at the Universe scope. This prevents timeout issues during the PostgreSQL 15 catalog upgrade process.
...
The `XClusterConfig` object returned by this API contains a list of namespaces. Each object in this list (XClusterNamespaceConfig) now includes three new required properties as described in the following table:
...
* Kubernetes
  ...
  * [YugabyteDB Kubernetes Operator](/v2025.1/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/). A powerful tool designed to automate deploying, scaling, and managing YugabyteDB clusters in Kubernetes environments. It streamlines database operations, reducing manual effort for developers and operators. {{<tags/feature/ga idea="1448">}}
...
* Fixes Hashicorp vault's KMS configuration to retain custom key names on edits and displays the key name under KMS configuration's Show Details area. PLAT-14968,PLAT-14966
...
* Adds a bootstrap summary to the DR configuration creation modal to clarify which tables will be bootstrapped. PLAT-15973
...
* Enhances node agent installation to be idempotent when using the node agent provisioning script. PLAT-17141
...
* Fixes an issue with listing customer configurations when DR metadata is inconsistent. PLAT-16026
...
* Enables `postmaster_cgroup` flag based on user intent without needing additional runtime configurations. PLAT-16142
...
* Increases GET DR configuration API reliability on unhealthy universes. PLAT-16169
...
* Ensures collect_metrics_wrapper.sh aligns with the node_exporter metrics path. PLAT-16581
...
* Blocks creation of cron-based universes when using node agent provisioning to prevent health check failures. PLAT-16879
...
* Appends `node_ip` to the configuration file to prevent race conditions. PLAT-16960
...
* Enables correct THP parameter settings in Ansible and node agent provisioning. PLAT-17167,PLAT-171678,PLAT-17171
...
* Adds YugabyteDB package support to the node agent provisioning module. PLAT-17260
* Allows configuring the SSHD daemon via node agent provisioning for custom SSH ports. PLAT-17283
* Corrects counting of failed tables for DR error banners. PLAT-17348
* Enhances handling and defaults of YBA installer configurations. PLAT-17361,PLAT-17426
...
* Enables database-scoped xCluster configuration creation by default. PLAT-16514
...
* Prompts users for confirmation if an existing configuration file will be overwritten. PLAT-16617
...
* Adds endpoint to list backup directories for selected storage configuration. PLAT-16900
...
* Updates the node agent provisioning log collection path in the support bundle. PLAT-17129
