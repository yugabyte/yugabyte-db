...
| {{<tags/feature/ga idea="2340">}}[Improvements to Kubernetes cert-manager](/stable/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/) | Added support for scenarios where Kubernetes cert-manager manages only partial certificate chains (specifically, cert-manager only holds the intermediate certificate(s), but not the root certificate).<hr style="margin: 0.5em 0;">**To use**: Feature enabled by default when using cert-manager for encryption in transit. |
