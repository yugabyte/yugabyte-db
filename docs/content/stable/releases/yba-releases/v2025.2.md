...
Update your node provisioning automation and workflows to support the [node agent script](/v2025.2/yugabyte-platform/prepare/server-nodes-software/software-on-prem/).

To migrate universe nodes to the new automated provisioning, follow the [node patching procedure](/v2025.2/yugabyte-platform/manage-deployments/upgrade-nodes/).

{{< /warning >}}

{{< warning title="S3 storage and certificate validation" >}}
v2025.2 and later requires certificate validation for [S3 storage](/v2025.2/yugabyte-platform/back-up-restore-universes/configure-backup-storage/#amazon-s3). If you are using S3 storage with custom self-signed or CA certificates, to connect to your S3 storage, you must add the certificates to the YugabyteDB Anywhere Trust Store _before upgrading_.
{{< /warning >}}

Before upgrading, review the information in [Prepare to upgrade YugabyteDB Anywhere](/v2025.2/yugabyte-platform/upgrade/prepare-to-upgrade/).

## v2025.2.6.0 - September 4, 2026 {#v2025.2.6.0}

{{< release-downloads version="2025.2.6.0" build="111" type="yba" >}}
...
* Upgrades OpenTelemetry to version 0.145.0 (using a custom fork for consistency across K8s and VM deployments), updates Loki export to otlphttp, and ensures compatibility with Loki 3.0+. PLAT-21676,PLAT-21415
...
* Fixes inaccuracies in the CPU Seconds Total dashboard metrics. PLAT-19856,PLAT-19286
* Adds a 10-second delay and checks boot_id changes to ensure node reboot detection. PLAT-21797
* Ensure node safety by running `AreNodesSafeToTakeDown` before replace and remove operations to prevent issues and add additional protection. PLAT-19594,PLAT-19801
* Create a migration to ensure dedicated universes have `masterDeviceInfo`, and add API validation to prevent their creation if missing. PLAT-21762,PLAT-21764
...
* Upgrades urllib to version 2.7.0 for enhanced security and stability. PLAT-21556
...
* Prevents retention of failed Read Replica state, enabling new creations. PLAT-21386,PLAT-21577
...
* Upgrade YBC version to 2.2.0.3-b22 to support multi-address, ported, and IPv6 `yb_tserver_address` for varz queries. PLAT-21842,PLAT-21843
...
* Automatically restarts `node_exporter` during provisioning if the path changes. PLAT-21095,PLAT-20978
...
* Fix prevents null pointer errors when editing Kubernetes read replica tasks by adding a necessary check for the primary cluster in version v2025.2, allowing successful node scaling without configuration failures. PLAT-22282,PLAT-22250
...
* Reduces xCluster and DR log verbosity by shifting certain logs to debug/trace levels. PLAT-18385
...
* Resets regions only when changed to prevent multi-region providers from forgetting placements. PLAT-20148
...
* Addresses multiple critical vulnerabilities in Yugaware components and upgrades key dependencies. PLAT-20974,PLAT-20976,PLAT-21206,PLAT-20975
* Restores the ability to upload self-signed CA certificates affected by FIPS changes. PLAT-21332
* Enables configurable Master and TServer debug hooks in Kubernetes setups. PLAT-21390
* Adds optional delay to prevent TServer stops during leader re-elections. PLAT-21391
...
* Enables editing of leader preference in v2025.2.3 for placement comparison adjustments. PLAT-20847
...
* Increases keep-alive time intervals to avoid GOAWAY errors. PLAT-21235
* Fixes environment variable inheritance in node agent for concurrent execution. PLAT-21276
* Ensures Kubernetes operator correctly identifies universes by UUID to prevent accidental renames and orphans. PLAT-21329
* Enhances replica distribution accuracy by accounting for node count in each AZ. PLAT-21356,PLAT-21355
* Corrects handling of nil values and enhances descriptions in node agent. PLAT-21388
* Ensures `SUDO_ACCESS` check correctly identifies root users. PLAT-21418
* Displays costs for paused clusters to reflect ongoing charges. PLAT-21444,PLAT-21445
...
* Automates changing the installation directory to the Yugabyte user's home directory. PLAT-20210,PLAT-19661,PLAT-20472,PLAT-20506
...
| <div style="width:150px">Feature</div> | Description |
| :-------- | :---------- |
| {{<tags/feature/ga idea="441">}}[Export metrics](/v2025.2/yugabyte-platform/alerts-monitoring/anywhere-export-configuration/#dynatrace) | Support for exporting universe metrics to third-party tools for analysis and customization, and adds Dynatrace as a metric export sink. {{<tags/feature/ea idea="441">}} <hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. On the universe **Metrics** tab, click the gear icon, choose **Export Metrics** and enable the **Export Metrics from this Universe** option. |
|{{<tags/feature/ga idea="394">}} [Backup and restore of roles and grants](/v2025.2/yugabyte-platform/back-up-restore-universes/back-up-universe-data/#create-backups)| Backups now capture PostgreSQL users (aka roles) and their permissions (aka grants).<hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. You can set role backup options as part of creating backups for your universe.  |
| {{<tags/feature/ga idea="2133">}} [Improved time synchronization across nodes for on-premises, AWS, and GCP environments](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone/#configure-clockbound-optional) | Time synchronization across nodes has been enhanced through the use of the [ClockBound](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone/#configure-clockbound-optional) library, resulting in improved P99 latency for certain workloads (those with near simultaneous writes and reads). <hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. ClockBound is automatically configured during node provisioning, and the universe creation task sets the [time_source](/v2025.2/reference/configuration/yb-master/#time-source) flag to `clockbound`.|
...
* Enables `Not preferred` value for leaderPreferred zones in cluster placement. PLAT-20193,PLAT-19459
* Enables backups during DDL operations by default. PLAT-19895
...
* Enforces strict INI parsing in Golang to prevent configuration errors and failures. PLAT-19623,PLAT-20023
...
* Adds toggle to disable chunked transfer encoding for S3 in AWS API config. PLAT-20246,PLAT-19375
...
* Upgrades Go dependencies and addresses vulnerabilities in various tools. PLAT-19147
...
* Upgrades key Python dependencies and node_exporter to enhance stability and security. PLAT-19190,PLAT-19679,PLAT-19675,PLAT-19677,PLAT-19695,PLAT-19678
* Upgrades Postgres to version 14.22, fixing multiple CVEs. PLAT-19701
* Resolves an issue with the INI template in the YNP branch, exclusively for backport. PLAT-19898
...
* Stores the `ynp_version` file in the hidden `.yugabyte` folder to prevent accidental deletion. PLAT-20033
...
* Adds node exporter port configuration for on-prem providers using the node agent provisioning script. PLAT-19897,PLAT-20252,PLAT-19804
* Upgrades Prometheus to version 3.10.0, addressing multiple security vulnerabilities. PLAT-19194
* Enhances Terraform support for specifying all keyspaces in scheduled backups. PLAT-17948,PLAT-19311
...
* Fixes an issue with invalid YAML output due to newline removal in $appLabelArgs assignment in templates. PLAT-19169
* Resolves chart rendering issues by correcting remaining instances of erroneous pattern in the template. PLAT-19169
* Blocks editing of provider settings that alter multi-AZ status during migrations. PLAT-19240
* Addresses the circular JSON error in the "Reboot Node" operation and enhances node management with clearer actions and integrated prechecks. PLAT-19504,PLAT-19838
* Ensures the UI displays a FIPS banner when FIPS mode is enabled. PLAT-19861
* Enhances operator import to handle null fields and unknown Kubernetes override fields. PLAT-19944,PLAT-19940,PLAT-19894
...
* Enables using `yugabyte_upgrade` user in TLS universes during PG upgrades preventing upgrade failures related to role issues.  PLAT-20145
* Removes obsolete `platformVersion` from universe details and checks YBA version from `yugaware_property` table. PLAT-20217,PLAT-20213
...
* Enhances metadata file retrieval with added timeout settings and improves thread safety during concurrent access. PLAT-20107,PLAT-20117,PLAT-20106
* Enables successful operator imports by filtering out storage configs used by deleted databases. PLAT-19465
* Allows users to configure CDC SDK replication slot expiry alert thresholds. PLAT-20037
...
* Resolves issues with full move operations when modifying attached volumes on AWS. PLAT-20532, PLAT-20134
* Restores functionality to the "View flags changes" component. PLAT-18893
* Ensures backup schedule UI correctly sends cron expressions and role usage. PLAT-20108
* Disables PostgreSQL log redaction to reduce CPU usage and support bundle creation time. PLAT-20628
* Enables stats dump on YBA by default for version 2025.2.3.0. PLAT-20675
* Fixes VM image upgrade failure by adjusting node agent removal sequence. PLAT-20695, PLAT-20576
* Fixes CPU metrics display for OpenShift by adjusting metric join conditions. PLAT-20743
* Ensures provider creation via CLI succeeds by validating access key inputs. PLAT-20774
* Fixes K8s restore failure by addressing shell script expansion issue. PLAT-20778
* Updates node agent alert to exclude Kubernetes universes. PLAT-20811
* Ensures the 'clockbound' parameter is set correctly during universe creation. PLAT-20841
...
| <div style="width:150px">Feature</div> | Description |
| :-------- | :---------- |
| {{<tags/feature/ga idea="1372">}}[Backup throughput limit](/v2025.2/yugabyte-platform/back-up-restore-universes/back-up-universe-data/#configure-backup-performance-parameters) | You can set a limit for backup throughput in MB/s, providing more precise control over resource allocation.<hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. You can set disk read/write bytes per second as part of configuring backup performance parameters for your universe. |
| {{<tags/feature/ea idea="2004">}}[YugabyteDB Kubernetes Operator import universe](/v2025.2/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#import-universe)| Added a capability to live-migrate an existing K8s universe that is managed by YBA to one that is managed by the K8s Operator. Additional improvements include adding CRDs that allow support for multi-region universes (spanning multiple K8s clusters).<hr style="margin: 0.5em 0;">**To use**: The feature can be used via API only and the K8s operator must be installed on YBA. See [Before you begin](/v2025.2/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#before-you-begin).|
...
| <div style="width:150px">Feature</div> | Description |
| :-------- | :---------- |
| {{<tags/feature/ga idea="153">}}[xCluster DR automatic mode](/v2025.2/yugabyte-platform/back-up-restore-universes/disaster-recovery/#automatic-mode) | Seamless automated replication of YSQL DDL changes across xCluster DR, eliminating the need to manually apply DDLs on both source and target clusters. This simplifies DDL operations and reduces management overhead.<hr style="margin: 0.5em 0;">**To use**: Enabled by default for universes running v2025.2.1 or later. |
| {{<tags/feature/ga idea="22">}}[Automatic YB-Master failover](/v2025.2/yugabyte-platform/manage-deployments/remove-nodes/#automatic-yb-master-failover) | If a node with Master process fails unexpectedly (reducing health, but not causing service downtime), YBA auto-heals the universe by starting a Master process on a suitable surviving node (if one exists).<hr style="margin: 0.5em 0;">**To use**: Enabled by default for universes running v2.20.3 or later. |
...
If a node with Master process fails unexpectedly (reducing health, but not causing service downtime), YBA now auto-heals the universe by starting a Master process on a suitable surviving node (if one exists). For more information, refer to [Automatic YB-Master failover](/v2025.2/yugabyte-platform/manage-deployments/remove-nodes/). -->

### New features

| <div style="width:150px">Feature</div> | Description |
| :-------- | :---------- |
| {{<tags/feature/ea idea="986">}}[Azure Managed Identity for VM-based backups](/v2025.2/yugabyte-platform/back-up-restore-universes/configure-backup-storage/#azure-managed-identity-authentication) | When taking backups of a VM-based universe to Azure Blob Storage, instead of using an SAS token, you can use the Azure IAM roles assigned to YugabyteDB Anywhere and database nodes to authenticate.<hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. Assign a managed identity to your YugabyteDB Anywhere and database node VMs, and create an Azure storage configuration that uses the same managed identity. |
| {{<tags/feature/ga idea="989">}}[Restore to a point in time](/v2025.2/yugabyte-platform/back-up-restore-universes/restore-universe-data/#restore-a-pitr-enabled-backup) | Restore PITR-enabled backups to a specific point in time. Similar to instant database cloning, quickly recover from accidental SQL or CQL human errors that cause data loss or corruption by restoring at a time immediately prior to the incident.<hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. Create a backup schedule with ability to restore to point-in-time. |
| {{<tags/feature/ga idea="2340">}}[Improvements to Kubernetes cert-manager](/v2025.2/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/) | Added support for scenarios where Kubernetes cert-manager manages only partial certificate chains (specifically, cert-manager only holds the intermediate certificate(s), but not the root certificate).<hr style="margin: 0.5em 0;">**To use**: Feature enabled by default when using cert-manager for encryption in transit. |
| {{<tags/feature/ga idea="2559">}}[IO2 EBS Volume Support on AWS](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone/#additional-aws-fields) | Use IO2 EBS volumes for disk storage in universes deployed on AWS.<hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. Select IO2 EBS volumes when creating or modifying universes. |

<!-- - [Improved Azure Managed Identity authentication for VM-based backups](/v2025.2/yugabyte-platform/back-up-restore-universes/configure-backup-storage/#azure-managed-identity-authentication). When taking backups of a VM-based universe to Azure Blob Storage, you can now use the Azure IAM roles assigned to YugabyteDB Anywhere and database nodes for authentication. {{<tags/feature/ea idea="986">}}

- [Restoring PITR-enabled backups into a different universe](/v2025.2/yugabyte-platform/back-up-restore-universes/restore-universe-data/#restore-a-pitr-enabled-backup).
 Previously, backups could only be restored to the moment in time when the backup was taken. Now, you can restore backups at any point in time. Like the [Instant database cloning](/v2025.2/manage/backup-restore/instant-db-cloning/) feature, this feature provides a safety net for rapid recovery from accidental SQL or CQL human errors that cause data loss or corruption. {{<tags/feature/ga idea="989">}}

- Improvements to [Kubernetes cert-manager](/v2025.2/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/) for encryption in transit (EIT). YugabyteDB Anywhere now supports scenarios in which cert-manager manages only partial certificate chains (specifically, cert-manager only holds the root CA, and not the intermediate certs). {{<tags/feature/ga idea="2340">}}

- [IO2 EBS Volume Support on AWS](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone/#additional-aws-fields). YugabyteDB Anywhere now supports IO2 EBS volumes in create and edit universe workflows, enabling more efficient cloud storage to be used with YugabyteDB. {{<tags/feature/ga idea="2559">}} -->
...
For more information, refer to [YSQL Audit Logs](/v2025.2/yugabyte-platform/alerts-monitoring/universe-logging/).
...
For information on deploying on Kubernetes, refer to [Create a multi-zone universe](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone-kubernetes/).
...
For more information, refer to [Support for LDAP/OIDC groups and custom roles in YugabyteDB Anywhere](/v2025.2/yugabyte-platform/administer-yugabyte-platform/oidc-authentication/#use-oidc-groups-with-yugabytedb-anywhere-roles).

**SuperAdmin LDAP/SSO user support** {{<tags/feature/ea idea="17540">}}

YugabyteDB Anywhere supports LDAP and SSO users becoming SuperAdmin through controlled RBAC and group mapping flows. The feature is guarded by the global runtime configuration flag `yb.security.allow_superadmin_user_group_mapping` (default `false`). General availability in v2026.1.2.

For more information, refer to [SuperAdmin LDAP/SSO user support](/stable/yugabyte-platform/administer-yugabyte-platform/superadmin-ldap-sso-users/).
...
Time synchronization across nodes has been enhanced through the use of the [ClockBound](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone/#configure-clockbound-optional) library, resulting in improved P99 latency for certain workloads (those with near simultaneous writes and reads).

For more information, refer to [ClockBound](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone/#configure-clockbound-optional).

### New features

* [CDC Observability](/v2025.2/yugabyte-platform/alerts-monitoring/cdc-observability/). YugabyteDB Anywhere supports monitoring the status of replication slots when using [PostgreSQL Logical Replication Protocol](/v2025.2/additional-features/change-data-capture/using-logical-replication/) for Change Data Capture (CDC). {{<tags/feature/ga idea="709">}}

* [Azure support for multiple Resource Groups (one per region)](/v2025.2/yugabyte-platform/configure-yugabyte-platform/azure/). Added support for Azure configurations with one Azure Resource Group per region (and also one network per Azure Resource Group), a configuration providing better network management and resilience. {{<tags/feature/ga idea="1606">}}

* [Non-root installation of YBA](/v2025.2/yugabyte-platform/install-yugabyte-platform/install-software/installer/#non-sudo-installation). You can install YugabyteDB Anywhere as a de-privileged user; a root-privileged user is no longer required.  This reduces friction and security concerns during initial testing, setup, and proof of concepts. {{<tags/feature/ga idea="1521">}}

* Kubernetes

  * [Kubernetes immutability](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone-kubernetes/#deploy-immutable-yb-controller-ybc). Added support to deploy code via immutable Helm images instead of at runtime, eliminating the use of 'kubectl cp' to push executables into the pod or container. {{<tags/feature/ga idea="1264">}}

  * [Kubernetes service endpoint](/v2025.2/yugabyte-platform/create-deployments/create-universe-multi-zone-kubernetes/#create-a-common-load-balancer-service-for-yb-masters-yb-tservers). For single-region universes in Kubernetes, added support for creating a unified service endpoint. This is in addition to the already-supported one service endpoint per availability zone (AZ). {{<tags/feature/ga idea ="1405">}}

  * [Configurable common names for Kubernetes](/v2025.2/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/#configurable-common-names). You can configure a custom common name suffix for Kubernetes cert-manager. {{<tags/feature/ga idea="2065">}}

  * [Rotate certificates](/v2025.2/yugabyte-platform/security/enable-encryption-in-transit/rotate-certificates/). Enables retry for the TLS toggle task in Kubernetes and resolves node task failure for ClientToNode only universes. {{<tags/feature/ga idea="1714">}}

  * [Operator: Specify placement information](/v2025.2/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#create-a-universe-with-placement-information). Specify `placementInfo` in the YBUniverse CRD to control regional and zonal placement of nodes. {{<tags/feature/ga idea ="2004">}}

* [Export metrics](/v2025.2/yugabyte-platform/alerts-monitoring/anywhere-export-configuration/#dynatrace). Support for exporting universe metrics to third-party tools for analysis and customization, and adds Dynatrace as a metric export sink. {{<tags/feature/ea idea="441">}}

* [CipherTrust support](/v2025.2/yugabyte-platform/security/create-kms-config/ciphertrust-kms/). You can now use Thales CipherTrust as the Key Management System (KMS) for encryption at rest. {{<tags/feature/ga idea="1227">}}

* [Alerts](/v2025.2/yugabyte-platform/alerts-monitoring/alert/). Removed the DB Queue Overflow Alert to reduce unnecessary alert noise and enhance user experience. {{<tags/feature/ea idea="905">}}

* Backup

  {{< warning title="S3 storage and certificate validation" >}}
v2025.2 and later now requires certificate validation for [S3 storage](../../../yugabyte-platform/back-up-restore-universes/configure-backup-storage/#amazon-s3). If you are using S3 storage with custom self-signed or CA certificates, to connect to your S3 storage, you must add the certificates to the YugabyteDB Anywhere Trust Store _before upgrading_.
  {{< /warning >}}

  * [Backups with DDL](/v2025.2/yugabyte-platform/back-up-restore-universes/). Backups succeed even when DDL changes are occurring during the backup process. {{<tags/feature/ea idea="1484">}}

  * Adjusts [backup directory structure](/v2025.2/yugabyte-platform/back-up-restore-universes/back-up-universe-data/#access-backups-in-storage) to include Universe Name, enhancing multi-tenancy and DB portability. {{<tags/feature/ga idea="1888">}}

  * Allows backup authorization for EKS using a [service account](/v2025.2/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#service-account-for-backup) annotated to each universe pod, eliminating the need for managing short-lived credentials. {{<tags/feature/ga idea="1924">}}

  * Backups now capture PostgreSQL users (aka roles) and their permissions (aka grants) (via API). {{<tags/feature/ea idea="394">}}

  * [Back up and restore YBA](/v2025.2/yugabyte-platform/administer-yugabyte-platform/back-up-restore-yba/). Adds support for regularly scheduled backups of your YugabyteDB Anywhere installation for recovery in case of the loss of the node running your YugabyteDB Anywhere instance. Store backups in remote storage buckets, and restore from the YugabyteDB Anywhere UI. {{<tags/feature/tp idea="1429">}}

* [Certify CIS-hardened RHEL 8 and RHEL 9 (x86)](/v2025.2/reference/configuration/operating-systems/#using-cis-hardened-operating-systems) Facilitates running YugabyteDB with a CIS-hardened RHEL 8 and RHEL 9 image, enhancing security measures. {{<tags/feature/ga idea="1193">}} <!--Also IDEA-1716-->
