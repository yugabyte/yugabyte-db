...
<ul class="nav yb-pills">
 <li>
   <a href="https://downloads.yugabyte.com/releases/2025.2.2.0/yba_installer_full-2025.2.2.0-b69-linux-x86_64.tar.gz">
...
| {{<tags/feature/ea idea="1804">}}[User-level OOM killer](/stable/yugabyte-platform/create-deployments/create-universe-multi-zone/#configure-earlyoom-optional)| YugabyteDB Anywhere now supports better handling of database nodes that run out of memory (OOM) by quickly detecting OOMs and intelligently terminating processes. <hr style="margin: 0.5em 0;">**To use**: Feature need to be enabled. <ul><li>To enable EarlyOOM, set the **Enables Earlyoom Installation on Nodes** Global Runtime Configuration option (config key `yb.ui.feature_flags.enable_earlyoom`) to true. </li><li> To enable by default for new universes, set the **Enable earlyoom by default** Global runtime configuration flag (config key `yb.node_agent.enable_earlyoom_by_default`) to true. </li><li> To disable EarlyOOM, set the **Enable earlyoom by default** Global runtime configuration flag (config key `yb.ui.feature_flags.enable_earlyoom`) to false, then perform a [rolling restart](/stable/yugabyte-platform/manage-deployments/edit-config-flags/#modify-configuration-flags) for the change to take effect. </li><li> For existing universes, enable or disable EarlyOOM as needed, then use **Edit Universe > Apply Changes** or a [rolling restart](/stable/yugabyte-platform/manage-deployments/edit-config-flags/#modify-configuration-flags) for the change to take effect.</li></ul> |
| {{<tags/feature/ea idea="2004">}}[YugabyteDB Kubernetes Operator import universe](/stable/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#operator-import-universe)| Added a capability to live-migrate an existing K8s universe that is managed by YBA to one that is managed by the K8s Operator. Additional improvements include adding CRDs that allow support for multi-region universes (spanning multiple K8s clusters).<hr style="margin: 0.5em 0;">**To use**: The feature can be used via API only and the K8s operator must be enabled on YBA. See [Before you begin](/stable/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#before-you-begin).|
