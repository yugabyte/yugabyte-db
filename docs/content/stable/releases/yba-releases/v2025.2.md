...
* Skips access key verification for on-prem universes provisioned with the node agent. PLAT-19412
...
* Allows specifying a custom home directory for automatic on-premises provisioning with the node agent. PLAT-18347
...
* Switches On-premises Node Reprovision task from using Ansible to node agent provisioning. PLAT-18202
...
* Ensures on-premises provisioning with the node agent supports custom home directories. PLAT-18445
...
* Enable users to set latitude and longitude during custom region setup in node agent provisioning. PLAT-16184
