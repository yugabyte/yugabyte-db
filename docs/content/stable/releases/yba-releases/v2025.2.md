...
* [xCluster Disaster Recovery (DR)](/stable/yugabyte-platform/back-up-restore-universes/disaster-recovery/). Enables seamless replication of schema changes across clusters in a DR setup (Automatic mode). {{<tags/feature/ga idea="2089">}}
...
* Allows easier customization and local execution of database tasks, like software and flag upgrades, on on-premises database nodes, making YugabyteDB Anywhere more scalable and debuggable. {{<tags/feature/ga idea="2091">}}

* Backups now capture PostgreSQL users (aka roles) and their permissions (aka grants) (via API). {{<tags/feature/ea idea="394">}}

{{< warning title="Certificate rotation" >}}

For universes with only client-to-node encryption enabled, certificate rotation must be performed using a rolling or non-rolling upgrade process. Avoid using the non-restart option for certificate rotation in this configuration.
