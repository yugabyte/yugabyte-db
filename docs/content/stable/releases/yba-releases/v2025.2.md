...
* Rejects xCluster restarts when target tables are already in another replication to ensure replication consistency. PLAT-19929. See [API/OpenAPI change](#api-openapi-change).
...
* Adds a flag to bypass credential validation on restore in YBC. PLAT-20205,PLAT-20360
...
* Reverts `bootstrapParams` as a required field in Swagger API docs to maintain API consistency in patch releases. PLAT-21685

### API/OpenAPI change

**Affected endpoint** : `POST /api/v1/customers/{cUUID}/xcluster_configs/{xccUUID}` (restart xCluster configuration)

The OpenAPI schema now lists `bootstrapParams` as required on `XClusterConfigRestartFormData`. Runtime behavior is unchanged; requests without `bootstrapParams` were already rejected with HTTP 400 (bootstrapParams / error.required) via existing form validation. Clients that already send bootstrapParams on restart are unaffected.
