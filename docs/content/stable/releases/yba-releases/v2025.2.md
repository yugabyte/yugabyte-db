...
Update your node provisioning automation and workflows to support the [node agent script](/stable/yugabyte-platform/prepare/server-nodes-software/software-on-prem/).

To migrate universe nodes to the new automated provisioning, follow the [node patching procedure](/stable/yugabyte-platform/manage-deployments/upgrade-nodes/).

{{< /warning >}}

{{< warning title="S3 storage and certificate validation" >}}
v2025.2 and later requires certificate validation for [S3 storage](/stable/yugabyte-platform/back-up-restore-universes/configure-backup-storage/#amazon-s3). If you are using S3 storage with custom self-signed or CA certificates, to connect to your S3 storage, you must add the certificates to the YugabyteDB Anywhere Trust Store _before upgrading_.
{{< /warning >}}
...
| <div style="width:150px">Feature</div> | Description |
| :-------- | :---------- |
| {{<tags/feature/ga idea="441">}}[Export metrics](/stable/yugabyte-platform/alerts-monitoring/anywhere-export-configuration/#dynatrace) | Support for exporting universe metrics to third-party tools for analysis and customization, and adds Dynatrace as a metric export sink. {{<tags/feature/ea idea="441">}} <hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. On the universe **Metrics** tab, click the gear icon, choose **Export Metrics** and enable the **Export Metrics from this Universe** option. |
|{{<tags/feature/ga idea="394">}} [Backup and restore of roles and grants](/stable/yugabyte-platform/back-up-restore-universes/back-up-universe-data/#create-backups)| Backups now capture PostgreSQL users (aka roles) and their permissions (aka grants).<hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. You can set role backup options as part of creating backups for your universe.  |
| {{<tags/feature/ga idea="2133">}} [Improved time synchronization across nodes for on-premises, AWS, and GCP environments](/stable/yugabyte-platform/create-deployments/create-universe-multi-zone/#configure-clockbound-optional) | Time synchronization across nodes has been enhanced through the use of the [ClockBound](/stable/yugabyte-platform/create-deployments/create-universe-multi-zone/#configure-clockbound-optional) library, resulting in improved P99 latency for certain workloads (those with near simultaneous writes and reads). <hr style="margin: 0.5em 0;">**To use**: Feature enabled by default. ClockBound is automatically configured during node provisioning, and the universe creation task sets the [time_source](/stable/reference/configuration/yb-master/#time-source) flag to `clockbound`.|
...
| <div style="width:150px">Feature</div> | Description |
| :-------- | :---------- |
| {{<tags/feature/ga idea="153">}}[xCluster DR automatic mode](/stable/yugabyte-platform/back-up-restore-universes/disaster-recovery/#automatic-mode) | Seamless automated replication of YSQL DDL changes across xCluster DR, eliminating the need to manually apply DDLs on both source and target clusters. This simplifies DDL operations and reduces management overhead.<hr style="margin: 0.5em 0;">**To use**: Enabled by default for universes running v2025.2.1 or later. |
| {{<tags/feature/ga idea="22">}}[Automatic YB-Master failover](/stable/yugabyte-platform/manage-deployments/remove-nodes/#automatic-yb-master-failover) | If a node with Master process fails unexpectedly (reducing health, but not causing service downtime), YBA auto-heals the universe by starting a Master process on a suitable surviving node (if one exists).<hr style="margin: 0.5em 0;">**To use**: Enabled by default for universes running v2.20.3 or later. |
