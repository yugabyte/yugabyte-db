...
{{% highlight-card title="Reprovision universe nodes" link="/stable/yugabyte-platform/manage-deployments/reprovision-nodes/" maturity="ga" %}}
Re-run node provisioning on existing universe nodes so OS-level settings match what YugabyteDB Anywhere expects. For universes still on legacy provisioning, the action also migrates nodes to new provisioning, including node agent and user-level systemd.

**To use:** Enabled by default. Open the universe and click **Actions > More > Reprovision Universe Nodes**. See [Reprovision universe nodes](/stable/yugabyte-platform/manage-deployments/reprovision-nodes/).
{{% /highlight-card %}}
-->
...
* Upgrades Prometheus to 3.12.0 to fix critical and high-severity CVEs. PLAT-21229
...
* Removes multiple CVEs from the YBA image to enhance security. PLAT-21321
