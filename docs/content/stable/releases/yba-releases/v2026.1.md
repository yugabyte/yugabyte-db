...
## v2026.1.0.0 - April 20, 2026 {#v2026.1.0.0}

**Build:** `2026.1.0.0-b109`

**Third-party licenses:** [YugabyteDB](https://downloads.yugabyte.com/releases/2026.1.0.0/yugabytedb-2026.1.0.0-b109-third-party-licenses.html), [YugabyteDB Anywhere](https://downloads.yugabyte.com/releases/2026.1.0.0/yugabytedb-anywhere-2026.1.0.0-b109-third-party-licenses.html)

### Download

<ul class="nav yb-pills">
 <li>
   <a href="https://downloads.yugabyte.com/releases/2026.1.0.0/yba_installer_full-2026.1.0.0-b109-linux-x86_64.tar.gz">
...
{{% highlight-card title="OIDC-based authentication in YCQL" link="/stable/secure/authentication/oidc-authentication-ycql/" maturity="ga" %}}
...
**To use:** Disabled by default. Set `ycql_use_jwt_auth=true` and configure `ycql_jwt_conf` on YB-TServer (with YCQL auth enabled). Create YCQL roles and use the JWT as the password. See [OIDC authentication](/stable/yugabyte-platform/security/authentication/oidc-authentication-aad/).
{{% /highlight-card %}}

{{% highlight-card title="Canary upgrades with automatic rollback" link="/stable/yugabyte-platform/manage-deployments/upgrade-software-install/#canary-upgrade" maturity="ea" %}}
...
{{% highlight-card title="YugabyteDB Kubernetes Operator import universe" link="/stable/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#import-universe" maturity="ga" %}}
The Kubernetes Operator reaches near-complete feature parity with YBA's VM-based workflows. Key additions include universe import (live migration of existing YBA-managed K8s universes to Operator management without downtime) and multi-region universe support via new CRDs that span multiple Kubernetes clusters.

**To use:** Available via API only; the K8s Operator must be installed on YBA. See [Before you begin](/stable/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#before-you-begin).
{{% /highlight-card %}}

{{% highlight-card title="YBA Operator High Availability" link="/stable/yugabyte-platform/administer-yugabyte-platform/operator-high-availability/" maturity="ea" %}}
The Operator now supports YBA's High Availability configuration by replicating Operator state — including applied YAMLs — from the primary YBA instance to standbys. This removes a single point of failure for the complete YugabyteDB stack in multi-region deployments.
...
| :-------- | :---------- |
| {{<tags/feature/ga idea="946">}}[OIDC-based authentication in YCQL](/stable/secure/authentication/oidc-authentication-ycql/) | Support for OIDC token-based authentication for YCQL databases. This allows YCQL database users to sign in to YugabyteDB Anywhere universes using their JSON Web Token (JWT) as their password. <hr style="margin: 0.5em 0;">**To use**: Disabled by default. Set `ycql_use_jwt_auth=true` and configure `ycql_jwt_conf` on YB-TServer (with YCQL auth enabled). Create YCQL roles and use the JWT as the password. See [OIDC authentication](/stable/yugabyte-platform/security/authentication/oidc-authentication-aad/). |
| {{<tags/feature/ea idea="486">}}[Canary upgrades with automatic rollback](/stable/yugabyte-platform/manage-deployments/upgrade-software-install/#canary-upgrade)| Upgrade production clusters with confidence using the new, optional canary upgrade mode. Instead of upgrading the entire cluster at once, YugabyteDB can now upgrade a single fault domain first, for example, all nodes in one availability zone — then pause for monitoring and validation before proceeding further. If an issue is detected during the canary phase, the upgrade rolls back automatically, leaving the rest of the cluster untouched. Once one's canary tests and monitoring phase pass, the upgrade continues across remaining fault domains. The result is a dramatically safer upgrade path for production deployments, with a built-in safety net that catches problems before they can affect your entire cluster.  <hr style="margin: 0.5em 0;">**To use**: Disabled by default. Set the **Enable Canary Upgrade Universe** Runtime Configuration option (config key `yb.upgrade.enable_canary_upgrade`) to true. |
| {{<tags/feature/ga idea="2004">}}[YugabyteDB Kubernetes Operator import universe](/stable/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#import-universe)|The Kubernetes Operator reaches a major milestone in this release, achieving near-complete feature parity with YBA's VM-based workflows. Existing YugabyteDB K8s universes can now be migrated to K8s Operator management without downtime, removing the last barrier to Operator adoption for teams with established deployments. Key additions include:<br><ul><li>Universe Import (Live Migration): Migrate existing YBA-managed Kubernetes universes to Operator management without downtime nor reconfiguration. </li><li>Multi-Region Universe Support: new Custom Resource Definitions (CRDs) enable universes that span multiple Kubernetes clusters. In short, YugabyteDB's uniquely differentiated multi-region stretch cluster support is now available to the K8s Operator, with one K8s cluster per region (this is contingent, of course, on network connectivity between the K8s clusters usually via MCS, Istio, or the equivalent).</li></ul><hr style="margin: 0.5em 0;">**To use**: The feature can be used via API only and the K8s operator must be installed on YBA. See [Before you begin](/stable/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#before-you-begin).|
| {{<tags/feature/ea idea="2460">}}[YBA Operator High Availability](/stable/yugabyte-platform/administer-yugabyte-platform/operator-high-availability/)| The Operator now supports YBA's High Availability configuration by replicating Operator state, including applied YAMLs, from the primary YBA instance to standbys, ensuring resilience of both the control plane and the K8s Operator itself. This removes a single point of failure for the complete YugabyteDB stack in multi-region deployments. <hr style="margin: 0.5em 0;">**To use**: Configure YBA HA between your active and standby instances, and enable YugabyteDB Kubernetes Operator on each YBA instance in the HA cluster. For the detailed steps, see [Prerequisites](/stable/yugabyte-platform/administer-yugabyte-platform/operator-high-availability/#prerequisites).|

### New features

| <div style="width:150px">Feature</div> | Description |
| :-------- | :---------- |
| {{<tags/feature/ea idea="56">}}[Replication Factor increase for existing universes](/stable/yugabyte-platform/manage-deployments/edit-universe/#edit-a-universe) | Support for increasing the Replication Factor (RF) on existing VM-based universes. RF decreases and support for K8s universes are planned for a future release.  <hr style="margin: 0.5em 0;">**To use**: You do this by adding new Availability Zones. Contact {{% support-platform %}} before modifying this field, for assistance on capacity planning and sizing appropriately. |
| {{<tags/feature/ga idea="2077">}}[Run all-nodes script to collect node data](/stable/yugabyte-platform/troubleshoot/universe-issues/#collect-node-data)| Added a utility to run the same shell command on all nodes in a cluster simultaneously, enabling faster and more consistent troubleshooting and operations across large deployments. <hr style="margin: 0.5em 0;">**To use**:  Disabled by default, set the **Enable All Nodes Script APIs** Universe Runtime Configuration option (config key yb.node_script.enabled) to true. You also need an API token and shell access to the YugabyteDB Anywhere host. See [Prerequisites](/stable/yugabyte-platform/troubleshoot/universe-issues/#prerequisites) |
| {{<tags/feature/ga idea="1701">}}[YBA operation safety pre-checks](/stable/yugabyte-platform/troubleshoot/universe-issues/#run-pre-checks-before-edit-and-upgrade-operations)| Added validation checks that run before YBA operations such as universe creation and node provisioning, rolling upgrades, and edit universe and edit GFlags, blocking operations that would result in a locked or degraded cluster state.  <hr style="margin: 0.5em 0;">**To use**: Enabled by default. Most checks can be turned off or tuned using universe-scoped runtime configuration parameters, `yb.checks.comprehensive_prechecks.enabled` and `yb.checks.comprehensive_prechecks.check_service_liveness_timeout`. See [Configure pre-checks](/stable/yugabyte-platform/troubleshoot/universe-issues/#configure-pre-checks).|
| {{<tags/feature/ga idea="2459">}} [Rotate certificates in Kubernetes cert-manager](/stable/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/#rotate-certificates-in-cert-manager)| For encryption in transit (EIT), YBA now supports scenarios in which cert-manager manages only partial certificate chains (specifically, cert-manager only holds the root cert, and not the intermediate certs). <hr style="margin: 0.5em 0;">**To use**:  See [Rotate server certificates](/stable/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/#rotate-server-certificates) and to [rotate cert-manager root certificates](/stable/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/#rotate-cert-manager-root-certificates). |
| {{<tags/feature/ea idea="2459">}}[Full move for Kubernetes universes](/stable/yugabyte-platform/manage-deployments/kubernetes-full-move/) | You can now modify storage attributes of K8s nodes such as volume count, storage class, and volume size on existing universes; under the hood, it's via a "full move" (in which a new pod for each TServer node is added and data is migrated from the old pods to the new ones). <hr style="margin: 0.5em 0;">**To use**: Disabled by default. Full move for Kubernetes universes requires YugabyteDB v2026.1.0.0 or later on the universe. Based on how your universe is managed, see [Non-operator universes](/stable/yugabyte-platform/manage-deployments/kubernetes-full-move/#non-operator-universes) or [Operator universes](/stable/yugabyte-platform/manage-deployments/kubernetes-full-move/#operator-universes).|
| {{<tags/feature/ga idea="2385">}}[Split-privilege mode for on-prem provisioning](/stable/yugabyte-platform/prepare/server-nodes-software/software-on-prem/#run-root-or-non-root) | For provisioning of VMs for "on-prem" infrastructure providers, the "New automatic on-premises node provisioning" script (node_agent_provision.sh) has added "split-privilege support" for separating root-privileged Linux commands (ulimits, THP, user creation, and so on) from user-privileged Linux commands (for example, cp, cat, and others). This enables deployment in environments where Linux admins have full root access, but DBAs have more restrictive user privileges. <hr style="margin: 0.5em 0;">**To use**: Run the node-agent-provision.sh script with `--noroot` or `--root` flag. See [Run root or non-root](/stable/yugabyte-platform/prepare/server-nodes-software/software-on-prem/#run-root-or-non-root).|
...
* Enables integration of Perf Advisor into YBA, not visible to end-users and default disabled. PLAT-18795
...
* Enables debug logging for YNP provisioning using `yb.node_agent.server.request_log_level`. PLAT-20701
...
* Restores visibility of LDAP passwords in API responses and logs. PLAT-18364
...
* Corrects dashboard calculation for CPU Seconds Total metrics. PLAT-19286
...
* Adds checks for node services liveness and SSH connectivity in UTs. PLAT-19338,PLAT-19339
...
* Corrects HA standby sync alert to avoid false positives after an upgrade. PLAT-18539
...
* Replaces `CheckSSHConnection` with `checkNodeCommandExecution` during universe edits. PLAT-20979
...
* Corrects universe creation failure due to Python version mismatch in on-prem Ubuntu OS patch tests. PLAT-18709
...
* Displays correct module names and error messages in YNP driver. PLAT-18915
...
* Fixes cluster creation failure in YBM after node agent enablement. PLAT-18947
...
* Adds support for YNP modules in YBM using Golang. PLAT-19274
...
* Fixes YBM service template copying and log filename issues. PLAT-19306
...
* Adds duplicate zone/region validation for provider API calls in YNP. PLAT-19467
...
* Adds task to convert legacy automatic nodes to YBA Node Agent/YNP, with rolling re-provisioning and new API endpoint. PLAT-19652
* Unifies node-level pre-checks across environments using YNP pre-flight checks. PLAT-19661
...
* Always populates the extra field in YNP config for easier conditional checks. PLAT-19745
...
* Adds version check for onprem nodes to ensure correct YNP provisioning. PLAT-19805
* Adds metrics to track and alert on YNP version skews across nodes. PLAT-19806
...
* Respects the skip_tls_verify setting in YNP connections. PLAT-19864
* Sets the node exporter port for YNP-created providers. PLAT-19897
...
* Ensures user systemd starts correctly in YNP split mode by handling privileges. PLAT-20198
...
* Handles `400 Bad Request` errors for non-existing resources in YNP, reducing errors. PLAT-20252
* Reduces memory leaks in YBM by using KeyLock for provider locks. PLAT-20289
...
* Disables YNP version skew alerts for Kubernetes universes. PLAT-20802
...
* Respects the `yb.node_agent.enable_ynp_version_check` flag consistency in YBA and YNP. PLAT-21172
...
* Documents now available for well-tested YBA HA APIs. PLAT-18823
...
* Enables correct dual NIC setup for YBM during YNP provisioning. PLAT-18968
* Ensures `configure_nic.sh` and `bootscript` run correctly with YNP provisioning. PLAT-18968
...
* Distinguishes between FATAL and NON-FATAL errors in YNP module failures. PLAT-19530
...
* Fixes AWS universe creation failure during the Setup YNP step. PLAT-19752
...
* Updates the S3 path in YBA Makefile for testing the Perf Advisor. PLAT-19883
...
* Enables adding and deleting geo-partitions in the Edit Universe flow. PLAT-17598
...
* Increases Storybook to v10.2.13, reducing install size by 29% and integrating Vitest. PLAT-19987
...
* Enables split mode in YNP for root and nonroot processes. PLAT-19354
...
* Fixes issue with missing config key preventing Integrations tab from loading. PLAT-20266
...
* Ensures YNP node provisioning remains consistent when executed multiple times in `noroot` mode. PLAT-20341
