...
{{% highlight-card title="STIG-compliant hardened Docker images" link="/stable/yugabyte-platform/install-yugabyte-platform/install-software/kubernetes/#run-containers-as-non-root" maturity="ga" %}}
Starting in v2026.1.2, YugabyteDB Anywhere and YugabyteDB Docker images are STIG-compliant and hardened, and run as a non-root user by default. Existing Kubernetes deployments that do not pin a user in the pod security context migrate to non-root on upgrade.

**To use:** Enabled by default. To keep running as root, set `runAsUser: 0` in the Helm security context before you upgrade. See [Run containers as non-root](/stable/yugabyte-platform/install-yugabyte-platform/install-software/kubernetes/#run-containers-as-non-root).
{{% /highlight-card %}}
-->
