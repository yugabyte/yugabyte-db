...
{{% highlight-card title="OIDC-based authentication in YCQL" link="/stable/yugabyte-platform/security/authentication/oidc-authentication-aad/" maturity="ga" idea="946" %}}
...
{{% highlight-card title="Kubernetes Operator enhancements" link="/stable/yugabyte-platform/anywhere-automation/yb-kubernetes-operator/#import-universe" maturity="ga" idea="2004" %}}
The YugabyteDB Kubernetes Operator reaches near-complete feature parity with YBA's VM-based workflows. Key additions include universe import (live migration of existing YBA-managed Kubernetes universes to Operator management without downtime) and multi-region universe support via new CRDs that span multiple Kubernetes clusters.
...
| {{<tags/feature/ga idea="2459">}} [Rotate cert-manager certificates](/stable/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/#rotate-certificates-in-cert-manager)| Added support for scenarios where Kubernetes cert-manager manages only partial certificate chains (specifically, cert-manager only holds the root certificate, and not the intermediate certificates). <hr style="margin: 0.5em 0;">**To use**:  See [Rotate server certificates](/stable/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/#rotate-server-certificates) and to [rotate cert-manager root certificates](/stable/yugabyte-platform/security/enable-encryption-in-transit/add-certificate-kubernetes/#rotate-cert-manager-root-certificates). |
