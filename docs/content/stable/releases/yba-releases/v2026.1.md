...
Added a Perf Advisor (PA) data collector for self-managed YugabyteDB installations, with PII redaction and a dedicated portal for accessing performance insights. Starting in YugabyteDB Anywhere v2026.1.1.0, the YBA installer automatically registers a Perf Advisor collector service during installation, and can be enabled giving the ablility to download PA support bundles.
...
YugabyteDB and YugabyteDB Anywhere (K8s only) supports FIPS 140-2–compliant encryption when configured during initial installation.

**To use:** Disabled by default. You can enable FIPS in your configuration values file when you install YugabyteDB Anywhere. See [Enable FIPS](/stable/yugabyte-platform/install-yugabyte-platform/install-software/kubernetes/#enable-fips).
{{% /highlight-card %}}
...
| :-------- | :---------- |
| {{<tags/feature/ga idea="2657">}}[Gflag requires restart indicator](/stable/yugabyte-platform/manage-deployments/edit-config-flags/#modify-configuration-flags) | When adding or editing configuration flags on a universe, the gflag details now display a **Requires restart** tag for flags whose changes take effect only after the universe is restarted. This makes it clear at selection time whether a flag change can be applied immediately or needs a rolling or concurrent restart. <br /> <img src="/icons/connect.svg" style="height: 1em; display: inline; margin: 0 0.25em 0 0; vertical-align: middle;" /> **To use**: Enabled by default. Navigate to the universe and click **Actions > Edit Flags**. See [Edit configuration flags](/stable/yugabyte-platform/manage-deployments/edit-config-flags/#modify-configuration-flags). |
