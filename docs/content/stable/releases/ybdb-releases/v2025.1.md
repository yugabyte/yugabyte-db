...
* [CDC data now retained for 24 hours by default](/stable/additional-features/change-data-capture/using-logical-replication/advanced-configuration/#retention-of-resources) in case the CDC client/consumer is offline. {{<tags/feature/ga idea="1171">}}

* Improved [YSQL Connection Manager](/stable/explore/going-beyond-sql/connection-mgr-ysql/) performance. Reduced the cost of binding logical to physical connections to improve performance and lower application-level latencies, specifically by optimizing the GUC workflow.
...
Added [Table-level locks](/stable/explore/transactions/explicit-locking/#table-level-locks) to support concurrent DDL and DML operations, improving workload concurrency and reducing conflicts during schema changes.
...
For more information on CDC, refer to [Change data capture (CDC)](/stable/additional-features/change-data-capture/).
...
For more information, refer to [xCluster replication](/stable/architecture/docdb-replication/async-replication/).

### New features

* Added support for SCRAM-based password authentication in [Connection manager](/stable/additional-features/connection-manager-ysql/ycm-setup/#authentication-methods), enhancing security beyond the previously supported MD5 method. {{<tags/feature/ea idea="2194">}}

* [Query diagnostics](/stable/launch-and-manage/monitor-and-alert/query-tuning/query-diagnostics/). You can use a new YSQL function `yb_query_diagnostics`, to create a diagnostics bundle for a normalised YSQL query for a time range to speed up troubleshooting. The bundle contains critical data like the EXPLAIN PLAN, bind variables, pg_statement_stats, schema details, and Active Session History. {{<tags/feature/ea idea="1591">}}
...
For more information, refer to [PostgreSQL 15 features](/stable/api/ysql/pg15-features/).
...
- [PITR-flashback query](/stable/manage/backup-restore/time-travel-query/). Allows querying historical database state by specifying a past timestamp, aiding in auditing, debugging, and data analysis. {{<tags/feature/ea idea="1182">}}
...
- [Query Plan Management (QPM)](/stable/launch-and-manage/monitor-and-alert/query-tuning/pg-hint-plan/) introduced to ensure stable and adaptable query performance by preventing plan regression and automatically detecting optimal new plans. {{<tags/feature/ea idea="1107">}}
...
- [YSQL lease mechanism](/stable/architecture/transactions/concurrency-control/#ysql-lease-mechanism). Enabled a robust lease mechanism between YB-TServer and YB-Master to enhance fault tolerance and data consistency during network partitions. {{<tags/feature/ga>}}
...
- Added [CDC Logical Replication support](/stable/additional-features/change-data-capture/using-logical-replication/get-started/) for xCluster feature where both CDC and xCluster can work simultaneously on the same source tables. {{<tags/feature/ea idea="82">}}

- [Active Session History](/stable/explore/observability/active-session-history/). Get real-time and historical information about active sessions to analyze and troubleshoot performance issues. {{<tags/feature/ga idea="830">}}

- [PostgreSQL Anonymizer](/stable/additional-features/pg-extensions/extension-pganon/) extension, which masks or replaces personally identifiable information (PII) or commercially sensitive data, is now bundled with Yugabyte. {{<tags/feature/ga idea="1497">}}
