...
* Fixed consistent failures in Java test TestPgRegressUpdateOptimized by adding ORDER BYs to SELECTs on the affected table. {{<issue 28988>}}
...
* Disables object locking in PggateTest suite to prevent failures. {{<issue 29382>}}
...
* Stabilizes the flaky test by using correct annotations. {{<issue 29603>}}
* Fixes test failure by starting the YSQL lease refresher. {{<issue 29100>}}
* Resolves deadlock detection issues in `PgLibPqTest.PagingReadRestart` by temporarily disabling it. {{<issue 28891>}}
* Disables object locking in specific tests to ensure DDLs execute with DMLs present. {{<issue 28807>}}
* Ensures DDL operations wait when DML is in progress with object locking. {{<issue 29150>}}
* Corrects `pg_locks` test failures by including object locks in expectations. {{<issue 29153>}}
...
* Fixes cleanup for mock handlers in `PgRecursiveAbortTest` to prevent test failures when table locks are enabled. {{<issue 29131>}}
...
* Resolved flakiness in the CDCSDKConsumptionConsistentChangesTest.TestCDCWithSavePoint test by incorporating a wait function in GetConsistentChangesFromCDC. {{<issue 29823>}}
...
* Enables vlogging in `PgAnalyzeTest` with YSQL lease compatibility. {{<issue 29249>}}
