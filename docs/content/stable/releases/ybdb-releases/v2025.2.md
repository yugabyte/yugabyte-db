...
| {{<tags/feature/ea idea="2525">}}[CDC: Use WAL Remote Origin to prevent circular loops](/stable/deploy/multi-dc/async-replication/async-transactional-setup-automatic/#set-up-automatic-mode-replication) | YugabyteDB now tracks the origin cluster of every SQL transaction, thereby enabling CDC to avoid infinite loops (A -> B -> A -> B -> A) when CDC is configured bi-directionally.<hr style="margin: 0.5em 0;">**To use**: Create replication origins using the `pg_replication_origin_create();` function before you start streaming changes from a replication slot. |
| {{<tags/feature/ea idea="2196">}}[Connection Manager enhancement](/stable/additional-features/connection-manager-ysql/ycm-setup/#configure) | Reduces latency by leveraging authentication passthrough mechanism for acquiring new logical connections, ensuring increased efficiency and correctness.<hr style="margin: 0.5em 0;">**To use**: Enable YSQL Connection Manager by setting the flag [enable_ysql_conn_mgr](/stable/additional-features/connection-manager-ysql/ycm-setup/#configure) on yb-tserver. |
