...
## v2025.2.0.0 - December 9, 2025 {#v2025.2.0.0}

**Build:** `2025.2.0.0-b130`

**Third-party licenses:** [YugabyteDB](https://downloads.yugabyte.com/releases/2025.2.0.0/yugabytedb-2025.2.0.0-b130-third-party-licenses.html), [YugabyteDB Anywhere](https://downloads.yugabyte.com/releases/2025.2.0.0/yugabytedb-anywhere-2025.2.0.0-b130-third-party-licenses.html)

### Downloads

<ul class="nav yb-pills">
  <li>
    <a href="https://downloads.yugabyte.com/releases/2025.2.0.0/yugabyte-2025.2.0.0-b130-darwin-x86_64.tar.gz">
      <i class="fa-brands fa-apple"></i>
      <span>macOS x86</span>
    </a>
  </li>
  <li>
    <a href="https://downloads.yugabyte.com/releases/2025.2.0.0/yugabyte-2025.2.0.0-b130-darwin-arm64.tar.gz">
      <i class="fa-brands fa-apple"></i>
      <span>macOS ARM</span>
    </a>
  </li>
  <li>
    <a href="https://downloads.yugabyte.com/releases/2025.2.0.0/yugabyte-2025.2.0.0-b130-linux-x86_64.tar.gz">
      <i class="fa-brands fa-linux"></i>
      <span>Linux x86</span>
    </a>
  </li>
  <li>
    <a href="https://downloads.yugabyte.com/releases/2025.2.0.0/yugabyte-2025.2.0.0-b130-el8-aarch64.tar.gz">
      <i class="fa-brands fa-linux"></i>
      <span>Linux ARM</span>
    </a>
  </li>
...
```sh
docker pull yugabytedb/yugabyte:2025.2.0.0-b130
```
...
- Introduces transaction-related enhancements for 2025.2, fixing consistency issues and read-restart errors. {{<tags/feature/ga idea="2336">}}

- [AWS ClockBound support](/stable/deploy/manual-deployment/system-config/#configure-clockbound). [ClockBound](https://github.com/aws/clock-bound) is an open source daemon that allows you to compare timestamps to determine order for events and transactions, independent of an instance's geographic location; it improves clock accuracy by several orders of magnitude. {{<tags/feature/ga idea="1807">}}

- Added [Table-level locks](/stable/explore/transactions/explicit-locking/#table-level-locks) to support concurrent DDL and DML operations, improving workload concurrency and reducing conflicts during schema changes. {{<tags/feature/ea idea="1114">}}

- Improves [load balancer](/stable/architecture/docdb-sharding/cluster-balancing/#monitoring-cluster-balancer-progress) observability and throughput by prioritizing recovery of leaderless tablets and production databases. {{<tags/feature/ga idea="1328">}}
...
- Enables TRUNCATE operations to be safely replicated in [xCluster](/stable/architecture/docdb-replication/async-replication/) environments, ensuring consistency across distributed clusters. {{<tags/feature/ga idea="1608">}}

- Enhances bulk load performance on distributed tables by implementing Skip Prefix Locks. {{<tags/feature/ea idea="2139">}}

- [Auto Analyze service](../../../additional-features/auto-analyze) Added automatic background statistics gathering to improve usability by eliminating the need for users to manually run ANALYZE. {{<tags/feature/ga idea="590">}}
...
- [Cluster tablet metadata](/stable/explore/observability/yb-tablet-metadata/) Offers a global view, `yb-tablet-metadata` to reveal tablet placement, leadership, and row-specific information for all tables and databases. {{<tags/feature/ea idea="2358">}}
...
* Refactors the PgGate fetch control, introduces new classes for combining results from multiple PgsqlOp objects, optimizes index scan read requests for copartitioned vector indexes, and removes unnecessary index requests. {{<issue 22826>}}
...
* Enhances Postgres logging integration by utilizing the ErrorData.backtrace field, adjusting the control of backtrace addition through the `backtrace_functions` parameter, and introducing a `yb_debug_original_stacktrace_format` flag to switch stacktrace formats. {{<issue 26447>}}
...
* Allows cleaner and when-needed building of dummy_seclabel in build_postgres.py instead of adding it as a default subdir. {{<issue 25892>}}
...
* Reduces load balancer's hold on `Catalocustomeranager::mutex_` to prevent potential master leader unavailability. {{<issue 29183>}}
...
* Disables object locking for PgTabletShutdownTest to allow DDLs like DROP/TRUNCATE to abort in progress DMLs. {{<issue 28161>}}
...
* Adjust assert logic in PgCatalogVersionTest to handle changes when object locking and transactional DDL are enabled. {{<issue 28413>}}
...
* Refactors code to address contention issues on `Catalocustomeranager::mutex_` in larger clusters, enhancing stability and performance. {{<issue 29186>}}
...
* Aligns the styling of assessment voyager page with the provided ficustomera design without disrupting schema migration page operation. {{<issue 28214>}}
* Updates the voyager analysis page in yugabyted according to the Ficustomera provided. {{<issue 28220>}}

### Bug fixes

#### YSQL

* Reduces sporadic "duplicate key value violates unique constraint" errors in the Long-running System Test framework. {{<issue 12449>}}
...
* Allows prompt response to Postgres interrupts, enhancing handling of statement timeouts and cancel requests. {{<issue 26284>}}
...
* Allows sharing of perform operation serial number counter among multiple Postgres processes to ensure unique serial numbers and better operation execution. {{<issue 23648>}}
...
* Fixes PgConnTest.SessionExpiration failure when table locking is enabled. {{<issue 29010>}}
...
* Adjusts DDL replication tables to be non-colocated directly in the extension using SQL for better compatibility with Postgres. {{<issue 27100>}}
...
* Resolves the issue of queue_time_us_stats being incremented twice, enhancing TestThreadPool.TestMetrics stability. {{<issue 28668>}}
* Adds a long operation tracker to CatalocustomeranagerBgTasks::RunOnceAsLeader for easier debugging. {{<issue 28712>}}
