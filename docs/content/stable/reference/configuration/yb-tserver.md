...
- If the shards value is set to *Default* (`-1`), then the server internally *updates* the flag with the intended value during start-up prior to version `2.18` and the flag remains *unchanged* starting from version `2.18`.
...
- This value must match on all `yb-master` and `yb-tserver` configurations of a YugabyteDB cluster.
- If the shards value is set to *Default* (`-1`), then the server internally *updates* the flag with intended value during startup prior to version `2.18` and the flag remains *unchanged* starting from version `2.18`.
