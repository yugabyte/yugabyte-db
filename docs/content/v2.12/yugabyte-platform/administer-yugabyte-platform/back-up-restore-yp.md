...
You need to download the backup script that corresponds to the version of YugabyteDB Platform that you are backing up. Set an environment variable for the version of YugabyteDB Platform that you are running as follows:
...
1. Copy the YugabyteDB Platform backup script `yb_platform_backup.sh` from the `yugabyte-db` repository using the following `wget` command:
