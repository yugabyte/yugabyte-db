...
After trying out [CQL](/community-edition/quick-start/test-cql/) and [Redis]() cli based commands on the local cluster, follow the instructions below to run some sample apps against the cluster as well as look at some of the configuration details of the cluster.
...
You can inspect the table created by the sample app using cqlsh. Connect to the local cluster [as before](/community-edition/quick-start/test-cql/). The sample app creates the `cassandrakeyvalue` table in the keyspace `ybdemo_keyspace`.
