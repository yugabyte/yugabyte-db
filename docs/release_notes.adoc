...
. The Java client's AbstractYBScannerBuilder#maxNumBytes() setter is now called
batchSizeBytes as is the corresponding property in AsyncYBScanner. This makes it
consistent with the C++ client.
. The "yb-admin" tool can now list and delete tables via its new subcommands
