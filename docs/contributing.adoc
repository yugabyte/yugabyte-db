...
Existing code uses `std::bind` and `std::function` for function binding and
...
Tests should run quickly (< 1s).::
  ...
  test, make the runtime dependent on `YBTest#AllowSlowTests`, which is
  enabled via the `YB_ALLOW_SLOW_TESTS` environment variable and is
