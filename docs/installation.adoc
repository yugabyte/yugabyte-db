...
sudo yum install yb-master                  # Kudu master init.d service script and default configuration
sudo yum install yb-tserver                 # Kudu tablet server init.d service script and default configuration
...
`yb-master` and `yb-tserver` packages on the appropriate hosts. These packages
...
sudo apt-get install yb-master              # Service scripts for managing yb-master
sudo apt-get install yb-tserver             # Service scripts for managing yb-tserver
...
. Verify that services are running using one of the following methods:
  - Examine the output of the `ps` command on servers to verify one or both of `yb-master`
  or `yb-tserver` processes is running.
...
in `/etc/default/yb-master` and `/etc/default/yb-tserver` files as well. You
...
[source,bash]
----
$ sudo service yb-master start
$ sudo service yb-tserver start
----
...
[source,bash]
----
$ sudo service yb-master stop
$ sudo service yb-tserver stop
----
// end::start_stop[]
...
[source,bash]
----
$ sudo chkconfig yb-master on                # RHEL / CentOS
$ sudo chkconfig yb-tserver on               # RHEL / CentOS

$ sudo update-rc.d yb-master defaults        # Debian / Ubuntu
$ sudo update-rc.d yb-tserver defaults       # Debian / Ubuntu
----
...
. Build any missing third-party requirements using the `build-thirdparty.sh` script.
+
[source,bash]
----
$ build-support/enable_devtoolset.sh thirdparty/build-thirdparty.sh
----
...
WARNING: Only build against the client libraries and headers (`yb_client.so` and `client.h`).
...
no headers are included in your installation. and you will need to <<build_yb,build
