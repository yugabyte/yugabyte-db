...
To see all available configuration options for the `yb-master` executable, run it
...
|--fs_wal_dirs | string | | Comma-separated list of directories where the Master
will place its write-ahead logs. May be the same list as '--fs_data_dirs`, but
cannot contain sub-directories of a data directories.
...
To see all available configuration options for the `yb-tserver` executable,
