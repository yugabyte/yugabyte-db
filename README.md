...
* **Distributed transactionas** The transactions design is based on the Google Spanner architecture. Strongly consistency of writes is achieved by using Raft consensus for replication and cluster-wide distributed ACID transactions using *hybrid logical clocks*. *Snapshot * and *serializable* isolation levels are supported.  Reads (queries) have strong consistency by default, but can be tuned dynamically to read from followers and read-replicas.

* **High availability** Extremely resilient to failures with native failover and repair. YugabyteDB can be configured to tolerate disk, node, zone and region failures automatically. For a typical deployment (YugabyteDB cluster deployed in one region across multiple zone in a public cloud), the RPO is 0 (meaning no data is lost on a failure) and the RTO is 3 seconds (meaning the data being served by the failed node is available in 3 seconds).
...
| [Automatic tablet splitting](https://github.com/yugabyte/yugabyte-db/blob/master/architecture/design/docdb-automatic-tablet-splitting.md) | PROGRESS  | v2.2 |  |  |
...
| [Row-level geo-partitioning](https://github.com/yugabyte/yugabyte-db/blob/master/architecture/design/ysql-row-level-partitioning.md) | PROGRESS  |   |  | This feature will be done in two phases: supporting PostgreSQL partitions followed by geo-placement support for partitions |
